  ...
  )
{
  ...
  if (Status == EFI_SUCCESS) {
    ...
    if (Status == EFI_SUCCESS) {
      ...
      //
      MmioWrite16 (MiscPeiPreMemConfig->MchBar + R_SA_MCHBAR_NCDECS_CR_SCRATCHPAD_NCU_0_REG, DisplayDebugNumber);
    }
#endif
  }
  return;
  ...
  )
{
  ...
  //
  // Intel Silicon View Technology (ISVT) IO Reading port 0x99 with EAX = 0x1000 for End of MRC
  //
