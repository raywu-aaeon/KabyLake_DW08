  ...
  )
{
  ...
  for (Dimm = 0; Dimm < SA_MC_MAX_SOCKETS; Dimm++) {
    ChannelIndex = Dimm >> 1;
    DimmIndex = Dimm & 0x1;
    DimmInfo = &mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex];
  ...
  MaximumCapacity += (MaxRankCapacity * SA_MC_MAX_SIDES * (MaxSockets - SkipSolderedDimmCapacityCount));
