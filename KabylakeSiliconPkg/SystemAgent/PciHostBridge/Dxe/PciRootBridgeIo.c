...
GLOBAL_REMOVE_IF_UNREFERENCED RESOURCE_CONFIGURATION  Configuration = {
  {
    {
      0x8A,
      0x2B,
  ...
  )
{
  ...
  if (Multiplicand >= DivU64x64Remainder (MAX_UINT64, Multiplier, NULL)) {
    ...
    // let Multiplicand = Multiplicand / 2 and recursively call itself until Multiplicand * Multiplier doesn't overflow.
    // The result will be multiplied with 2 to get the correct result.
    //
    Result = MultThenDivU64x64x32 (RShiftU64 (Multiplicand, 1), Multiplier, Divisor, &Temp32);
    return LShiftU64 (Result, 1) + DivU64x32Remainder (LShiftU64 (Temp32, 1), Divisor, Remainder);
  } else {
  ...
  )
{
  ...
  *CurrentTick = GetPerformanceCounter();
  if (StartTick < EndTick) {
    //AMI_OVERRIDE_START >>> EIP490159 - External RAID Card Function Fail in EFI Mode on KBL
    if (PreviousTick < *CurrentTick) {
      return *CurrentTick - PreviousTick;
    } else {
      return (EndTick - PreviousTick) + (*CurrentTick - StartTick);
    }
  } else {
    if (*CurrentTick < PreviousTick) {
      return PreviousTick - *CurrentTick;
    } else {
      return (StartTick - *CurrentTick) + (PreviousTick - EndTick);
    }
    //AMI_OVERRIDE_END <<< EIP490159 - External RAID Card Function Fail in EFI Mode on KBL
  }
}
