  ...
  )
{
  ...
  //
  Status = TouchSensorSetMemWindowEx ((UINT8 *)(UINTN)iTouchDev->Me2HidBufferDevAddr, (UINT8 *)(UINTN)iTouchDev->FeedbackBufferDevAddr, (UINT8 *)(UINTN)iTouchDev->Hid2MeBufferDevAddr, iTouchDev->FeedbackBufferSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "IntegratedTouchInitialize TouchSensorSetMemWindowEx Status=%r\n", Status));
    return Status;
  }
