/** @file
  ...
  NOTE: This module does not work if CSM is off.
...
#define VARIABLE_BUFFER_SIZE  10
...
/**
    ...
    @param TRUE: Enable Alternate Access Mode
...
)
{
    ...
    // Wait for the Bit is set/reset.
    do {
        PchPcrRead32( PID_ITSS, R_PCH_PCR_ITSS_GIC, &Buffer32 );
    } while ((Buffer32 & B_PCH_PCR_ITSS_GIC_AME) != (Data32Or & B_PCH_PCR_ITSS_GIC_AME));    
}
...
{
    static volatile TIMER_INT_DATA  TimerIntData[10] = {0};
    static volatile UINT32  Index = 0;
    UINT8                   StoreCMOSData = 0;
    static volatile UINT8   RtcRegA;
    static UINT32           AltAcc;
...
//    BOOLEAN                 IntState;
    ...
    if (Save) { 
        ...
        PchPcrRead32( PID_ITSS, R_PCH_PCR_ITSS_GIC, &AltAcc );
        AltAcc &= B_PCH_PCR_ITSS_GIC_AME;
        // Enable Alternate Access Mode
        SwitchAlternateAccessMode (TRUE);       
        StoreCMOSData = IoRead8(0x70); // Save 0x70
        // Restore Alternate Access Mode
        if (AltAcc == 0) SwitchAlternateAccessMode (FALSE); 
        
        if ((Index != 0) && \
            (IoRead32(0xcf8) == TimerIntData[Index - 1].StoreCF8) && \
            (StoreCMOSData == TimerIntData[Index - 1].StoreCMOS) && \
            (IoRead8(0x76) == TimerIntData[Index - 1].StoreExtCMOS) && \
            (TimerIntData[Index - 1].Count < 0xFFFFFFFF)) {
          TimerIntData[Index - 1].Count ++;
        } else {
          TimerIntData[Index].StoreCF8 = IoRead32(0xcf8);    // Store CF8 (PCI index)        
          TimerIntData[Index].StoreCMOS = StoreCMOSData;
          TimerIntData[Index].StoreExtCMOS = IoRead8(0x76);  // Save 0x76 
          Index ++; 
        }
    } else {
        Index --;
        ...
        IoWrite8(0x70, TimerIntData[Index].StoreCMOS);     // Restore 0x70
        IoWrite8(0x76,TimerIntData[Index].StoreExtCMOS); // Restore 0x76
        IoWrite32(0xcf8, TimerIntData[Index].StoreCF8);    // Restore 0xCF8 (PCI index)
