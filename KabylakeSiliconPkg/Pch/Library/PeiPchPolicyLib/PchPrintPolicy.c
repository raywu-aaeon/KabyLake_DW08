  ...
  )
{
  ...
  for (Index = 0; Index < GetPchMaxPciePortNum (); Index++) {
    ...
    DEBUG ((DEBUG_INFO, " RootPort[%d] EnableHotPlugSci = %x\n", Index, PcieConfig->RootPort[Index].EnableHotPlugSci));
    ...
    DEBUG ((DEBUG_INFO, " RootPort[%d] ClkSrcNumber= %x\n", Index, PcieConfig->RootPort[Index].ClkSrcNumber));
    ...
    DEBUG ((DEBUG_INFO, " RootPort[%d] ForceClkDisableWhenRpDisable= %x\n", Index, PcieConfig->RootPort[Index].ForceClkDisableWhenRpDisable));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FlashCfgLockDown= %x\n", SpiConfig->FlashCfgLockDown));
}
...
/**
  Print HYBRID_STORAGE_CONFIG.

  @param[in] HybridStorageConfig         Pointer to a HYBRID_STORAGE_CONFIG that provides the Hybrid Storage device setting

**/
VOID
PchPrintHybridStorageConfig (
  IN CONST HYBRID_STORAGE_CONFIG   *HybridStorageConfig
  )
{
  ...
  )
{
  ...
  PchPrintHybridStorageConfig (HybridStorageConfig);
