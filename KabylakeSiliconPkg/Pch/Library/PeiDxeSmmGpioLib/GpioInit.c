...
// AMI_OVERRIDE_PCH0010_START >>> For debug GPIO setting
//      DEBUG ((DEBUG_ERROR, "Group:%x | Pad: %x | ", Group, PadNumber));
// AMI_OVERRIDE_PCH0010_END <<< 
      DEBUG_CODE_END ();
...
// AMI_OVERRIDE_PCH0010_START >>> For debug GPIO setting
//  DEBUG_CODE_BEGIN();
//      DEBUG ((DEBUG_ERROR, "PadCfg Reg Addr0: %x | PadCfg Reg Addr1: %x | ", PCH_PCR_ADDRESS (GpioGroupInfo[GroupIndex].Community, PadCfgReg), PCH_PCR_ADDRESS (GpioGroupInfo[GroupIndex].Community, PadCfgReg + 0x4)));
//      DEBUG ((DEBUG_ERROR, "PadCfgReg Val0: %x(%x) | PadCfgReg Val1: %x(%x) \n", Dw0Reg, ~(UINT32)Dw0RegMask, Dw1Reg, ~(UINT32)Dw1RegMask));
//  DEBUG_CODE_END();
// AMI_OVERRIDE_PCH0010_END <<<
      //
