...
{
  UINTN SpiRegBase;
  //AMI_OVERRIDE_START >>> EIP441868 - System will hang in Method (TPTS, 1) with black screen after S4 aging.
  UINT32 BiosControl;
  UINT32 Timeout;
  //AMI_OVERRIDE_END <<< EIP441868 - System will hang in Method (TPTS, 1) with black screen after S4 aging.
  ...
  Timeout = 1000;
  do {
    BiosControl = MmioRead32 (SpiRegBase + R_PCH_SPI_BC);
    Timeout--;
  } while ((BiosControl & B_PCH_SPI_BC_SYNC_SS) && (Timeout > 0));
  //AMI_OVERRIDE_END <<< EIP441868 - System will hang in Method (TPTS, 1) with black screen after S4 aging.
  //
  // Any TCO-based status bits require special handling.
  // SMI_STS.TCO_STS must be cleared in addition to the status bit in the TCO registers
