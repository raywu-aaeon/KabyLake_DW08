  ...
  )
{
  ...
  //
  if (!SciEn) {
    if ((IoRead32 (mAcpiBaseAddr + R_PCH_ACPI_GPE0_EN_127_96) & B_PCH_ACPI_GPE0_EN_127_96_WADT) &&
        (IoRead32 (mAcpiBaseAddr + R_PCH_ACPI_GPE0_STS_127_96) & B_PCH_ACPI_GPE0_STS_127_96_WADT)) {
      IoWrite32 (mAcpiBaseAddr + R_PCH_ACPI_GPE0_STS_127_96, B_PCH_ACPI_GPE0_STS_127_96_WADT);
    }
  }
  ...
  )
{
  ...
  UINT32              SmiEnValue;
  UINT32              SmiStsValue;
// AMI_OVERRIDE_PCH0023_START >>> reduce the Smi times, it's the same in SbSmmCorePlatformHookLib.c
#if 0   
  UINT8               Port74Save;
  UINT8               Port76Save;
#endif
// AMI_OVERRIDE_PCH0023_END <<<  reduce the Smi times, it's the same in SbSmmCorePlatformHookLib.c    
