...
Scope(\_GPE) {
  Method(_L6D, 0, Serialized) {
    \_SB.PCI0.XHC.GPEH()
// AMI_OVERRIDE_PCH0002_START >>> Support UpServer RC.   
#ifndef ASL_UP_ACPI_SUPPORT
    \_SB.PCI0.HDAS.GPEH()
#endif //ASL_UP_ACPI_SUPPORT
// AMI_OVERRIDE_PCH0002_END <<<
    \_SB.PCI0.GLAN.GPEH()
    \_SB.PCI0.XDCI.GPEH()
  }
}
...
}// AMI_OVERRIDE_PCH0003_START >>>
#if 0  // AMI_OVERRIDE_PCH0003, Device LPCB is generated in PCI_TREE.ASL
  //
...
// AMI_OVERRIDE_PCH0004_START >>>
Scope (\_SB.PCI0) {
// AMI_OVERRIDE_PCH0004_END <<<
  //
...
Scope(\_SB_.PCI0) {
  ...
  //
// AMI_OVERRIDE_PCH0005_START >>> Support UpServer RC.   
#ifndef ASL_UP_ACPI_SUPPORT
  include("PchHda.asl")
#endif //ASL_UP_ACPI_SUPPORT
// AMI_OVERRIDE_PCH0005_END <<<

#if 0   // AMI_OVERRIDE_PCH0006_START >>>
  //
  // PCIE Root Port #01
  //
  Device(RP01) {
...
#endif // AMI_OVERRIDE_PCH0006_END <<<
  ...
  // Based on CIOE,When it is enable, we are publishing ACPI Device (CIO2) (Which
  // is for PCHLP < C0 Stepping). We are publishing it based on _HID = "INT343E".
  // So if CIOE is disable, We are publishing ACPI Device (CIO2) based on
  // PCI BUS DEVICE FUNCTION address (Which is for PchLp>=C0 Stepping).
  //
  Device (CIO2) {
    Method (_STA) {
      Return (0x0F)
...
Scope(\_SB){
  //
  // PCH Core Voltage Margining support
  //
  Name (HDAA, 0)    // HD Audio Active state flag
  Name (DISA, 1)    // Display Active state flag

  //
  // Display ON event handler for Voltage Margining
  // To be called in 'GUAM - Global User Absent Mode' or similar screen/display on event handler
  //
  Method(DION, 0) {
    VMMH(1, 1) // CS Exit event - Display state ON
  }

  //
  // Display OFF event handler for Voltage Margining
  // To be called in 'GUAM - Global User Absent Mode' or similar screen/display off event handler
  //
  Method(DIOF, 0) {
    VMMH(1, 0) // CS Entry event - Display state OFF
  }

  //
  // Voltage Margining handler
  // Arg0 - Caller: 0 - HD Audio, 1 - Display
  // Arg1 - Device state: 0 - disactive, 1 - active
  //
  Method(VMMH, 2, Serialized) {

    // Check if voltage margining is supported on the platform
    If(LOr(LNot(CondRefOf(\_SB.VMON)), LNot(CondRefOf(\_SB.VMOF)))) {
      Return // VMON/VMOF platform specific methods not available - VM not supported
    }
    ...
    If (LAnd(LNot(DISA), LNot(HDAA))) {
      // Diplay OFF and Audio in D3
      // 1) CPPMVRIC (0x31Ch[22]) = 0, 24MHz Crystal Shutdown Qualification Disable (XTALSDQDIS)
      // 2) CPPMVRIC (0x31Ch[13]) = 1, SLP_S0# Low Voltage Mode Enable (SLPS0LVEN)
      // 3) Enable VM - call VMON method
      Store(0, XSQD)
      Store(1, SLS0)
      \_SB.VMON()  // Enable voltage margining - platform specific
    } Else {
      // 1) CPPMVRIC (0x31Ch[22]) = 1, 24MHz Crystal Shutdown Qualification Disable (XTALSDQDIS)
      // 2) CPPMVRIC (0x31Ch[13]) = 0, SLP_S0# Low Voltage Mode Enable (SLPS0LVEN)
      // 3) Disable VM  - call VMOF method
      Store(1, XSQD)
      Store(0, SLS0)
      \_SB.VMOF()  // Disable voltage margining - platform specific
    }
  }
}
//
...
Include ("SerialIoDevices.asl")                // AdvancedFeaturesContent
