...
extern ASM_PFX(CpuInitFloatPointUnit)
...
;
        pause
        cmp         qword [esi+CProcedureLocation], 0
...
;
        ...
        mov         edi, esi
        add         edi, BistBuffer
        mov         ecx, dword [edi + 8 * ebx]    ; RCX = CpuNumber
...
CheckWakeUpCounterInit64:
        ...
        cmp         dword [esi], WakeUpApCounterInit
...
;
        xor         rcx, rcx
        mov         qword [rsp +  0], rcx         ; BreakToRunApSignal
        mov         qword [rsp +  8], rcx         ; HltLoopBreakCounter
        mov         qword [rsp + 16], rcx         ; MwaitLoopBreakCounter
        mov         qword [rsp + 24], rcx         ; RunLoopBreakCounter
        mov         qword [rsp + 32], rcx         ; MwaitLoopBreakCounter32
        mov         qword [rsp + 40], rcx         ; RunLoopBreakCounter32
        mov         qword [rsp + 48], rcx         ; WakeUpApVectorChangeFlag
        mov         qword [rsp + 56], rcx         ; MwaitTargetCstate
...
ApMwaitLoop64:
        ...
        mov         rax, qword [rsp + 56]         ; Mwait Target C-State per rax[7:4]
        DB          0fh, 1, 0c9h                      ; MWAIT

CheckRunSignal64:

        cmp         qword [rsp], rbx              ; Check if run signal correct?
...
CheckWakeUpManner32:
        ...
        pause
        cmp         dword [rsi], WakeUpApPerMwaitLoop32                    ; Use rsi for esi per compling in 64-bits mode
        ...
        xor         edx, edx
        DB          0fh, 1, 0c8h                      ; MONITOR
        mov         eax, dword [rsp + 56]         ; Mwait Target C-State per eax[7:4]
        DB          0fh, 1, 0c9h                      ; MWAIT


CheckRunSignal32:

        cmp         dword [rsp], ebx              ; Check if run signal correct?
...
ASM_PFX(AsmGetAddressMap):
        ...
        mov         qword [rcx+18h], LongModeStart - RendezvousFunnelProcStart
        mov         qword [rcx+20h], LONG_JUMP - RendezvousFunnelProcStart
        mov         qword [rcx+28h], RendezvousFunnelProcEnd - RendezvousFunnelProcStart
...
LockObtained1:
        mov         byte [rsi + 1], CPU_SWITCH_STATE_STORED
        xchg        al, byte [rsi]
...
LockObtained3:
        mov         byte [rsi+1], CPU_SWITCH_STATE_LOADED
        xchg        al, byte [rsi]
...
extern ASM_PFX(mOrigChgWakeUpBufferDelta)
extern ASM_PFX(mCpuCountSync)

global ASM_PFX(AsmRelocateApsInMwaitLoop)
ASM_PFX(AsmRelocateApsInMwaitLoop):

        mov       rax, ASM_PFX(mOrigChgWakeUpBufferDelta)
  mov       rax, [rax]
  add       [rsp], rax                ;Update return Address
        add       [rsp + 28h], rax          ;Update rdi
        add       [rsp + 30h], rax          ;Update rsi
        add       [rsp + 40h], rax          ;Update rbp

        mov       edx, [rbp]                ;Read compatibility 32-bit address.
        add       edx, eax                  ;Adjust
        mov       [rbp + rax], edx          ;Save

        mov       qword [rsp + 48h], 0  ;Clear BREAK_TO_RUN_AP_SIGNAL
        mov       rax,  ASM_PFX(mCpuCountSync)
        lock      dec   dword [rax]
        ret
