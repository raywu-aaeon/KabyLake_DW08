...
GLOBAL_REMOVE_IF_UNREFERENCED INTERRUPT_GATE_DESCRIPTOR gIdtTable[INTERRUPT_VECTOR_NUMBER];

INTERRUPT_GATE_DESCRIPTOR gOrigIdtTable[INTERRUPT_VECTOR_NUMBER];
  ...
  )
{
  ...
  if (PcdGetBool(PcdSourceDebugEnable) == TRUE) {
    //
  ...
  )
{
  ...
  if (ComplementBits != 0) {
    ...
    ///
    PreMtrrChange ();
    for (MsrNum = MSR_IA32_MTRR_PHYSBASE0;
         MsrNum < (MSR_IA32_MTRR_PHYSBASE0 + (Msr.Bits.VCNT * 2) - 1);
         MsrNum += 2
         ) {
  ...
  )
{
  ...
  Status = AllocateReservedMemoryBelow4G (
             TotalCpusForThisSystem * STACK_SIZE_PER_PROC,
             StackAddressStart
             );
  ...
  )
{
  ...
  ///
  Status = AllocateAlignedReservedMemoryBelow4G (
             IdtrForBsp.Limit + 1,
             8,
             (VOID **) &IdtForAp
             );
