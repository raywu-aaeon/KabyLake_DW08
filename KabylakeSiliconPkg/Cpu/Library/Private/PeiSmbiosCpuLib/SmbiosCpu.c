/** @file
  ...
  create SMBIOS CPU HOBs appropriately.
  ...
  )
{
  ...
  GetEnabledCount (NULL, NULL, NULL, &TotalNumberOfSockets);

  for (CurrentSocketNumber = 0; CurrentSocketNumber < TotalNumberOfSockets; CurrentSocketNumber++) {
    InitializeSmbiosCacheInfoHobs (CurrentSocketNumber);
    InitializeSmbiosProcessorInfoHob (TotalNumberOfSockets, CurrentSocketNumber);
  }
