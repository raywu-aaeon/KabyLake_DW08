  ...
  )
{
  ...
  ///
  /// Call HWP SMI Initialization
  ///
  if (CpuConfigData->Hwp > 0) {
    DEBUG ((DEBUG_INFO, " Install PowerMgmt Hardware P-state \n"));
    InitPowerMgmtHwpLvt ();
  }

  ///
  /// Call HDC SMI Initialization
  ///
  if (CpuConfigData->HdcControl == 1) {
    DEBUG ((DEBUG_INFO, " Install PowerMgmt Hardware Duty Cycle \n"));
    InitPowerMgmtHdc ();
  }
