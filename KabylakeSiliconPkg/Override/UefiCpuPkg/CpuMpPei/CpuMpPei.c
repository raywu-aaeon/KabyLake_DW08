...
{
// SICO-KBL-0005: Royal Park Begin
#ifndef FSP_FLAG
// SICO-KBL-0005: Royal Park End
...
#else
  //
  // For FSP, this must be a fixed address with fixed size.
  //
  ASSERT(WakeupBufferSize == EFI_PAGE_SIZE);

  //
  // Memory HOB for FSP is built in KabylakeFspPkg\FspInit\FspInitPreMem.c function MemoryDiscoveredPpiNotifyCallback.
  //

  return PcdGet32 (PcdFspCpuPeiApWakeupBufferAddr);
#endif
// SICO-KBL-0005: Royal Park End
}
  ...
  )
{
  ...
  UINTN                         WakeupBufferSizeAligned4k; // SICO-KBL-0005: Royal Park Content
...
// SICO-KBL-0005: Royal Park Begin
  WakeupBufferSizeAligned4k = (WakeupBufferSize + SIZE_4KB - 1) & ~(SIZE_4KB - 1);
  WakeupBuffer     = GetWakeupBuffer (WakeupBufferSizeAligned4k);
  ASSERT (WakeupBuffer != (UINTN) -1);

  DEBUG ((EFI_D_INFO, "CpuMpPei: WakeupBuffer = 0x%x\n", WakeupBuffer));
  DEBUG ((EFI_D_INFO, "CpuMpPei: WakeupBufferSizeAligned4k = 0x%x\n", WakeupBufferSizeAligned4k));
// SICO-KBL-0005: Royal Park End
  ...
  )
{
  ...
  //
  // SICO-KBL-0006: RoyalParkOverrideBegin
  //
  IA32_DESCRIPTOR      Gdtr;
  VOID                 *GdtInMemory;
  //
  // SICO-KBL-0006: RoyalParkOverrideEnd
  //
...
*/
  ...
  //
  // Check if GDT is stored in flash.
  //

  if (Gdtr.Base >= (UINTN) (SIZE_4GB - SIZE_16MB)) {

    Status = (*PeiServices)->AllocatePool (PeiServices, Gdtr.Limit + 1, &GdtInMemory);
    ASSERT_EFI_ERROR (Status);

    //
    // Copy GDT from flash to memory.
    //

    CopyMem(GdtInMemory, (VOID *)Gdtr.Base, Gdtr.Limit + 1);

    //
    // Set new GDT.
    //

    Gdtr.Base = (UINTN) GdtInMemory;
    AsmWriteGdtr (&Gdtr);
  }
