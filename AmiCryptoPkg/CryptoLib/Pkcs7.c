...
struct x509_certificate * Pkcs7_return_signing_cerificate(struct pkcs7_signed_data_st *SignedData)
{
    ...
    wpa_printf(MSG_DEBUG, "PKCS7: Signing certificate Issuer: %a", buf);
#endif
...
// Compare Name and Serial Number
    ...
    wpa_printf(MSG_DEBUG, "PKCS7: Signing certificate found!");
 ...
 * Returns: 0 if operation successful, -1 if not
 */
