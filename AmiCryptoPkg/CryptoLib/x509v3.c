...
//		x509_certificate_free(cert);
	...
	pos = hdr.payload;
	if (*pos) {
		wpa_printf(MSG_DEBUG, "X509: Signature : BITSTRING - %d unused bits", *pos);
		wpa_printf(MSG_DEBUG, "PKCS #1 v1.5 10.2.1: "
...
#endif
	// AMI
	if(0 != crypto_calculate_digest(hashType, &cert->tbs_cert_start, &cert->tbs_cert_len, hash, &hash_len))
		return -1;
