...
**/
        ...
        case x509GetSubjectsCommonNameStr:
            if(Data && DataSize) {
                err = Pkcs7_x509_return_SubjectNameStr((UINT8*)P7Data, P7Size, *Data, (size_t)*DataSize );
            }
            break;
...
)
{
    ...
    pFwKeyHob = GetEfiConfigurationTable(SystemTable, &gEfiHobListGuid);
    if (pFwKeyHob!=NULL) {
  ...
  )
{
    ...
    if(AlgId == NULL || Key == NULL || Key->Blob == NULL)
        return EFI_INVALID_PARAMETER;
