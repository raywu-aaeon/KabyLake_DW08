...
/**
  ...
  @param  Length                Received buffer length.		// AMI PORTING
  ...
  )
{
  ...
  CHAR8                 *QueryName;
  UINT32                QueryNameLen;		// AMI PORTING
  DNS_QUERY_SECTION     *QuerySection;
  ...
  EFI_STATUS            Status;
  UINT32                RemainingLength;		// AMI PORTING
  ...
  *Completed       = TRUE;
  ...
  // Get query section
  //
  QuerySection = (DNS_QUERY_SECTION *) (QueryName + QueryNameLen);	// AMI PORTING
  ...
  )
{
  ...
  Len = Packet->TotalSize;		// AMI PORTING
  ...
  // Parse Dns Response
  //
  ParseDnsResponse (Instance, RcvString, Len, &Completed);		// AMI PORTING
