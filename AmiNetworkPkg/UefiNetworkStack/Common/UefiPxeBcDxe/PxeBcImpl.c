...
//
// AMI PORTING START : Pint MAC address on PXE boot screen.
//
/**
...
//
// AMI PORTING END : Print MAC address on PXE boot screen.
//
#endif 
  ...
  )
{
  ...
  EFI_STATUS              Status;
  //
  // AMI PORTING START : Print MAC address on PXE boot screen.
  //
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
  ...
  //
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
  ZeroMem (&ClientMacAddress, sizeof (EFI_MAC_ADDRESS));   
  NetLibGetMacAddress (Private->Controller, &ClientMacAddress, &HwAddressSize);
#endif  
  //
  // AMI PORTING END : Print MAC address on PXE boot screen.
  //
  if (Mode->UsingIpv6) {
    ...
    // AMI PORTING : Print MAC address on PXE boot screen.
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
    PxeBootShowMacAddr (&ClientMacAddress);
#endif
    //
    // AMI PORTING START : Added to support the PXE abort using ESC key.
    //
