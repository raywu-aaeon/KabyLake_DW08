  ...
  )
{
  ...
  *BufferSize = 0;
  ...
  ZeroMem (&Private->ServerIp, sizeof (EFI_IP_ADDRESS)); //  AMI PORTING - For handling the failure to boot the second PXE server among 2 connected servers EIP - 215070
  ...
  )
{
  ...
  PXEBC_DHCP_PACKET_CACHE    *Cache=NULL;
  PXEBC_VENDOR_OPTION        *VendorOpt=NULL;
  //
  // AMI PORTING ENDS 
  //
  ...
  else  // //  AMI PORTING - For handling ESC key from UEFI PXE boot menu
  {
      // According to the PXE specification 2.1, Table 2-1 PXE DHCP Options,
      // we must not consider a boot prompt or boot menu if all of the following hold:
      //   - the PXE_DISCOVERY_CONTROL tag(6) is present inside the Vendor Options(43), and has bit 3 set  
	  ...
	  if( (EFI_ABORTED == Status) && !(IS_DISABLE_PROMPT_MENU (VendorOpt->DiscoverCtrl) &&
           Cache->Dhcp4.OptList[PXEBC_DHCP4_TAG_INDEX_BOOTFILE] != NULL) )
		  return Status;
  }
