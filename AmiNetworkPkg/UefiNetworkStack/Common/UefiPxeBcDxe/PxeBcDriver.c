  ...
  )
{
 ...
 //
 CHAR16                           IAID[]=L"_IAIDPXE", *TempBuffer;
 CHAR16                          *MacString;
 EFI_STATUS                       Status;	
 //  
 ...
 //
  ...
  TempBuffer = AllocateZeroPool ( (StrLen(MacString) + StrLen(IAID) + 1) * sizeof(CHAR16) );
  if (TempBuffer != NULL) {
      //
  ...
  )
{
  ...
  if (IpVersion == IP_VERSION_4) {
	...
	//
    if (NetworkStackSetupData.Ipv4Pxe == 0)  return EFI_UNSUPPORTED;
  } else {
    //
    // Check whether PxeIpv6 is disabled in setup if so return EFI_UNSUPPORTED.
    //
    if (NetworkStackSetupData.Ipv6Pxe == 0)  return EFI_UNSUPPORTED;
  }
  ...
  )
{
  ...
  if (IpVersion == IP_VERSION_4) {
    ...
    //
#if (LOAD_DRIVER_ONLY_ON_NWSTACK_ENABLE == 1)	   // AMI PORTING.  
    if (NetworkStackSetupData.Ipv4Pxe == 1) 
#endif	    
    ...
    //
#if (LOAD_DRIVER_ONLY_ON_NWSTACK_ENABLE == 1)      // AMI PORTING.
    if (NetworkStackSetupData.Ipv6Pxe == 1)  
#endif	    
