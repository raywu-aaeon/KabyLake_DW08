  ...
  )
{
  ...
  // Only accept either http or https URI because for empty URI, 
  // BDS will already create an HTTP boot option for empty URI.
  if(StrLen(Uri) == 0)
	  return EFI_INVALID_PARAMETER;
  if((StrnCmp(Uri, L"http://", 7) != 0) && (StrnCmp(Uri, L"https://", 8) != 0))
	  return EFI_INVALID_PARAMETER;
  ...
  )
{
  ...
  case KEY_INITIATOR_URI:
	...
	// AMI porting Start to fix coverty error
    if(Uri == NULL) {
    	return EFI_INVALID_PARAMETER;
    }  
	// AMI porting End      
    ...
    if (StrLen (Uri) != 0) {
      ...
      if (Status == EFI_INVALID_PARAMETER) {
        ...
        DEBUG_RAYDEBUG ((-1, "HttpBootFormCallback: %r.\n", Status));
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    // Append MAC string in the menu Title string
    //
    OldMenuString = HiiGetString (
                          CallbackInfo->RegisteredHandle, 
