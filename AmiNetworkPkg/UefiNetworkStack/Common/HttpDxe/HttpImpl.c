  ...
  )
{
  ...
  //
  // Only support GET, HEAD, PATCH, PUT, DELETE and POST method in current implementation.
  //
  ...
  )
{
  ...
  UINT64                        DynamicValue; //AMI PORTING
  ...
  //AMI PORTING START- Use dynamic pcd PcdHttpResponseTimeout instead of the value HTTP_RESPONSE_TIMEOUT
  DynamicValue = PcdGetEx64(&gEfiNetworkPkgTokenSpaceGuid,PcdHttpResponseTimeout);
  //AMI PORTING ENDS- Use dynamic pcd PcdHttpResponseTimeout instead of the value HTTP_RESPONSE_TIMEOUT
  ...
  if (HttpMsg->Data.Response != NULL) {
    ...
    //
//AMI PORTING START- Use dynamic pcd PcdHttpResponseTimeout instead of the value HTTP_RESPONSE_TIMEOUT
    Status = gBS->SetTimer (HttpInstance->TimeoutEvent, TimerRelative, DynamicValue * TICKS_PER_SECOND);
//AMI PORTING ENDS- Use dynamic pcd PcdHttpResponseTimeout instead of the value HTTP_RESPONSE_TIMEOUT
    if (EFI_ERROR (Status)) {
      goto Error;
    }
