  ...
  )
{
  ...
  if (CacheEntry != NULL) {
    ...
    if (CacheEntry == NULL) {
      DEBUG_RAYDEBUG ((-1, "ArpAdd: Failed to allocate pool for CacheEntry.\n"));
  ...
  )
{
  ...
  if (RequestContext == NULL) {
    DEBUG_RAYDEBUG ((-1, "ArpRequest: Allocate memory for RequestContext failed.\n"));
  ...
  if (CacheEntry != NULL) {
    ...
    CacheEntry = ArpAllocCacheEntry (Instance);
    if (CacheEntry == NULL) {
      DEBUG_RAYDEBUG ((-1, "ArpRequest: Allocate memory for CacheEntry failed.\n"));
