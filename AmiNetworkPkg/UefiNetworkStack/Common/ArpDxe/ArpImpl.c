...
RESTART_RECEIVE:
  ...
  DEBUG_CODE (
    if (EFI_ERROR (Status)) {
      DEBUG_RAYDEBUG ((-1, "ArpOnFrameRcvd: ArpService->Mnp->Receive "
        "failed, %r\n.", Status));
    }
  );
}
  ...
  )
{
  ...
  DEBUG_CODE (
    if (EFI_ERROR (TxToken->Status)) {
      DEBUG_RAYDEBUG ((-1, "ArpOnFrameSent: TxToken->Status, %r.\n", TxToken->Status));
    }
  );
  ...
  if (ConfigData != NULL) {
    ...
    if (Instance->Configured) {
      ...
      if (OldConfigData->StationAddress == NULL) {
        DEBUG_RAYDEBUG ((-1, "ArpConfigInstance: AllocatePool for the StationAddress "
          "failed.\n"));
  ...
  )
{
  ...
  if (TxToken == NULL) {
    DEBUG_RAYDEBUG ((-1, "ArpSendFrame: Allocate memory for TxToken failed.\n"));
    return;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG_RAYDEBUG ((-1, "ArpSendFrame: CreateEvent failed for TxToken->Event.\n"));
    goto CLEAN_EXIT;
  }
  ...
  if (TxData == NULL) {
    DEBUG_RAYDEBUG ((-1, "ArpSendFrame: Allocate memory for TxData failed.\n"));
    goto CLEAN_EXIT;
  }
  ...
  if (Packet == NULL) {
    DEBUG_RAYDEBUG ((-1, "ArpSendFrame: Allocate memory for Packet failed.\n"));
    ASSERT (Packet != NULL);
  }
  ...
  *(UINT16 *)TmpPtr = HTONS (ARP_ETHER_PROTO_TYPE);
  ...
  if (EFI_ERROR (Status)) {
    DEBUG_RAYDEBUG ((-1, "Mnp->Transmit failed, %r.\n", Status));
    goto CLEAN_EXIT;
  }
  ...
  )
{
  ...
  if (FindData == NULL) {
    DEBUG_RAYDEBUG ((-1, "ArpFindCacheEntry: Failed to allocate memory.\n"));
