  ...
  )
{
  ...
  for (Index = IpInstance->GroupCount; Index > 0 ; Index--) {
    Group = 0;  	  // AMI porting to initialise Group to 0 to fix coverty error
    if(IpInstance->Groups != NULL) {   // AMI porting if condition starts to fix coverty error
  	  Group = IpInstance->Groups[Index - 1];
  	}   // AMI porting if condition ends to fix coverty error
