...
// AMI PORTING START
  ...
  #if (PRESERVE_IP4_CONFIG_VARIABLE_SUPPORT)
    #define                 MAC_STRING_SIZE 24  // 2 * AABBCCDDEEFF
  ...
  )
{
  ...
  if (IfrFromNvData->DhcpEnable == 0) {
	...
	// AMI PORTING START
    CopyMem (IfrNvData->StationAddress, IfrFromNvData->StationAddress, sizeof(IfrFromNvData->StationAddress));
    CopyMem (IfrNvData->SubnetMask, IfrFromNvData->SubnetMask, sizeof(IfrFromNvData->SubnetMask));
    CopyMem (IfrNvData->GatewayAddress, IfrFromNvData->GatewayAddress, sizeof(IfrFromNvData->GatewayAddress));
    CopyMem (IfrNvData->DnsAddress, IfrFromNvData->DnsAddress, sizeof(IfrFromNvData->DnsAddress));
    goto Exit;
	// AMI PORTING END
  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGED) {
   ...
   IpSb = IP4_SERVICE_FROM_IP4_CONFIG2_INSTANCE (Instance);
    ...
    case KEY_SAVE_CHANGES:
  ...
  #if (PRESERVE_IP4_CONFIG_VARIABLE_SUPPORT)
      // To Preserve IP4 Config2 Variable for this MAC
    #if (RT_ACCESS_SUPPORT_IN_HPKTOOL) 
      ...
      {
        MACListSize = 0;
        MACList = AllocateZeroPool( MAC_STRING_SIZE );
        if (MACList == NULL) {
          return EFI_OUT_OF_RESOURCES;
        }
        
        Status = gRT->GetVariable(L"MACNamesListVar",
        ...
        {
           MACListSize = MAC_STRING_SIZE;   
           CopyMem(MACList, IpSb->MacString, MAC_STRING_SIZE); 
           Status =gRT->SetVariable(L"MACNamesListVar",
                                   &MACNamesListVarDataGuid,
                                   Attributes,
                                   MACListSize,
                                   MACList
        ...
        else if (Status == EFI_BUFFER_TOO_SMALL)
        {
            ...
            MACList = AllocateZeroPool( MACListSize );
            ...
            MACList = ReallocatePool (MACListSize, MACListSize + MAC_STRING_SIZE, MACList);
            CopyMem( ((UINT8*)MACList + MACListSize), IpSb->MacString, MAC_STRING_SIZE);
            MACListSize += MAC_STRING_SIZE;
            Status =gRT->SetVariable(L"MACNamesListVar",
                                     &MACNamesListVarDataGuid,
                                     Attributes,
       ...
       // To Preserve IP4 Config2 Variable for this MAC
  #endif // PRESERVE_IP4_CONFIG_VARIABLE_SUPPORT
      
#endif // NWSTACK_CONFIGURE_IP
      //
	  // AMI PORTING END
	  //
