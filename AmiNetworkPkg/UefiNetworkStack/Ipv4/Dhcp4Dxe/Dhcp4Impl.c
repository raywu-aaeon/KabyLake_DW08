...
#endif
  BOOLEAN                   MediaPresent; // AMI PORTING
...
// AMI PORTING STARTS
//
#if (NET_FAIL_SAFE_DHCP4_CLIENT_ADDR)

DHCP_SERVICE	*gFailSafeDhcpSb;	// DHCP Service Binding Instance to get ClientAddr.
BOOLEAN			gIsClientAddrPresentInToken = TRUE;	// If this is false, ClientAddr was retrived from gFailSafeDhcpSb.
IP4_ADDR		gFailSafeClientAddr = 0;	// IP4 address retrieved from gFailSafeDhcpSb will be copied into this variable.

/**
  Function to store the ClientAddr if there is no IP address available in
  EFI_DHCP4_TRANSMIT_RECEIVE_TOKEN.

  @param[in] FailSafeInstance		A pointer to DHCP_PROTOCOL where the ClientAddr is available.

  @retval[out]	EFI_SUCCESS			If the ClientAddr is copied successfully from the DHCP_PROTOCOL.
  				EFI_NO_MAPPING		If the ClientAddr is not available in the DHCP_PROTOCOL also.

**/
EFI_STATUS
FailSafeLock(DHCP_PROTOCOL  *FailSafeInstance)
{
	gFailSafeDhcpSb = FailSafeInstance->Service;
	gFailSafeClientAddr = gFailSafeDhcpSb->ClientAddr;
	if (gFailSafeClientAddr != 0)
	{
		gIsClientAddrPresentInToken = FALSE;
		return EFI_SUCCESS;
	}
	return EFI_NO_MAPPING;
}
#endif	// NET_FAIL_SAFE_DHCP4_CLIENT_ADDR
//
// AMI PORTING ENDS
//
  ...
  )
{
  ...
  //
#if (NET_FAIL_SAFE_DHCP4_CLIENT_ADDR)
  // If no client address is present in Token->Packet->Dhcp4.Header.ClientAddr, 
  // see if gFailSafeClientAddr has and copy it to ClientAddr.
  if(ClientAddr == 0 && !gIsClientAddrPresentInToken) {
	  ClientAddr = gFailSafeClientAddr;
	  gIsClientAddrPresentInToken = TRUE;
  }
#endif	// NET_FAIL_SAFE_DHCP4_CLIENT_ADDR
  //
  // AMI PORTING ENDS
...
#if (NET_FAIL_SAFE_DHCP4_CLIENT_ADDR)
	  // If no client address is present in Token->Packet->Dhcp4.Header.ClientAddr,
	  // get it from Instance->ServiceClientAddr
	  Status = FailSafeLock(Instance);
	  if(EFI_ERROR(Status)) {
#endif	// NET_FAIL_SAFE_DHCP4_CLIENT_ADDR
	  //
	  // AMI PORTING ENDS
