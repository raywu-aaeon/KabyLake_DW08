...
**/
typedef struct {
    UINT16                    PeerSegmentGroupNumber;
    UINT8                     PeerBusNumber;
    UINT8                     PeerDeviceFunctionNumber;
    UINT8                     PeerBusDataWidth;
} PEER_DEVICE;

typedef struct {                                        // Type 9
    UINT8                     CurrentUsage[NUMBER_OF_SYSTEM_SLOTS];             // 3.3.10.3
    UINT8                     BusNumber[NUMBER_OF_SYSTEM_SLOTS];
    UINT8                     BaseDataBusWidth[NUMBER_OF_SYSTEM_SLOTS];
    UINT8                     PeerCount[NUMBER_OF_SYSTEM_SLOTS];
    PEER_DEVICE               PeerDevice[NUMBER_OF_SYSTEM_SLOTS][MAX_PEER_GROUPING_COUNT];
...
typedef struct {                                        // Type 17
    ...
    UINT16                    MemoryOpModeCap;
    UINT8                     FirmwareVersion;
    UINT16                    ModuleManufacturerId;
    UINT16                    ModuleProductId;
    UINT16                    MemorySubContManId;
    UINT16                    MemorySubContProdId;
    UINT64                    NonVolatileSize;          // 0 for no non-volatile portion. 0xffffffffffffffff for unknown
    UINT64                    VolatileSize;             // 0 for no volatile portion. 0xffffffffffffffff for unknown
    UINT64                    CacheSize;                // 0 for no cache portion. 0xffffffffffffffff for unknown
    UINT64                    LogicalSize;              // Size of Logical memory device in bytes. 0xffffffffffffffff for unknown
} AMI_MEMORY_DEVICE;
...
#if (defined(SMBIOSDYNAMICDATA_HEADER_VERSION) && (SMBIOSDYNAMICDATA_HEADER_VERSION == 1))
    MEM_ARRAY_MAP_ADDR        MemArrayMapAddr[MAX_NUMBER_MEMORY_REGION];            // Type 19
#else
...
EFI_STATUS EFIAPI CreateBaseBoardDataForSMBios();
EFI_STATUS EFIAPI CreateChassisDataForSMBios();
EFI_STATUS EFIAPI CreateCPUDataForSMBios();
EFI_STATUS EFIAPI CreateSlotDataForSMBios();
EFI_STATUS EFIAPI CreateOnBoardDevDataForSMBios();
EFI_STATUS EFIAPI CreateBatteryDataForSMBios();
EFI_STATUS EFIAPI CreateOnBoardDevExtInfoForSMBios();
EFI_STATUS EFIAPI EnableSmbusController();
EFI_STATUS EFIAPI RestoreSmbusController();
EFI_STATUS EFIAPI GetSpdByte(EFI_SMBUS_DEVICE_ADDRESS SpdAddr, UINTN Offset, UINT8 *Data);
VOID EFIAPI      OemUpdate();
