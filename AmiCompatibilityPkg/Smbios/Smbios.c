...
// ===== TYPE17_STRUCTURE =====
                                              ...
                                              {17, 8, 0x04, 16, 1},
#endif
#if (A1_MEMORY_SOCKETS > 8)
                                              {17, 9, 0x04, 16, 1},
#endif
#if (A1_MEMORY_SOCKETS > 9)
                                              {17, 10, 0x04, 16, 1},
#endif
#if (A1_MEMORY_SOCKETS > 10)
                                              {17, 11, 0x04, 16, 1},
#endif
#if (A1_MEMORY_SOCKETS > 11)
                                              {17, 12, 0x04, 16, 1},
#endif
#if (A1_MEMORY_SOCKETS > 12)
                                              {17, 13, 0x04, 16, 1},
#endif
#if (A1_MEMORY_SOCKETS > 13)
                                              {17, 14, 0x04, 16, 1},
#endif
#if (A1_MEMORY_SOCKETS > 14)
                                              {17, 15, 0x04, 16, 1},
#endif
#if (A1_MEMORY_SOCKETS > 15)
                                              {17, 16, 0x04, 16, 1},
#endif
// ===== TYPE19_STRUCTURE =====
                                              {19, 1, 0x0c, 16, 1},
#if (MAX_NUMBER_MEMORY_REGION > 1)
                                              {19, 2, 0x0c, 16, 1},
#endif
#if (MAX_NUMBER_MEMORY_REGION > 2)
                                              {19, 3, 0x0c, 16, 1},
#endif
#if (MAX_NUMBER_MEMORY_REGION > 3)
                                              {19, 4, 0x0c, 16, 1},
#endif
// ===== TYPE20_STRUCTURE =====
#if TYPE20_STRUCTURE
...
#if AMI_SMBIOS_DBG_MSG_SUPPORT == 1
...
)
{
    ...
    // Replace string to be deleted with " " as space holder
    *(UINT16*)StructPtr = (UINT16)0x0020;
    StructPtr += 2;
...
)
{
    ...
    if (NvramData != NULL) {
        ...
        // Height field - Offset 0x11 (Test Bit19)
        if (NvramData->Flag & 0x00080000) {
            ((SMBIOS_SYSTEM_ENCLOSURE_INFO*)TempBuffer)->Height = NvramData->Height;
        }
...
)
{
    ...
    Count = ((SMBIOS_OEM_STRINGS_INFO*)SrcBuffer)->Count;
    for (i = 0; i < Count; i++) {
        if (NvramData->Flag[i] == 1) {
            TestFlag = 1;
            break;
        }
    }
...
)
{
    ...
    Count = ((SMBIOS_SYSTEM_CONFIG_INFO*)SrcBuffer)->Count;
    for (i = 0; i < Count; i++) {
        if (NvramData->Flag[i] == 1) {
            TestFlag = 1;
            break;
...
#if (defined(SmbiosDMIEdit_SUPPORT) && (SmbiosDMIEdit_SUPPORT != 0))
/**
    Validate the size of an ASCII string
...
)
{
    ...
    if ((Size == 0) || (String[Size - 1] != 0)){    // Size is NVARM variable string size including zero terminator
        return FALSE;
    }

    if (Size < MAX_NVRAM_STRING_SIZE) {
        Size = AsciiStrnLenS(String, MAX_NVRAM_STRING_SIZE);    // Actual string size
        if ((Size != 0) && (Size != MAX_NVRAM_STRING_SIZE)) {
            while (Size) {
                AsiiChar = String[Size - 1];
                if ((AsiiChar < 0x20) || (AsiiChar == 0x7f)) {  // Only allow printable character
                    return FALSE;
                }
...
)
{
...
#if (TYPE2_STRUCTURE == 1)
    Status = pBS->AllocatePool(EfiBootServicesData, NUMBER_OF_BASEBOARDS * sizeof(UINT16), (void**)&Type2Handle);
    ASSERT_EFI_ERROR(Status);
    Status = pBS->AllocatePool(EfiBootServicesData, NUMBER_OF_BASEBOARDS * sizeof(SMBIOS_NVRAM_TYPE2), (void**)&NvramType2);
    ASSERT_EFI_ERROR(Status);
...
#endif                                      // TYPE2_STRUCTURE
#if (TYPE3_STRUCTURE == 1)
    Status = pBS->AllocatePool(EfiBootServicesData, NUMBER_OF_SYSTEM_CHASSIS * sizeof(UINT16), (void**)&Type3Handle);
    ASSERT_EFI_ERROR(Status);
    Status = pBS->AllocatePool(EfiBootServicesData, NUMBER_OF_SYSTEM_CHASSIS * sizeof(SMBIOS_NVRAM_TYPE3), (void**)&NvramType3);
    ASSERT_EFI_ERROR(Status);
    ...
    Status = pBS->AllocatePool(EfiBootServicesData, gSmbiosBoardProtocol->NumberOfBatteries * sizeof(UINT16), (void**)&Type22Handle);
    ASSERT_EFI_ERROR(Status);
    Status = pBS->AllocatePool(EfiBootServicesData, gSmbiosBoardProtocol->NumberOfBatteries * sizeof(SMBIOS_NVRAM_TYPE22), (void**)&NvramType22);
    ASSERT_EFI_ERROR(Status);
    for (i = 0; i < gSmbiosBoardProtocol->NumberOfBatteries; i++) {
        Type22Handle[i] = 0;
        NvramType22[i].Flag = 0;
    }
    Status = pBS->AllocatePool(EfiBootServicesData, NUMBER_OF_POWER_SUPPLY * sizeof(UINT16), (void**)&Type39Handle);
    ASSERT_EFI_ERROR(Status);
    Status = pBS->AllocatePool(EfiBootServicesData, NUMBER_OF_POWER_SUPPLY * sizeof(SMBIOS_NVRAM_TYPE39), (void**)&NvramType39);
    ASSERT_EFI_ERROR(Status);
                    ...
                    case 3  :   {
                                    ...
                                    if (gSmbiosBoardProtocol->SysChassisInfoSupport) {
                                        ...
                                        if (i < NUMBER_OF_SYSTEM_CHASSIS) {
                                            ...
                                            if (((TABLE_INFO*)FlashDataPtr)->Offset == 0x11) {
                                                MemCpy(&NvramType3[i].Height, FlashDataPtr + sizeof(TABLE_INFO), 1);
                                                NvramType3[i].Flag |= 0x00080000;               // New Height addition
                                                UpdateFlag = TRUE;
                                            }
...
#else                                       // SMBIOS_DMIEDIT_DATA_LOC
{
    ...
    Status = pBS->AllocatePool(EfiBootServicesData, DmiArraySize, (void**)&DmiArray);
    ASSERT_EFI_ERROR(Status);
        ...
        for (Index = 0; Index < Count; Index++) {
            ...
            if (Flags & DMIEDIT_DELETE_STRUC) {
                ...
                if (Status == EFI_SUCCESS) {
                    ...
                    switch (Type) {
                        case 00:
                                if (ValidateString(DmiData, DmiDataSize)) {
                        ...
                        case 01:
                                ...
                                else if (ValidateString(DmiData, DmiDataSize)) {
                        ...
                        case 03:
                                if (gSmbiosBoardProtocol->SysChassisInfoSupport) {
                                    ...
                                    if (i < NUMBER_OF_SYSTEM_CHASSIS) {
                                        ...
                                        if (Offset == 0x05) {               // Type
                                        ...
                                        else if (Offset == 0x11) {          // Height
                                            if (DmiDataSize == 1) {
                                                NvramType3[i].Height = *(UINT8*)DmiData;
                                                NvramType3[i].Flag |= 0x00080000;   // New Height addition
                                            }
                                        }
                                            ...
                                            if (Offset == 0x04) {               // Manufacturer
                                                ...
                                                // SKU Number
                                                if (Offset == 0x15 + (((SMBIOS_SYSTEM_ENCLOSURE_INFO*)TempBuffer)->ElementCount *\
                                                        ((SMBIOS_SYSTEM_ENCLOSURE_INFO*)TempBuffer)->ElementRecordLength)) {
                        ...
                        case 22:
                                ...
                                if (i < gSmbiosBoardProtocol->NumberOfBatteries) {
                                    ...
                                    if (Offset == 0x09) {               // Device Chemistry
                                        if (DmiDataSize == 1) {
...
#endif
        Status = pBS->AllocatePool(EfiBootServicesData, BufferSize, (void**)&TempBuffer);
        ASSERT_EFI_ERROR(Status);
...
#if (defined(SMBIOSDYNAMICDATA_HEADER_VERSION) && (SMBIOSDYNAMICDATA_HEADER_VERSION == 1))
          CurCpu->ProcessorType = DynamicData->CpuData.ProcessorType;
#endif
...
EFI_STATUS
UpdateType9Record(
    IN SMBIOS_SYSTEM_SLOTS_INFO     *StructurePtr,
    IN SYSTEM_SLOT_DYNAMIC_DATA     *DynamicData,
    IN UINT8                        index
)
{
    EFI_STATUS      Status;
    ...
    Size = sizeof(SMBIOS_SYSTEM_SLOTS_INFO);
    MemCpy(Buffer, StructurePtr, Size);
    Ptr = Buffer + Size;
    RemainingSize = GetStructureLength((UINT8*)StructurePtr) - (UINT16)Size;
    MemCpy(Buffer + 0x800, (UINT8*)StructurePtr + Size, RemainingSize);

    *Ptr = DynamicData->BaseDataBusWidth[index];
    Size++;
    Ptr++;

    *Ptr = DynamicData->PeerCount[index];
    Size++;
    Ptr++;

    if (DynamicData->PeerCount[index] != 0) {
        for (i = 0; i < DynamicData->PeerCount[index]; i++) {
            MemCpy(Ptr, &DynamicData->PeerDevice[index][i], sizeof(PEER_DEVICE));
            Size += sizeof(PEER_DEVICE);
            Ptr += sizeof(PEER_DEVICE);
        }
    }

    *(Buffer + 1) = (UINT8)Size;                // Update size in structure header
    MemCpy(Ptr, Buffer + 0x800, RemainingSize);
    Size += RemainingSize;
    Status = WriteStructureByHandle(StructurePtr->StructureType.Handle,
                                    Buffer,
                                    (UINT16)Size
                                   );

    pBS->FreePages((EFI_PHYSICAL_ADDRESS)Buffer, 1);
    return Status;
}
...
)
{
    ...
    Status = pBS->AllocatePool(EfiBootServicesData, SmbiosVariableSize, (void**)&SmbiosVariableBuffer);
    ASSERT_EFI_ERROR(Status);
    ...
    if (Status != EFI_NOT_FOUND) {
        for (i = 0; i < gSmbiosBoardProtocol->NumberOfSystemSlots; i++) {
            SrcBuffer = Buffer;
            if (FindStructureType(&SrcBuffer, &StructureFoundPtr, 9, i + 1)) {
                ...
                if (((SYSTEM_SLOT_DYNAMIC_DATA*)SmbiosVariableBuffer)->BaseDataBusWidth[i] != 0 ||
                    ((SYSTEM_SLOT_DYNAMIC_DATA*)SmbiosVariableBuffer)->PeerCount[i] != 0) {
                    // Dynamically create and replace SMBIOS Type 9 record with dynamic Peer Devices data
                    Status = UpdateType9Record((SMBIOS_SYSTEM_SLOTS_INFO*)StructureFoundPtr,
                                               (SYSTEM_SLOT_DYNAMIC_DATA*)SmbiosVariableBuffer,
                                               i
                                              );

                    if (EFI_ERROR(Status)) {
                        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to overwrite Smbios Slot structure\n"));
                    }
                }
            }
        }
...
)
{
    ...
    if (Strlen(StringData) == 0) {
        DeleteString(DestStructPtr, StringNum);
    }
    else {
...
#if (defined(SMBIOSDYNAMICDATA_HEADER_VERSION) && (SMBIOSDYNAMICDATA_HEADER_VERSION == 1))
            for (j = 0; j < gSmbiosBoardProtocol->NumberOfMemoryRegion; j++) {
                // Update Type 19
                if (FindStructureType(&SrcBuffer, &StructureFoundPtr, 19, 1)) {
                    SmbAmap = (SMBIOS_MEM_ARRAY_MAP_ADDR_INFO*)StructureFoundPtr;
                    SmbAmap->StartingAddress = DynArray->MemArrayMapAddr[j].StartingAddress;
                    SmbAmap->EndingAddress = DynArray->MemArrayMapAddr[j].EndingAddress;
                    SmbAmap->PartitionWidth = DynArray->MemArrayMapAddr[j].PartitionWidth;
                    SmbAmap->ExtendedStartAddr = DynArray->MemArrayMapAddr[j].ExtendedStartAddr;
                    SmbAmap->ExtendedEndAddr = DynArray->MemArrayMapAddr[j].ExtendedEndAddr;
                    if (SmbAmap->StartingAddress == 0 && SmbAmap->EndingAddress == 0) {
                        SrcBuffer = StructureFoundPtr;
                        DeleteStructureByHandle (SmbAmap->StructureType.Handle);
                        continue;
                    }
                }
            }
            ...
            for (j = 0; j < gSmbiosBoardProtocol->NumberOfMemorySlots[i]; j++)
            {
                ...
                if (FindStructureType(&SrcBuffer, &StructureFoundPtr, 17, 1)){
                    ...
                    if (SmbDevice->Size == 0) {
                        ...
                        SmbDevice->MemoryTechnology = 2;
                        SmbDevice->MemoryOpModeCap = 4;
                        SmbDevice->ModuleManufacturerId = 0;
                        SmbDevice->ModuleProductId = 0;
                        SmbDevice->MemorySubContManId = 0;
                        SmbDevice->MemorySubContProdId = 0;
                        SmbDevice->NonVolatileSize = 0;
                        SmbDevice->VolatileSize = 0;
                        SmbDevice->CacheSize = 0;
                        SmbDevice->LogicalSize = 0;
                    }
                    else {
#if AMI_SMBIOS_DBG_MSG_SUPPORT == 1
                        ...
                        SmbDevice->MemoryTechnology = DynDevice->Type17.MemoryTechnology;
                        SmbDevice->MemoryOpModeCap = DynDevice->Type17.MemoryOpModeCap;
                        SmbDevice->ModuleManufacturerId = DynDevice->Type17.ModuleManufacturerId;
                        SmbDevice->ModuleProductId = DynDevice->Type17.ModuleProductId;
                        SmbDevice->MemorySubContManId = DynDevice->Type17.MemorySubContManId;
                        SmbDevice->MemorySubContProdId = DynDevice->Type17.MemorySubContProdId;
                        SmbDevice->NonVolatileSize = DynDevice->Type17.NonVolatileSize;
                        SmbDevice->VolatileSize = DynDevice->Type17.VolatileSize;
                        SmbDevice->CacheSize = DynDevice->Type17.CacheSize;
                        SmbDevice->LogicalSize = DynDevice->Type17.LogicalSize;
                        ...
                        if (pHobList) {
                            ...
                            SrcBuffer += GetStructureLength(TempBuffer) - GetStructureLength((UINT8*)SmbDevice);
...
DynamicUpdateFlashMemoryData(VOID)
{
    ...
    *PartNumber++ = 0;
    ...
    while (1) {
        if (!FindStructureType(&Buffer, &FlashMemoryStructurePtr, 17, 1)) {
            break;
        }
        else {
...
#if SMBIOS_2X_SUPPORT
    UpdateEPSHeader();            // EPS 2.x Header in runtime memory
...
#if SMBIOS_3X_SUPPORT
    UpdateEPSHeader_3();        // EPS 3.0 Header in runtime memory
...
)
{
    ...
    EFI_STATUS              TableCopyStatus = EFI_NOT_FOUND;
        ...
        case ADD_STRUCTURE_BY_INDEX:
                ...
                if (Size > (MaximumTableSize - TableLength)) {
                    ...
                    if (SmbiosTableAtE000) {
                        ...
                        if (BeforeEndofDxe) {
                            ...
                            AllocatedMemory = (UINT8*)(UINTN)0xFFFFFFFF;
                            Status = pBS->AllocatePages(
                                        AllocateMaxAddress,
                ...
                if (Operation == ADD_STRUCTURE_BY_INDEX) {
                    ...
                    if (ADD_STRUCTURE_LOCATION == 1) {
                        ...
                        TempPtr2 = TempPtr;
                        if (FindStructureType(&TempPtr2, &TempPtr, 127, 1)) {
                ...
                if (FindStructureType(&MemoryPtr, &Type127, 127, 1)) {
...
GetFreeHandle(VOID)
{
    ...
    return 0xffff; // No available Handle to use;
}
...
/**
    Finalize Smbios Table location. Save and lock various pointers to NVRAM variables
    to be used by DmiEdit component
...
)
{
...
#if AMI_SMBIOS_DBG_MSG_SUPPORT == 1   
    DEBUG((DEBUG_INFO, "*** SmbiosEndOfDxe Entry ***\n"));
#endif
...
#if (CSM16_VERSION_MINOR >= 64)
    {
        ...
        if (LegacyRegionProtocol != NULL) {
            ...
            if (*(UINT32*)(UINTN)(0xf0000 + Legacy16Data->Compatibility16TableOfs) == CHR_I32('$', 'E', 'F', 'I')) {
#if SMBIOS_2X_SUPPORT
    ...
    pRS->SetVariable(L"SmbiosEntryPointTableF000",
                    &gAmiSmbiosNvramGuid,
                    EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                    sizeof(SMBIOS_TABLE_ENTRY_POINT*),
                    &pSmbiosTableEntryPointF000);
...
#if AMI_SMBIOS_DBG_MSG_SUPPORT == 1    
    DEBUG((DEBUG_INFO, "*** SmbiosEndOfDxe End ***\n"));
#endif
...
#if (defined(SB_WAKEUP_TYPE_FN) && (SB_WAKEUP_TYPE_FN == 1))
{
    ...
    if ((BootMode == BOOT_ON_S4_RESUME) || (BootMode == BOOT_ON_S5_RESUME)) {
        ...
        if (FindStructureType(&Buffer, &FoundPtr, 1, 1)) {
            ...
            if ((UINT32)(UINTN)FoundPtr < 0xfffff) {
...
#if ( defined(CSM_SUPPORT) && (CSM_SUPPORT != 0) )
{
...
#endif
    SmbiosDataTable = (UINT8*)(UINTN)0xFFFFFFFF;
    Status = pBS->AllocatePages(
                            AllocateMaxAddress,
                            EfiRuntimeServicesData,
...
#endif
    //
    // Register the event handling final Smbios Table location and its
    // related pointers.
    //
    Status = pBS->CreateEventEx (
                                EVT_NOTIFY_SIGNAL,
                                SMBIOS_DYNAMIC_UPDATE_TPL,
                                (EFI_EVENT_NOTIFY)SmbiosEndOfDxe,
                                NULL,
                                &gEfiEndOfDxeEventGroupGuid,
                                &EndOfDxeEvent
                                );
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to create event for SmbiosEndOfDxe\n"));
    }
    ...
    // structures.
    //
    Status = CreateReadyToBootEvent(SMBIOS_DYNAMIC_UPDATE_TPL,
