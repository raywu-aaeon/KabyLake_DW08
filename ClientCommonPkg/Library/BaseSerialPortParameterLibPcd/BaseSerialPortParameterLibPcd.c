...
#include <Library/CmosAccessLib.h> // AMI_OVERRIDE
...
{
// AMI_OVERRIDE_START >>> Token define values.    
//  return PcdGet8(PcdStatusCodeFlags);
  UINT8 StatusCodeFlags = AMI_DEBUG_INTERFACE_PARAMETER;
  
#ifdef TRACE_HUB_DEBUGLIB_USAGE
#ifdef EFIx64  // Avoid build error in PEI.
  if (CmosRead8(AMI_TRACE_HUB_ENABLE_REG) == 1)
#endif    
    StatusCodeFlags |= 0x20;  // STATUS_CODE_USE_TRACEHUB: BIT5
#endif
  
  return StatusCodeFlags;
// AMI_OVERRIDE_END <<<
}
