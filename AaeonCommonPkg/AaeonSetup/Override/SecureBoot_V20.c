...
VOID UpdateSecureBootBrowserInfo ()
{
    ...
    for(i=0;i<Size;i++) TRACE((-1," %x:",bKey[i]));
    TRACE((-1,"\n"));
...
// Ami Test Rsa2048 key
                ...
                Size3 = 0;
                while(Size3 < Size) {
                    TRACE((-1,"SigList->SignatureType %g\n",  &(SigList->SignatureType)));
                    if (!guidcmp ((EFI_GUID*) &(SigList->SignatureType), &gEfiCertRsa2048Guid) 
                        ...
                        TRACE((-1, "CertStartOffs %x, CertLen %x\n", (UINT32)(CertStart-Buf1), CertLen));
                        if(mAmiSig == NULL)    {
                        ...
                        if(mAmiSig) {
                            ...
                            TRACE((-1, "Compare Key[%d]_1 == FW Key : %s\n", 1,(!err)?"Yes":"No"));
                            if(err) {
                                err = MemCmp(AmiPkKeyHash2, mDigest, SHA256_DIGEST_SIZE);
                                TRACE((-1, "Compare Key[%d]_2 == FW Key : %s\n", 1,(!err)?"Yes":"No"));
                            }
                            if(!err) {
...
ForceSetupModeCallback(EFI_HII_HANDLE HiiHandle, UINT16 Class, UINT16 SubClass, UINT16 Key)
{
    ...
    UINT8       Sel = 0;
    UINTN       Size;
    UINT8       AuditMode, DeployedMode, SetupMode;
...
//            UpdateSecureVariableBrowserInfo();
    ...
    switch(Key) {
		...
		case  AAEON_SECURE_BOOT_MENU_REFRESH:
            UpdateSecureBootBrowserInfo();
            break;
    ...
    if(!EFI_ERROR(Status)) {
        ...
        if(SecureBootSetup.DefaultKeyProvision == 1) {
            ...
            if(!EFI_ERROR(Status) && Byte == SETUP_MODE) 
            {
                ...
                if(!EFI_ERROR(Status) && (SecureBootSetup.SecureBootSupport==SECURE_BOOT) )
                {
