  ...
  )
{
  ...
  if (Wrapped) {
    ...
    *WrapData     = local_malloc (*WrapDataSize);
    if (*WrapData == NULL) {
      *WrapFlag = Wrapped;
      return FALSE;
    }
  ...
  )
{
  ...
  for (Index = 0; ; Index++) {
    ...
    CertBuf    = local_malloc (BufferSize);
  ...
  if (CertBuf != NULL) {
    ...
    *TrustedCert = local_malloc (*CertLength);
    if (*TrustedCert == NULL) {
      goto _Exit;
    }
