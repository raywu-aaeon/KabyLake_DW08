...
/**
  @internal
  This function calculates required LineHeight and BaselineOffset from the Cell data 
  of the characters present in a Row.

  @param  RowInfo                 Instance which holds Row information of a single line.
  @param  Cell                    Instance which holds character cell information.
  
  @endinternal
**/
VOID
FindLineHeightAndBaselineOffset (
  IN OUT  EFI_HII_ROW_INFO   *RowInfo,    
  IN      EFI_HII_GLYPH_INFO *Cell
)
{
   UINTN   Index;
   ...
   for (Index = RowInfo->StartIndex; Index <= RowInfo->EndIndex; Index++){
      if (BaseLine < Cell[Index].Height + Cell[Index].OffsetY) {
          BaseLine = (UINT16) (Cell[Index].Height + Cell[Index].OffsetY);
       }
      if (MinOffsetY > Cell[Index].OffsetY) {
          MinOffsetY = Cell[Index].OffsetY;
      }
   }
   RowInfo->LineHeight = (UINT16) (BaseLine - MinOffsetY);
   RowInfo->BaselineOffset = RowInfo->LineHeight - BaseLine;
}
//*** AMI PORTING END ***//
...
//*** AMI PORTING BEGIN ***//
  ...
  if (*Blt == NULL)
  {
      ...
      UINT16 RunningDisplayHeight = 0;
      UINT16 BaseLineInRow = 0;
      INT16  MinOffsetYInRow = 0;
      ...
      while( *StringTmp != 0)
      {
        ...
        if(BaseLineInRow < (Cell[Index].Height + Cell[Index].OffsetY) )
            BaseLineInRow = (Cell[Index].Height + Cell[Index].OffsetY);
       
        //The largest delta( change in y ) encountered below the baseline in a line will be MinOffsetY.
        //MinOffsetY will be a negative value or Zero if None.
        if (MinOffsetYInRow > Cell[Index].OffsetY) {
            MinOffsetYInRow = Cell[Index].OffsetY;
        }
        ...
        {
            //A height is a sum of the largest BaseLine and largest delta of MinOffsetY among the characters in a line
            RunningDisplayHeight = (UINT16)(BaseLineInRow - MinOffsetYInRow);
            RequiredDisplayHeight += RunningDisplayHeight ;
        ...
        if( *(StringTmp+1) == 0)
        {
            ...
            RunningDisplayHeight = (UINT16)(BaseLineInRow - MinOffsetYInRow);
            RequiredDisplayHeight += RunningDisplayHeight ;
            LastLineHeight = RunningDisplayHeight;
            MaxRowNum += 1;
        }
...
//*** AMI PORTING BEGIN ***// 
 // If MaxRowNum and LastLineHeight are not updated previously, then it is calculated here.
 if (MaxRowNum == 0) {
//*** AMI PORTING END ***// 
...
//*** AMI PORTING BEGIN ***//
      //Calculate Required LineHeight and BaselineOffset from the Cell data of the characters present in a Row.
      FindLineHeightAndBaselineOffset(&RowInfo[RowIndex] ,Cell);
      BaseLine =(UINT16) (RowInfo[RowIndex].LineHeight - RowInfo[RowIndex].BaselineOffset);
...
//*** AMI PORTING END ***//
      ...
      if (Index > RowInfo[RowIndex].StartIndex) {
        ...
        RowInfo[RowIndex].LineWidth      = LineWidth;
//*** AMI PORTING BEGIN ***//
        //Calculates Required LineHeight and BaselineOffset from the Cell data of the characters present in a Row.
        FindLineHeightAndBaselineOffset(&RowInfo[RowIndex] ,Cell);
        BaseLine = (UINT16) (RowInfo[RowIndex].LineHeight - RowInfo[RowIndex].BaselineOffset);
//        RowInfo[RowIndex].LineHeight     = LineHeight;
//        RowInfo[RowIndex].BaselineOffset = BaseLineOffset;
//*** AMI PORTING END ***//
    ...
    //*** AMI PORTING BEGIN ***//    
    // LineHeight can't exceed Image Height.
    if (RowInfo[RowIndex].LineHeight + BltY > Image->Height) {
      RowInfo[RowIndex].LineHeight = Image->Height - BltY;
    }
    //*** AMI PORTING END ***//
    //
