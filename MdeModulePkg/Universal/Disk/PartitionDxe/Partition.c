...
PARTITION_DETECT_ROUTINE mPartitionDetectRoutineTable[] = {
  ...
  //*** AMI PORTING BEGIN ***//
  // Aptio compatibility: Disable EDK UDK detection code.
  // Aptio Partition driver already has UDK and ISO9660 detection code in PartitionInstallElToritoChildHandles
  // compatible with Aptio file system drivers.
  ...
  )
{
  ...
  if ((PartitionInfo->System == 1)) {
    ...
    if (PartitionInfo->Type == PARTITION_TYPE_GPT){
        // Since this is just copying the pointer to the GUID not copying the GUID, we must use the 
        //  Address of the GUID in the Private structure and not the one in the PartitionInfo 
        //  Parameter passed into the function
      Private->EspGuid = &Private->PartitionInfo.Info.Gpt.PartitionTypeGUID;
	} else
    //*** AMI PORTING END ***//
    //
