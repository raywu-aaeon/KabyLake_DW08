  ...
  )
{
  ...
  VOID                            *DummyInterface;
  //*** AMI PORTING BEGIN ***//
  // We do not need ReadyToBoot event, because we are locating Acpi Protocols in the entry point
  //EFI_EVENT                       Event;
  //*** AMI PORTING END ***//
  //
  ...
  //*** AMI PORTING BEGIN ***//
  RamDiskAcpiCheck(NULL, NULL);

  // We do not need callback on ReadyToBoot event, because we are locating Acpi Protocols in the entry point
/*
