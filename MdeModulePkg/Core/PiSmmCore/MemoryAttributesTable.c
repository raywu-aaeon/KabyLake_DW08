  ...
  )
{
  ...
  if ((SectionAlignment & (RUNTIME_PAGE_ALLOCATION_GRANULARITY - 1)) != 0) {
    //*** AMI PORTING BEGIN ***//  
    // Improvement: Since memory attributes table is an optional feature, do not report the message with EFI_D_WARN category.
    //DEBUG ((DEBUG_WARN, "SMM !!!!!!!!  InsertImageRecord - Section Alignment(0x%x) is not %dK  !!!!!!!!\n",
    DEBUG ((EFI_D_VERBOSE, "SMM !!!!!!!!  InsertImageRecord - Section Alignment(0x%x) is not %dK  !!!!!!!!\n",    
    ...
    if (PdbPointer != NULL) {
      ...
      DEBUG ((EFI_D_VERBOSE, "SMM !!!!!!!!  Image - %a  !!!!!!!!\n", PdbPointer));
      //*** AMI PORTING END *****//
    }
    goto Finish;
  }
