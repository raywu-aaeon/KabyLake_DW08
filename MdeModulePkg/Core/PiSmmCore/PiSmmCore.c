...
)
{
  ...
  //
  //*** AMI PORTING BEGIN ***//
  // Improvement: Original code calls platform hook prior to setting gSmmCorePrivate->InSmm flag used by gSmmBase2->InSmm.
  // As a result, gSmmBase2->InSmm returns FALSE when called from the hook function, which leads to undesired side effects.
  // For example, interrupts are enabled in gBS->RestoreTPL (see CoreSetInterruptState in Tpl.c).
  // The code is modified to set gSmmBase2->InSmm before calling the platform hook.
  if (!mInLegacyBoot) {
  ...
  if (!InLegacyBoot) {
    ...
    //
    //*** AMI PORTING BEGIN ***//
    // Improvement: Set gSmmBase2->InSmm before calling the platform hook. See comment above for details.
    // gSmmCorePrivate->InSmm = TRUE;
    //*** AMI PORTING END *****//
