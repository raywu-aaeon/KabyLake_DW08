  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // Bug fix: FixLoaddingAddress is an address. %x will trim upper 4 bytes on x64 projects. Use %p instead.
...
//*** AMI PORTING BEGIN ***//  
#define EfiFileNameSize 256
  CHAR8 EfiFileName[EfiFileNameSize];
  EfiFileName[0] = 0;
//*** AMI PORTING END *****//
...
// Move debug message
  ...
  DEBUG_CODE_BEGIN ();
    ...
    CHAR8 *temp;
    CHAR8 *PdbPointer;
    EFI_LOADED_IMAGE_PROTOCOL *Image = DriverEntry->LoadedImage;
    
    if (    Image != NULL 
         && Image->ImageBase != NULL 
         && ( PdbPointer = PeCoffLoaderGetPdbPointer(Image->ImageBase) ) != NULL 
    ) {
        length = AsciiStrLen(PdbPointer);
        if ( length>4 && ( !AsciiStrCmp(&PdbPointer[length-4],".pdb") || !AsciiStrCmp(&PdbPointer[length-4],".dll")) ) {
            for(temp = &PdbPointer[length-5]; temp>=PdbPointer && *temp!='\\' && *temp!='/'; temp--);
            temp++;
            length = PdbPointer+length-4-temp;
            if ( length >= EfiFileNameSize ) length = EfiFileNameSize-1;
    ...
    if (length==0){
      if (   Image->FilePath != NULL 
        ...
        AsciiSPrint(EfiFileName,EfiFileNameSize,"[%g]",&((MEDIA_FW_VOL_FILEPATH_DEVICE_PATH*)Image->FilePath)->FvFileName);      
      } else {
        AsciiStrCpyS(EfiFileName,EfiFileNameSize,"Unknown");
      }
    }
