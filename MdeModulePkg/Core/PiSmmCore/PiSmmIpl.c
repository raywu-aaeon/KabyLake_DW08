  ...
  )
{
  ...
  if (!gSmmCorePrivate->InSmm && gSmmCorePrivate->SmmEntryPointRegistered) {
    ...
    //
    //*** AMI PORTING BEGIN ***//
    // Bug fix. Non-interrupt safe buffer address/size initialization.
    // Original code was initializing buffer address prior to buffer size.
    // PiSmmCore uses non-NULL gSmmCorePrivate->CommunicationBuffer as an indicator of a pending SMM Communication request.
    // PiSmmCore was getting in invalid buffer size in case on HW SMI between buffer address and buffer size assignments.
