...
#include "PiSmmCore.h"
//*** AMI PORTING BEGIN ***//
// Enhancement: Display protocol name instead of protocol GUID
#include <GetProtocolName.c>
//*** AMI PORTING END *****//
  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // Enhancements: 
  //  1. Display protocol name instead of protocol GUID
  //  2. Update debug mask here for a consistency in displaying interface publishing debug messages.
  //     Similar debug messages in DxeMain and PeiMain are reported with just the DEBUG_INFO mask.
  // DEBUG((DEBUG_LOAD | DEBUG_INFO, "SmmInstallProtocolInterface: %g %p\n", Protocol, Interface));
  DEBUG_CODE(
	CHAR8 *Name = GetProtocolName(Protocol);
	if (Name==NULL) DEBUG((DEBUG_INFO, "SmmInstallProtocolInterface: %g %p\n", Protocol, Interface));
	else DEBUG((DEBUG_INFO, "SmmInstallProtocolInterface: %a %p\n", Name, Interface));
  );
  //*** AMI PORTING END *****//
