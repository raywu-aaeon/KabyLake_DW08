...
//*** AMI PORTING BEGIN ***//
// Bug fix. Function that swaps code segment lists. See comments at the end of SwapImageRecord for details.
STATIC VOID SwapCodeSegmentLists (
  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // Bug fix. SwapListEntries is misused here. The function is intended to swap two elements of the same list,
  // whereas we're dealing with two separate lists.
  // Usage of SwapListEntries generates assert when PcdVerifyNodeInList is TRUE.
  // Replace SwapListEntries with the SwapCodeSegmentLists function defined above.
  
  //SwapListEntries (&FirstImageRecord->CodeSegmentList, &SecondImageRecord->CodeSegmentList);
  SwapCodeSegmentLists (&FirstImageRecord->CodeSegmentList, &SecondImageRecord->CodeSegmentList);
  //*** AMI PORTING END *****//
}
  ...
  )
{
  ...
  for (Index = 0; Index < Hdr.Pe32->FileHeader.NumberOfSections; Index++) {
    ...
    if ((Section[Index].Characteristics & EFI_IMAGE_SCN_CNT_CODE) != 0) {
      ...
      //*** AMI PORTING BEGIN ***//
      // Bug fix. All elements of the Memory Attributes Table (and properties table)
      // have to be aligned on RUNTIME_PAGE_ALLOCATION_GRANULARITY boundary.
      // It's safe to align the size because code section alignment has already been verified 
      // above by calling SetPropertiesTableSectionAlignment.
      // ImageRecordCodeSection->CodeSegmentSize = Section[Index].SizeOfRawData;
      ImageRecordCodeSection->CodeSegmentSize = ALIGN_VALUE(Section[Index].SizeOfRawData,RUNTIME_PAGE_ALLOCATION_GRANULARITY);
      //*** AMI PORTING END *****//
  ...
  //*** AMI PORTING BEGIN ***//
  // Bug fix. SortImageRecord changes content of the record referred by ImageRecord.
  // As a result if-statement below deals with a different image record and does not work as expected.
  // Call SortImageRecord after updating mImagePropertiesPrivateData.CodeSegmentCountMax
