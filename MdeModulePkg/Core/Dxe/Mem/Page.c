...
EFI_MEMORY_TYPE_STATISTICS mMemoryTypeStatistics[EfiMaxMemoryType + 1] = {
  ...
  { 0, MAX_ADDRESS, 0, 0, EfiMaxMemoryType, TRUE, FALSE },  // EfiBootServicesData
  //*** AMI PORTING END *****//
      ...
      if (Entry->Type == EfiConventionalMemory) {
        ...
        if (mMemoryTypeStatistics[Entry->Type].Special                          &&
          ...
          continue;
            ...
            for (Type = (EFI_MEMORY_TYPE) 0; Type < EfiMaxMemoryType; Type++) {
              if (Type != Entry->Type                                        &&
            	  mMemoryTypeStatistics[Type].Special                        &&
                  mMemoryTypeStatistics[Type].NumberOfPages > 0              &&
                  Entry->Start >= mMemoryTypeStatistics[Type].BaseAddress    &&
                  Entry->End   <= mMemoryTypeStatistics[Type].MaximumAddress) {
                BufferSize += Size;
  ...
  for (Link = mGcdMemorySpaceMap.ForwardLink; ; Link = Link->ForwardLink) {
    ...
    if ((MergeGcdMapEntry.GcdMemoryType == EfiGcdMemoryTypeReserved) ||
      ...
      if (MergeGcdMapEntry.GcdMemoryType == EfiGcdMemoryTypeReserved) {
        ...
        In general there should be no such entries:
        ...
        Prior to PI 1.4, the PI specification did not define how to reflect GCD memory maps entries
        in the UEFI memory map, which made it up to implementation.
        Prior implementation ignored all non-runtime GCD entries.
        So, the only way to report reserved GCD entry to OS (which only deals with UEFI memory map),
        was to set a runtime attribute.
        Now that PI specification is updated and all reserved GCD entries are included into UEFI memory map,
        there is no need to use reserved entries with runtime attributes.
        Once all the projects are updated not to use such entries, the workaround can be removed.
