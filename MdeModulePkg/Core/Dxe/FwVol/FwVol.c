...
//*** AMI PORTING BEGIN ***//
// Support driver override from the secure path FV
#include <Library/AmiSecurePatchFvLib.h>
//*** AMI PORTING END *****//
  ...
  )
{
  ...
  if (FwVolHeader->ExtHeaderOffset != 0) {
    ...
    //*** AMI PORTING BEGIN ***//
    // Bug fix(USRT 1887): FFS file header address has to be always aligned on 8-byte boundary.
    // Original code aligned only if FV had extended header.
    // Alignment statement is moved outside of the if-statement.
  ...
  //*** AMI PORTING BEGIN ***//
  // Bug fix: FFS file header address has to be always aligned on 8-byte boundary.
  // Original code aligned only if FV had extended header.
  FfsHeader = (EFI_FFS_FILE_HEADER *) ALIGN_POINTER (FfsHeader, 8);
  //*** AMI PORTING END *****//
  ...
  while (((UINTN) FfsHeader >= (UINTN) FvDevice->CachedFv) && ((UINTN) FfsHeader <= (UINTN) ((UINTN) TopFvAddress - sizeof (EFI_FFS_FILE_HEADER)))) {
    ...
    //*** AMI PORTING BEGIN ***//
    // Enhancement: Support driver override from the secure path FV
    if( EFI_ERROR(Status =AmiFvOverrideCheck( Fvb, CacheFfsHeader, &FvDevice->FfsFileListHeader, &FileCached, FileState)) )
        goto Done; 
    //*** AMI PORTING END *****//
