...
//*** AMI PORTING BEGIN ***//
// Enhancement: Replace FFS file GUID and protocol GUID with names in debug messages printed during DepEx evaluation
// (CoreIsSchedulable, CoreDisplayDiscoveredNotDispatched, and a few other dispatcher functions).
CHAR8 *GetProtocolName(CONST EFI_GUID* Guid); // defined in GetProtocolName.c generated by the build process

VOID PrintDepexPushDebugMessage(CONST EFI_GUID *ProtocolGuid, BOOLEAN Result){
  DEBUG_CODE(
    CHAR8 *Name = GetProtocolName(ProtocolGuid);
    CHAR8 *ResultStr = (Result) ? "TRUE" : "FALSE";
    if (Name==NULL) DEBUG ((DEBUG_DISPATCH, "  PUSH GUID(%g) = %a\n", ProtocolGuid, ResultStr));
    else DEBUG ((DEBUG_DISPATCH, "  PUSH GUID(%a) = %a\n", Name, ResultStr));
  );
}

VOID GetDriverEntryName(EFI_CORE_DRIVER_ENTRY *DriverEntry, CHAR8 *Name, UINTN Size){
  // Precondition: DriverEntry->FileName is not NULL.

  // We're not using DriverEntry->ImageHandle to get driver name because this function
  // is used during DepEx evaluation when DriverEntry->ImageHandle is NULL (driver has not been loaded yet).
  DEBUG_CODE(
    EFI_STATUS Status = EFI_UNSUPPORTED;

    if (DriverEntry->Fv != NULL){
      UINT32 AuthenticationStatus;
      UINTN TmpSize = Size;
      UINTN Index;

      // Read driver name from UI section
      Status = DriverEntry->Fv->ReadSection(
        DriverEntry->Fv, &DriverEntry->FileName, EFI_SECTION_USER_INTERFACE, 0,
        &Name, &TmpSize, &AuthenticationStatus
      );
      if (!EFI_ERROR(Status)){
          // UI section is in Unicode format. Convert to ASCII.
          for(Index = 1; Index < TmpSize / sizeof(CHAR16); Index++){
    ...
    if (EFI_ERROR(Status)){
      AsciiSPrint(Name, Size, "%g", &DriverEntry->FileName);
    }
  );
}
//*** AMI PORTING END *****//
  ...
  while (TRUE) {
    ...
    case EFI_DEP_PUSH:
      ...
      if (EFI_ERROR (Status)) {
        ...
        PrintDepexPushDebugMessage(&DriverGuid, FALSE);
        //*** AMI PORTING END *****//
        ...
        PrintDepexPushDebugMessage(&DriverGuid, TRUE);
        //*** AMI PORTING END *****//
