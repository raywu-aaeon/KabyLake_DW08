...
//*** AMI PORTING BEGIN ***//
// Enhancement: Replace FFS file GUID and protocol GUID with names in debug messages printed during DepEx evaluation
VOID GetDriverEntryName(EFI_CORE_DRIVER_ENTRY *DriverEntry, CHAR8 *Name, UINTN Size); // Defined in Dependency.c
//*** AMI PORTING END *****//
    ...
    for (Link = mDiscoveredList.ForwardLink; Link != &mDiscoveredList; Link = Link->ForwardLink) {
      ...
      if (DriverEntry->Dependent) {
        ...
        if (DriverEntry->Unrequested) {
          ...
          DEBUG_CODE_BEGIN();
            CHAR8 Name[0x100];
            GetDriverEntryName( DriverEntry, Name, sizeof(Name) );
            DEBUG ((DEBUG_DISPATCH, "Evaluate DXE DEPEX for FFS(%a)\n", Name));
          DEBUG_CODE_END();
          //*** AMI PORTING END *****//
  ...
  )
{
  ...
  for (Link = mDiscoveredList.ForwardLink;Link !=&mDiscoveredList; Link = Link->ForwardLink) {
    ...
    if (DriverEntry->Dependent) {
      //*** AMI PORTING BEGIN ***//
      // Enhancement: Replace FFS file GUID and protocol GUID with names in debug messages
      //DEBUG ((DEBUG_LOAD, "Driver %g was discovered but not loaded!!\n", &DriverEntry->FileName));
      DEBUG_CODE_BEGIN();
        CHAR8 Name[0x100];
        GetDriverEntryName( DriverEntry, Name, sizeof(Name) );
        DEBUG ((DEBUG_LOAD, "Driver %a was discovered but not loaded!!\n", Name));
      DEBUG_CODE_END();
      //*** AMI PORTING END *****//
    }
  }
