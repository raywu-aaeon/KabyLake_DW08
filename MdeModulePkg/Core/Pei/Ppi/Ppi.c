...
#include "PeiMain.h"
//*** AMI PORTING BEGIN ***//
// Enhancement: Display PPI name instead of PPI GUID
#include <GetPpiName.c>
#define PPI_DEBUG_MESSAGE(Action,Guid) \
  DEBUG_CODE(\
	  CHAR8 *Name = GetPpiName(Guid);\
	  if (Name==NULL) DEBUG((EFI_D_INFO, Action": %g\n", (Guid)));\
	  else DEBUG((DEBUG_INFO, Action": %a\n", Name));\
  );
//*** AMI PORTING END *****//
...
{
  //*** AMI PORTING BEGIN ***//
  // Bug fix: PcdPeiCoreMaxPpiSupported is defined as UINT32. The original code was not compiling with PCD values 256 or higher.
  //UINT8                 Index;
  UINT16                 Index;
  //*** AMI PORTING END ***//
  UINT8                 IndexHole;
  ...
  //*** AMI PORTING BEGIN ***//
  ASSERT(PcdGet32 (PcdPeiCoreMaxPpiSupported) <= MAX_UINT16);
  //*** AMI PORTING END ***//
  ...
  )
{
  ...
  for (;;) {
    ...
    PPI_DEBUG_MESSAGE("Install PPI",PpiList->Guid)
    //*** AMI PORTING END *****//
  ...
  )
{
  ...
  PPI_DEBUG_MESSAGE("Reinstall PPI",NewPpi->Guid)
  //*** AMI PORTING END *****//
  ...
  )
{
  ...
  for (;;) {
    ...
    PPI_DEBUG_MESSAGE("Register PPI Notify",NotifyList->Guid)
    //*** AMI PORTING END *****//
    if (Single) {
      //
  ...
  for (Index1 = NotifyStartIndex; Index1 > NotifyStopIndex; Index1--) {
    ...
    for (Index2 = InstallStartIndex; Index2 < InstallStopIndex; Index2++) {
      ...
      if ((((INT32 *)SearchGuid)[0] == ((INT32 *)CheckGuid)[0]) &&
        ...
        //  ));
        DEBUG_CODE(
          CHAR8 *Name = GetPpiName(SearchGuid);
          if (Name==NULL)
            DEBUG ((EFI_D_INFO, "Notify: PPI: %g, Peim notify entry point: %p\n", SearchGuid, NotifyDescriptor->Notify));
          else
        	DEBUG ((EFI_D_INFO, "Notify: PPI: %a, Peim notify entry point: %p\n", Name, NotifyDescriptor->Notify));
        );
        //*** AMI PORTING END *****//
        NotifyDescriptor->Notify (
