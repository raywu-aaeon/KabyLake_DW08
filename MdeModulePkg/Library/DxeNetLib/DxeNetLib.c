...
{
  //*** AMI PORTING BEGIN ***//
  // Enhancement: improve "randomness" of the values returned by the function
  UINT32                    Seed;
  struct {
	EFI_GUID SystemGuid;
  ...
  gRT->GetTime (&Data.Time, NULL);
  gBS->GetNextMonotonicCount (&Data.MonotonicCount);
  NetLibGetSystemGuid(&Data.SystemGuid);
  if (EFI_ERROR(gBS->CalculateCrc32(&Data, sizeof(Data), &Seed))){
	Seed = (~Data.Time.Hour << 24 | Data.Time.Day << 16 | Data.Time.Minute << 8 | Data.Time.Second);
	Seed ^= Data.Time.Nanosecond;
	Seed ^= Data.Time.Year << 7;
	Seed += (UINT32) Data.MonotonicCount;
  }
/*	
  EFI_TIME                  Time;
  UINT32                    Seed;
