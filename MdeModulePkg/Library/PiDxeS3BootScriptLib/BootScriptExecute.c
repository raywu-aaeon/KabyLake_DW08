...
static AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR *gMemoryRangesStr = NULL;
//*** AMI PORTING END *****//
  ...
  )
{
  ...
  DEBUG_CODE(
	  UINTN Length = 0;
	  BOOLEAN GoodMemory = TRUE;
	  UINTN  i;

	  if (AddressStride !=0)
		  Length = AddressStride*Count;
	  else
		  Length = (UINT32)(1 << (Width & 0x03));
	  
	  for (i = 0; i < gMemoryRangesStr->NumberOfMemoryRanges; i ++) {
	          if (    OriginalAddress >= gMemoryRangesStr->MemRanges[i].Address
	               && OriginalAddress < gMemoryRangesStr->MemRanges[i].Address + gMemoryRangesStr->MemRanges[i].Size
	          ){
	              GoodMemory = FALSE; // Buffer starts in the illegal region
	          } else if (    OriginalAddress < gMemoryRangesStr->MemRanges[i].Address
	               && OriginalAddress+Length > gMemoryRangesStr->MemRanges[i].Address
	          ){
	              GoodMemory = FALSE; // Buffer overlaps with the illegal region
	          }
	      }
	  
	  if (!GoodMemory) 
	  {
		  DEBUG ((EFI_D_ERROR, "ERROR: S3 Boot Script overwrites OS memory at Address %p; Size = %x !!!\n", OriginalAddress, Length));
		  ASSERT(FALSE);
	  }	  
  );
  //*** AMI PORTING END ***//
  ...
  )
{
  ...
  // Improvement. Make sure boot script does not overwrite OS-owned memory.
  DEBUG_CODE(
    gMemoryRangesStr = (AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR*)(UINTN)mS3BootScriptTablePtr->MemoryRangesStr;
    ASSERT (gMemoryRangesStr != NULL);
  );
  //*** AMI PORTING END *****//
