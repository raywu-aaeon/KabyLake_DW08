...
//*** AMI PORTING BEGIN ***//
// Improvement. Make sure boot script does not overwrite OS-owned memory.
  ...
  )
{
  ...
  DEBUG_CODE_BEGIN ();
	    ...
	    EFI_GUID    mAmiMemoryRangesStrGuid = { 0x6f8d5d48, 0xa285, 0x44e8, { 0xaf, 0x6a, 0xd, 0x4, 0x29, 0x51, 0x13, 0xc6 } };
	    EFI_GUID	mAmiMemoryRangesBufferGuid = { 0xf754f36f, 0x1767, 0x414e, { 0x8a, 0xf2, 0x8b, 0xd4, 0x99, 0xab, 0x6e, 0x2d } };
	    AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR *MemoryRangesStr = NULL;
	    
	    mS3BootScriptTablePtr->MemoryRangesStr = NULL;
	    Size = 0;
	    ...
	    ASSERT_EFI_ERROR(Status);
	    if (EFI_ERROR(Status)){
	        DEBUG ((EFI_D_ERROR, "ERROR: No memory to save MemMap !!!\n"));
	        return;
	    }
	    Status = gBS->GetMemoryMap(&Size, MemMap, &MapKey, &DescriptorSize, &DescriptorVersion);
	    ASSERT_EFI_ERROR(Status);
	    if (EFI_ERROR(Status)){
	        gBS->FreePool(MemMap);
	        DEBUG ((EFI_D_ERROR, "ERROR in SaveBootScriptDataToLockBox: GetMemoryMap2 failed with %r Status !!!\n", Status));
	        return;
	    }
	    Buffer = SIZE_4GB - 1;
	    Status = gBS->AllocatePages (
	                        ...
	                        EFI_SIZE_TO_PAGES(sizeof(AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR)),
	                        &Buffer
	                        );
	    ASSERT_EFI_ERROR(Status);
	    if (EFI_ERROR(Status)){
	        DEBUG ((EFI_D_ERROR, "ERROR in SaveBootScriptDataToLockBox: AllocatePages MemoryRangesStr failed with %r Status !!!\n", Status));
	    	return;
	    }
	    MemoryRangesStr = (VOID*)(UINTN)Buffer;
	    MemoryRangesStr->NumberOfMemoryRanges = 0;
	    for(   Descriptor = MemMap
	         ; (UINT8*)Descriptor < (UINT8*)MemMap+Size
	         ; Descriptor = NEXT_MEMORY_DESCRIPTOR(Descriptor,DescriptorSize)
	    ){
	        if (   Descriptor->Type != EfiRuntimeServicesData && Descriptor->Type != EfiRuntimeServicesCode
		             && Descriptor->Type != EfiReservedMemoryType && Descriptor->Type != EfiACPIReclaimMemory
		             && Descriptor->Type != EfiACPIMemoryNVS && Descriptor->Type != EfiMemoryMappedIO
		             && Descriptor->Type != EfiMemoryMappedIOPortSpace
	        ) MemoryRangesStr->NumberOfMemoryRanges ++;
	    }
	    Buffer = SIZE_4GB - 1;
	    Status = gBS->AllocatePages (
	    	                        AllocateMaxAddress,
	    	                        EfiReservedMemoryType,
	    	                        EFI_SIZE_TO_PAGES(MemoryRangesStr->NumberOfMemoryRanges * sizeof(AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE)),
	    	                        &Buffer
	    	                        );
	    ASSERT_EFI_ERROR(Status);
	    if (EFI_ERROR(Status)){
	        DEBUG ((EFI_D_ERROR, "ERROR in SaveBootScriptDataToLockBox: AllocatePages gMemoryRangesStr2  %r Status !!!\n", Status));
	    	return;
	    }
	    
	    MemoryRangesStr->MemRanges = (VOID *)(UINTN)Buffer;
	    ASSERT (MemoryRangesStr->MemRanges != NULL);
	    if (MemoryRangesStr->MemRanges == NULL){
	    	MemoryRangesStr->NumberOfMemoryRanges  = 0;
	        gBS->FreePool(MemMap);
	        DEBUG ((EFI_D_ERROR, "ERROR: No memory to save MemMap !!!\n"));
	        return;
	    ...
	    for(   Descriptor = MemMap, i=0
	        ...
	        if (    Descriptor->Type != EfiRuntimeServicesData && Descriptor->Type != EfiRuntimeServicesCode
	            ...
	            MemoryRangesStr->MemRanges[i].Address = Descriptor->PhysicalStart;
	            MemoryRangesStr->MemRanges[i].Size = EFI_PAGES_TO_SIZE((UINTN)Descriptor->NumberOfPages);
	            i++;
	        }
	    }
	    ASSERT(i==MemoryRangesStr->NumberOfMemoryRanges );
	    gBS->FreePool(MemMap);
	    Status = SaveLockBox (
	               &mAmiMemoryRangesStrGuid,
	               (VOID *)(UINTN)MemoryRangesStr,
	               sizeof(AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR)
	               );
	    ASSERT_EFI_ERROR (Status);
	    
	    Status = SetLockBoxAttributes (&mAmiMemoryRangesStrGuid, LOCK_BOX_ATTRIBUTE_RESTORE_IN_PLACE);
	    ASSERT_EFI_ERROR (Status);
	    
	    Status = SaveLockBox (
	    	               &mAmiMemoryRangesBufferGuid,
	    	               (VOID *)(UINTN)&MemoryRangesStr->MemRanges,
	    	               MemoryRangesStr->NumberOfMemoryRanges * sizeof(AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE)
	    	               );
	    ASSERT_EFI_ERROR (Status);
	   
	    Status = SetLockBoxAttributes (&mAmiMemoryRangesBufferGuid, LOCK_BOX_ATTRIBUTE_RESTORE_IN_PLACE);
	    ASSERT_EFI_ERROR (Status);
	    mS3BootScriptTablePtr->MemoryRangesStr = (VOID*)(UINTN)MemoryRangesStr;
  DEBUG_CODE_END ();
  //*** AMI PORTING END *****//
}
