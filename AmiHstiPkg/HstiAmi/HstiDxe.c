  ...
  )
{
  ...
  DEBUG((-1, "HstiRecordFeaturesVerified: \n\r"));
  
  DEBUG((-1, "ByteIndex = %X     Bit = %X      Set = %X\n\r", ByteIndex, Bit, Set));

  Aip = InternalHstiFindAip (Role, ImplementationID, (VOID **)&Hsti, &HstiSize);
  if (Aip == NULL) {
      DEBUG((-1, "Can't find the table: \n\r"));
    return EFI_NOT_STARTED;
  }

  if (ByteIndex >= Hsti->SecurityFeaturesSize) {
      DEBUG((-1, "Byte index too large: \n\r"));
  ...
  )
{
  ...
  DEBUG((-1, "HstiRecordWindowsSecurityFeatureVerified: \n\r"));

  DEBUG((-1, "ByteIndex = %X     Bit = %X      Set = %X\n\r", ByteIndex, Bit, Set));
  
  Aip = InternalHstiFindAip (Role, ImplementationID, (VOID **)&Hsti, &HstiSize);
  if (Aip == NULL) {
      DEBUG((-1, "Error: HSTI IBV table not found\n\r"));
    return EFI_NOT_STARTED;
  }

  if (ByteIndex >= HSTI_WINDOWS_SECURITY_FEATURE_MAX_BYTES) {
      DEBUG((-1, "Error: Pointing to byte beyond the end of the windows security feature bitmap\n\r"));
      return EFI_UNSUPPORTED;
  }
