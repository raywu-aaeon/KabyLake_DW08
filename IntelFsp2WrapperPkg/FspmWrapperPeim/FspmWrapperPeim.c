...
// AMI_OVERRIDE_START
  Status = PeiServicesInstallPpi (&mAmiPeiBeforeMrcDesc[0]);  
...
// AMI_OVERRIDE_START
  Status = PeiServicesInstallPpi (&mAmiPeiCompelteMrcDesc[0]);  
  ASSERT_EFI_ERROR (Status);
...
// AMI_OVERRIDE_START
  Status = PeiServicesInstallPpi (&mAmiPeiEndOfMrcDesc[0]);  
  ASSERT_EFI_ERROR (Status);
  DEBUG ((EFI_D_ERROR, "Install End of MRC Ppi.\n"));
// AMI_OVERRIDE_END   
