...
// AMI_OVERRIDE_START >>> EIP393808 - Reserved and untested memory in GCD map is forbidden as SMM communication buffer
...
// AMI_OVERRIDE_START >>> Set UEFI runtime memory with EFI_MEMORY_RO as not present.
  //
...
// AMI_OVERRIDE_START >>> EIP415920 - The memory below 0x100000 should can be accessed when CSM is enabled.
        if ((SmmAmiBufferValidationLibCsmEnabled == FALSE) || (MemoryMap->PhysicalStart >= 0x100000)) {
// AMI_OVERRIDE_END <<<
...
// AMI_OVERRIDE_START >>> EIP415920 - The memory below 0x100000 should can be accessed when CSM is enabled.
          ...
          if ((MemoryMap->PhysicalStart + (UINT64)EFI_PAGES_TO_SIZE((UINTN)MemoryMap->NumberOfPages)) > 0x100000) {
            EFI_PHYSICAL_ADDRESS  ChangedAddress;
            UINT64                ChangedNumberOfPages;
            UINT64                Below1MNumberOfPages;

            Below1MNumberOfPages = EFI_SIZE_TO_PAGES(0x100000 - MemoryMap->PhysicalStart);
            ChangedNumberOfPages = MemoryMap->NumberOfPages - Below1MNumberOfPages;
            ChangedAddress = MemoryMap->PhysicalStart + (UINT64)EFI_PAGES_TO_SIZE((UINTN)Below1MNumberOfPages);
            Status = SmmSetMemoryAttributes (
                   ChangedAddress,
                   EFI_PAGES_TO_SIZE((UINTN)ChangedNumberOfPages),
                   EFI_MEMORY_RP
                   );
            DEBUG ((
              DEBUG_INFO,
              "UefiMemory protection: 0x%lx - 0x%lx %r\n",
              ChangedAddress,
              ChangedAddress + (UINT64)EFI_PAGES_TO_SIZE((UINTN)ChangedNumberOfPages),
              Status
            ));
          }
        }
