...
//*** AMI PORTING BEGIN ***// [ EIP431743 ]
// Enhancement: Support capsule scheduling from code that is not part of this driver.
// Original implementation assumes that capsule that persists across system reset 
// can only be scheduled by this driver.
// In Aptio capsule can also be scheduled by SMI handler.
// This function fills CapsuleVarName buffer with a capsule variable name that has not been used yet.
VOID BuildCapsuleVariableName(CHAR16* CapsuleVarName, UINTN MaxSize)
{
    CHAR16 *TempVarName;  
    UINTN Size;
    UINTN Index;
  ...
  )
{
  ...
  // Enhancement: Support capsule scheduling from code that is not part of this driver.
  //CHAR16                    *TempVarName;
  EFI_GUID                  CrashDumpGuid = {0x0E1D2972, 0x65AF, 0x4AC1, {0xBF, 0xA3, 0xCE, 0xF4, 0xAB, 0x0C, 0x38, 0xFE}};
  //*** AMI PORTING END *****// [ EIP431743 ]
  ...
  for (ArrayNumber = 0; ArrayNumber < CapsuleCount; ArrayNumber++) {
	...
	// [ EIP431743 ]
    if (CompareGuid(&CapsuleHeader->CapsuleGuid, &CrashDumpGuid)) {
       return EFI_INVALID_PARAMETER;
    }
  ...
  //*** AMI PORTING BEGIN ***// [ EIP431743 ]
  // Enhancement: Support capsule scheduling from code that is not part of this driver.
  BuildCapsuleVariableName(CapsuleVarName, sizeof(CapsuleVarName)/sizeof(CHAR16));
