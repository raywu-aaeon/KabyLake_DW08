...
//
//*************************************************************************
// $Header: /Alaska/BIN/IO/Fintek/F81804/F81804 Board/F81804SmartFan.c 1     7/20/11 4:23a Kasalinyi $Revision: 
//
// $Date:
//*************************************************************************
// Revision History
...
//--------------------------------------------------------------------------
VOID F81804SmartFunction (VOID);
VOID F81804_FAN_Full();
static VOID SmartFanFuncStart(SETUP_DATA *SetupData);

void F81804_FAN_Full(VOID)
{
  // Full Speed 
  // Manual Duty Mode

#if (F81804_SMF1_SUPPORT == 1)
  UINT8   i, temp;
#endif  
  ...
  for(i=0; i<( (sizeof(DXE_FAN1_INIT_TABLE))/(sizeof(DXE_SMF_INIT_DATA)) ); i++) {
      IoWrite8(F81804_HWM_INDEX_PORT, DXE_FAN1_INIT_TABLE[i].Reg8 ); 
      if(DXE_FAN1_INIT_TABLE[i].AndData8 == 0xFF) {
          temp = DXE_FAN1_INIT_TABLE[i].OrData8;
          } else {
          temp = IoRead8(F81804_HWM_DATA_PORT);
          temp &= DXE_FAN1_INIT_TABLE[i].AndData8;
          temp |= DXE_FAN1_INIT_TABLE[i].OrData8;
      }
      IoWrite8(F81804_HWM_DATA_PORT, temp); 
  }  
#endif
}
...
//---------------------------------------------------------------------------
//<AMI_PHDR_END>
void  F81804_SMF1_INIT(SETUP_DATA *SetupData)
{
	UINT8   i, temp;
...
EFI_STATUS SmartFanBootScript(EFI_S3_SAVE_STATE_PROTOCOL *bss)
{
    ...
    for (i=0; i < ((sizeof(HwmS3SaveRegisterTbl))/(sizeof(S3_SAVE_DATA))); i++) {
        ...
        if ( bank !=  0xFF ) {
          bank_reg = 0x4e;
          IoWrite8(F81804_HWM_INDEX_PORT, bank_reg);
          IoWrite8(F81804_HWM_DATA_PORT, bank);
        } 
        //select register to read actual data
        IoWrite8(F81804_HWM_INDEX_PORT, r);
        v = IoRead8(F81804_HWM_DATA_PORT);
        //save to s3 bootscrip table
        if ( bank != 0xFF ) {
          BOOT_SCRIPT_S3_IO_WRITE_MACRO(bss,EfiBootScriptWidthUint8,F81804_HWM_INDEX_PORT, 1, &bank_reg);
          BOOT_SCRIPT_S3_IO_WRITE_MACRO(bss,EfiBootScriptWidthUint8,F81804_HWM_DATA_PORT, 1, &bank);
        }

        BOOT_SCRIPT_S3_IO_WRITE_MACRO(bss,EfiBootScriptWidthUint8,F81804_HWM_INDEX_PORT, 1, &r);
        BOOT_SCRIPT_S3_IO_WRITE_MACRO(bss,EfiBootScriptWidthUint8,F81804_HWM_DATA_PORT, 1, &v);
    }
...
static VOID SmartFanFuncStart(SETUP_DATA *SetupData)
{
    ...
    //--------------------------------------------------------------------------
	#if defined(F81804_SMF1_SUPPORT) && (F81804_SMF1_SUPPORT == 1)
    F81804_SMF1_INIT(SetupData);
	#endif
}

//<AMI_PHDR_START>
//---------------------------------------------------------------------------
// Procedure:    F81804SmartFunction
//
