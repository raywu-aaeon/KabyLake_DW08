...
//----add your other device,if no,please cut and modify Package number----------//
        0xFF,    //   10 - CIR
        0x00,    //   11 - Serial C (SP3)
        0x01,    //   12 - Serial D (SP4)
        0x02,    //   13 - Serial E (SP5)
        0x03,    //   14 - Serial F (SP6)
    })
...
//<AMI_PHDR_END>
    Method(DSR3, 2) {
        ...
        //  Or(INTT, 0x01, INTT)
	    // AAEON_OVERRIDE >>
        //Set IRQ sender(F81216D) mode according to Arg0 (expecting it is resource description ACPI OS get from UART3~6.asl)
	    //IRQ Receiver - ACPI IRQS: bit4:_SHR,bit3:_LL,bit0:_HE
	    //IRQ Sender - Fintek URAIRQ_MODE(OPT0[5]):Fintek URAIRQ_SHAR(OPT0[4])
	    //    0 : PCI IRQ sharing mode.
	    //    1 : ISA IRQ sharing mode. 
        Or(INTT, 0x01, INTT)	// URAIRQ_SHAR(INTT[4]) IRQ Shared? 1 - Shared
        If(And(IRQS,0x10)){		    // _SHR IRQ Shared? 1 - Shared
            If(And(IRQS,0x01))		//  _HE IRQ Mode? 1 - Edge(ISA), 0 -Level(PCI)
            {
                // Receiver: Edge-Triggered
                // Sender: 1 : ISA IRQ sharing mode.
                Or(INTT, 0x02, INTT)
            }							
            Else{
                // Receiver: Active-Triggered
                // Sender: 0 : Sharing IRQ active low Level mode(PCI).
                And(INTT, 0xFD, INTT)
            }
        }
        Else{
