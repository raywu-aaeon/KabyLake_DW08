...
extern VOID F81966SmartFunction(VOID);
...
EFI_STATUS F81966ErpModeCfg(VOID)
{
	...
	TRACE((TRACE_ALWAYS, "[F81966ErpModeCfg]Enter F81966ErpModeCfg\n"));
	...
	IoWrite8(F81966_CONFIG_DATA, F81966_LDN_PME);

#if AAEONPOWERMODE_SUPPORT
	if((SetupData->F81966ErpMode != 0x00) && (SetupData->AaeonPowerMode == 0x0))
#else
	if(SetupData->F81966ErpMode != 0x00)
#endif
	{
	    if(SetupData->F81966ErpMode == 1)
	    {
	    	//Intel DSW mode
#if AAEONPOWERMODE_SUPPORT
			if(SetupData->AaeonRestoreACPowerLoss == 2)
       		...
       		if((SetupData->F81966RestoreACPowerLoss == 0x02) || ((SetupData->F81966RestoreACPowerLoss == 0x03) && (SetupData.StateAfterG3 == MapToACPowerLoss[AAEON_ACLOSS_ALWAYS_OFF])))
#else
    		if((SetupData->F81966RestoreACPowerLoss == 0x02) || ((SetupData->F81966RestoreACPowerLoss == 0x03) && (SetupData->LastState == MapToACPowerLoss[AAEON_ACLOSS_ALWAYS_OFF])))
#endif
#endif
		    {
		    	//Always off: Set ERP on for S5 and AC_Failure
		    	IoWrite8(F81966_CONFIG_INDEX, 0xE1);
		    	reg8 = (IoRead8(F81966_CONFIG_DATA) & 0xF0) | 0x05;
		    	IoWrite8(F81966_CONFIG_DATA, reg8); 
			} else
		    {
		    	//Always on/Last state-on: Set ERP off for AC_Failure, ERP on for S5
		    	IoWrite8(F81966_CONFIG_INDEX, 0xE1);
		    	reg8 = (IoRead8(F81966_CONFIG_DATA) & 0xF0) | 0x04;
		    	IoWrite8(F81966_CONFIG_DATA, reg8); 
			}
		} else
	    {
...
#endif
		    {
		    	//Always off: Set ERP on for S5 and AC_Failure
		    	IoWrite8(F81966_CONFIG_INDEX, 0xE1);
		    	reg8 = (IoRead8(F81966_CONFIG_DATA) & 0xF0) | 0x0F;
		    	IoWrite8(F81966_CONFIG_DATA, reg8); 
			} else
		    {
		    	//Always on/Last state-on: Set ERP off for AC_Failure, ERP on for S5
		    	IoWrite8(F81966_CONFIG_INDEX, 0xE1);
		    	reg8 = (IoRead8(F81966_CONFIG_DATA) & 0xF0) | 0x0C;
		    	IoWrite8(F81966_CONFIG_DATA, reg8); 
			}
		}
		
		IoWrite8(F81966_CONFIG_INDEX, 0xEC);
#if INTEL_DEEP_SX_SUPPORT
	    if(SetupData->F81966ErpMode == 1)
	    	reg8 = (IoRead8(F81966_CONFIG_DATA) & 0x3F) | 0xC0;
	    else
	    	reg8 = IoRead8(F81966_CONFIG_DATA) & 0x3F;
//	    	reg8 = (IoRead8(F81966_CONFIG_DATA) & 0x3F) | 0x40;	    
    	...
    	IoWrite8(F81966_CONFIG_INDEX, 0xE0);
		reg8 = IoRead8(F81966_CONFIG_DATA) | 0x80;
    	IoWrite8(F81966_CONFIG_DATA, reg8); 
	} else
	{
		IoWrite8(F81966_CONFIG_INDEX, 0xE0);
		reg8 = IoRead8(F81966_CONFIG_DATA) & 0x7F;
    	IoWrite8(F81966_CONFIG_DATA, reg8); 
	}
...
//----------------------------------------------------------------------
//
// Procedure: F81966_ClearDevResource
//
// Description:
...
)
{
    ...
    //Seclect device
    IoWrite8(F81966_CONFIG_INDEX, F81966_LDN_SEL_REGISTER);
    IoWrite8(F81966_CONFIG_DATA, Dev->DeviceInfo->Ldn);
    //Clear Base Address
...
//----------------------------------------------------------------------
// <AMI_PHDR_END>
//#if F81966_SERIAL_PORT1_PRESENT | F81966_SERIAL_PORT2_PRESENT
EFI_STATUS F81966_COM_Init(
    IN AMI_BOARD_INIT_PROTOCOL      *This,
    IN UINTN                        *Function,
    IN OUT VOID                     *ParameterBlock
)
...
//        if(F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid].AndData8 == 0xFF) {
//            rv=F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid].OrData8;
//        } else {
//            Status=AmiSio->Access(AmiSio, FALSE, FALSE, F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid].Reg8,&rv);
//            ASSERT_EFI_ERROR(Status);
//            rv &= F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid].AndData8;
//            rv |= F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid].OrData8;
//        }
//        Status=AmiSio->Access(AmiSio,TRUE,FALSE,F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid].Reg8,&rv);
//        ASSERT_EFI_ERROR(Status);
//        //Program COM Clock Source Registers.
//        if(F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid+6].AndData8 == 0xFF) {
//            rv=F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid+6].OrData8;
//        } else {
//            Status=AmiSio->Access(AmiSio, FALSE, FALSE, F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid+6].Reg8,&rv);
//            ASSERT_EFI_ERROR(Status);
//            rv &= F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid+6].AndData8;
//            rv |= F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid+6].OrData8;
//        }
//        Status=AmiSio->Access(AmiSio,TRUE,FALSE,F81966_DXE_COM_Mode_Init_Table[Dev->DeviceInfo->Uid+6].Reg8,&rv);
//        ASSERT_EFI_ERROR(Status);
...
//                if(EFI_ERROR(Status)) {
//                    SIO_TRACE((TRACE_SIO,"F81966_COM_Init: Fail to Get DSDT - returned %r\n", Status));
//                    ASSERT_EFI_ERROR(Status);
    ...
    case isPrsSelect:
        ...
        if(Dev->NvData.DevMode >3 ) {  //7 mode
            ...
            if(ACPI_SUPPORT) {
                ...
                //last parameter is 0-based index in F81966_DevLst[] table.
            ...
            Status=LibGetDsdt(&a,EFI_ACPI_TABLE_VERSION_ALL);
            if(EFI_ERROR(Status)) {
                SIO_TRACE((TRACE_SIO,"F81966_LPT_Init: Fail to Get DSDT - returned %r\n", Status));
                ASSERT_EFI_ERROR(Status);
            } else dsdt=(ACPI_HDR*)a;
...
)
{
    ...
    case isBeforeActivate:
        ...
        ProgramIsaRegisterTable(F81966_CONFIG_INDEX, F81966_CONFIG_DATA,\
...
)
{
    ...
    case isAfterBootScript:
        ...
        SioLib_BootScriptSioS3SaveTable(F81966_HWM_INDEX_PORT, F81966_HWM_DATA_PORT, \
