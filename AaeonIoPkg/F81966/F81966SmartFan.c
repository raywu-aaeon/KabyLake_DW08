...
void F81966_FAN_Full(VOID)
{
  ...
  for(i=0; i<( (sizeof(DXE_FAN1_INIT_TABLE))/(sizeof(DXE_SMF_INIT_DATA)) ); i++) {
      IoWrite8(F81966_HWM_INDEX_PORT, DXE_FAN1_INIT_TABLE[i].Reg8 ); 
  ...
  for(i=0; i<( (sizeof(DXE_FAN2_INIT_TABLE))/(sizeof(DXE_SMF_INIT_DATA)) ); i++) {
      IoWrite8(F81966_HWM_INDEX_PORT, DXE_FAN2_INIT_TABLE[i].Reg8 ); 
  ...
  for(i=0; i<( (sizeof(DXE_FAN3_INIT_TABLE))/(sizeof(DXE_SMF_INIT_DATA)) ); i++) {
      IoWrite8(F81966_HWM_INDEX_PORT, DXE_FAN3_INIT_TABLE[i].Reg8 ); 
...
EFI_STATUS SmartFanBootScript(EFI_S3_SAVE_STATE_PROTOCOL *bss)
{
    ...
    for (i=0; i < ((sizeof(HwmS3SaveRegisterTbl))/(sizeof(S3_SAVE_DATA))); i++) {
        ...
        if ( bank !=  0xFF ) {
          bank_reg = 0x4e;
          IoWrite8(F81966_HWM_INDEX_PORT, bank_reg);
          IoWrite8(F81966_HWM_DATA_PORT, bank);
        } 
        //select register to read actual data
        IoWrite8(F81966_HWM_INDEX_PORT, r);
        v = IoRead8(F81966_HWM_DATA_PORT);
        //save to s3 bootscrip table
        if ( bank != 0xFF ) {
          BOOT_SCRIPT_S3_IO_WRITE_MACRO(bss,EfiBootScriptWidthUint8,F81966_HWM_INDEX_PORT, 1, &bank_reg);
          BOOT_SCRIPT_S3_IO_WRITE_MACRO(bss,EfiBootScriptWidthUint8,F81966_HWM_DATA_PORT, 1, &bank);
        }

        BOOT_SCRIPT_S3_IO_WRITE_MACRO(bss,EfiBootScriptWidthUint8,F81966_HWM_INDEX_PORT, 1, &r);
        BOOT_SCRIPT_S3_IO_WRITE_MACRO(bss,EfiBootScriptWidthUint8,F81966_HWM_DATA_PORT, 1, &v);
    }
