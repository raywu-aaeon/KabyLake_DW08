...
///
static UINTN        mDynamicSmmPciExpressLibPciExpressBaseAddress = 0;
...
EFI_STATUS
EFIAPI
DynamicSmmPciExpressLibConstructor (
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "DynamicSmmPciExpressLibConstructor - Start\n"));

  if( gAmiCspSmmBase2 == NULL ) {
      Status = gBS->LocateProtocol( &gEfiSmmBase2ProtocolGuid, NULL, &gAmiCspSmmBase2 );
...
#ifndef PEI_BUILD
  if (gAmiCspInSmm) {
      mDynamicSmmPciExpressLibPciExpressBaseAddress = (PciCf8Read32 ((UINT32) PCI_CF8_LIB_ADDRESS(0, 0, 0, 0x60)) & 0xFF000000);
      DEBUG ((DEBUG_INFO, "mDynamicSmmPciExpressLibPciExpressBaseAddress: %X\n", mDynamicSmmPciExpressLibPciExpressBaseAddress));
  }
#endif
...
EFI_STATUS
EFIAPI
DynamicSmmPciExpressLibDestructor (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  )
{
  ...
  DEBUG ((DEBUG_INFO, "DynamicSmmPciExpressLibDestructor - End\n"));
...
{
#ifndef PEI_BUILD
  if (gAmiCspInSmm) return (VOID*)(UINTN) mDynamicSmmPciExpressLibPciExpressBaseAddress;
#endif
