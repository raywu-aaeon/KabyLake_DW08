...
)
{
    ...
    if ( FitTableEntry < BiosRomStaringAddress ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] The Fit entry(%x) is incorrect\n", FitTableEntry));
        return 0; 
    }
    ...
    DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] FitTableOffset = %x\n", FitTableOffset));
    DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] FitEntry[0].Address = %x\n", FitEntry[0].Address));
    
    if (FitEntry[0].Address != *(UINT64 *)"_FIT_   ") {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] The signature of Fit table is incorrect\n"));
        return 0;
    }
    if (FitEntry[0].Type != FIT_TABLE_TYPE_HEADER) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] FitEntry[0].Type is incorrect\n"));
        return 0;
    }
    EntryNum = *(UINT32 *)(&FitEntry[0].Size[0]) & 0xFFFFFF;
    for (Index = 0; Index < EntryNum; Index++) {
        if (FitEntry[Index].Type == Type) {
        	DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] The Fit type[%x] Physical Address = %x\n", Type, FitEntry[Index].Address ));
            return FitEntry[Index].Address;
        }
    }

    DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] The Fit type isn't found\n"));
    return 0;
}
...
)
{
    ...
    if ( Acm == NULL ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] Acm isn't found\n"));
        return EFI_SECURITY_VIOLATION;
    }

    //
    // 2.4.1 Boot Guard ACM Integration Requirement
    // The Boot Guard ACM must be uncompressed and minimally size aligned and the Boot
    // Guard ACM must be within the range of 4GB to 4GB minus 2MB. Supported
    // alignments are 32KB/64KB/128KB. It is recommended for Platform BIOS to choose
    // 64KB alignment as the ACM size might increase in the future (avoiding future BIOS flash map changes).
    //

    if ( (PhysicalAcmAddress & 0x0000FFFF) != 0 ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] Acm isn't 64KB alignment\n"));
        return EFI_SECURITY_VIOLATION;
    }

    if ( PhysicalAcmAddress < (UINT32)BASE_4GB - 0x200000 ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] Boot Guard ACM must be within the range of 4GB to 4GB minus 2MB\n"));
        return EFI_SECURITY_VIOLATION;
    }
    ...
    if ( MmioRead32 (MmPciBase (ME_BUS, ME_DEVICE_NUMBER, HECI_FUNCTION_NUMBER) + ME_VENDOR_ID) == 0xFFFFFFFF  ) {
    	DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] ME is disabled on the target platform, we can't verify the Boot Guard ACM\n"));
        return EFI_SUCCESS;
    }

    HFS = MmioRead32 (MmPciBase (ME_BUS, ME_DEVICE_NUMBER, HECI_FUNCTION_NUMBER) + R_ME_HFS_6);

    TargetACMSVN = (HFS & (BIT10 | BIT11 | BIT12 | BIT13)) >> 10;
    if ( Acm->AcmSvn < TargetACMSVN ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] ACMSVN is incorrect for target platform %x, %x\n", Acm->AcmSvn,TargetACMSVN));
        return EFI_SECURITY_VIOLATION;
    }

    DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] ACM is correct for the platform\n"));
    return EFI_SUCCESS;
}
...
)
{
    ...
    if ( Km == NULL ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] Km isn't found\n"));
        return EFI_SECURITY_VIOLATION;
    }
    ...
    if ( (PhysicalKmAddress & 0xF) != 0 ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] KM must be aligned to 16 bytes boundary.\n"));
        return EFI_SECURITY_VIOLATION;
    }

    if ( PhysicalKmAddress < (UINT32)(BASE_4GB - 0x400000) ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] KM must be uncompressed and reside within 4GB to 4GB minus 4MB region.\n"));
        return EFI_SECURITY_VIOLATION;
    }
    ...
    if ( MmioRead32 (MmPciBase (ME_BUS, ME_DEVICE_NUMBER, HECI_FUNCTION_NUMBER) + ME_VENDOR_ID) == 0xFFFFFFFF  ) {
    	DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] ME is disabled on the target platform, we can't verify the Boot Guard Km\n"));
        return EFI_SUCCESS;
    }
    ...
    if ( Km->KmSvn < TargetKMSVN ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] KMSVN is incorrect for target platform\n"));
        return EFI_SECURITY_VIOLATION;
    }

    DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] KM is correct for the platform \n"));
    return EFI_SUCCESS;
}
...
)
{
    ...
    if ( Bpm == NULL ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] Bpm isn't found\n"));
        return EFI_SECURITY_VIOLATION;
    }
    ...
    if ( (PhysicalBpmAddress & 0xF) != 0 ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] BPM must be aligned to 16 bytes boundary.\n"));
        return EFI_SECURITY_VIOLATION;
    }

    if ( PhysicalBpmAddress < (UINT32)(BASE_4GB - 0x400000) ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] BPM must be uncompressed and reside within 4GB to 4GB minus 4MB region.\n"));
        return EFI_SECURITY_VIOLATION;
    }
    ...
    if ( MmioRead32 (MmPciBase (ME_BUS, ME_DEVICE_NUMBER, HECI_FUNCTION_NUMBER) + ME_VENDOR_ID) == 0xFFFFFFFF  ) {
    	DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] ME is disabled on the target platform, we can't verify the Boot Guard Bpm\n"));
        return EFI_SUCCESS;
    }
    ...
    if ( Bpm->BPSVN < TargetBPMSVN ) {
    	DEBUG((DEBUG_ERROR, "[VerifyFwBootGuard.c] BPMSVN is incorrect for target platform\n"));
        return EFI_SECURITY_VIOLATION;
    }

    DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] BPM is correct for the platform\n"));
    return EFI_SUCCESS;
}
...
BootGuardValidate (
    ...
    if(!guidcmp((EFI_GUID*)&FWCapsuleHdr->CapHdr.CapsuleGuid, &gFWCapsuleGuid)) {
    	DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] CapsuleGuid OK\n"));
    ...
    }
    {
        ...
        if (!guidcmp((EFI_GUID*)(*pFwCapsule + FWCapsuleHdr->RomImageOffset + 0x60), &gMeFwCapsuleGuid)) return EFI_SUCCESS;
    }    
    
    DEBUG((DEBUG_INFO, "[VerifyFwBootGuard.c] BiosRom = %x, BiosRomSize = %x\n", BiosRom, BiosRomSize));    
...
BootGuardSetFlUpdMethod (
    ...
    if ( EFI_ERROR(Status) ) {
    	DEBUG((DEBUG_ERROR,"[VerifyFwBootGuard.c] Secure Flash verifies the BIOS ROM is fail\n\n"));
        return Status;
    }
    ...
    if ( EFI_ERROR(Status) ) {
    	DEBUG((DEBUG_ERROR,"[VerifyFwBootGuard.c] Boot Guard verifies the BIOS ROM is fail\n\n"));
        return Status;
    }

    DEBUG((DEBUG_INFO,"[VerifyFwBootGuard.c] Verify ROM is Pass\n"));
...
)
{
    ...
    DEBUG((DEBUG_INFO,"[VerifyFwBootGuard.c] InSmmFunction - Start\n"));

    Status = pSmst->SmmLocateProtocol(&gSecureSMIFlashProtocolGuid, NULL, &SecSmiFlash);
    if ( EFI_ERROR(Status) ) {
    	DEBUG((DEBUG_ERROR,"[VerifyFwBootGuard.c] Locate gSecureSMIFlashProtocolGuid is fail\n"));
        return Status;
    }
    ...
    DEBUG((DEBUG_INFO,"[VerifyFwBootGuard.c] InSmmFunction - End\n"));
    return Status;
}
