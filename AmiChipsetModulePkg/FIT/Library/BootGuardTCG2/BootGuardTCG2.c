...
)
{
    ...
    DEBUG(( DEBUG_INFO, "[BootGuardTCG2.c] : ACM_PostSuccess : EFI_UNSUPPORTED\n"));
...
)
{
    ...
    DEBUG(( DEBUG_INFO, "BootGuardTCG2.c[%d] : Enter CreateBootGuardTPM12Event(...)\n", __LINE__));
...
//    UINTN               DigestSize = SHA1_DIGEST_SIZE;
    ...
    DEBUG(( DEBUG_INFO, "BootGuardTCG2.c[%d] : Enter CreateBootGuardTPM20Event(...)\n", __LINE__));
...
//    DEBUG(( DEBUG_INFO, "[%x]: ptrEvent[%x], Len[%x]\n", __LINE__, ptrEvent, Len));
...
)
{
    ...
    do
    {
        ...
        if ( ( 0xFFFFFFFF == (UINTN)FitEntry ) || ( NULL == FitEntry ) )
        {
            ...
            DEBUG(( DEBUG_ERROR, "[BootGuardTCG2.c] : FitEntryPointer(%lx) is Invalid\n", FitEntryPointer));
            break;
        }
...
{
    // For Skylake/Kabylake, we don't need to add the startup locality Event
    // Mabe later check the status
   return FALSE;
}
...
)
{
    ...
    DEBUG(( DEBUG_INFO,  "[BootGuardTCG2.c] : Enter LogAuthorityPCREvent(...)\n"));
...
)
{
    ...
    DEBUG(( DEBUG_INFO,  "[BootGuardTCG2.c] : Enter LogDetailPCREvent(...)\n"));
    ...
    VOID *_TrEEPeiPpi )
{
    ...
    DEBUG(( DEBUG_INFO, "BootGuardTCG2.c[%d]: Enter BootGuardTcg2MeasureCRTMVersion\n", __LINE__));
    ...
    if( !EFI_ERROR(Status) )
    {
        Status = ACM_TpmStsChk( PeiServices );
        if( !EFI_ERROR(Status) ) {
