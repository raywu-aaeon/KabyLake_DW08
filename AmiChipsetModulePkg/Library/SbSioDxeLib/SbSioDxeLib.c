...
EFI_STATUS GetPrsFromTable(SIO_DEV2* Dev, UINTN Index){
	...
	T_ITEM_LIST					*dl;
//-----------------------------------
    DEBUG((DEBUG_INFO, "Dev[%d]: PRS Tbl=> \n", Index));

    if (Dev->DeviceInfo->Flags==SIO_NO_RES){
        DEBUG((DEBUG_INFO, " Device NOT USING Resources FLAGS=0x%X\n", Dev->DeviceInfo->Flags));
        return EFI_SUCCESS;
    }

	i = de->ResLen[0] | de->ResLen[1] | de->IrqMask[0] | de->IrqMask[1] | de->DmaMask[0] | de->DmaMask[1];
	if(!i){
		DEBUG((DEBUG_ERROR, "Entry is Empty! Check 'YourSioName_DevLst' table.\n"));
	...
	DEBUG((DEBUG_INFO, "DMA1=%X; ",de->DmaMask[0]));
	...
	DEBUG((DEBUG_INFO, "DMA2=%X;",de->DmaMask[1]));
	...
	DEBUG((DEBUG_INFO, "DepFn=%X,ItemCount=%d; ",(EFI_ASL_DepFn*)dl,((EFI_ASL_DepFn*)dl)->DepRes.ItemCount));
    for(i=0;i<6;i++) {
    	DEBUG((DEBUG_INFO, "Item[%d]=%X  ",i,((EFI_ASL_DepFn*)dl)->DepRes.Items[i]));
    }
	DEBUG((DEBUG_INFO, "\n <= Dev[%d]: PRS Tbl End \n",Index));
...
//---------------------------
	...
	if(EFI_ERROR(Status)) {
		DEBUG((DEBUG_ERROR, "SIO[%d]: Aml=> Failed to Locate ASL Object %a in DSDT at %08X\n",Index,&de->AslName[0], gDsdtPtr));
	...
	if(EFI_ERROR(Status)) {
		DEBUG((DEBUG_ERROR, "SIO[%d]: Aml=> Failed to Locate _PRS Object in %a Object Scope at %08X\n",Index, &de->AslName[0], obj.Object));
		return GetPrsFromTable(Dev, Index);
	}
	...
	//---------------------
		...
		//------------------------------
			...
			if((prsb+bl)<p){ //check if we still inside _PRS buffer or went out..
				DEBUG((DEBUG_ERROR, "SIO[%d]: Aml=> Failed to Locate EndDepFn in _PRS buffer of %a Object at %08X\n",Index ,&de->AslName[0],prs.Object));
				return GetPrsFromTable(Dev, Index);
			}
			rd=(ASLRF_S_HDR*)p;
		...
		if(!Dev->PRS.ItemCount){
			DEBUG((DEBUG_ERROR, "SIO[%d]: Aml=> Failed to Locate StartDepFn in _PRS buffer of %a Object at %08X\n",Index,&de->AslName[0],prs.Object));
			return GetPrsFromTable(Dev, Index);
		}
		DEBUG((DEBUG_INFO, "SIO[%d]: Aml=> Collected %d DepFunc Items of %a Object \n",Index,Dev->PRS.ItemCount,&de->AslName[0]));
	} else {
		DEBUG((DEBUG_ERROR, "SIO[%d]: Aml=> _PRS Object of %a Object is not a Buffer Type at %08X\n",Index,&de->AslName[0],prs.Object));
		return GetPrsFromTable(Dev, Index);
	}
	return Status;
}
...
//------------------------
	...
	if(CompDevInfo->InitRoutine!=NULL){
		DEBUG((DEBUG_INFO, "GSIO: Compatible InitRoutine(is = %d) ... ",InitStep)); 
...
//------------------------
	...
	if(InitProtocol==NULL){
		DEBUG((DEBUG_ERROR, "GSIO: InitProtocol Not Present! \n"));
		return EFI_SUCCESS;
	}
	
	if(InitProtocol->FunctionCount < InitFunction) {
		DEBUG((DEBUG_ERROR, "GSIO: InitProtocol->FunctionCount(%d) < InitFunction (%d)!\n", InitProtocol->FunctionCount, InitFunction));
