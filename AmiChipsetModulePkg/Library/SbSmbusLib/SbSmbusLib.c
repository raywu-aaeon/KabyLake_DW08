...
#include <Library/PciLib.h> // For Dynamic Pcie Length Adjustment feature
  ...
  )
{
  ...
  SmBusBase = MmioRead16 ((UINTN) SMBUS_PCIE_CFG_ADDRESS(R_SB_SMBUS_BASE_ADDR)) & 0xFFF0;
  SmBusCmd  = MmioRead16 ((UINTN) SMBUS_PCIE_CFG_ADDRESS(R_SB_SMBUS_PCICMD));

  if (SmBusBase == 0) { // Assign a new I/O if the original address is 0
      MmioWrite16 (
          (UINTN) SMBUS_PCIE_CFG_ADDRESS(R_SB_SMBUS_BASE_ADDR),
          SMBUS_BASE_ADDRESS
          );
  }
  ...
  if ((SmBusCmd & 1) == 0) { // Enable I/O command if needed.
      MmioWrite16 (
          (UINTN) SMBUS_PCIE_CFG_ADDRESS(R_SB_SMBUS_PCICMD),
          3
          );
  }

  ReturnStatus = SmbusExec (
                  SmbusDeviceAddress,
