...
)
{
    ...
    if ( !((ReadMsr (MSR_PLATFORM_INFO) & B_MSR_PLATFORM_INFO_BIOSGUARD_AVAIL) &&
        ...
        //   	
    	DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BIOS Guard is disabled. \n"));       
      return FALSE;
    } else {
    	DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BIOS Guard is enabled. \n"));      
    }
...
)
{
    ...
    if(IsBiosGuardFlashInitialized()) {
    	...
    	DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BiosGuardFlashEraseCommand = %lX \n", (UINTN)(pBlockAddress - BIOS_GUARD_FLASH_START_ADDRESS)));    
#endif        
    ...
    //    }
        ...
        if( EFI_ERROR(Status) ) {
#if BIOS_GUARD_DEBUG_MODE
            DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BiosGuardFlashEraseCommand RETRY. \n"));           
#endif
            BiosGuardInstance->Erase( BiosGuardInstance, (UINTN)(pBlockAddress - BIOS_GUARD_FLASH_START_ADDRESS) );
...
#if BIOS_GUARD_DEBUG_MODE
    	DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] Non-BiosGuardFlashEraseCommand. \n"));      
#endif        
...
#if BIOS_GUARD_DEBUG_MODE
    DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BiosGuardIsEraseCompleted. \n")); 
#endif
...
)
{
    ...
    if(IsBiosGuardFlashInitialized()) {
#if BIOS_GUARD_DEBUG_MODE	    
    	DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BiosGuardProgramCommand = %lX \n", (UINTN)(pByteAddress - BIOS_GUARD_FLASH_START_ADDRESS)));       
#endif        
     ...
     //   }
        ...
        if( EFI_ERROR(Status) ) {
#if BIOS_GUARD_DEBUG_MODE        	
            DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BiosGuardProgramCommand RETRY. \n"));          
#endif            
            BiosGuardInstance->Write(BiosGuardInstance,
                                     (UINTN)(pByteAddress - BIOS_GUARD_FLASH_START_ADDRESS),
                                     *Length, Byte);
...
#if BIOS_GUARD_DEBUG_MODE	    
    	DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] Non-BiosGuardProgramCommand. \n"));    
#endif        
        if ( FlashInitialized == NULL) return FALSE;
...
#if BIOS_GUARD_DEBUG_MODE
    DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BiosGuardProgramCompleted. \n"));   
#endif    
  ...
  )
{
...
#if BIOS_GUARD_DEBUG_MODE    
        DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c] BiosGuardFlashLibConstructor Start\n"));
#endif        
    ...
    if ( !((ReadMsr (MSR_PLATFORM_INFO) & B_MSR_PLATFORM_INFO_BIOSGUARD_AVAIL) &&
        ...
        //
#if BIOS_GUARD_DEBUG_MODE	              
        DEBUG((DEBUG_INFO, "[BiosGuardFlashLib.c - BiosGuardFlashLibConstructor] BIOS Guard  is disabled. \n"));
#endif        
        return EFI_SUCCESS;
    }
