...
)
{
    ...
    BiosGuardBiosStartRomAddress = 0xFFFFFFFF - FULL_BIOS_IMAGE_SIZE + BiosGuardBiosStartRomAddress;
    DEBUG((DEBUG_INFO, "BiosGuardBiosStartRomAddress = %x\n", BiosGuardBiosStartRomAddress));
    ...
    for (i = 0, Index = 0; BiosGuardRecoveryMap[i].Name != NULL; i++) {
        ...
        for (j = StringLength(RecoveryRomAddress + sizeof(UINT32)); j < AmiBiosGuardHeaderLength; j++ ) {
            ...
            DEBUG ((DEBUG_INFO, "Current data = %X \n", *(RecoveryRomAddress + j) ));
...
#if BIOS_GUARD_DEBUG_MODE            
            DEBUG ((DEBUG_INFO, "BiosGuardRecoveryMap[%d].NumOfBlocks = %d \n", i , BiosGuardRecoveryMap[i].NumOfBlocks));
#endif
            GetBiosGuardBlockAddress (Index, (UINT32*)&BiosGuardRecoveryMap[i].BlockAddress, (UINT32*)&Source);
            if ((BiosGuardRecoveryMap[i].BlockAddress > BiosGuardBiosStartRomAddress) && (FlashAddressBase < BiosGuardBiosStartRomAddress)) {
                BiosGuardRecoveryMap[i].BlockAddress = FLASH_BIOS_BASE + (BiosGuardRecoveryMap[i].BlockAddress - BiosGuardBiosStartRomAddress - 1);
            }
#if BIOS_GUARD_DEBUG_MODE    	    
            DEBUG ((DEBUG_INFO, "Index = %d \n", Index));            
            DEBUG ((DEBUG_INFO, "BiosGuardRecoveryMap[%d].BlockAddress = %X \n", i , BiosGuardRecoveryMap[i].BlockAddress));
#endif
...
)
{
    ...
    DEBUG((DEBUG_INFO, "BiosGuardFlashProgram(%x)\n", Block->BlockAddress));
...
)
{
    ...
    if ( !((ReadMsr (MSR_PLATFORM_INFO) & B_MSR_PLATFORM_INFO_BIOSGUARD_AVAIL) &&
        ...
        //
        DEBUG ((DEBUG_INFO, "BiosGuardRecoveryWorker.c : BIOS Guard is disabled or not supported\n"));
        return;
    }
...
//        ASSERT_EFI_ERROR (Status);
    ...
    //EIP429536 >>
    if (((RECOVERY_IMAGE_HOB*)HobList)->ImageSize == BIOSGUARD_MEUD_BIOS_SIZE){
    ...
    if (((RECOVERY_IMAGE_HOB*)HobList)->ImageSize == (BIOSGUARD_MEUD_BIOS_SIZE + FV_MEFW_CAPSULE_SIZE + EC_FIRMWARE_BIN_SIZE)) {
        FlashAddressBase = FLASH_BIOS_BASE;
    }
#endif
#endif
#if defined (BIOS_GUARD_ECFW_UPDATE_SUPPORT) && (BIOS_GUARD_ECFW_UPDATE_SUPPORT == 1)
    if (((RECOVERY_IMAGE_HOB*)HobList)->ImageSize == (BIOSGUARD_MEUD_BIOS_SIZE + EC_FIRMWARE_BIN_SIZE)) {
        FlashAddressBase = FLASH_BIOS_BASE;
    }    
#endif
    ...
    for(i = 0; BlocksToUpdate[i].Type != FvTypeMax; i++) {
        for (j = 0; BiosGuardRecoveryMap[j].Name != NULL; j++) {
            ...
            DEBUG ((DEBUG_INFO, "BiosGuardRecoveryMap[%d].Type = %X \n", j , BiosGuardRecoveryMap[j].Type));
        	
            DEBUG ((DEBUG_INFO, "BlocksToUpdate[%d].BlockAddress = %X \n",i,  BlocksToUpdate[i].BlockAddress));
            DEBUG ((DEBUG_INFO, "BlocksToUpdate[%d].Type = %X \n", i, BlocksToUpdate[i].Type));
            ...
            if ((BiosGuardRecoveryMap[j].BlockAddress == (UINT32)BlocksToUpdate[i].BlockAddress) && \
                ...
                DEBUG ((DEBUG_INFO, "Found.BlockAddress = %X \n", BiosGuardRecoveryMap[j].BlockAddress));
                DEBUG ((DEBUG_INFO, "Found.Type = %X \n", BiosGuardRecoveryMap[j].Type));        	
