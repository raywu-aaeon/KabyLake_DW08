...
External(ASL_PCI_SCOPE.DVI1)
External(ASL_PCI_SCOPE.CBC1)
External(ASL_PCI_SCOPE.DVI9)
External(ASL_PCI_SCOPE.CBC9)
External(ASL_PCI_SCOPE.HGON, MethodObj)
External(ASL_PCI_SCOPE.HGOF, MethodObj)
 ...
 Scope(ASL_PCI_SCOPE) 
 {
        Device (WMI2)
        {
            ...
            Method (WMOP, 3, NotSerialized)
            {
                ...
                {
                    // Power on DGPU
                    ASL_DGPUPCH_SCOPE._PS0()                      // IBV_customize: root port path
                ...
                If (LEqual (Arg1, 2) )
                {
                    ...
                    If (LEqual (ASL_DGPUPCH_SCOPE.DGPS, Zero))    // IBV_customize: root port path
                    {
                        Return(0x10)
                    } 
                    Else
