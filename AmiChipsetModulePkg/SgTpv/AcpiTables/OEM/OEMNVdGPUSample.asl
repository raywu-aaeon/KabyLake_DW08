...
//<AMI_SHDR_END>
    ...
    OperationRegion(PCIM, SystemMemory, Add(\XBAS,ShiftLeft(\_SB.PCI0.PEG0.PRBN,20)), 0x0600)
...
Device(EDP1)
{
  ...
  Method (_BCM,1)
  {
    ...
    //
    // It is by OEM board to port it, below sample code is for Intel IGFX
...
//
// CMPB() - Compare two buffers.
// Arg0,1 - buffers.
// Exit - 0: buffers not the same
//        1: buffers are the same
//
    Method(CMPB, 2, NotSerialized)
    {
        ...
        {
            Return(Zero)                        // If size of buffers not the same, return 0.
        }
        Store(0x00, Local0)
        While(LLess(Local0, Local1))
        {
            ...
            {
                Return(Zero)                    // any single mismacthed, return 0.
            }
            Increment(Local0)
        }
        Return(One)                             // Every bytes matched, return 1.
    }
//<AMI_PHDR_START>
//------------------------------------------------------------------------
//
...
//<AMI_PHDR_END>
    Method(_DSM,4,SERIALIZED)
    {
        ...
        // common _DSM for dGPU and iGPU: NBCI, SG DSM, Ventura
            ...
            If (CMPB(Arg0, Buffer(){0x75,0x0B,0xA5,0xD4,0xC7,0x65,0xF7,0x46,0xBF,0xB7,0x41,0x51,0x4C,0xEA,0x02,0x44}))
            {
                Return( NBCI(Arg0, Arg1, Arg2, Arg3) )
            }
#if ASL_NV_GPS_SUPPORT == 1
            //GPS_DSM_GUID     {A3132D01-8CDA-49BA-A52E-BC9D46DF6B81}
            Elseif (CMPB(Arg0, Buffer(){0x01,0x2D,0x13,0xA3,0xDA,0x8C,0xBA,0x49,0xA5,0x2E,0xBC,0x9D,0x46,0xDF,0x6B,0x81}))
            {
                Return( ASL_DGPU_SCOPE.GPS(Arg0, Arg1, Arg2, Arg3) )
            }
#endif
#if ASL_NVSR_SUPPORT
            //JT_DSM_GUID      {CBECA351-067B-4924-9CBD-B46B00B86F34}
            Elseif (CMPB(Arg0, Buffer(){0x51,0xA3,0xEC,0xCB,0x7B,0x06,0x24,0x49,0x9C,0xBD,0xB4,0x6B,0x00,0xB8,0x6F,0x34}))
            {
                Return( ASL_DGPU_SCOPE.NVJT(Arg0, Arg1, Arg2, Arg3) )
            }
#endif
            Elseif (LEqual (Arg0, Buffer (0x10)
                {
        ...
        Name(GSV1, Buffer(0x01)
        {
             0xff
        })
        Name(GSV2, Buffer(0x01)
        {
            0xff
        })
        Name(GSDR, Buffer(0x01)
        {
            0xff
        })
        Method(NBCI, 4, Serialized)
        {
            ...
            If(LEqual(Arg2, Zero))              // NBCI_FUNC_SUPPORT
            {
                Return(Buffer(0x04)
                {
                    0x03, 0x00, 0x01, 0x00      // sub-func: 0,1,16 supported                
                  //  0x03, 0x00, 0x11, 0x00      // sub-func: 0,1,16,20 supported
                })
            }
            If(LEqual(Arg2, One))               // NBCI_FUNC_PLATCAPS
            {
            ...
            If(LEqual(Arg2, 0x10))              // NBCI_FUNC_GETOBJBYTYE
            {
            ...
            If(LEqual(Arg2, 0x14))              // NBCI_FUNC_GETBACKLIGHT
            {
                Return(Package(32)
                {
                 // OEM_customize: Package returned in this interface is panel related. OEM SBIOS need to follow definition in NBSI Spec to reflect real panel feature in this pacakge. 
                  //  Programming by OEM board design
...
//<AMI_PHDR_END> 
    ...
    Method(_ON, 0, Serialized)
    {
        ...
        ASL_PCI_SCOPE.PGON(ASL_SG_H_PORT_FUNC)
