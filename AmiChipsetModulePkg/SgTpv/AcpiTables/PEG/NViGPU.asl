...
//x     if(CMPB(Arg0, Buffer(){0xFD,0x88,0xDB,0x95,0x0A,0x94,0x53,0x42,0xA4,0x46,0x70,0xCE,0x05,0x04,0xAE,0xDF}))
        ...
        {
            return ( ASL_DGPU_SCOPE.SPB(Arg0, Arg1, Arg2, Arg3))
        }
#endif
...
//x     if(CMPB(Arg0, Buffer(){0x01, 0x2D, 0x13, 0xA3, 0xDA, 0x8C, 0xBA, 0x49,0xA5, 0x2E, 0xBC, 0x9D, 0x46, 0xDF, 0x6B, 0x81}))
        ...
        {
            return ( ASL_DGPU_SCOPE.NGC6(Arg0, Arg1, Arg2, Arg3))                 
        }
#endif
        ...
        P8DB(0xEE, Arg2, 1000)
            ...
            If(LEqual(Arg2,0))
            {
                if(SGCI){
                    ...
                    if(NBCI){
                        Return(Buffer()ASL_NBCI_SUPPORT_FUNCTION)
                    }
                    else {
                        if(OPCI){
...
//                            Return(ToBuffer(0x0c010001))
            ...
            If(LEqual(Arg2,3))
            {
                ...
                If(LEqual(Local0,2))
                {
                    ASL_PCI_SCOPE.PGOF(ASL_SG_H_PORT_FUNC)
                }
    
                //dGPU_PWROK is not working. Using dGPU_PWR_EN# instead as w/a
                //Or(STS2,ASL_DGPU_SCOPE.MPOK,STS2)
                If(LEqual(ASL_DGPU_SCOPE.SGST(), 0xF))
                {
                    Or(STS2,0x1,STS2)
                }
...
// common for SGCI and NBCI
            ...
            If(LEqual(Arg2,16))
            {
                ...
                // Key start -
                
                // Key end -
                })
...
// common for SGCI and NBCI
		...
		CreateField(Arg3,0x02,One,DVSC)
                If(ToInteger(FLCH))
                {
                    Store(OMPR, ASL_DGPU_SCOPE.OPCE)                   // Optimus Power Control Enable - From DD
                }
                // On return
		...
		If(ToInteger(DVSC))
		{
			If(ToInteger(DVSR))
			{
				Store(One, ASL_DGPU_SCOPE.GPRF)
			}
			Else
			{
				Store(Zero, ASL_DGPU_SCOPE.GPRF)
			}
                } 
		Store(ASL_DGPU_SCOPE.GPRF, SNSR)
                If(LNotEqual(ASL_DGPU_SCOPE.SGST(), 0))
                {
...
//      return (ASL_DGPU_SCOPE.HDSM(Arg0, Arg1, Arg2, Arg3))
      Return (0x80000001)
    }
} // end PCI0.GFX0 scope
//**********************************************************************
