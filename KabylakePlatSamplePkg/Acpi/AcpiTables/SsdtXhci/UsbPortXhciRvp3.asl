...
DefinitionBlock (
    ...
    "xh_rvp03", // OEM table ID  (8 byte string)
...
// AMI_OVERRIDE_START - Fix HLK USB termination fail on RVP3
#if 0
    Method(_UPC) { Return (TUPC(9)) }
    Method(_PLD) { Return (TPLD(1,1)) } //USB-C Port 1
#else
    Method(_UPC) { Return (GUPC(1)) }
    Method(_PLD) { Return (GPLD(1,1)) }
#endif
// AMI_OVERRIDE_END - Fix HLK USB termination fail on RVP3
  }
  ...
  Scope (\_SB.PCI0.XHC.RHUB.HS02) {
    ...
    Method(_PLD) {
      If(LEqual(USME, 0)) {
          Return (GPLD(1,2)) //walkup port 4
      } Else {
          Return (TPLD(1,2)) //TBT USB-C Port 2
      }
    }
  }
  ...
  Scope (\_SB.PCI0.XHC.RHUB.HS04) {
    Method(_UPC) { Return (GUPC(1)) }
    Method(_PLD) { Return (GPLD(0,4)) } //webcam
  }
  ...
  Scope (\_SB.PCI0.XHC.RHUB.HS05) {
    ...
    Method(_PLD) {
      If(LEqual(USME, 0)) {
          Return (GPLD(1,5)) //walkup port 4
      } Else {
          Return (TPLD(1,5)) //TBT USB-C Port 2
      }
    }
  }
...
#else
    Method(_UPC) { Return (GUPC(1)) }
    Method(_PLD) { Return (GPLD(1,1)) } //USB-C Port 1
#endif
// AMI_OVERRIDE_END - Fix HLK USB termination fail on RVP3 
  }
  ...
  Scope (\_SB.PCI0.XHC.RHUB.SS03) {
    Method(_UPC) { Return (GUPC(0)) }
    Method(_PLD) { Return (GPLD(0,0)) } // DS4 Camera
