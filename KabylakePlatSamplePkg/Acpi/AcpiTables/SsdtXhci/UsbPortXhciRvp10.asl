...
DefinitionBlock (
    "UsbPortXhciRvp10.aml",
    "SSDT",
    0x02, // SHAD revision.
        // A Revision field value greater than or equal to 2 signifies that integers
        // declared within the Definition Block are to be evaluated as 64-bit values
    "INTEL", // OEM ID (6 byte string)
    "xh_rvp10", // OEM table ID  (8 byte string)
    ...
    )
{
  ...
  If(LEqual(PCHV(),SPTH)) {
    Scope (\_SB.PCI0.XHC.RHUB.HS11) {
      Method(_UPC) { Return (GUPC(1)) }
      Method(_PLD) { Return (GPLD(0,11)) } //m.2
    }
  ...
  Scope (\_SB.PCI0.XHC.RHUB.SS01) {
    Method(_UPC) { Return (GUPC(1)) }
    Method(_PLD) { Return (GPLD(1,14)) } //walkup or docking station port
  }
  ...
  If(LEqual(PCHV(),SPTH)) {
    ...
    Scope (\_SB.PCI0.XHC.RHUB.SS09) {
      Method(_UPC) { Return (GUPC(1)) }
      Method(_PLD) { Return (GPLD(1,13)) } //walkup or docking station port
    }
