...
DefinitionBlock (
    "UsbPortXhciRvp16.aml",
    "SSDT",
    0x02, // SHAD revision.
        // A Revision field value greater than or equal to 2 signifies that integers
        // declared within the Definition Block are to be evaluated as 64-bit values
    "INTEL", // OEM ID (6 byte string)
    "xh_rvp16", // OEM table ID  (8 byte string)
    ...
    )
{
  ...
  Scope (\_SB.PCI0.XHC.RHUB.HS01) {     // USB type C USB2
  ...
  If(LEqual(PCHV(),SPTH)) {
    ...
    Scope (\_SB.PCI0.XHC.RHUB.HS12) {
      Method(_UPC) { Return (GUPC(1)) }
      Method(_PLD) { Return (GPLD(0,12)) } //touchpanel
    }
  ...
  Scope (\_SB.PCI0.XHC.RHUB.SS01) {     // USB tyep C USB3
    Method(_UPC) { Return (TUPC(9)) }
    Method(_PLD) { Return (TPLD(1,1)) }
  }
  ...
  If(LEqual(PCHV(),SPTH)) {
    ...
    Scope (\_SB.PCI0.XHC.RHUB.SS09) {
      Method(_UPC) { Return (GUPC(1)) }
      Method(_PLD) { Return (GPLD(1,9)) } //walkup or docking station port
    }
