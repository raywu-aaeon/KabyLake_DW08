...
DefinitionBlock (
    "UsbPortXhciAmlAep.aml",
    "SSDT",
    0x02, // SHAD revision.
        // A Revision field value greater than or equal to 2 signifies that integers
        // declared within the Definition Block are to be evaluated as 64-bit values
    "INTEL", // OEM ID (6 byte string)
    "xh_mlaep", // OEM table ID  (8 byte string)
...
//end scope RHUB
  ...
  Scope (\_SB.PCI0.XHC.RHUB.HS02) {
    ...
    Method(_PLD) { Return (GPLD(0,2)) } //webcam
  }
  ...
  Scope (\_SB.PCI0.XHC.RHUB.HS04) {
    Method(_UPC) { Return (TUPC(9)) }
    Method(_PLD) { Return (TPLD(1,4)) } //USB-C Port 2
  }

  Scope (\_SB.PCI0.XHC.RHUB.HS05) {
    Method(_UPC) { Return (GUPC(1)) }
    Method(_PLD) { Return (GPLD(0,5)) } //m.2 connector
  }
