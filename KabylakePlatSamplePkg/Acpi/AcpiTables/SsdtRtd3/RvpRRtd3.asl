/** @file
  ACPI RTD3 SSDT table for KBL RVPR
...
External(\_SB.PCI0.XHC.RHUB.HS06, DeviceObj) //xHCI HSP port 6, WWAN
...
External(\_SB.PCI0.RP09.PXSX.PAHC, MethodObj)
External(\_SB.PCI0.RP09.PXSX.PNVM, MethodObj)
...
External(\_SB.PCI0.RP12.PXSX, DeviceObj) // For WWAN

External(\_SB.PCI0.XHC.RHUB.HS03, DeviceObj)
External(\_SB.PCI0.RP06.PXSX.WIST, MethodObj)
...
// PCIe root ports - START
  ...
  If(LEqual(\RTBT, 1)) {
    ///
    /// PCIe RTD3 - onboard TBT AR
    ///
    Scope(\_SB.PCI0.RP01)
    {
      // reset pin = IOEX_P10(0,8) - Requires rework
      // power pin = IOEX_P14(0,12)
      // wake pin  = GPIO_SKL_LP_GPP_B18
      // CLK_REQ   = Mask Enable of Mapped CLKREQ# for CLKOUT_SRC0 (MSKCRQSRC0):
      Name(SLOT, 1)   ///- port #1
      // Note: By default, KBL-R board doesn't support RSTG. Rework is needed to enable RTD3 RST support.
      Name(RSTG, Package() {
  ...
  If (\_SB.PCI0.RP06.PXSX.WIST ()) {
    ...
    Scope(\_SB.PCI0.RP06)
    {
      ...
      Name(WAKG, Package() {
            1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
            0, // IOEX #
            GPIO_SKL_LP_GPP_B16, // GPIO pad #/IOEX pin #
            1  // wake en polarity
          })
      Name(SCLK, Package() {
          1,    // 0-disable, 1-enable
...
// AMI_OVERRIDE_END - Fix Linux build error.
  ...
  If (LEqual(And(RCG0, 0x80), 0x80)) {
    ...
    Scope(\_SB.PCI0.RP12)
    {
      // reset pin = IOEX(0,22)
      // power pin = IOEX(1,0)
      // wake pin  = GPIO(GPIO_SKL_LP_GPP_E1)
      // CLK_REQ   = Mask Enable of Mapped CLKREQ# for CLKOUT_SRC5 (MSKCRQSRC5)
      Name(SLOT, 12)   ///- port #12
      Name(RSTG, Package() {
            2, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
            0, // IOEX #
      ...
      Name(WAKG, Package() { //WAKG[2] refers to GPIO_SKL_LP_GPP_E1
            1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
            0, // IOEX #
            GPIO_SKL_LP_GPP_E1, // GPIO pad #/IOEX pin #
            1  // wake en polarity
          })
      Name(SCLK, Package() {
          1,    // 0-disable, 1-enable
      ...
      Include("Rtd3SptPcieWWAN.asl")

      Scope (PXSX) {
        // For WWAN endpoint device
        Method(_PR0, 0) {
          Return (Package(){\_SB.PCI0.RP12.PXP})
        }
    ...
    // Based on GpioGroup to GPE0_DWx mapping GPP_E1 (WAKG) will map to _L41
    Scope(\_GPE) {
      Method(_L41) {
        \_SB.SHPO(GPIO_SKL_LP_GPP_E1, 1) // set gpio ownership to driver(0=ACPI mode, 1=GPIO mode)
      ...
      Method(_PR0, Zero, NotSerialized, 0, PkgObj){
        If(LOr(PAHC(), PNVM())){ // Check if PCIe AHCI controller or NVMe
          If (LAnd(LGreaterEqual(\OSYS, 2015), LEqual(\EMOD, 1))) {
            ...
            If (CondRefOf(\_SB.PCI0.RP09.PXP)) {
              Return(Package(){\_SB.PCI0.RP09.PXP})
            }
          }
        }
        Return(Package(){})
...
// SATA - START
  ...
  /// Port 0 is on Flex - no power controll suported.
  If(And(RCG1_RTD3_PRT0_ENABLED,RCG1)) {
    Scope(\_SB.PCI0.SAT0.PRT0) {
      ...
      // include sata port power resource
// AMI_OVERRIDE_START - Fix Linux build error.      
  ...
  Scope(\_SB.PCI0.XHC.RHUB.SS03)        // USB Superspeed Port 3 - IVCAM
    ...
    If(LEqual(And(XHPR,0xF0),0x20)){
      Scope(\_SB.PCI0.XHC.RHUB.SS02){
        ...
        Name(_PR2, Package(){\PX02})
        Name(_PR3, Package(){\PX02})
      } //  End of Scope(\_SB.PCI0.XHC.RHUB.SS02)
    }
