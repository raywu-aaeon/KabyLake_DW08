...
// PCIe root ports - START
  If(LEqual(\RTBT, 1)) {
    ...
    Scope(\_SB.PCI0.RP01)
    {
      ...
      Method (PPRW ,0) {
        // The method set for RP05_PRW GPE
        Return (GPRW (GGPE (GPIO_SKL_LP_GPP_B18), 4))
      }
    }
  }
    ...
    Scope(\_SB.PCI0.RP08)
    {
      ...
      Name(RSTG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_F11, // GPIO pad #/IOEX pin #
              0  // reset pin de-assert polarity
            })
      Name(PWRG, Package() {
...
// AMI_OVERRIDE_END - Fix Linux build error.
    ...
    Scope(\_SB.PCI0.RP09)
    {
      ...
      Name(RSTG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_F10, // GPIO pad #/IOEX pin #
              0  // reset pin de-assert polarity
            })
      Name(PWRG, Package() {
              0, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
...
// AMI_OVERRIDE_END - Fix Linux build error.
    ...
    If(LEqual(And(RCG0, 0x80), 0x80)) {
      ...
      {
        // reset pin = GPIO_SKL_LP_GPP_C13
        // power pin = IOEX1_P00(1,0)
        // wake pin  = GPIO_SKL_LP_GPP_C15
        // CLK_REQ   = Mask Enable of Mapped CLKREQ# for CLKOUT_SRC4 (MSKCRQSRC4):
        Name(SLOT, 10)   ///- port #10
        Name(RSTG, Package() {
                1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
                0, // IOEX #
                GPIO_SKL_LP_GPP_C13, // GPIO pad #/IOEX pin #
                0  // reset pin de-assert polarity
              })
        Name(PWRG, Package() {
                2, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
        ...
        Name(WAKG, Package() {
                1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
                0, // IOEX #
                GPIO_SKL_LP_GPP_C14, // GPIO pad #/IOEX pin #
                1  // wake en polarity
              })
        Name(SCLK, Package() {
                1,    // 0-disable, 1-enable
...
//GPE Event handling - Start
  Scope(\_GPE) {
    ...
    Method(AL6F) {
      ...
      // WWAN Wake event
      If (\_SB.ISME(GPIO_SKL_LP_GPP_C14))
      {
        \_SB.SHPO(GPIO_SKL_LP_GPP_C14, 1) // set gpio ownership to driver(0=ACPI mode, 1=GPIO mode)
        Notify(WWAN_PCIE_ROOT_PORT, 0x02)   // device wake
        \_SB.CAGS(GPIO_SKL_LP_GPP_C14)
      }
    }
  } //Scope(\_GPE)
