/** @file
  ACPI RTD3 SSDT table for SKL RVP7
...
External(\_SB.PCI0.RP01.PXSX.PAHC, MethodObj)
External(\_SB.PCI0.RP01.PXSX.PNVM, MethodObj)
...
// AMI_OVERRIDE_END - Fix Linux build error.      
    ...
    ///
    /// PCIe RTD3 - m.2 PCIe SSD (default disabled on RVP7)
    ///
    If(LNotEqual(SGMD,2)) {
      Scope(\_SB.PCI0.RP09)
      {
...
// USB - START
  ...
  Scope(\_SB.PCI0.XHC.RHUB.SS03)        // USB Superspeed Port 3 - IVCAM
    ...
    ///
    /// If WWAN RTD3 support is enabled, associate _PR0, PR2, PR3 for USB Highspeed Port 6 with \ref xhc_pr_px06
    ///@addtogroup xhc_scope
    If(LEqual(And(RCG0,0x80),0x80)) {
      Scope(\_SB.PCI0.XHC.RHUB.HS06){
        // USB power resource 6
        // power pin = IOEX2,P00
      ...
      //      Notify(\_SB.PCI0.XHC.RHUB.HS06, 0x02)     //  DeviceWake: Notify PCIe RootPort
      //    }
      }
    } // If(LEqual(And(RCG0,0x80),0x80))
