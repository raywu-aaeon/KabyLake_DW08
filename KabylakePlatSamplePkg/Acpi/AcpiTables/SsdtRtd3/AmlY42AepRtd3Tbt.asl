/** @file
  ACPI RTD3 SSDT table for AML Y42 TBT
...
External(WRTO)
#define WWAN_PCIE_ROOT_PORT \_SB.PCI0.RP10
...
External(\_SB.PCI0.XHC.RHUB.HS05, DeviceObj) //xHCI HSP port 5, WWAN
...
// PCIe root ports - START
  If(LEqual(\RTBT, 1)) {
    ...
    Scope(\_SB.PCI0.RP01)
    {
      // reset pin = GPIO_SKL_LP_GPP_B15
      // power pin = GPIO_SKL_LP_GPP_E2
      // wake pin  = GPIO_SKL_LP_GPP_E15
      // CLK_REQ   = Mask Enable of Mapped CLKREQ# for CLKOUT_SRC2 (MSKCRQSRC2):
      Name(SLOT, 1)   ///- port #1
      Name(RSTG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_B15, // GPIO pad #/IOEX pin #
              1  // reset pin de-assert polarity
            })
      Name(PWRG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_E2, // GPIO pad #/IOEX pin #
              1  // power on polarity
            })
      Name(WAKG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_E15, // GPIO pad #/IOEX pin #
              1  // wake en polarity
            })
      Name(SCLK, Package() {
      ...
      Name(TUID, 0) // TBT RP Number 0 for RP01
      Include("Rtd3SptPcieTbt.asl")
    }
  }

    ///
    /// PCIe RTD3 - m.2 PCIe SSD
    ///
    Scope(\_SB.PCI0.RP05)
    {
      // reset pin = GPIO_SKL_LP_GPP_C12
      // power pin = GPIO_SKL_LP_GPP_C15
      // wake pin  = none
      ...
      Name(RSTG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_C12, // GPIO pad #/IOEX pin #
              1  // reset pin de-assert polarity
            })
      Name(PWRG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_C15, // GPIO pad #/IOEX pin #
              1  // power on polarity
            })
      Name(WAKG, Package() {
...
// AMI_OVERRIDE_END - Fix Linux build error.
    ...
    Scope(\_SB.PCI0.RP09)
    {
      // reset pin = GPIO_SKL_LP_GPP_E0
      // power pin = GPIO_SKL_LP_GPP_C11
      // wake pin  = GPIO_SKL_LP_GPP_E8
      // CLK_REQ   = Mask Enable of Mapped CLKREQ# for CLKOUT_SRC5 (MSKCRQSRC5)
      Name(SLOT, 9)   ///- Slot #9
      Name(RSTG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_E0, // GPIO pad #/IOEX pin #
              1  // reset pin de-assert polarity
            })
      Name(PWRG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_C11, // GPIO pad #/IOEX pin #
              1  // power on polarity
            })
      Name(WAKG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_E8, // GPIO pad #/IOEX pin #
              1  // wake en polarity
            })
      Name(SCLK, Package() {
              1,    // 0-disable, 1-enable
...
// AMI_OVERRIDE_END - Fix Linux build error.
    ...
    {
      // reset pin = GPIO_SKL_LP_GPP_C6
      // power pin = GPIO_SKL_LP_GPP_C22
      // wake pin  = GPIO_SKL_LP_GPP_G0
      // CLK_REQ   = Mask Enable of Mapped CLKREQ# for CLKOUT_SRC4 (MSKCRQSRC4)
      Name(SLOT, 10)   ///- port #10
      Name(RSTG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_C6, // GPIO pad #/IOEX pin #
              1  // reset pin de-assert polarity
            })
      Name(PWRG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_C22, // GPIO pad #/IOEX pin #
              1  // power on polarity
            })
      Name(WAKG, Package() {
              1, // 0-disable, 1-enable GPIO, 2-enable IOEX(IO Expander)
              0, // IOEX #
              GPIO_SKL_LP_GPP_G0, // GPIO pad #/IOEX pin #
              1  // wake en polarity
            })
      Name(SCLK, Package() {
              1,    // 0-disable, 1-enable
      ...
      Include("Rvp3Rtd3SptPcieWWAN.asl")
    }
// PCIe root ports - END
...
// USB - START
/* Commenting DUAM method as there no GPIO or IO EXP to control USB D3 flow or power saving of USB port.
...
/* There no GPIO or IO EXP to control USB D3 flow or power saving of USB port.
...
*/
    ...
    If(LEqual(And(RCG0,2),2)) {
      Scope(\_SB.PCI0.XHC.RHUB.HS02)        // USB Superspeed Port 3 - IVCAM
      {
        Name(_S0W, 4)
        Method(_PR0) {
          If (LAnd(LGreaterEqual(\OSYS, 2015), LEqual(\EMOD, 1))) {
    ...
    If(LEqual(And(RCG0,0x80),0x80)) {
      Scope(\_SB.PCI0.XHC.RHUB){
        ...
        PowerResource(PX05, 0, 0){
          ...
          Name(TIM1, 0)
          Method(_STA){                                         /// _STA method
            If(LEqual(\_SB.GGOV(GPIO_SKL_LP_GPP_C22),1)) {
              Return(0x01)
            } Else {
              Return(0x00)
            }
          ...
          Method(_ON, 0)                                        /// _ON Method
          {
            ...
            \_SB.SGOV(GPIO_SKL_LP_GPP_C22,1)                        // set power pin to high
          }
          Method(_OFF, 0)                                       /// _OFF Method
          {
            Store(Timer(), ^TIM1)                               // Start OFF timer here.

            // drive RESET Pin low (GPP_A_16)
            \_SB.SGOV(\WGUR, 0)
            ...
            \_SB.SGOV(GPIO_SKL_LP_GPP_C22,0)                        // set power pin to low
      ...
      //      Notify(\_SB.PCI0.XHC.RHUB.HS05, 0x02)     //  DeviceWake: Notify PCIe RootPort
      //    }
      }
    } // If(LEqual(And(RCG0,0x80),0x80))
...
// Serial IO - START
// Commenting out Power Resource for Audio Codec as gpio pin for Audio Codec is not available
      ...
      PowerResource(PAUD, 0, 0) {
        ...
        Method(PUAM, 0, Serialized)
        {
          ...
          If (LAnd(LEqual(^_STA, Zero), LNotEqual(\UAMS, Zero))) { ///New state = OFF Check if (_STA ==0 && \UAMS != 0) \n
            ...
            //If(LEqual(\_SB.PCI0.GEXP.GEPS(1, 5), 1)) { // to do (Gpio resource for Audio not available)
              // drive pwr low
...
// Check HDAS (HD-Audio) controller present
  ...
  // PCH I2C1 - TouchPanel Power control
    Scope(\_SB.PCI0.I2C1){
      ...
      PowerResource(PXTC, 0, 0){ /// Power rail for Touch Panel (GPP, F, 8)
        Method(_STA){
          If (LEqual(\EMOD, 1)) {
            Return(1)
          } Else {
      ...
      Method(PSTA){
        If(LEqual(\_SB.GGOV(GPIO_SKL_LP_GPP_F8),1)) { ///- Check GPIO_F8 NOTE: 1=> ON; 0=> OFF
          Return(0x01)
        } Else {
          Return(0x00)
        }
      ...
      {
        // De-Assert GPIO RST (GPP_D_9)
        \_SB.SGOV(GPIO_SKL_LP_GPP_D9, 1)
        // drive pwr high (GPP_F_8)
        \_SB.SGOV(GPIO_SKL_LP_GPP_F8, 1)
        // update ONTM
        Store(Timer(), ONTM)
        // enable int line (GPP_D_10)
        \_SB.SGRA(GPIO_SKL_LP_GPP_D10, 1)
      }

      Method(POFF, 0)  /// _OFF method \n Turn off
      {
        // disable int line (GPP_D_10)
        \_SB.SGRA(GPIO_SKL_LP_GPP_D10, 0)
        // drive pwr low (GPP_F_8)
        \_SB.SGOV(GPIO_SKL_LP_GPP_F8, 0)
        // Assert GPIO RST (GPP_D_9)
        \_SB.SGOV(GPIO_SKL_LP_GPP_D9, 0)
        // update ONTM
        Store(Zero , ONTM)  ///- Clear ONTM
      }
...
//GPE Event handling - Start
  Scope(\_GPE) {
    ...
    Method(AL6F) {
      If(LNotEqual(SGMD,2)) {
        // TBT wake event
        If (\_SB.ISME(GPIO_SKL_LP_GPP_E15))
        {
          \_SB.SHPO(GPIO_SKL_LP_GPP_E15, 1) // set gpio ownership to driver(0=ACPI mode, 1=GPIO mode)
          Notify(\_SB.PCI0.RP01, 0x02)   // device wake
          \_SB.CAGS(GPIO_SKL_LP_GPP_E15)
        }
      }
      // WLAN wake event
      If (\_SB.ISME(GPIO_SKL_LP_GPP_E8))
      {
        \_SB.SHPO(GPIO_SKL_LP_GPP_E8, 1) // set gpio ownership to driver(0=ACPI mode, 1=GPIO mode)
        Notify(\_SB.PCI0.RP09, 0x02)   // device wake
        \_SB.CAGS(GPIO_SKL_LP_GPP_E8)
      }
      // WWAN Wake event
      If (\_SB.ISME(GPIO_SKL_LP_GPP_G0))
      {
        \_SB.SHPO(GPIO_SKL_LP_GPP_G0, 1) // set gpio ownership to driver(0=ACPI mode, 1=GPIO mode)
        Notify(WWAN_PCIE_ROOT_PORT, 0x02)   // device wake
        \_SB.CAGS(GPIO_SKL_LP_GPP_G0)
      }
    }
