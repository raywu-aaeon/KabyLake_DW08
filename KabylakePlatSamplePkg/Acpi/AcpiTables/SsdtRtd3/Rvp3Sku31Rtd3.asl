...
External(DS4D, IntObj)
External(S0ID)
External(OSYS)
External(SGMD)
External(\_SB.ICAM.IDFU, IntObj)              /// Variable to check for IVCAM DFU Mode
External(\_SB.ICAM.IPWR, IntObj)              /// Variable to check IVCAM Power set by _DSM.
...
// USB - START
    ...
    ///HighSpeed Port 4 (Camera), SuperSpeed Port 3 (IVCAM Camera) with \ref xhc_pr_pcam
    ...
    Scope(\_SB.PCI0.XHC.RHUB)
    {
      ...
      Method(CMSA) {                              /// _STA method
        ...
        If (CondRefOf(\_SB.PCI0.GEXP.GEPS)) {
          If(LEqual(\_SB.PCI0.GEXP.GEPS(0,  17), 1)) {   ///- Get power status base on GPIO P21 from IoExpander
            Return(0x01)
          }
        }
        Return(0x00)
      ...
      Method(CMON, 0) {                            /// _ON method
        If (LNotEqual(\_SB.PCI0.XHC.RHUB.CPWR, 1)) {
          If(LEqual(CAMT, 0)){  // Check for CameraType = IVCAM
            If (CondRefOf(\_SB.ICAM.IPWR)) {
              If(LNotEqual(\_SB.ICAM.IPWR, 1)) {      // If IVCAM PWR is not disabled by _DSM.
                // IoExpander {TCA6424A}
                If (CondRefOf(\_SB.PCI0.GEXP.SGEP)) {
                  \_SB.PCI0.GEXP.SGEP(0, 17, 1)     // Power up Camera.
                  Store(1,\_SB.PCI0.XHC.RHUB.CPWR)
                }
              }
            }
          } Else {
      ...
      Method(COFF, 0) {                           /// _OFF method
        If (LNotEqual(\_SB.PCI0.XHC.RHUB.CPWR,0)) {
          If(LEqual(CAMT, 0)){  // Check for CameraType = IVCAM
            If (CondRefOf(\_SB.ICAM.IDFU)) {
              If (LEqual(\_SB.ICAM.IDFU, 0)) {        /// If IVCAM DFU is disabled
                // IoExpander {TCA6424A}
                If (CondRefOf(\_SB.PCI0.GEXP.SGEP)) {
                  \_SB.PCI0.GEXP.SGEP(0, 17, 0)     // Power down Camera.
                  Store(0,\_SB.PCI0.XHC.RHUB.CPWR)
                }
              }
            }
          } Else {
  ...
  Scope(\_SB.PCI0.XHC.RHUB.SS03)        // USB Superspeed Port 3 - IVCAM
    ...
    ///
    /// If WWAN RTD3 support is enabled, associate _PR0, PR2, PR3 for USB Highspeed Port 5 with \ref xhc_pr_px05
    ///@addtogroup xhc_scope
