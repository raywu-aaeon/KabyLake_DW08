...
BOOLEAN                                     mStmActive = FALSE; // RPPO-SKL-0018
  ...
  )
{
  ...
  Psd = (TXT_PROCESSOR_SMM_DESCRIPTOR*)(mCpuHotPlugData.SmBase[CpuIndex] + SMM_PSD_OFFSET); // RPPO-SKL-0018
  ...
  )
{
  ...
  if ((PageTable[PTIndex] & IA32_PG_PS) != 0) {
    ...
    if (NewPageTableAddress == NULL) {  // RPPO-SKL-0022
  ...
  )
{
  ...
  //
  for (Index = 0; Index < mMaxNumberOfCpus; Index++) {
    Psd = (TXT_PROCESSOR_SMM_DESCRIPTOR *)(VOID *)(UINTN)(mCpuHotPlugData.SmBase[Index] + SMM_PSD_OFFSET); // RPPO-SKL-0018
