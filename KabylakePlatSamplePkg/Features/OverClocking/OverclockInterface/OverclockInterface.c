...
//AMI_OVERRIDE_START - Fix BCLK setting is wrong after S3 resuming.
#include <Library/DxeMeLib.h>
//AMI_OVERRIDE_END - Fix BCLK setting is wrong after S3 resuming.
...
#define XTU_SMI_MEMORY_SIZE                 (3 * 1024) //3 3kb buffer
...
// Desktop EV / CRB
//
GLOBAL_REMOVE_IF_UNREFERENCED const UINT16              MemoryVoltageTableDesktop[13][3] = {
  //
  // Index      Value    Volts
  //
    {0x00,      0x00,    0000},
  ...
  )
{
  ...
  UINT32                Data32;
#if 0  //AMI_OVERRIDE_START - Modify some code for updating RC 0.7.0.
  ICC_GETSET_CLOCK_SETTINGS_REQRSP BclkSettings;
#else
  ICC_CLK_RANGE_DEF_REC Crdr;
  ICC_GETSET_CLOCK_SETTINGS_REQRSP Clock2Settings;
#endif //AMI_OVERRIDE_END - Modify some code for updating RC 0.7.0.
...
//    gBclkMaxValue = 10000;//10 * (UINT16) (Clock2Settings.MaxFreq / 100000);
    ...
    if (EFI_ERROR(Status)) {
      DEBUG ((DEBUG_ERROR, "ERROR:OverClocLibDxe Get Clock Range Definition Record message failed. Status = %X\n", Status));
      return;
    }
    ...
    //
    SetMem(&Clock2Settings, sizeof (ICC_GETSET_CLOCK_SETTINGS_REQRSP), 0);

    //Clock2Settings.ReqClock = CLOCK2;
    Clock2Settings.ReqClock = 1;
    Status = HeciGetIccClockSettings(&Clock2Settings);
    if (EFI_ERROR(Status)) {
      DEBUG ((DEBUG_ERROR, "ERROR:OverClocLibDxe Get Clock Settings messge failed, clock 2. Status = %X\n", Status));
      return;
    }
    ...
    //
    //gBclkMinValue = 100;//10 * (UINT16) (Clock2Settings.MinFreq / 100000);
    //gBclkMaxValue = 100;//10 * (UINT16) (Clock2Settings.MaxFreq / 100000);
    gBclkMinValue = 10 * (UINT16) (Clock2Settings.MinFreq / 100000);
    gBclkMaxValue = 10 * (UINT16) (Clock2Settings.MaxFreq / 100000);
    DEBUG ((DEBUG_INFO, "OverClockInterface: ClockRanges.FrequencyMin = 0x%X\n", Clock2Settings.MinFreq));
    DEBUG ((DEBUG_INFO, "OverClockInterface: ClockRanges.FrequencyMax = 0x%X\n", Clock2Settings.MaxFreq));
#endif
//AMI_OVERRIDE_END - Modify some code for updating RC 0.7.0.
...
//AMI_OVERRIDE_START - Install ACPI SSDT for XTU use
EFI_STATUS
InitializeOcASLAcpiTables(
  void
  )
{
  EFI_STATUS                    Status;
  ...
  //UINT8                         *EndPtr;
  //UINT32                        *Signature;
  EFI_ACPI_DESCRIPTION_HEADER   *OcAcpiTable;
  EFI_ACPI_TABLE_PROTOCOL       *AcpiTable;
  EFI_GUID			            Guid = OCASL_FFS_TABLE_STORAGE_GUID;
  ...
  ///
  DEBUG ((DEBUG_INFO, "Init OverClock SSDT table\n"));
  ...
  while (Status == EFI_SUCCESS) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      ///
      //if (((EFI_ACPI_DESCRIPTION_HEADER *) CurrentTable)->OemTableId == SIGNATURE_64 ('P', 'e', 'r', 'f', 'T', 'u', 'n', 'e')) {
        OcAcpiTable = (EFI_ACPI_DESCRIPTION_HEADER *) CurrentTable;
        ///
...
//AMI_OVERRIDE_END - Install ACPI SSDT for XTU use
  ...
  )
{
  ...
  //
  // Allocate XTU SMI memory in ACPI NVS
  //
  Status = AllocateMemoryBelow4G (EfiACPIMemoryNVS, XTU_SMI_MEMORY_SIZE, (VOID **) &XtuSmiMemoryAddress);
  ASSERT_EFI_ERROR (Status);
  if (EFI_ERROR (Status)) {
    return 0;
  }
  ...
  mGlobalNvsAreaPtr->XtuSmiBaseAddress = (UINT32) (UINTN) XtuSmiMemoryAddress;
  DEBUG ((DEBUG_INFO, "OverClockInterface: mGlobalNvsAreaPtr->XtuSmiBaseAddress = 0x%X\n", mGlobalNvsAreaPtr->XtuSmiBaseAddress));

  //
  //  Call Xtu Common Buffer SMI to save buffer address to SMRAM
  //
  DEBUG ((DEBUG_INFO, "Calling XTU SMI for common buffer.\n"));
  IoWrite8 (SW_SMI_IO_ADDRESS, SW_SMI_XTU_COMMON_BUFFER);
//AMI_OVERRIDE_START - Install ACPI SSDT for XTU use
  Status = InitializeOcASLAcpiTables();
//AMI_OVERRIDE_END - Install ACPI SSDT for XTU use
  ...
  )
{
  ...
  if ((PlatformInfo->PlatformFlavor == FlavorMobile) ||
      (PlatformInfo->PlatformFlavor == FlavorEmbedded)) {
  ...
  //
  // Allocate XTU Control ID memory in ACPI NVS
  //
  ...
  mGlobalNvsAreaPtr->XTUBaseAddress = (UINT32) (UINTN) ControlBuff;
  mGlobalNvsAreaPtr->DDRReferenceFreq = mSaSetup.DdrRefClk;
  DEBUG ((DEBUG_INFO, "OverClockInterface: mGlobalNvsAreaPtr->XTUBaseAddress = 0x%X\n", mGlobalNvsAreaPtr->XTUBaseAddress));

  //
  // Allocate XMP memory in ACPI NVS
  //
  ...
  mGlobalNvsAreaPtr->XMPBaseAddress = (UINT32) (UINTN) XmpBuff;
  DEBUG ((DEBUG_INFO, "OverClockInterface: mGlobalNvsAreaPtr->XMPBaseAddress = 0x%X\n", mGlobalNvsAreaPtr->XMPBaseAddress));
...
//AMI_OVERRIDE_START - Fix memory voltage value not match with setup
    ...
    AddControlBuffData(&ControlBuff->ControlIDData[Count],
                        ...
                        PRECISION_THREE,
                        ZERO,
                        mSaSetup.MemoryVoltage,
                        MemoryVoltageTableDesktop[0][0],
                        MemoryVoltageTableDesktop[NoOfVoltages - 1][0],
                        MemoryVoltageTableDesktop[0][2],
                        MemoryVoltageTableDesktop[NoOfVoltages - 1][2],
                        &Count );    
#endif
...
// AMI_OVERRIDE_END - Fix one building error in OC module after updating RC 1.0.1.
  ...
  // tWR
  //
// AMI_OVERRIDE_START - Modify the functions overclockinterface for updating module NVRAM_07.
#if 0        
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      BIOS_DEVICE_tWR,
                      0xFFFF,
  ...
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      ...
                      &Count );
#endif
// AMI_OVERRIDE_END - Modify the functions overclockinterface for updating module NVRAM_07.
  //
  // tRFC
  //
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
...
// AMI_OVERRIDE_START - Modify the functions overclockinterface for updating module NVRAM_07.
  ...
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      ...
                      0,
                      1,
                      MEMORY_CLOCK_FREQ_MINIMUM,
                      MEMORY_CLOCK_FREQ_MAXIMUM,
                      &Count );
#endif
// AMI_OVERRIDE_END - Modify the functions overclockinterface for updating module NVRAM_07.
...
// AMI_OVERRIDE_START - Fix that Turbo power and power limitation don't sync with setup value and waring messages in BET tool.
#if 0            
          if ( (mCpuSetup.LongDurationPwrLimitOverride == 0) || (mCpuSetup.TurboPowerLimitLock == 1)) {
            //
            //  If locked, this will prevent the ability to edit PL1 in XTU, but still display the default value.
            //
            AddControlBuffData(&ControlBuff->ControlIDData[Count],
          ...
          if( (mCpuSetup.LongDurationPwrLimitOverride == 0) || (mCpuSetup.TurboPowerLimitLock == 1)) {
            ...
            AddControlBuffData(&ControlBuff->ControlIDData[Count],
                                ...
                                PRECISION_THREE,
                                ZERO,
                                mCpuSetup.PowerLimit1Default / 125,
                                mCpuSetup.PowerLimit1Default / 125,
                                mCpuSetup.PowerLimit1Default / 125,
                                mCpuSetup.PowerLimit1Default / 125,
                                mCpuSetup.PowerLimit1Default / 125,
            ...
            AddControlBuffData(&ControlBuff->ControlIDData[Count],
                                ...
                                gMinTurboPowerLimit,
                                gMaxTurboPowerLimit,
                                gMinTurboPowerLimit * 125,
                                gMaxTurboPowerLimit * 125,
                                &Count );
          }
#endif
// AMI_OVERRIDE_END - Fix that Turbo power and power limitation don't sync with setup value and waring messages in BET tool.
        ...
        AddControlBuffData(&ControlBuff->ControlIDData[Count],
                            ...
                            PRECISION_NONE,
                            ZERO,
#if 0 // AMI_OVERRIDE_START - Fix wrong value for Short Window Package Power Limit.
                            mCpuSetup.PowerLimit2,
#else
                            1,
#endif // AMI_OVERRIDE_END - Fix wrong value for Short Window Package Power Limit.
                            0,
                            1,
                            0,
                            1,
...
// AMI_OVERRIDE_START - Fix that Turbo power and power limitation don't sync with setup value and waring messages in BET tool.
        ...
        AddControlBuffData(&ControlBuff->ControlIDData[Count],
                            ...
                            3,
                            ZERO,
                            mCpuSetup.PowerLimit2Default / 125,
                            gMinTurboPowerLimit,
                            (0x7fff/gPowerUnit),
                            gMinTurboPowerLimit * 125,
                            (0x7fff/gPowerUnit) * 125,
                            &Count );        
#endif
// AMI_OVERRIDE_END - Fix that Turbo power and power limitation don't sync with setup value and waring messages in BET tool.
...
//AMI_OVERRIDE_START - Fix EIP239426
    switch (Profile) {
      case 0:
#if 0
      ...
      case 1:
        ...
        MemoryTiming  = &MemInfoHobProtocol->MemInfoData.Timing[2];
#else
        MemoryTiming  = &MemInfoHobProtocol->MemInfoData.Timing[3];
#endif
        break;
//AMI_OVERRIDE_END - Fix EIP239426
...
//AMI_OVERRIDE_START - Fix SetupData error Fix BIOS_DEVICE_DDR_MULTIPLIER setting error
#if 0  
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
...
//AMI_OVERRIDE_END - Fix SetupData error
  ...
  // Voltage Override
  //
  if(mSaSetup.GtsVoltageModeDefault == VOLTAGE_MODE_ADAPTIVE) {
    AddControlBuffData(&ControlBuff->ControlIDData[Count],
                        BIOS_DEVICE_IGFX_VOLTAGE_OVERRIDE,
                        IVR_VOLTAGE_MAXIMUM + 1,
                        PRECISION_THREE,
                        ZERO,
                        mSaSetup.GtsExtraTurboVoltageDefault,
                        IVR_VOLTAGE_MINIMUM,
                        IVR_VOLTAGE_MAXIMUM,
                        IVR_VOLTAGE_MINIMUM,
                        IVR_VOLTAGE_MAXIMUM,
    ...
    AddControlBuffData(&ControlBuff->ControlIDData[Count],
                        ...
                        PRECISION_THREE,
                        ZERO,
                        mSaSetup.GtsVoltageOverrideDefault,
                        IVR_VOLTAGE_MINIMUM,
                        IVR_VOLTAGE_MAXIMUM,
                        IVR_VOLTAGE_MINIMUM,
                        IVR_VOLTAGE_MAXIMUM,
  ...
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      ...
                      PRECISION_NONE,
                      ZERO,
                      mSaSetup.GtsVoltageModeDefault,
                      0,
                      1,
                      0,
                      1,
  ...
  if(mSaSetup.GtusVoltageModeDefault == VOLTAGE_MODE_ADAPTIVE) {
    AddControlBuffData(&ControlBuff->ControlIDData[Count],
                        BIOS_DEVICE_IGFX_UNSLICED_VOLTAGE_OVERRIDE,
                        IVR_VOLTAGE_MAXIMUM + 1,
                        PRECISION_THREE,
                        ZERO,
  ...
  //
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      BIOS_DEVICE_IGFX_UNSLICED_VOLTAGE_MODE,
                      0x2,
                      PRECISION_NONE,
                      ZERO,
  ...
  //
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      BIOS_DEVICE_IGFX_UNSLICED_VOLTAGE_OFFSET,
                      IVR_VOLTAGE_OFFSET_MAX_DATA_VAL + 1,
                      PRECISION_NONE,
                      ZERO,
                      IVR_VOLTAGE_OFFSET_DEFAULT_VAL,
  ...
  //
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      BIOS_DEVICE_IGFX_UNSLICED_FREQUENCY,
                      (((IGD_FREQ_RATIO_LIMIT_MAXIMUM - IGD_FREQ_RATIO_LIMIT_MINIMUM) / IGD_FREQ_RATIO_LIMIT_STEP_SIZE) + 1),
                      PRECISION_ONE,
                      ZERO,
  ...
  //
  // VR Settings ( SA = 0, IA = 1, GTUS = 2, GTS = 3)
  //
// AMI_OVERRIDE_START - Fix SetupData error and waring messages in BET tool.
  ...
  if (PlatformInfo->PlatformFlavor == FlavorMobile) {
      AddControlBuffData(&ControlBuff->ControlIDData[Count],
                    ...
                    2,
                    2,
                    mCpuSetup.IccMax[0],      // setup value / 4 = value of XTU
                    0,
                    1023,
                    0,
                    25575,
  ...
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      ...
                      2,
                      2,
                      mCpuSetup.IccMax[1],      // setup value / 4 = value of XTU
                      0,
                      1023,
                      0,
                      25575,
  ...
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      ...
                      2,
                      2,
                      mCpuSetup.IccMax[2],       // setup value / 4 = value of XTU
                      0,
                      1023,
                      0,
                      25575,
  ...
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      ...
                      2,
                      2,
                      mCpuSetup.IccMax[3],       // setup value / 4 = value of XTU
                      0,
                      1023,
                      0,
                      25575,
  ...
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      BIOS_DEVICE_TURBO_BOOST_POWER_TIME_WINDOWS,
                      0xFFFF,
                      0,
                      2,
                      8,
...
// AMI_OVERRIDE_END - Fix SetupData error and waring messages in BET tool.
  ...
  //
  AddControlBuffData(&ControlBuff->ControlIDData[Count],
                      BIOS_DEVICE_REAL_TIME_MEMORY_TIMING_ENABLE,
                      2,
                      PRECISION_NONE,
                      ZERO,
                      mSaSetup.RealtimeMemoryTiming,
                      0,
                      1,
                      0,
                      1,
  ...
  mGlobalNvsAreaPtr->XTUSize = sizeof(CONTROLID_DATA)*(Count);
  DEBUG ((DEBUG_INFO, "mGlobalNvsAreaPtr->XTUSize = %X Count=%d\n", mGlobalNvsAreaPtr->XTUSize, Count));
...
//       MemoryVoltageSettingEntry[Index].VoltageValueEntry[0].Offset  = R_PCH_GPIO_LVL2;
  ...
  ContLogicalBiosSetting->AutoModeSupport     = NOT_SUPPORTED;
  ContLogicalBiosSetting->PerformanceFlag     = MIN_SETTING_LOW_PERFORMANCE;
  ContLogicalBiosSetting->DefaultSetting      = mSaSetup.GtsMaxOcRatioDefault;
  Dddt = (DDDT *) (ContLogicalBiosSetting + 1);
