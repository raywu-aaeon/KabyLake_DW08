...
#include "PchAccess.h"
#include "OemSetup.h"
//AMI_OVERRIDE_START - Add this for search XTU SSDT from FV
#include <Protocol/FirmwareVolume2.h>
//AMI_OVERRIDE_END - Add this for search XTU SSDT from FV
...
// AMI_OVERRIDE_START - Move code to OverClockSetupOverride.c and delete the ModulePart OverClockSetup and waring messages in BET tool.
//#define SUPPORTED_CONTROL_ID_COUNT 49
#if (((CRB_BOARD < 3) || (CRB_BOARD > 5)) && (CRB_BOARD != 9))  // Mobile platform
#define SUPPORTED_CONTROL_ID_COUNT 56 //54 - 3(RING_VOLTAGE) - 1(RING_IccMax) + 1(AVX ratio offset) + 4 (IGFX_UNSLICED VOLTAGE_MODE/VOLTAGE_OVERRIDE/VOLTAGE_OFFSET/FREQUENCY) + 1 (BIOS_DEVICE_REAL_TIME_MEMORY_TIMING_ENABLE)
#else  // DT platform
#define SUPPORTED_CONTROL_ID_COUNT 59 //54 - 3(RING_VOLTAGE) - 1(RING_IccMax) + 1(AVX ratio offset) -1 (SA IccMax) + 4 (IGFX_UNSLICED VOLTAGE_MODE/VOLTAGE_OVERRIDE/VOLTAGE_OFFSET/FREQUENCY) + 4 (Core 5/6/7/8) + 1 (BIOS_DEVICE_REAL_TIME_MEMORY_TIMING_ENABLE)
#endif
// AMI_OVERRIDE_END - Move code to OverClockSetupOverride.c and delete the ModulePart OverClockSetup and waring messages in BET tool.
