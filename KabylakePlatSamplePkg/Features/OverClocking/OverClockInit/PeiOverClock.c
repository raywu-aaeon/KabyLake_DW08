...
// AMI_OVERRIDE_START - Disable OC WDT in different cases.
#include <PchAccess.h>
...
#endif
// AMI_OVERRIDE_END - Disable OC WDT in different cases.
...
// AMI_OVERRIDE_START - Disable OC WDT in different cases.
#if Recovery_SUPPORT
EFI_PEI_NOTIFY_DESCRIPTOR gOCInRecoveryNotifyList = {
...
)
{
	...
	PmBase = MmioRead16(PCI_LIB_ADDRESS(DEFAULT_PCI_BUS_NUMBER_PCH,PCI_DEVICE_NUMBER_PCH_PMC,PCI_FUNCTION_NUMBER_PCH_PMC,R_PCH_PMC_ACPI_BASE)) & B_PCH_PMC_ACPI_BASE_BAR;
...
// AMI_OVERRIDE_START - Disable OC WDT in different cases.  
...
#if Recovery_SUPPORT
  (**PeiServices).NotifyPpi(PeiServices, &gOCInRecoveryNotifyList);
#endif
  
  // Enter to capsule mode
#if Capsule_SUPPORT
  ...
  Status = ReadOnlyVariable->GetVariable( 
                               ...
                               &CapsuleData );
  if (!EFI_ERROR(Status)) {
	PmBase = MmioRead16(PCI_LIB_ADDRESS(DEFAULT_PCI_BUS_NUMBER_PCH,
