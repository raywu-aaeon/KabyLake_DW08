...
External(\PVSC, IntObj) // PcdVsctTableForSens
...
// KblGBegin
//
// Discrete Graphics Power Share Participants and Policy
External(\DGCE, IntObj) // EnableDGFCParticipant
External(\DCAT, IntObj) // ActiveThermalTripPointDGFC
External(\DCPT, IntObj) // PassiveThermalTripPointDGFC
External(\DCCT, IntObj) // CriticalThermalTripPointDGFC
External(\DCS3, IntObj) // CriticalThermalTripPointDGFCS3
External(\DCHT, IntObj) // HotThermalTripPointDGFC
External(\SSPC, IntObj) // SensorSamplingPeriodDGFC
External(\DCSZ, IntObj) // PpccStepSizeDGFC
External(\DCL0, IntObj) // MinPowerLimit0DGFC

External(\DGME, IntObj) // EnableDGHMParticipant
External(\DMAT, IntObj) // ActiveThermalTripPointDGHM
External(\DMPT, IntObj) // PassiveThermalTripPointDGHM
External(\DMCT, IntObj) // CriticalThermalTripPointDGHM
External(\DMS3, IntObj) // CriticalThermalTripPointDGHMS3
External(\DMHT, IntObj) // HotThermalTripPointDGHM
External(\SSPM, IntObj) // SensorSamplingPeriodDGHM

External(\MCPE, IntObj) // EnableMCPParticipant
External(\MCSZ, IntObj) // PpccStepSizeMCP
External(\MCL0, IntObj) // MinPowerLimit0MCP

External(\PSPE, IntObj) // EnablePowerSharePolicy
//
// KblGEnd
//
...
Scope(\_SB)
{
  ...
  Device(IETM)
  {
    ...
    Name(TMPP,Package()
    {
      ...
      ToUUID("00000000-0000-0000-0000-000000000000"),
      ToUUID("00000000-0000-0000-0000-000000000000"),
      ToUUID("00000000-0000-0000-0000-000000000000"),  // KblGContent
      ToUUID("00000000-0000-0000-0000-000000000000")
    })
...
// KblGBegin
//
        // Discrete Graphics Power Share Policy GUID
        If(LAnd(LEqual(\PSPE,1),CondRefOf(DGPS))){
          Store(DeRefOf(Index(DGPS,0)), Index(TMPP,TMPI))
          Increment(TMPI)
        }
//
// KblGEnd
//
    ...
    Method(_OSC, 4,Serialized,,BuffObj,{BuffObj,IntObj,IntObj,BuffObj})
    {
      ...
      // PID Policy GUID
      //
      If(LAnd(LEqual(\PIDE,1),CondRefOf(\_PR.APSV))){ // Check if PID policy is enabled in setup
        If(LEqual(PSEM,0)){
          Store(1,PSEM)
          Store(\_PR.APSV,PTRP)  // use semaphore so variable is only initialized once
        }
...
// AMI_OVERRIDE_START - Arg2 should be used for Acoustic Limit.
//      If(LOr(LGreaterEqual(Arg1,0),LLessEqual(Arg1,5))) // bounds check
      If(LOr(LGreaterEqual(Arg2,0),LLessEqual(Arg2,5))) // Acoustic Limit check
// AMI_OVERRIDE_END - Arg2 should be used for Acoustic Limit.
      {
...
// AMI_OVERRIDE_START - Arg3 should be used for Power Limit.
//      If(LOr(LGreaterEqual(Arg1,0),LLessEqual(Arg1,5))) // bounds check
      If(LOr(LGreaterEqual(Arg3,0),LLessEqual(Arg3,5))) // Power Limit check
// AMI_OVERRIDE_END - Arg3 should be used for Power Limit.
      {
...
//AMI_OVERRIDE_START >>> EIP431173 - Fix Gcc Build Failed
  Include("KblG/DGFCParticipant.asl")
  Include("KblG/DGHMParticipant.asl")
  Include("KblG/MCPParticipant.asl")
