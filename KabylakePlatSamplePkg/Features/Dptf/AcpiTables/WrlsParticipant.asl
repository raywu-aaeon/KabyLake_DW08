...
Scope(\_SB)
{
  ...
  Device(WRLS)  // WRLS temperature sensor
  {
    Name(_HID, EISAID("INT3408"))  // Intel DPTF Wireless Participant Device
    Name(_UID, "WRLS")
    Name(_STR, Unicode("Intel DPTF Wireless Participant Device"))
    Name(PTYP, 0x07)
    Name(CTYP,0)  // Mode
    ...
    Method(_AC0,0,Serialized)
    {
      If(CTYP)
      {
        If (LEqual(WFPT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(WFPT),Local1) // Passive Cooling Policy
      } Else {
        If (LEqual(WFAT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(WFAT),Local1) // Active Cooling Policy
      }
    ...
    Method(_PSV,0,Serialized)
    {
      ...
      {
        If (LEqual(WFAT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Return(\_SB.IETM.CTOK(WFAT)) // Active Cooling Policy
      } Else {
        If (LEqual(WFPT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Return(\_SB.IETM.CTOK(WFPT)) // Passive Cooling Policy
      }
    }
    ...
    {
      If (LEqual(WFCT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(WFC3,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(WFHT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
