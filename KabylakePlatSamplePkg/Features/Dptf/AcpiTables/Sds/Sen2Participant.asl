...
Scope(\_SB.PCI0.LPCB.H_EC)
{
  ...
  Device(SEN2)
  {
    ...
    Name(FAUX, 0)
    Name(SAUX, 0)
    Name(_STR, Unicode ("Sensor 2 A-Skin Skycam Chassis U21 IR"))
    Name(PTYP, 0x03)
    Name(CTYP,0)  // Mode
    ...
    {
      Store(Arg0,LSTM)
      Notify(\_SB.PCI0.LPCB.H_EC.SEN2, 0x91) // notify the participant of a trip point change event
    }
    ...
    Method(_AC0,0,Serialized)
    {
      If(CTYP)
      {
        If (LEqual(S2PT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S2PT),Local1) // Passive Cooling Policy
      } Else {
        If (LEqual(S2AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S2AT),Local1) // Active Cooling Policy
      }
    ...
    Method(_PSV,0,Serialized)
    {
      ...
      {
        If (LEqual(S2AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Return(\_SB.IETM.CTOK(S2AT)) // Active Cooling Policy
      } Else {
        If (LEqual(S2PT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Return(\_SB.IETM.CTOK(S2PT)) // Passive Cooling Policy
      }
    }
    ...
    {
      If (LEqual(S2CT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(S2S3,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(S2HT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
