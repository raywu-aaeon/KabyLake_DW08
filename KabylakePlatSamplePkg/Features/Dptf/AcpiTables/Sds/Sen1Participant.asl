...
Scope(\_SB.PCI0.LPCB.H_EC)
{
  ...
  Device(SEN1)
  {
    ...
    Name(FAUX, 0)
    Name(SAUX, 0)
    Name(_STR, Unicode ("Sensor 1 A-Skin Skycam Hotspot U21"))
    Name(PTYP, 0x03)
    Name(CTYP,0)  // Mode
    ...
    {                          // Field Name : Field Type
      ...
      Package ()               // Power Limit 1
      {
        ...
        1000,                  // TimeWindowMinimum : DWordConst
        28000,                 // TimeWindowMaximum : DWordConst
    ...
    {
      Store(Arg0,LSTM)
      Notify(\_SB.PCI0.LPCB.H_EC.SEN1, 0x91) // notify the participant of a trip point change event
    }
    ...
    Method(_AC1,0,Serialized)
    {
      ...
      {
        If (LEqual(S1PT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S1PT),Local0) // Passive Cooling Policy
      }
      Else
      {
        If (LEqual(S1AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S1AT),Local0) // Active Cooling Policy
      }
