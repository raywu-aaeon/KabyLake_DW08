...
Scope(\_SB.PCI0.LPCB.H_EC)
{
  ...
  Device(SEN4)
  {
    ...
    Name(FAUX, 0)
    Name(SAUX, 0)
    Name(_STR, Unicode ("Sensor 4 B-Skin Display LED Chasiss U1 IR"))
    Name(PTYP, 0x03)
    Name(CTYP,0)  // Mode
    ...
    {
      Store(Arg0,LSTM)
      Notify(\_SB.PCI0.LPCB.H_EC.SEN4, 0x91) // notify the participant of a trip point change event
    }
    ...
    Method(_AC0,0,Serialized)
    {
      If(CTYP)
      {
        If (LEqual(S4PT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S4PT),Local1) // Passive Cooling Policy
      } Else {
        If (LEqual(S4AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(S4CT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(S4S3,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(S4HT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
