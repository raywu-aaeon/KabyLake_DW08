...
Scope(\_SB.PCI0.LPCB.H_EC)
{
  ...
  Device(GEN1)  // Generic participant
  {
    ...
    Name(_UID, "GEN1")
    Name(_STR, Unicode ("Thermistor 1 SDRAM_hotspot_RT5B1"))
    Name(PTYP, 0x12)
    Name(CTYP,0)  // Mode
    ...
    Method(_TMP,0,Serialized)
    {
      ...
      {
        Return(ShiftLeft(\_SB.PCI0.LPCB.H_EC.ECRD(RefOf(\_SB.PCI0.LPCB.H_EC.TER1)),2)) // Thermistor value is 10 bit value, Bit[1:0] are ignored
      } Else {
    ...
    Method(_AC0,0,Serialized)
    {
      If(CTYP)
      {
        ...
        Store(\_SB.IETM.CTOK(G1PT),Local1) // Passive Cooling Policy
      } Else {
        If (LEqual(G1AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(G1AT),Local1) // Active Cooling Policy
      }
      Return(Local1)
    }
