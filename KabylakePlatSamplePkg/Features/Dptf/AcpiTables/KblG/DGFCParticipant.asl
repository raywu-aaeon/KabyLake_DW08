...
Scope(\_SB)
{
  ...
  Device(DGFC)
  {
    Name(_HID, EISAID("INT340D"))  // Discrete Graphics Core
    Name(_UID, "DGFC")
    Name(_STR, Unicode ("Intel DPTF DGFX Core Participant"))  // Intel DPTF DGFX Core Participant
    Name(PTYP, 0x24)
    ...
    {
      If (LEqual(\DCPT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
      Store(\_SB.IETM.CTOK(\DCPT),Local1) // Passive Cooling Policy
      Return (Local1)
    }
    ...
    {
      If (LEqual(\DCAT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
      Return(\_SB.IETM.CTOK(\DCAT)) // Active Cooling Policy
    ...
    {
      If (LEqual(\DCCT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(\DCS3,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(\DCHT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      Store (\DCSZ,Index(DerefOf(Index(\_SB.DGFC.NPCC,1)),5))  // StepSize
      Store (\DCL0,Index(DerefOf(Index(\_SB.DGFC.NPCC,1)),1))  // PowerLimitMinimum
      Return(NPCC)
    }
    ...
    {                          // Field Name : Field Type
      ...
      Package ()               // Power Limit 1
      {
        ...
        55000,                 // PowerLimitMaximum : DWordConst
