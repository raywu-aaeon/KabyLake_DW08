...
Scope(\_SB)
{
  Device(PRCD)  // Perceptual Computing Camera Participant, DS4
  {
    Name(_HID, EISAID("INT340B"))
    Name(_UID, "PRCD")
    Name(PTYP, 0x1E)
    Name(_STR, Unicode("Intel(R) RealSense(TM) DS4 Camera"))
    ...
    {
      Store(Arg0, LSTM)
      Notify(\_SB.PRCD, 0x91) // notify the participant of a trip point change event
    }
    ...
    {
      If (LEqual(\PEAT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(\PEPV,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(\PEC3,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(\PEHT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(\PECR,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
