...
Scope(\_SB.PCI0.LPCB.H_EC)
{
  ...
  Device(GEN2)  // Generic participant
  {
    ...
    Name(_UID, "GEN2")
    Name(_STR, Unicode ("Thermistor 2 NGFF_slot_RT6G1"))
    Name(PTYP, 0x12)
    Name(CTYP,0)  // Mode
    ...
    Method(_TMP,0,Serialized)
    {
      ...
      {
        Return(ShiftLeft(\_SB.PCI0.LPCB.H_EC.ECRD(RefOf(\_SB.PCI0.LPCB.H_EC.TER2)),2)) // Thermistor value is 10 bit value, Bit[1:0] are ignored
      } Else {
    ...
    Method(_AC0,0,Serialized)
    {
      If(CTYP)
      {
        ...
        Store(\_SB.IETM.CTOK(G2PT),Local1) // Passive Cooling Policy
      } Else {
        If (LEqual(G2AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(G2AT),Local1) // Active Cooling Policy
      }
      Return(Local1)
    }
