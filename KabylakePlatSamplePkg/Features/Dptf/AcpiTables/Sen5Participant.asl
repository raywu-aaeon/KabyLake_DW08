...
Scope(\_SB.PCI0.LPCB.H_EC)
{
  ...
  Device(SEN5)
  {
    ...
    Name(FAUX, 0)
    Name(SAUX, 0)
    Name(_STR, Unicode ("Sensor 5 board_hotspot2_Q3G1"))
    Name(PTYP, 0x03)
    Name(CTYP,0)  // Mode
    ...
    {
      Store(Arg0,LSTM)
      Notify(\_SB.PCI0.LPCB.H_EC.SEN5, 0x91) // notify the participant of a trip point change event
    }
    ...
    Method(_AC0,0,Serialized)
    {
      If(CTYP)
      {
        If (LEqual(S5PT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S5PT),Local1) // Passive Cooling Policy
      } Else {
        If (LEqual(S5AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S5AT),Local1) // Active Cooling Policy
      }
    ...
    Method(_AC1,0,Serialized)
    {
      ...
      {
        If (LEqual(S5PT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S5PT),Local0) // Passive Cooling Policy
      } Else {
        If (LEqual(S5AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(S5AT),Local0) // Active Cooling Policy
      }
    ...
    Method(_PSV,0,Serialized)
    {
      ...
      {
        If (LEqual(S5AT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Return(\_SB.IETM.CTOK(S5AT)) // Active Cooling Policy
      } Else {
        If (LEqual(S5PT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
        Return(\_SB.IETM.CTOK(S5PT)) // Passive Cooling Policy
      }
    }
    ...
    {
      If (LEqual(S5CT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(S5S3,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
    ...
    {
      If (LEqual(S5HT,0)) {Return(0xFFFFFFFF)} // signal DPTF driver to disable trip point
