  ...
  )
{
  ...
  while (Status == EFI_SUCCESS) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      // Look for IRMT table.
      //
// AMI_OVERRIDE_START - Use setup item to enable/disable Irmt table.Change iRMT asl code.
#if 0
      if (((EFI_ACPI_DESCRIPTION_HEADER *) CurrentTable)->OemTableId == SIGNATURE_64 ('I', 'r', 'm', 't', 'T', 'a', 'b', 'l')) {
#else
      if (((EFI_ACPI_DESCRIPTION_HEADER *) CurrentTable)->OemTableId == SIGNATURE_64 ('I', 'n', 't', 'e', 'l', 'R', 'M', 'T')) { 
#endif
// AMI_OVERRIDE_END - Use setup item to enable/disable Irmt table.Change iRMT asl code.
