  ...
  )
{
  ...
  if (Length <= 1 && Key.UnicodeChar != CHAR_CARRIAGE_RETURN) {
    return EFI_NOT_READY;
  }

  UnicodeStrToAsciiStrS (Unicode, Ascii, (sizeof (Ascii)) - 1);
...
// AMI_OVERRIDE_START - Fix garbage display.
//  TitleSize = (UStrLen (Title)+1) * sizeof (CHAR16);
  TitleSize = (UStrLen (Title)+2) * sizeof (CHAR16); // 0-base + L'\0'
// AMI_OVERRIDE_END - Fix garbage display.
...
#else    
    CopyMem (
        &Line[(mConOutCols - (UStrLen (mScreenTitle)+1))/2],
        mScreenTitle,
        (UStrLen (mScreenTitle)+1) * sizeof (CHAR16)
        );    
#endif
// AMI_OVERRIDE_END - Fix garbage display.          
  }
