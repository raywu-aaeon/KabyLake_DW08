...
// AMI_OVERRIDE_START >>> EIP307386
        if (EFI_ERROR (AtaStatus)) {
		  //Once get Hdd failed, try to get device identify data
          AtaStatus = GetDeviceIdentifyData (AtaDevice, Port, PortMultiplierPort, &IdentifyData);
          if (EFI_ERROR (AtaStatus)) {
            return AtaStatus;
          }
          //Skip ATAPI device, bit 15 should be cleared with ATA device
          if ((IdentifyData.config & BIT15) == BIT15)
            continue;
        }
// AMI_OVERRIDE_END <<< EIP307386
