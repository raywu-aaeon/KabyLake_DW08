...
)
{
    ...
    for ( ;(UINTN)pStartAddr < (UINTN)(dStartAddress + dSize); pStartAddr++ )
        if ( *(UINT32 *)pStartAddr == dSignature ) return ((UINT32)(UINTN)pStartAddr);
    return FALSE;
}
...
LegacyBiosLockLocalKeyboard (VOID)
{
    ...
    Status = pBS->LocateProtocol(&gEfiLegacyRegion2ProtocolGuid, NULL, (VOID**) &iRegion);
    if (EFI_ERROR(Status)) return;
    ...
    *(UINT32*)(UINTN)(dAMTInt9Ptr + 4) = (UINT32)(((UINTN)dSRedirFlagPtr & 0xFFFF) + \
...
)
{
    ...
    Status = pBS->LocateProtocol(&gEfiLegacySredirProtocolGuid, NULL, (VOID**) &pLegacySredir);
    if(EFI_ERROR(Status)) return;
...
**/
EFI_STATUS EFIAPI AMTLOCKKBDEntryPoint(
