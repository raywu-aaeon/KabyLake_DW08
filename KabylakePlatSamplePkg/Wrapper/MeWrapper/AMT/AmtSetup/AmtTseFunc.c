...
VOID iAMTProcessProceedToBootHook ( EFI_EVENT Event, VOID *Context)
{
    ...
    //if (pAmtWrapper == NULL) {
    Status = gBS->LocateProtocol(&gEfiAmtWrapperProtocolGuid, NULL, (VOID **) &pAmtWrapper);
    //}
...
VOID iAMTProcessEnterSetupHook  ( EFI_EVENT Event, VOID *Context)
{
    ...
    //AMI_OVERRIDE_START >>> Fix cpp check error
	//if (pAmtWrapper == NULL) 
    gBS->LocateProtocol(&gEfiAmtWrapperProtocolGuid, NULL, (VOID **) &pAmtWrapper);
	//AMI_OVERRIDE_END <<< Fix cpp check error
    if (pAmtWrapper != NULL) {		  
...
BOOLEAN iAMTProcessConInAvailabilityHook  (VOID)
{
    ...
    if(pAmtWrapper->ActiveManagementIsEnterSetupEnabled())
    {
        gEnterSetup = TRUE;
    }
