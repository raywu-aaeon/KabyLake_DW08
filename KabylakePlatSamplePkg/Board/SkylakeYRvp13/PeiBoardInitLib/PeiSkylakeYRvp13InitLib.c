...
// RVP13 Init Table
//
BOARD_INIT_TABLE mRvp13BoardInit [] = {
  {BoardIdSkylakeYRvp13, PeiSkylakeYRvp13Init},
  {BoardIdAmberLakeLpddr3Rvp13, PeiSkylakeYRvp13Init},
  { 0, 0 }
};
...
PeiSkylakeYRvp13Init (
  IN VOID *Content
  )
{
  EFI_STATUS Status;

  Status = PeiSkylakeA0Rvp3Init (Content);
...
EFI_STATUS
EFIAPI
PeiSkylakeYRvp13BoardInitLibConstructor (
  IN EFI_PEI_FILE_HANDLE     FileHandle,
  IN CONST EFI_PEI_SERVICES  **PeiServices
  )
{
  return CommonInitInternalLibConstructor (mRvp13BoardInit, TABLE_LIST_NUM);
}