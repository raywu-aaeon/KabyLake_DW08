/** @file
  Source code for the Skylake-Y RVP13 configuration init function in DXE init phase.
...
#include "SkylakeYRvp13InitLib.h"
...
//
BOARD_INIT_TABLE mSkylakeYRvp13Init [] = {
  {BoardIdSkylakeYRvp13, SkylakeYRvp13Init},
  {BoardIdAmberLakeLpddr3Rvp13, SkylakeYRvp13Init},
  { 0, 0 }
};
...
EFI_STATUS
SkylakeYRvp13AcpiConfigBlockInit (
...
EFI_STATUS
SkylakeYRvp13CommonConfigBlockInit (
  IN PLATFORM_INFO *PlatformInfo
)
{
  PCD64_BLOB Data64;
...
/**
  SkylaeYRvp13 board configuration init function for DXE memory phase.
  ...
  )
{
  ...
  Status = SkylakeYRvp13AcpiConfigBlockInit (PlatformInfo);
  ASSERT_EFI_ERROR(Status);

  Status = SkylakeYRvp13CommonConfigBlockInit (PlatformInfo);
  ASSERT_EFI_ERROR(Status);
...
EFI_STATUS
EFIAPI
DxeSkylakeYRvp13InitLibConstructor (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  )
{
  return CommonInitInternalLibConstructor (mSkylakeYRvp13Init, TABLE_LIST_NUM);
}
