/** @file
  Source code for the Skylake-Halo DDR4 RVP11 configuration init function in DXE init phase.
...
#include "SkylakeHaloDdr4Rvp11InitLib.h"
...
//
BOARD_INIT_TABLE mSkylakeHaloDdr4Rvp11Init [] = {
  {BoardIdSkylakeHaloDdr4Rvp11, SkylakeHaloDdr4Rvp11Init},
  {BoardIdSkylakeHaloDdr4Rvp11Ppv, SkylakeHaloDdr4Rvp11PpvInit},
  { 0, 0 }
};
...
EFI_STATUS
SkylakeHaloDdr4Rvp11AcpiConfigBlockInit (
...
EFI_STATUS
SkylakeHaloDdr4Rvp11CommonConfigBlockInit (
  IN PLATFORM_INFO *PlatformInfo
)
{
  EFI_STATUS                   Status;
  ...
  //
  //
  // SPT-Halo, Group B, Pad 14
  //
  PcdSet32S (PcdEcSmiGpio, GPIO_SKL_H_GPP_B14);
  PcdSet32S (PcdEcLowPowerExitGpio, GPIO_SKL_H_GPP_A16);
  //
  ...
  if (BoardIdOrgValue == BoardIdHaloDdr4uSFF) {
    Data64.BoardGpioConfig.u.Pin = GPIO_SKL_H_GPP_D23;
  } else {
  ...
  //
  if (BoardIdOrgValue == BoardIdHaloDdr4uSFF) {
    PcdSet32S(PcdSmcRuntimeSciPin, (UINT32)GPIO_SKL_H_GPP_C15);
  } else {
...
/**
  SkylaeHaloDdr4Rvp11 board configuration init function for DXE memory phase.
  ...
  )
{
  ...
  Status = SkylakeHaloDdr4Rvp11AcpiConfigBlockInit (PlatformInfo);
  ASSERT_EFI_ERROR(Status);

  Status = SkylakeHaloDdr4Rvp11CommonConfigBlockInit (PlatformInfo);
  ASSERT_EFI_ERROR(Status);
...
EFI_STATUS
EFIAPI
SkylakeHaloDdr4Rvp11PpvInit (
  IN VOID *Content
  )
{
  return SkylakeHaloDdr4Rvp11Init (Content);
}
...
EFI_STATUS
EFIAPI
DxeSkylakeHaloDdr4Rvp11InitLibConstructor (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  )
{
  return CommonInitInternalLibConstructor (mSkylakeHaloDdr4Rvp11Init, TABLE_LIST_NUM);
}
