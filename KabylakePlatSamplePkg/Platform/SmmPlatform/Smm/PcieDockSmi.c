  ...
  )
{
  ...
  UINT32                BusNumber32;
  UINT32                SecBusNum, SecBusNum1, SecBusNum2;
  UINT32                EndBusNum, EndBusNum1, EndBusNum2;
  UINTN                 DevIndex;
  UINT8                 Command8;
  UINT8                 BusWidth8 = 1;
  UINT16                Dev_id;
  UINT32                i;
  UINT32                *PCIControlRegister = NULL;

  //
  // Check if PEG (0,1,0) is populated
  //
  VenDev_id2 = 0;
  if (MmioRead16 (MmPciBase (0, 0x1, 0) + 0x00) != 0xFFFF) {
    //
    // Checking for the Turbo dock. Turbo dock has PLX 8547 for the PEG. 8547 sits on the Bus 0, dev1, fun 0.
    //
    BusNumber32 = MmioRead32 (MmPciBase (0, 0x1, 0) + 0x18);

    SecBusNum1 = (BusNumber32 >> 8) & 0xFF;                // Get Secondary Bus Number
    EndBusNum1 = (BusNumber32 >> 16) & 0xFF;               // Get Subordinary Bus Number


    // Read the vendor id to check for the PLX8547 on Secbusnum, dev 0 and func 0
    VenDev_id2 = MmioRead32 (MmPciBase (SecBusNum1, 0, 0) + 0);


    // Read the bus numbers for the PLX8547 device
    BusNumber32 = MmioRead32 (MmPciBase (SecBusNum1, 0, 0) + 0x18);
    ...
    if (VenDev_id2 == TURBODOCK_PLX_DEVID) { //if there is Turbo dock device
      for(i=SecBusNum2;i<=EndBusNum2;i++) {
        Dev_id = MmioRead16 (MmPciBase (i, 0, 0) + 2);
        if (Dev_id == 0x0191) { //If dev id is for NVidia card.
          PCIControlRegister = PCIE_CFG_ADDR(i, 0, 0, 0x114);
          *PCIControlRegister = 0x80000001;
        }
      }
    }
  }
  ...
  //
  BusNumber32 = MmioRead32 (MmPciBase (0, 0x1C, 6) + 0x18);
  ...
  VenDev_id1 = MmioRead32 (MmPciBase (SecBusNum, 0, 0) + 0);
  ...
  MmioAndThenOr32 (MmPciBase (SecBusNum, 0, 0) + 0x18, 0, BusNumber32);

  //
  // Get PCIe Root Port 7 I/O Base & Limit Register
  //
  IoBaseLimit = MmioRead16 (MmPciBase (0, 0x1C, 6) + 0x1C);

  //
  // Set PCIe dock first level Bridge I/O Base & Limit Register
  //
  MmioAndThenOr16 (MmPciBase (SecBusNum, 0, 0) + 0x1C, 0, IoBaseLimit);
  MmioAndThenOr32 (MmPciBase (SecBusNum, 0, 0) + 0x30, 0, 0);  // I/O Base/Limit Upper 16 Bits Regs

  //
  // Get PCIe Root Port 7 MEM Base & Limit Register
  //
  MemBaseLimit = MmioRead32 (MmPciBase (0, 0x1C, 6) + 0x20);

  //
  // Set PCIe dock first level Bridge MEM BAR0
  //
  BaseLimit32 = (MemBaseLimit & 0xFFFF0000);            // Set top 1M MEM for BAR0
  MmioAndThenOr32 (MmPciBase (SecBusNum, 0, 0) + 0x10, 0, BaseLimit32);

  //
  // Set PCIe dock first level Bridge MEM Base & Limit Register
  //
  MemBaseLimit -= 0x100000;                             // Exclude the top 1M MEM Range
  MmioAndThenOr32 (MmPciBase (SecBusNum, 0, 0) + 0x20, 0, MemBaseLimit);

  //
  // Get PCIe Root Port 7 Prefetchable MEM Base & Limit Register
  //
  PfmBaseLimit = MmioRead32(MmPciBase (0, 0x1C, 6) + 0x24);

  //
  // Set PCIe dock first level Bridge Prefetchable MEM Base & Limit Register
  //
  MmioAndThenOr32 (MmPciBase (SecBusNum, 0, 0) + 0x24, 0, PfmBaseLimit);

  //
  // Set PCIe dock first level Bridge Command Register
  //
  Command8 = 7;
  MmioAndThenOr8 (MmPciBase (SecBusNum, 0, 0) + 0x04, 0, Command8);
  ...
  for (DevIndex = 0; DevIndex <= 9; DevIndex ++) {
    ...
    //
    if (MmioRead16 (MmPciBase ((SecBusNum + 1), (UINT32)DevIndex, 0) + 0x00) != 0xFFFF) {
      //
      // I/O Base & Limit Register
      //
      MmioAndThenOr16 (MmPciBase ((SecBusNum + 1), (UINT32)DevIndex, 0) + 0x1C, 0, BaseLimit16);
      //
      // I/O Base/Limit Upper 16 Bits Register
      //
      MmioAndThenOr32 (MmPciBase ((SecBusNum + 1), (UINT32)DevIndex, 0) + 0x30, 0, 0);
      //
      ...
      //
      MmioAndThenOr32 (MmPciBase ((SecBusNum + 1), (UINT32)DevIndex, 0) + 0x20, 0, BaseLimit32);
      //
      ...
      //
      MmioAndThenOr32 (MmPciBase ((SecBusNum + 1), (UINT32)DevIndex, 0) + 0x24, 0, PfmBaseLimit32);
      //
      ...
      //
      MmioAndThenOr32 (MmPciBase ((SecBusNum + 1), (UINT32)DevIndex, 0) + 0x18, 0, BusNumber32);

      //
      // Set Command Register
      //
      MmioAndThenOr8 (MmPciBase ((SecBusNum + 1), (UINT32)DevIndex, 0) + 0x04, 0, Command8);
    }
  }

  SmmIoWrite32 (0x80, 0x4DCE);                          // Display port 80h with value 4DCEh
}
