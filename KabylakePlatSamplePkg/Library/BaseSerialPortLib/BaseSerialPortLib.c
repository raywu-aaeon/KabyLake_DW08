  ...
  )
{
  ...
  if ((!(CmosStatusCodeFlags & STATUS_CODE_CMOS_VALID)) || (CmosStatusCodeFlags & STATUS_CODE_CMOS_INVALID)) {
    CmosStatusCodeFlags = STATUS_CODE_USE_ISA_SERIAL | STATUS_CODE_USE_TRACEHUB | STATUS_CODE_CMOS_VALID;

    //
    // @todo: Remove once Setup option malfunction is fixed.
    //        Intel Advanced Menu -> Debug Configuration -> USB3[]
    //
    if (PcdGetBool (PcdStatusCodeUseUsb3Serial) == TRUE) {
      CmosStatusCodeFlags = CmosStatusCodeFlags | STATUS_CODE_USE_USB3;
    }

    DEBUG_CODE(CmosStatusCodeFlags |= STATUS_CODE_USE_SERIALIO;);
    SetDebugInterface (CmosStatusCodeFlags);
  }
  // no init for MEM

  if (CmosStatusCodeFlags & STATUS_CODE_USE_ISA_SERIAL) {
    UartInitialize ();
  }
...
/*
  ...
  if (CmosStatusCodeFlags & STATUS_CODE_USE_USB3) {
...
// AMI_OVERRIDE_START - EIP226329 : POST hang on "0x0D" when use debug-mode with UART2.
//    PchSerialIoUartInit (2,TRUE,115200,3,FALSE);
	  PchSerialIoUartInit (PcdGet8 (PcdSerialIoUartNumber),TRUE,115200,3,FALSE);
// AMI_OVERRIDE_END - EIP226329 : POST hang on "0x0D" when use debug-mode with UART2.
  }
...
// AMI_OVERRIDE_START - AMI not use.
/*  
  if (CmosStatusCodeFlags & STATUS_CODE_USE_RAM) {
    RamDebugWrite (Buffer, NumberOfBytes);
  }
*/
// AMI_OVERRIDE_END - AMI not use.
...
/*
  if (CmosStatusCodeFlags & STATUS_CODE_USE_USB3) {
    Usb3DebugPortWrite (Buffer, NumberOfBytes);
  }
*/
// AMI_OVERRIDE_END - AMI not use.
  ...
  if (CmosStatusCodeFlags & STATUS_CODE_USE_SERIALIO) {
// AMI_OVERRIDE_START - Fixed PCH UART can't connect to SerialDebugger feature.
// AMI_OVERRIDE_START - EIP226329 : POST hang on "0x0D" when use debug-mode with UART2.
    // PchSerialIoUartOut (2, Buffer, NumberOfBytes);
	NumberOfBytes = PchSerialIoUartOut (PcdGet8 (PcdSerialIoUartNumber), Buffer, NumberOfBytes);
// AMI_OVERRIDE_END - EIP226329 : POST hang on "0x0D" when use debug-mode with UART2.
// AMI_OVERRIDE_END - Fixed PCH UART can't connect to SerialDebugger feature.
  }

#ifdef TRACE_HUB_DEBUGLIB_USAGE
  if ((CmosStatusCodeFlags & STATUS_CODE_USE_TRACEHUB) &&
...
// AMI_OVERRIDE_END - Fixed PCH UART can't connect to SerialDebugger feature.
  ...
  CmosStatusCodeFlags = GetDebugInterface();
  //CmosStatusCodeFlags = ReadCmos8(PcdGet8(PcdStatusCodeFl agsCmosIndex));
  if ((!(CmosStatusCodeFlags & STATUS_CODE_CMOS_VALID)) || (CmosStatusCodeFlags & STATUS_CODE_CMOS_INVALID)) {
    //
    // invalid cmos value, it means action was attempted before Init
    //
...
/*  
  if (CmosStatusCodeFlags & STATUS_CODE_USE_USB3) {
    return Usb3DebugPortRead (Buffer, NumberOfBytes);
  }
*/
// AMI_OVERRIDE_END - AMI not use.
...
// AMI_OVERRIDE_START - Fixed PCH UART can't connect to SerialDebugger feature.
// AMI_OVERRIDE_START - EIP226329 : POST hang on "0x0D" when use debug-mode with UART2. 
//    return PchSerialIoUartIn (2, Buffer, NumberOfBytes, FALSE);
    Result = PchSerialIoUartIn (PcdGet8 (PcdSerialIoUartNumber), Buffer, NumberOfBytes, FALSE);
// AMI_OVERRIDE_END - EIP226329 : POST hang on "0x0D" when use debug-mode with UART2.
// AMI_OVERRIDE_END - Fixed PCH UART can't connect to SerialDebugger feature.
  }
// AMI_OVERRIDE_START - Fixed PCH UART can't connect to SerialDebugger feature.  
