	...
	//{
		switch( CheckControlCondition( &popupstring->ControlData ) )
		{
			...
			case COND_GRAYOUT:
				...
				ColorLabel = CONTROL_GRAYOUT_COLOR;		// to display the control value in grayout color
				break;
			default:
...
EFI_STATUS PopupStringHandleAction( POPUP_STRING_DATA *popupstring, ACTION_DATA *Data)
{
	...
	if ( Data->Input.Type == ACTION_TYPE_KEY )
    {
		...
		{
            UINT16 TmpNewPopupStringTextWidth = 0;
			...
			TmpNewPopupStringTextWidth = (UINT16)EfiStrLen(popupstring->Text);
			
			// Check for Range. If its a valid string annd not between min and max report invalid error msg
			if((popupstring->Text) &&((TmpNewPopupStringTextWidth < MinVal) || (TmpNewPopupStringTextWidth > MaxVal)))
			{
	...
	if(retValue ==0)
	{
	    ...
	    *Text = EfiLibAllocateZeroPool( (gPopupEditCb.Length+1) * sizeof(CHAR16) );
		...
		return EFI_OUT_OF_RESOURCES;
	     ...
	     StrCpyS (*Text, (gPopupEditCb.Length)+1, gPopupEditCb.Data);		//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
