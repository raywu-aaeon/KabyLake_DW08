...
/**
    This function destroys OrderedList control
    Every member ptr within control structure has to be freed regardless of freeMem (true/false). 
    When freeMem is TRUE, free control data [ORD_LISTBOX_DATA*]
...
EFI_STATUS _OrdListMultiLIneDraw(ORD_LISTBOX_DATA *OrdList)
{
	...
	else
	{
        ...
        do
		{
            ...
            if ( Len > (UINTN)(OrdList->Width - OrdList->LabelMargin -2) )
			    EfiStrCpy(&text1[HiiFindStrPrintBoundary(text1 ,(UINTN)(OrdList->Width- OrdList->LabelMargin -5))],L"..." );
