...
extern VOID EvalauteDisableifExpressionOverControls(VOID);
extern VOID EvalauteDisableifControlsInFormset(EFI_HII_HANDLE Handle);
extern UINTN GetAMITSESETUPSize();
extern UINTN GetAMISilentBootOffset();
...
BOOLEAN gIsDisableIfExpressionEvalauted;
...
VOID EFIAPI InTseGopNotificiationFunciton( EFI_EVENT Event, VOID *NotificationContext );
...
static EFI_GUID gAmiUefiLibPopupProtocolGuid = AMI_UEFI_LIB_POPUP_PROTOCOL_GUID;
...
EFI_STATUS MiniSetupEntry(VOID)
{
	...
	gIsDisableIfExpressionEvalauted = FALSE;
    ...
    // disable controls that evaluate to COND_DISABLE, 
    // call callbacks for those controls that were not evaluated to COND_DISABLE.
    gDisableFlag = EvaluateDisableOpControls;   
    EvalauteDisableifExpressionOverControls(); // Check Disable if over controls of all formset. 
    gIsDisableIfExpressionEvalauted = TRUE;
    // call the callbacks for evaluated controls only
    UEFICallSetupFormCallBack(AMI_CALLBACK_FORM_OPEN);
    UEFICallSetupFormCallBack(AMI_CALLBACK_RETRIEVE);
    //reset gDisableFlag; Control evaluation will be happening normally
    gDisableFlag = 0;
   HandleReadOpOnRetrieve();		//Evaluating Read opcode for all the control in all the pages after AMI_CALLBACK_RETRIEVE callback done while load previous values
   

	if(!NoVarStoreSupport())
	{
		AMITSESETUP *TempSaveData = (AMITSESETUP *)NULL;
		TempSaveData =(AMITSESETUP *)VarGetNvram( VARIABLE_ID_AMITSESETUP, &size );
		if(size < GetAMITSESETUPSize())
		{
			pSetupVariable = (AMITSESETUP *) EfiLibAllocateZeroPool(GetAMITSESETUPSize());
			...
			VarSetNvram( VARIABLE_ID_AMITSESETUP, pSetupVariable, GetAMITSESETUPSize());
			VarUpdateDefaults(VARIABLE_ID_AMITSESETUP);
		}
 		if(TempSaveData) //For MemoryLeak Fix 
...
UINT32 GetValidPageId(UINT32 PageId)
{
    ...
    if (NULL == PageInfo || 0 == PageInfo->PageHandle || PageInfo->PageHandle == (VOID*)(UINTN)0xffff) 
    {
        ...
        if (conditionPtr && *conditionPtr)
        {
            ...
            {
                if ( *conditionPtr != bootFlowPtr->Condition )
                    continue;
                ...
                for(Page = 0; Page < gPages->PageCount ; Page++)
                {
                    ...
                    if (
                        ( bootFlowPtr->PageClass == PageIdInfo->PageClass ) &&
                        ( bootFlowPtr->PageSubClass == PageIdInfo->PageSubClass ) &&
...
//	SetDesiredTextMode();		//StyleInit will set the desired text mode
	...
	if(FormBrowserHandleValid() || (NULL == gApp) )//Don't re-create Hii Data if already done
	{
	...
	if(FormBrowserHandleValid() || (NULL == gApp))//Don't re-init gApp if already done
	{
	...
	if (gStartPage) 
	{
	    gStartPage = GetValidPageId(gStartPage);
	}
...
//			Language = VarGetNvramName (L"PlatformLang", &gEfiGlobalVariableGuid, NULL, &Size);
			...
			if (NULL != Language && Size < sizeof(gPlatformLang))		//Checking for Size to avoid overlapping the boundary
			{
...
Exit:
	MemFreePointer ((VOID **)&Language); // Memory leak fix - Free memory allocated for last page
	StyleExit();
...
DONE:
	...
	if(BootMenu) //For MemoryLeak Fix
	{
		MemFreePointer((void **)&BootMenu);
		BootMenu = NULL;
	}
	return Status;
}
...
VOID UIUpdateCallback(VOID * Handle, UINT32 OldVariableCount)
{
	...
	if(gApp == NULL)
	{
	   if(gIsDisableIfExpressionEvalauted)// If pack happens before evaluating disableif expression for entire setup, skip this part. It will handle in EvalauteDisableifExpressionOverControls().
	   {
	       //This part will execute only if pack update happens in between after EvalauteDisableifExpressionOverControls() and before gApp initialize in MainSetupLoop().
	       gDisableFlag = EvaluateDisableOpControls;   
	       EvalauteDisableifControlsInFormset(Handle); // Check for disbaleIF controls before updating Frame details.
	       gDisableFlag = 0;
	   }
...
**/
VOID EFIAPI _CompleteRedrawNotificationFunction (EFI_EVENT Event, VOID *Context)
{
	if(gApp)
	{
		gApp->CompleteRedraw = TRUE;
