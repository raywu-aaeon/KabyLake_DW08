...
UINT8* gExpEvaluation = (UINT8*)NULL;
...
EFI_STATUS FrameDestroy( FRAME_DATA *frame, BOOLEAN freeMem )
{
	...
	if(gExpEvaluation)
        MemFreePointer((VOID**)&gExpEvaluation);
...
**/
BOOLEAN	IsScrollSupportRequired(FRAME_DATA *frame, UINT8* ExpEvaluation )
{
	CONTROL_DATA **control;
	UINT16 i, Height;
	UINT8 u8ChkResult;
	...
	for ( i = 0; i < frame->ControlCount; i++, control++ )
	{
		if(!control || !(*control)->ControlData.ControlFlags.ControlVisible)
			continue;
		
	    if(frame->FrameData.FrameType == MAIN_FRAME && ExpEvaluation)
	        u8ChkResult = ExpEvaluation[i];
	    else
...
EFI_STATUS FrameDraw( FRAME_DATA *frame )
{
    ...
    UINT8* ExpEvaluation = (UINT8*)NULL;
	...
	if ( control != NULL )
	{
		if(!(*control)->ControlData.ControlFlags.ControlVisible)
		{
		...
		{
          	// Static code analysis may report a dereferencing (*control) error here. This is a false positive
			// If the Dynamic update happens while drawing the control (via Evalutate
			// Expression->VarGetValue->ExtractConfig). The controls may be invalidated. 
			// In that case don't proceed with Frame Draw
...
ClearFrame:
	...
	if(frame->FrameData.FrameType == MAIN_FRAME)
	{
        ExpEvaluation = EfiLibAllocateZeroPool(frame->ControlCount);
        control = frame->ControlList;
        for ( i = 0; i < frame->ControlCount; i++, control++)
        {
            if(!control || !((*control)->ControlData.ControlFlags.ControlVisible))
            {
                ExpEvaluation[i] = COND_HIDDEN;
                continue;
            }
            ExpEvaluation[i] = CheckControlCondition( &((*control)->ControlData) );
            if(*control == NULL)
            {
                    gApp->CompleteRedraw = TRUE;
                    MemFreePointer((VOID**)&ExpEvaluation);
                    return EFI_UNSUPPORTED;
            }
		}
	}
	//Check whether the scroll support required before proceeding.
	if(!IsScrollSupportRequired( frame, ExpEvaluation )) {
	...
	for (count = 0; count < frame->ControlCount; count++)
	{
		...
		if(!frame->ControlList[count] || !frame->ControlList[count]->ControlData.ControlFlags.ControlVisible)
			continue;

        if (frame->FrameData.FrameType == MAIN_FRAME)
            u8ChkResult = ExpEvaluation[count];
        else 
...
EFI_STATUS FrameHandleAction( FRAME_DATA *frame, ACTION_DATA *action )
{
	...
	switch ( action->Input.Type )
	{
        ...
        case ACTION_TYPE_KEY:
            if ( control != NULL && (*control)->ControlData.ControlFlags.ControlVisible)
    ...
    switch ( bScrollUp )
    {
        case TRUE:
			...
			if (!bDone)
			{
                ...
                {
                    thisControl--;
                    if(!frame->ControlList[thisControl] || !frame->ControlList[thisControl]->ControlData.ControlFlags.ControlVisible)
                    	continue;
  
                    if((MAIN_FRAME == frame->FrameData.FrameType) && gExpEvaluation)
                        u8ChkResult = gExpEvaluation[thisControl];
                    else
...
VOID _FrameSubmenuCallback( FRAME_DATA *frame, SUBMENU_DATA *submenu, VOID *cookie )
{
	...
	if ( ! EFI_ERROR( Status ) )
	{
		...
		{
		    // need to update control focus value
		    UINTN       i ;
		    PAGE_DATA   *PageData = gApp->PageList[gApp->CurrentPage];
		    FRAME_DATA  *MainFrame = NULL;
		    
            MainFrame = PageData->FrameList[StyleFrameIndexOf(MAIN_FRAME)] ;
		    for (i = 0 ; i < MainFrame->ControlCount ; i++)
		    {
                if (_FrameSetControlFocus(MainFrame, (UINT32)i))
                {
                    MainFrame->FirstVisibleCtrl = 0;
                    break;
                }
		    }
		}
...
VOID _FrameVariableCallback( FRAME_DATA *frame, CONTROL_DATA *control, VOID *cookie )
{
	...
	HandleWriteOpOnChanged(&(control->ControlData));		//Evaluating Write opcode for a current control when the control value is edited
	HandleReadOpOnChanged(gApp->CurrentPage);				//Evaluating Read opcode for all the controls in current page to update to the latest edited value
...
VOID _FramePasswordCallback( FRAME_DATA *frame, CONTROL_DATA *control, VOID *cookie )
{
	...
	if(FramePwdCallbackIdePasswordUpdate ( control,saveData) != EFI_SUCCESS)
	{
        ...
        {   
            if ( VARIABLE_ID_AMITSESETUP != callbackData->Variable)
            {   
                //VARIABLE_ID_AMITSESETUP - is an exception, 
                //use TsePasswordVariableLength even though callbackData->Length will be less 
                if (callbackData->Length < TsePasswordVariableLength * sizeof(CHAR16))
                {

                    UINT8   Sel = 0;
                    CHAR16  *Title = HiiGetString (gHiiHandle, STRING_TOKEN (STR_ERROR));
                    CHAR16  *Text = HiiGetString (gHiiHandle, STRING_TOKEN (STR_ENCODING_ERROR));
            ...
            //for encoding Use TsePasswordVariableLength instead of callbackData->Length
            ...
            if (!EfiCompareMem( TestPassword, callbackData->Data, callbackData->Length))  // clearing the password 
            {   // need a bigger zero buffer if the incoming one is too small     
                if (callbackData->Length < TsePasswordVariableLength * sizeof(CHAR16))
                    saveData = TestPassword;
            }
            else // not clearing the password so saveData should be valid
            {
                saveData = PasswordUpdate( callbackData->Data, (TsePasswordVariableLength)*sizeof(CHAR16));
                if ((!saveData)|| (!EfiCompareMem( TestPassword, saveData, TsePasswordVariableLength * sizeof(CHAR16))) )
                    return;
            }
            ...
            if((TSEPwdSavetoNvram())&&( VARIABLE_ID_AMITSESETUP == callbackData->Variable ))
            {
                ...
                UINTN size=0;
                size = GetAMITSESETUPSize();
                TempSaveData = VarGetNvram(VARIABLE_ID_AMITSESETUP, &size);
                MemCopy( ((UINT8*)(TempSaveData))+(callbackData->Offset), saveData, TsePasswordVariableLength * sizeof(CHAR16));
                VarSetNvram( VARIABLE_ID_AMITSESETUP, TempSaveData, GetAMITSESETUPSize());
                MemFreePointer( (VOID **)&TempSaveData );	       	
            }
            else if ( VARIABLE_ID_AMITSESETUP == callbackData->Variable )
                VarSetValue( callbackData->Variable, callbackData->Offset, TsePasswordVariableLength*sizeof(CHAR16), saveData );
            else 
                VarSetValue( callbackData->Variable, callbackData->Offset, callbackData->Length, saveData );
        }
        else // no encoded enabled, save as is
        {
            VarSetValue( callbackData->Variable, callbackData->Offset, callbackData->Length, saveData );
        }
        ...
        if ( saveData != callbackData->Data )
            StringZeroFreeMemory( (VOID **)&saveData );
  	}
...
EFI_STATUS _FrameAddControls( FRAME_DATA *frame, PAGE_INFO *data )
{
	...
	{
		CONTROL_INFO* controlInfo = (CONTROL_INFO *) ((UINT8 *)gControlInfo + data->PageControls.ControlList[i]);

		if(controlInfo && controlInfo->ControlFlags.ControlVisible)
		{
...
EFI_STATUS _FrameAddTitle( FRAME_DATA *frame, UINT32 frameType, PAGE_INFO *data )
{
	...
	{	
		(*control)->FGColor = (UINT8)StyleGetFocusColor();
	}
	frame->ControlCount++;
	...
	if(frame->UseScrollbar)
	{
		...
		if(MouseInfo.Left == (UINT32) frame->FrameData.Left + frame->FrameData.Width)
		{
					...
					)
			{
				...
				if ( (UINT32)lButtonDownInitialPosition != MouseInfo.Top )
				{
...
EFI_STATUS TSEMouseHelpFrameHandleAction( FRAME_DATA *frame, ACTION_DATA *action,CONTROL_DATA **control )
{
	...
	{	
		...
		tempText = StringWrapText( text, memo->Width, &height );
		MemFreePointer( (VOID **)&text );
		...
		if(bScrollBar && (action->Input.Data.MouseInfo.ButtonStatus == TSEMOUSE_LEFT_CLICK))
		{
			...
			if((action->Input.Data.MouseInfo.Top == (UINT32)(frame->FrameData.Top + frame->FrameData.Height -1))&&(action->Input.Data.MouseInfo.Left == (UINT32)(frame->FrameData.Width + frame->FrameData.Left -1) ))
				 	_HelpFrameScroll( memo, FALSE);
			
			if(((action->Input.Data.MouseInfo.Top == (UINT32)frame->FrameData.Top) ||(action->Input.Data.MouseInfo.Top == (UINT32)(frame->FrameData.Top -1)))
			   && (action->Input.Data.MouseInfo.Left == (UINT32)(frame->FrameData.Width + frame->FrameData.Left -1)) )
			...
			for ( i = 0; i < frame->ControlCount; i++ )
				{
					...
					if(control != NULL && control->ControlData.ControlFlags.ControlVisible)
						...
						//Check conditional
						u8ChkResult = CheckControlCondition(&(control->ControlData ));
	       				if((COND_SUPPRESS != u8ChkResult) && (COND_DISABLE != u8ChkResult) )
