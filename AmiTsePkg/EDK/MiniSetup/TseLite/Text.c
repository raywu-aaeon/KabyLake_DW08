...
*/
    ...
    if(IsTSETextExcessSupport() && (StrLen(txt2) == 0) && (text->ControlData.ControlFlags.ControlInteractive == 0)) // check txt2 have strings or not in mutlicontrolSupported case
    {
        temp_width = text->Width - gLabelLeftMargin; 
        ...
        else
        {
            ...
            if ( (TestPrintLength( text->LabelText ) / (NG_SIZE)) > temp_width)
                    text->LabelText[HiiFindStrPrintBoundary(text->LabelText , temp_width)] = L'\0';
	...
	if ( text->Text != NULL )
	{
		...
		if( IsTSEMultilineControlSupported())
		{
				DrawMultiLineStringWithAttribute( (UINTN)text->Left + text->LabelMargin, (UINTN) text->Top, 
						(UINTN)(text->Width - text->LabelMargin +1),(UINTN)text->Height,
		...
		else
		{
		    ...
		    if ( (TestPrintLength( text->Text ) / (NG_SIZE)) >= (UINTN)(text->Width - text->LabelMargin +1) )
	      	  EfiStrCpy(&text->Text[HiiFindStrPrintBoundary(text->Text ,(UINTN)(text->Width - text->LabelMargin +1 -3))],L"..." ); 
...
EFI_STATUS TextGetControlHeight( TEXT_DATA *text, VOID* frame,  UINT16 *height )
{
	...
	if(IsTSEMultilineControlSupported())
	{
		...
		//
		newText = StringWrapText( text->Text, (UINT16)(text->Width - text->LabelMargin + 1), &tmpHeight );
		*height = tmpHeight;
		MemFreePointer( (VOID **)&newText );

		if(IsTSETextExcessSupport())
		{
			if(Text2 && (EfiStrLen(Text2) == 0)) // check printing the string on both txt and txt2 width
			{
				Width = (UINT16)(text->Width - gLabelLeftMargin); // height will be adjusted according to width
			}
		}
