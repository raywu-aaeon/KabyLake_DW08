	...
	)
{
	...
	case MSG_DNS_DP:
	    n = (((DNS_DEVICE_PATH*)Path)->Header.Length[0] - 5)/sizeof(EFI_IP_ADDRESS);//number of instances of the DNS server address in EFI_IP_ADDRESS. 5 is the size of other variables(Header+IsIPv6) in DNS_DEVICE_PATH structure.
	    SPrint(Str,0,L"Dns(");
	    Offset = EfiStrLen(L"Dns(");
	    if(((DNS_DEVICE_PATH*)Path)->IsIPv6)
	    {
            for(i=0;i<n;i++)
            {
                SPrint(Str + Offset,0,L"%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x,",
                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[0],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[1],
                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[2],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[3],
                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[4],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[5],
                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[6],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[7],
                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[8],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[9],
                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[10],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[11],
                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[12],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[13],
                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[14],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v6.Addr[15]
                    );
                Offset = EfiStrLen(Str);
            }
            return SPrint(Str + Offset - 1,0,L")\\");
	    }
	    else
	    {
	        for(i=0;i<n;i++)
            {
	            SPrint(Str + Offset,0,L"%d.%d.%d.%d,",((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v4.Addr[0],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v4.Addr[1],
	                    ((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v4.Addr[2],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v4.Addr[3]);
	            Offset = EfiStrLen(Str);
            }
	        return SPrint(Str + Offset - 1,0,L")\\");
	    }
...
CHAR16* GetFileNameFromDevicePath(EFI_DEVICE_PATH_PROTOCOL *Path)
{
    ...
    for(devPath = Path;!IsDevicePathEnd(devPath);devPath = NextDevicePathNode(devPath))
    {
        if (devPath->Type == MEDIA_DEVICE_PATH && devPath->SubType == MEDIA_FILEPATH_DP )
        {
            Count = _MEDIAToStr(devPath, p);
            break;
        }
    }
    Length = (EfiStrLen(p)+1)* sizeof(CHAR16);
    Str = (UINT16 *)EfiLibAllocateZeroPool(Length);
    MemCopy(Str, Buffer, Length-4);
    return Str;
}
...
UINT16 *_DevicePathToStr(EFI_DEVICE_PATH_PROTOCOL *Path)
{
    ...
    {
    	Status = gBS->LocateProtocol(&DevPathToTextGuid,NULL,(void**)&DevPathToTextProtocol);
...
*/
	...
	if(DevPathToTextProtocol)
		MemFreePointer((void**)&DevPathToTextProtocol);
...
VOID BootAddBootOption( VOID )
{
	...
	//	CallbackShowMessageBox( (UINTN)gAddBootOptionEmpty, MSGBOX_TYPE_OK );
    	...
    	MemFreePointer ((VOID **)&newBoot); //For MemoryLeak Fix
		return;
    }
...
VOID DriverAddDriverOption(VOID)
{
	...
	if (( EfiStrLen (newDriver->DriverPath) == 0 ) || (EfiStrLen (newDriver->Label) == 0))
    {
		//CallbackShowMessageBox( (UINTN)gAddDriverOptionEmpty, MSGBOX_TYPE_OK );
		 ...
		 MemFreePointer ((VOID **)&newDriver); //For MemoryLeak Fix
		return;
    }
...
VOID DiscardAddDelBootOptions(VOID)
{
    ...
    for(i=0;i<gDelOptionCount;i++)
    {
        ...
        {
        	count++;
            DelOptionListCount++;// Holds the count of deleted existing options that needs to be re-added.
        }
    }
...
// If newly add/Del option discorded.
	if(AddOptionListCount || DelOptionListCount)
	{
        ...
        for(j=0;j<count;j++)
		{
			...
			{
				if ((UINTN)size >= ((j+1)*sizeof (UINT16)) )// Don't go beyond the current valid boot order size 
				{
					...
					size-=sizeof(UINT16);
					j--;//re-check the same index since the bytes are left shifted
				}
			}
		}
		...
		if(gBootOptionCount >= (count + DelOptionListCount)) // Don't copy beyond allocated memory.
    ...
    MemFreePointer((VOID **) &DelOptionList); // Memory Leak fix - to avoid leak if deleted only newly added optoin, not old options.
...
// If newly add/Del option discorded.
	if(AddOptionListCount || DelOptionListCount)
	{
		...
		if(gDriverOptionCount >= (count + DelOptionListCount)) // Don't copy beyond allocated memory.
...
VOID SaveAddDelBootOptions(VOID)
{
	...
	{
		if(pOldOptimal && BootGetBootData(pOldOptimal[i])) // pOldOptimal may be NULL
	    {
	        pNewOptimal[OptimalIndex] = pOldOptimal[i];
	        OptimalIndex++;
	    }	
	}
    size = gFailsafeDefaults[ VARIABLE_ID_BOOT_ORDER ].Size;
    for(i=0;i<size/sizeof(UINT16);i++)
    {
		if(pOldFailsafe && BootGetBootData(pOldFailsafe[i]))// pOldFailsafe may be NULL
        {
	...
	for(i=0;i<gBootOptionCount;i++)
	{
		if(gBootData[i].bNewOption)
		{
			...
			OptimalIndex++;
			FailsafeIndex++;
			MemFreePointer((void**)&pOption);//For MemoryLeak Fix 
		}
	}
   ...
   //All the time changing the default buffer is not good
	if (0 == gOptimalDefaults [VARIABLE_ID_BOOT_ORDER].Size)
	{
		...
		if(pNewOptimal) //For MemoryLeak Fix 
			MemFreePointer((VOID **)&pNewOptimal);
	}
	else
	{		 	 
...
VOID SaveAddDelDriverOptions(VOID)
{
	...
	for(i=0;i<size/sizeof(UINT16);i++)
	{
		if (pOldOptimal && DriverGetDriverData (pOldOptimal [i])) // pOldOptimal may be NULL
	    {
	        pNewOptimal[OptimalIndex] = pOldOptimal[i];
	        OptimalIndex++;
	    }
    ...
    for(i=0;i<size/sizeof(UINT16);i++)
    {
		if(pOldFailsafe && DriverGetDriverData (pOldFailsafe[i])) // pOldFailsafe may be NULL
        {
            pNewFailsafe[FailsafeIndex] = pOldFailsafe[i];
            FailsafeIndex++;
        }
 ...
 EFI_STATUS CreateFileList(EFI_FILE_PROTOCOL *FileProtocol, FILE_TYPE **FileList, UINTN *FileCount)
{
    ...
    while(BufferSize != 0 && !EFI_ERROR(Status))
    {
        ...
        if(Status == EFI_BUFFER_TOO_SMALL)
        {
            ...
            else // file info corrupted; reset the status and continue reading
            {
                Status = EFI_SUCCESS;
                continue;
            }
...
static EFI_STATUS DisplayFileListMenu(FILE_TYPE *FileList, UINTN FileCount, UINT16 *Selection, UINT32 Variable)
{
    ...
    if(FileCount != 0 && FileList != NULL)
    {
        ...
        Status = mPostMgr->DisplayPostMenu(gHiiHandle,
                                            ...
                                            (UINT16)FileCount,
                                            Selection);
        MemFreePointer((void**)&List);//For MemoryLeak Fix 
    }
    return Status;
}
        ...
        if(PossibleFileSystems != NULL)
        {
            ...
            for(i = 0; i < Count; i++)
            {
                ...
                if(!EFI_ERROR(Status))
                {
                    ...
                    MemFreePointer((void**)&Name);//For MemoryLeak Fix 
                }
                else
                {
...
//    EFI_HANDLE Handle = 0; Unused
	...
	while(!EFI_ERROR(Status) && gFs != NULL)
	{
		...
		if(Status == EFI_ABORTED) {
		    MemFreePointer((VOID**)&FilePath);//For MemoryLeak Fix 
			return Status;
		}
	}
...
EFI_STATUS FileBrowserChooseFile(UINT32 Variable)
{
    EFI_STATUS Status;
    EFI_DEVICE_PATH_PROTOCOL *dp;
    CHAR16 FileType[] = L"EFI";
    CHAR16* Name;
    UINT16 FsIndex = 0;
    Status = ChooseFileWrapper(FileType,&dp,&FsIndex) ;

    if(!EFI_ERROR(Status))
    {
        Name = GetFileNameFromDevicePath(dp); 
        if (VARIABLE_ID_ADD_BOOT_OPTION == Variable)
        {
            VarSetValue(VARIABLE_ID_ADD_BOOT_OPTION, STRUCT_OFFSET(NEW_BOOT_OPTION,SelFs), sizeof(UINT16), (VOID*)&FsIndex);
            VarSetValue(VARIABLE_ID_ADD_BOOT_OPTION, STRUCT_OFFSET(NEW_BOOT_OPTION,Path), ((EfiStrLen(Name)+1)*sizeof(CHAR16)), Name);
        }
        if (VARIABLE_ID_ADD_DRIVER_OPTION == Variable) 
        {
            VarSetValue(VARIABLE_ID_ADD_DRIVER_OPTION, STRUCT_OFFSET(NEW_DRIVER_OPTION,SelFs), sizeof(UINT16), (VOID*)&FsIndex);
            VarSetValue(VARIABLE_ID_ADD_DRIVER_OPTION, STRUCT_OFFSET(NEW_DRIVER_OPTION,DriverPath), ((EfiStrLen(Name)+1)*sizeof(CHAR16)), Name);
        }
