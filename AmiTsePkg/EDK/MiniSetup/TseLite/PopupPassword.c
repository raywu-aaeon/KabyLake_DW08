...
UINT32 PopupPasswordCheckInstalledLocal(POPUP_PASSWORD_DATA *popuppassword)
{
    ...
    if  (VARIABLE_ID_AMITSESETUP == popuppassword->ControlData.ControlVariable)
        size = (UINT8)TsePasswordVariableLength; 
	else
	    size = (UINT8)UefiGetMaxValue(data);
	    
	
	// read real password from nvram
	RealPassword = EfiLibAllocateZeroPool( (size+1) * sizeof(CHAR16) );
	if ( RealPassword == NULL )
        return Installed;
	MemSet( RealPassword, ((size+1) * sizeof(CHAR16)), 0 );

	VarGetValue( popuppassword->ControlData.ControlVariable, UefiGetQuestionOffset(data), size*2, RealPassword );

	for(i=0;i<size;i++)
	{
        if ( RealPassword[i] != L'\0' ){
            Installed = 1;
            break;
        }
	}
...
BOOLEAN PopupPasswordAuthenticateLocal( POPUP_PASSWORD_DATA *popuppassword, CHAR16 *Password )
{
	...
	TempEncoded = EfiLibAllocateZeroPool( (size+1) * sizeof(CHAR16) );
	if ( TempEncoded == NULL )
        return AuthenticateStatus;

	MemSet( TempEncoded, (size+1) * sizeof(CHAR16), 0 );
	...
	if( IsPasswordEncodeEnabled( &popuppassword->ControlData )){
		PasswordEncodeHook( TempEncoded, size * sizeof(CHAR16));
	}
    if( EfiCompareMem(TempEncoded, RealPassword, size * sizeof(CHAR16)))
		AuthenticateStatus = FALSE;
	else
		AuthenticateStatus = TRUE;

    StringZeroFreeMemory ( (VOID **)&RealPassword );
...
EFI_STATUS _PopupPasswordActivate(POPUP_PASSWORD_DATA *popuppassword)
{
	...
	if(UefiIsInteractive(&popuppassword->ControlData))
	{
		...
		else
		{
			...
			else if((Status == EFI_SUCCESS) && (*Text == L'\0')) //for empty old password
			{
				CallbackShowMessageBox( (UINTN)gInvalidPasswordFailMsgBox, MSGBOX_TYPE_OK );
				goto Done;
...
Password:
	...
	//CheckIsAllowedPasswordCharCombination() allows to verfiy the input string is allowed for passwordcontrol.If not we don't 
	//ask for confirm passowrd.
		//handling for ESC over new password window.
		if(Status || (NewPswd && NewPwLen && !CheckIsAllowedPasswordCharCombination(NewPswd)))
		{
		...
		if ( !Status )
		{
			...
			if ( ( NULL!=ConfirmNewPswd ) && EfiStrCmp(NewPswd, ConfirmNewPswd)==0 ) // Avoid crashing when give empty password
			{
					...
					if(popuppassword->Text != NULL){
					    StringZeroFreeMemory( (VOID **)&popuppassword->Text );
					}
					...
					if(UefiIsInteractive(&popuppassword->ControlData))
					{
							...
							Key = UefiGetControlKey(&(popuppassword->ControlData));
							if(NewPwLen){
								TempEncoded = EfiLibAllocateZeroPool( ((UINT8)UefiGetMaxValue(data)+1) * sizeof(CHAR16) );
								if ( TempEncoded == NULL )
									return EFI_NOT_READY;
...
EFI_STATUS _DoPopupEdit( POPUP_PASSWORD_DATA *PopupPassword, UINT16 Title, CHAR16 **Text)
{
	...
	if (IsTSECursorSupport() && (gST->ConOut))
	{
