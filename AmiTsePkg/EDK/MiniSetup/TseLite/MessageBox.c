...
/**
    This function destroys MsgBox control
    Every member ptr within control structure has to be freed regardless of freeMem (true/false). 
    When freeMem is TRUE, free control data [MSGBOX_DATA*]
    Destroy() has to be called with FALSE inside another Destroy(), 
    unless it’s an explicit member object that is allocated during Initialize or Create [gMemo].
...
EFI_STATUS MsgBoxDestroy( MSGBOX_DATA *msgbox, BOOLEAN freeMem )
{
	...
	Status = gMemo.Destroy( msgbox->Memo, TRUE );
	...
	if( (TSE_POST_STATUS_IN_POST_SCREEN == gPostStatus) || (TSE_POST_STATUS_IN_QUITE_BOOT_SCREEN == gPostStatus) )
	{
		...
		{    
			...
			MemFreePointer( (VOID **)&(msgbox->ScreenBuf) );
		}
		RestoreGraphicsScreen();
	}
	//Restore saved screen only inside TSE
	if ((TSE_POST_STATUS_IN_TSE == gPostStatus) ||(TSE_POST_STATUS_IN_BOOT_TIME_OUT == gPostStatus))
	{
...
// MAX_MSGBOX_WIDTH macro changed as token and handled from binary
    ...
    if ( msgbox->Width < MIN_MSGBOX_WIDTH)
       	 msgbox->Width = MIN_MSGBOX_WIDTH;

	if( (TSE_POST_STATUS_IN_POST_SCREEN == gPostStatus) ||  (TSE_POST_STATUS_IN_QUITE_BOOT_SCREEN == gPostStatus) ) // Dont do save screen for quietboot screen if TSE_GOP_NOTIFICATION_SUPPORT token enabled
	{
		SaveGraphicsScreen();
	}

	//Save the screen only inside TSE and also during POST
	if( (TSE_POST_STATUS_IN_TSE == gPostStatus) || (TSE_POST_STATUS_IN_POST_SCREEN == gPostStatus) || (TSE_POST_STATUS_IN_BOOT_TIME_OUT == gPostStatus) )
	...
	{ 
		...
		switch ( msgbox->MsgBoxType & MSGBOX_TYPE_MASK )
		{
			...
			case MSGBOX_TYPE_YESNO:
				...
				W = ( msgbox->Width)/2 ;   //MSGBOX_TYPE_YESNO type having 2 buttons si dividing message box width as two parts 
				msgbox->Button1Len = (UINT8)_MsgBoxOptSize(txt );
				if((msgbox->Button1Len > (UINT16)(W-2)) )  // For avoiding string overlap with PopUp borders
                {
                    txt[HiiFindStrPrintBoundary(txt, W-2)] = L'\0';
                    msgbox->Button1Len =(UINT8) (TestPrintLength( txt ) / (NG_SIZE));
                }
				DrawStringWithAttribute((UINTN) (msgbox->Left + (W-msgbox->Button1Len)/2) , (UINTN)(msgbox->Top + msgbox->Height -2), txt, (UINT8) (((msgbox->MsgboxSel==0)? EFI_BACKGROUND_BLACK|EFI_WHITE : msgbox->BGColor|msgbox->FGColor)));
				...
				pos = (UINTN) (msgbox->Left + W ) ;
				if((msgbox->Button2Len > (UINT16)(W-2)) )  // For avoiding string overlap with PopUp borders
				{
				    txt[HiiFindStrPrintBoundary(txt, W-2)] = L'\0';
				    msgbox->Button2Len = (UINT8)(TestPrintLength( txt ) / (NG_SIZE));
				}
				DrawStringWithAttribute((UINTN)(pos + (W-msgbox->Button2Len)/2) ,(UINTN)(msgbox->Top + msgbox->Height -2), txt, (UINT8) (((msgbox->MsgboxSel==1)? EFI_BACKGROUND_BLACK|EFI_WHITE : msgbox->BGColor|msgbox->FGColor)));
				MemFreePointer( (VOID **)&txt);
			...
			case MSGBOX_TYPE_HII_POP:
				...
				{ 
					...
					for ( Index = 0; Index < msgbox->OptionCount ; Index++ )
					{
						...
						if( printLength > (UINT16)(W-2))  // For avoiding string overlap with PopUp borders 
						{
						    txt[HiiFindStrPrintBoundary(txt, W-2)] = L'\0';
						    printLength = (TestPrintLength( txt ) / (NG_SIZE));
						}
		...
		///Removed the logic due to instability with variable progress box width and hardcoded to increment progress for every 2%
			...
			default:
				...
				msgbox->Button1Len = (UINT8)_MsgBoxOptSize(txt );
				msgbox->Button2Len = 0;
			    DrawStringWithAttribute( msgbox->Left + (msgbox->Width - msgbox->Button1Len)/2  , msgbox->Top + msgbox->Height -2, txt, EFI_BACKGROUND_BLACK|EFI_WHITE );
				MemFreePointer( (VOID **)&txt);
				break;
		} 
	...
	if(Data->Input.Type == ACTION_TYPE_KEY)
	{
      CONTROL_ACTION Action;
		...
		if( ( (msgbox->MsgBoxType & MSGBOX_TYPE_MASK) != MSGBOX_TYPE_OK )&&
				...
				( (msgbox->MsgBoxType & MSGBOX_TYPE_MASK) != MSGBOX_TYPE_HII_POP ) )
    		{
    				if ( msgbox->MsgboxSel == 1 )
    					msgbox->MsgboxSel = 0;
    				
    		}
		else if(((msgbox->MsgBoxType & MSGBOX_TYPE_MASK) == MSGBOX_TYPE_CUSTOM ) || ((msgbox->MsgBoxType & MSGBOX_TYPE_MASK) == MSGBOX_TYPE_HII_POP))
		{
		...
		else if(((msgbox->MsgBoxType & MSGBOX_TYPE_MASK) == MSGBOX_TYPE_CUSTOM)  || ((msgbox->MsgBoxType & MSGBOX_TYPE_MASK) == MSGBOX_TYPE_HII_POP) )
		{
   		...
   		Redraw = TRUE;
            ...
            case ControlActionAbort:
                if(msgbox->MsgBoxType != MSGBOX_TYPE_HII_POP)
                {
...
EFI_STATUS TSEMouseMsgBoxHandleAction( MSGBOX_DATA *msgbox, ACTION_DATA *Data,BOOLEAN * pRedraw)
{
    ...
    if((
        ...
        (Data->Input.Data.MouseInfo.Left > (UINT32)(msgbox->Left + msgbox->Width)) 
	 		...
	 		) && (msgbox->MsgBoxType != MSGBOX_TYPE_HII_POP)
	  )
    {
		msgbox->MsgboxSel = MSGBOX_CANCEL;
		_MsgBoxCallCallback(msgbox,msgbox->MsgboxSel);
        ...
        if(Data->Input.Data.MouseInfo.Top == (UINT32)(msgbox->Top + msgbox->Height -2))
        {
            switch(msgbox->MsgBoxType & MSGBOX_TYPE_MASK)
            {
				...
				case MSGBOX_TYPE_HII_POP:
					...
					{ 
						...
						for ( Index = 0; Index < msgbox->OptionCount ; Index++ )
						{
							...
							if( (Data->Input.Data.MouseInfo.Left)>=(UINT32)(pos + (W-printLength)/2 )  &&
				        			(Data->Input.Data.MouseInfo.Left)<=(UINT32)((pos + (W-printLength)/2 )+printLength-1))	
				    			{
    			...
    			case MSGBOX_TYPE_OKCANCEL:
    			    ...
    			    W = ( msgbox->Width)/2 ;
				    if( (Data->Input.Data.MouseInfo.Left)>=(UINT32)(msgbox->Left +(W-msgbox->Button1Len)/2)   &&
				        (Data->Input.Data.MouseInfo.Left)<=(UINT32)((msgbox->Left + (W-msgbox->Button1Len)/2)+msgbox->Button1Len-1)) //msgbox->Button1Len-1 is for getting proper string length
					    {
					        msgbox->MsgboxSel = 0;
                            ButtonClicked = TRUE;
					    }
				    pos = (UINTN) (msgbox->Left + W ) ;
    				if( (Data->Input.Data.MouseInfo.Left)>=(UINT32)( pos + (W-msgbox->Button2Len)/2)   &&
				        (Data->Input.Data.MouseInfo.Left)<=(UINT32)(( pos + (W-msgbox->Button2Len)/2)+msgbox->Button2Len-1) )	
					    {
					        msgbox->MsgboxSel = 1;
                            ButtonClicked = TRUE;
    					}
    			...
    			default:
    				...
    				if( (Data->Input.Data.MouseInfo.Left)>=(UINT32)(msgbox->Left + (msgbox->Width - msgbox->Button1Len)/2)   &&
    				    (Data->Input.Data.MouseInfo.Left)<=(UINT32)((msgbox->Left + (msgbox->Width - msgbox->Button1Len)/2)+msgbox->Button1Len-1) )	
				    	{
        					msgbox->MsgboxSel = 0;
                            ButtonClicked = TRUE;
    					}
