...
extern UINTN           gSpecialTokenCount;
extern UINT16          *gSpecialTokenArray;
/**
...
VOID MiniSetupUIExit(VOID)
{
    ...
    */ //For memleak
    ...
    //UEFICallSetupFormCallBack(AMI_CALLBACK_FORM_CLOSE);// Calling the Wrapper function with AMI_CALLBACK_FORM_CLOSE action before HiiExit() 
    FreeTseAllocMemories (); //For Memleak Fix [The Memory Allocated by the Optimal/Failsafe Buffer for Dynamic Variables is not freed]
    HiiExit( );
    ...
    if(gProcessedForms) //For Memleak Fix
    {
   	 MemFreePointer((VOID **)&gProcessedForms);
   	 gProFormOffsetAllocCount = 0;
    }
    
    if(gSpecialTokenArray) //For Memleak Fix
    {
        MemFreePointer((VOID **)&gSpecialTokenArray);
        gSpecialTokenCount = 0;
    }
