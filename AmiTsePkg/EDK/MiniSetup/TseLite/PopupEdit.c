...
/**
    This function destroys PopupEdit control
    Every member ptr within control structure has to be freed regardless of freeMem (true/false). 
    When freeMem is TRUE, free control data [POPUPEDIT_DATA]
    Destroy() has to be called with FALSE inside another Destroy(), 
    unless it’s an explicit member object that is allocated during Initialize or Create.
...
//		if ( IsTSEMultilineControlSupported() ) //have to increase editable width area for both multiline enable and disable case
	...
	StrCpyS (TempText, (PopupEdit->TextWidth+2), PopupEdit->Text);	//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
//	MemFreePointer((CHAR16**)&PopupEdit->Text);
    ...
    {
        TempText = StringWrapText(PopupEdit->Title, PopupEdit->Width - 4, &TempHeight);
	...
	if (Data->Input.Type == ACTION_TYPE_KEY)
    {
        ...
        default:
			...
			if(PopupEditHandlePassword( PopupEdit, Data) == EFI_SUCCESS)
				...
				{   // Avoid carriage return when press CTRL+ENTER in popupedit controls. or other combinations with control characters with TAB and BACK
	                if ((Data->Input.Data.AmiKey.Key.UnicodeChar) && 
	                        (CHAR_CARRIAGE_RETURN != Data->Input.Data.AmiKey.Key.UnicodeChar)&&
	                        (CHAR_BACKSPACE != Data->Input.Data.AmiKey.Key.UnicodeChar)&&
	                        (CHAR_TAB != Data->Input.Data.AmiKey.Key.UnicodeChar)) 
	                {
...
EFI_STATUS PopupEditSetText(POPUPEDIT_DATA *PopupEdit, CHAR16 *String )
{
    ...
    else
	{
        if(String != NULL)
        {
            StrCpyS (PopupEdit->Text, (PopupEdit->TextWidth*2+2), String);		//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
        }
	    else
           EfiStrCpy(PopupEdit->Text, L"");
