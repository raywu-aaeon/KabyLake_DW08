...
/**
    This function destroys ListBox control
    Every member ptr within control structure has to be freed regardless of freeMem (true/false). 
    When freeMem is TRUE, free control data [LISTBOX_DATA*]
    Destroy() has to be called with FALSE inside another Destroy(), unless it’s an explicit member object that is allocated during Initialize or Create.
...
EFI_STATUS ListBoxInitialize( LISTBOX_DATA *listbox, VOID *data )
{
	...
	listbox->PtrItems = EfiLibAllocateZeroPool( listbox->ItemCount * sizeof(POSTMENU_TEMPLATE) ); 
	...
	if ( listbox->UseScrollbar )
	{
			...
			)
		{
					...
					)
			{
				...
				if ( (UINT32)lButtonInitialPosition != Data->Input.Data.MouseInfo.Top )
				{
