...
BOOLEAN PcdPasswordPolicy(VOID);
...
UINT32 CheckSystemPassword(UINT32 EmptyPasswordType, UINTN *NoOfRetries, UINTN *TimeOut)
{
    ...
    while(*NoOfRetries)
    {
        ...
        if((!PcdPasswordPolicy())&&(PasswordEntered[0] == L'\0') && (AMI_PASSWORD_NONE == PasswordCheck))
        {
    ...
    StringZeroFreeMemory((VOID **)&PasswordEntered);
...
VOID _DrawPasswordWindow(UINT16 PromptToken, UINTN PasswordLength, UINTN *CurrXPos, UINTN *CurrYPos)
{
   ...
   CHAR16 *PasswordPrompt = NULL;
 	CHAR16 *Title = NULL, *tempText = NULL;
	EFI_STATUS Status = EFI_SUCCESS;
			...
			if ( IsTSEMultilineControlSupported() ) //If multiline supported
				{
					...
					tempText = StringWrapText(PasswordPrompt,(UINT16)DlogWidth,(UINT16*)(&(Height)));
...
VOID EFIAPI SetPwdTimeOut( EFI_EVENT Event, BOOLEAN *timeout )
{
	if ( timeout != NULL )
		*timeout = TRUE;
}
