...
#if TSE_FOR_EDKII_SUPPORT
		//If Blt for the character is not found, add blank space in gActiveBuffer in its place. Otherwise it will be skipped during draw and give screen corruption.
		Blt = NULL;
		Status = gHiiFont->GetGlyph(gHiiFont, String[nIndex], NULL, &Blt, &Baseline);
		if(Blt)
		    gActiveBuffer->Chars[Offset] = String[nIndex];
		else
		    gActiveBuffer->Chars[Offset] = L' ';
		if(Blt)
        {
...
{
    CHAR16 		*newText, *text;
    UINTN 		TextLen, i, j, newTextLen, StrLenforBoundaryCheck = 0;
    ...
    if (!IsANSIEscapeCodeSupported ())
    {
    	...
    	StrLenforBoundaryCheck = (TextLen + 1 + gMaxRows);
    }
    else
    {
    	newText = EfiLibAllocateZeroPool( (TextLen + 1 + (gMaxRows * 12)) * sizeof(CHAR16));			//If ANSI supported then chances are there to fill
        StrLenforBoundaryCheck = (TextLen + 1 + (gMaxRows * 12));                     //ANSI code in all rows so 12 * gMaxRows bytes allocated
    }																								
    if (NULL == newText)
    {
    	return NULL;
    ...
    *height=0;
    ...
    for(i=0; i<TextLen; (*height)++)
    {
        ...
        if (ESCCode && *ESCCode)
        {
            if (newTextLen + EfiStrLen(ThisLine) + EfiStrLen(ESCCode) + 1 >= StrLenforBoundaryCheck)	//Checking for the boundary to avoid crossing boundary
                break;
        } 
        else if (newTextLen + EfiStrLen(ThisLine) + 1 >= StrLenforBoundaryCheck)		//Checking for the boundary to avoid crossing boundary
            break;
...
VOID DisplayActivePostMsgs(UINTN LogoHeight)
{
	...
	if(!stPostScreenActive && (stStartLine<LogoHeight))
	{
