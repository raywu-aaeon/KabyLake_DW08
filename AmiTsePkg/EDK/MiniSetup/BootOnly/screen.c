...
VOID UpdateScreen( VOID )
{
	...
	if ( gUgaDraw != NULL )
	{
		...
		//Revert the existing resolution. If existing resolution not supported in new GOP, then free the gPostscreenwithlogo whose memory allocated based on existing resolution. If not, screen will corrupt. 
		if(gGraphicsHorizontalResolution && gGraphicsVerticalResolution && ((gGraphicsHorizontalResolution != HorizontalResolution)||(gGraphicsVerticalResolution != VerticalResolution)))
		{
			SetScreenResolution(gGraphicsHorizontalResolution,gGraphicsVerticalResolution);
		}
	}
#endif	
