...
VOID GetArgumentsFromImage(EFI_HANDLE ImageHandle, CHAR16 ***pArgv, UINTN *pArgc)
{
    ...
    status = gBS->HandleProtocol( ImageHandle, &loadedImageProtocol, (void**)&image );
...
EFI_STATUS GetDefaultPassword (UINT32 PasswordInstalled, void **DefaultPassword)
{
	...
	if ((PasswordInstalled == AMI_PASSWORD_ADMIN) || (AMI_PASSWORD_NONE == PasswordInstalled))
	{
		...
		{
			tmpPasswd =  (CHAR16 *)EfiLibAllocateZeroPool((TsePasswordVariableLength +1)*sizeof(CHAR16));
			if (NULL == tmpPasswd)
			{
				return EFI_OUT_OF_RESOURCES;
			}
			StrCpyS (tmpPasswd, TsePasswordVariableLength+1, usrPwd);		//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
			StringZeroFreeMemory ((VOID **) &usrPwd);
			PasswordEncodeHook (tmpPasswd, TsePasswordVariableLength*sizeof(CHAR16));
			MemCopy (*DefaultPassword, tmpPasswd, TsePasswordVariableLength*sizeof(CHAR16));
            StringZeroFreeMemory ((VOID **) &tmpPasswd);
			Status = EFI_SUCCESS;
		}
	}
	...
	if ((PasswordInstalled == AMI_PASSWORD_USER) || (AMI_PASSWORD_NONE == PasswordInstalled))
	{
		...
		if ((NULL != adminPwd) && (EfiStrCmp (adminPwd, L"\"\"")))
		{
			...
			StrCpyS (tmpPasswd, TsePasswordVariableLength+1, adminPwd);		//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
			StringZeroFreeMemory ((VOID **) &adminPwd);
			PasswordEncodeHook (tmpPasswd, TsePasswordVariableLength*sizeof(CHAR16));
			MemCopy ((CHAR16*)(*DefaultPassword) + TsePasswordVariableLength, tmpPasswd, TsePasswordVariableLength*sizeof(CHAR16));
			StringZeroFreeMemory ((VOID **) &tmpPasswd);          
			Status = EFI_SUCCESS;
		}
	}
...
EFI_STATUS SetDefaultPassword (VOID)
{
	...
	if(size < GetAMITSESETUPSize())
	{
      ...
      {
   		VarSetNvramName( L"AMITSESetup", &amitsesetupGuid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_RUNTIME_ACCESS, pSetupVariable, GetAMITSESETUPSize());
      }
      else
      {
         VarSetNvramName( L"AMITSESetup", &amitsesetupGuid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE, pSetupVariable, GetAMITSESETUPSize());
      }  
	...
	Status = GetDefaultPassword (PasswordInstalled, (void**)&pSetupVariable);
...
VOID EFIAPI InTseGopNotificiationFunciton( EFI_EVENT Event, VOID *NotificationContext )
{
    ...
    if((TSE_POST_STATUS_BEFORE_POST_SCREEN == gPostStatus) || (TSE_POST_STATUS_PROCEED_TO_BOOT == gPostStatus)) // Don't do anything before post screen or entered into boot
		return;

	UpdateGoPUgaDraw();    
#if !TSE_USE_EDK_LIBRARY	
    if(gConsoleControl)// update screen resolution only after Post or Quiet boot screen activated
#endif
    	UpdateScreen();
