...
#pragma warning( disable : 4366 ) // To suppress the warning - The result of the unary '&' operator may be unaligned
#endif
...
#include EFI_GUID_DEFINITION(FileInfo)


#if SETUP_ITK_COMPATIBILITY
#include "..\ITK\OemBadgingSupport\EfiOEMBadging.h"
#else
#include EFI_PROTOCOL_DEFINITION(OEMBadging)
#endif
...
typedef struct {
  EFI_DEVICE_PATH_PROTOCOL        Header;
  UINT8                           IsIPv6; // Indicates the DNS server address is IPv4 or IPv6 address.
  EFI_IP_ADDRESS                  DnsServerIp[];  // Instance of the DNS server address.
} DNS_DEVICE_PATH;
#endif
...
VOID EFIAPI NotificationFunction( EFI_EVENT Event, VOID *Context );
...
UINT16 GetMsgboxWidth(VOID);//EIP74963 : MAX_MSGBOX_WIDTH macro changed as token and handled from binary
extern const UINTN TsePasswordLength;
extern const UINTN TsePasswordVariableLength;
...
struct _TSE_SETUP_GLOBAL_DATA
{
	...
	UINTN FakeVariableCount;
	TSELITE_GLOBALS TseLite;
	TSE_SETUP_GLOBAL_DATA *Next;
};
#pragma pack()

#define TSE_FAKE_VARIABLE_GUID {0x9b9b610b, 0x1887, 0x465e, {0xb2, 0x1b, 0x33, 0xd9, 0xe0, 0x5f, 0x44, 0xa7}}

extern EFI_STATUS SaveBltBuffer(
    EFI_UGA_PIXEL *UgaBlt,
    CO_ORD_ATTRIBUTE Attribute,
    UINTN   Width,
    UINTN   Height,
...
#define SkipDisableOp               1
#define EvaluateDisableOpControls   2
#endif /* _MINISETUP_H_ */
