...
VOID BBSSetBootPriorities( BOOT_DATA *pBootData, UINT16 *pOrder, UINTN u16OrderCount)
{
    ...
    if(pOrder)
    {
        ...
        for ( i = 0; i < (BootOrderCount - u16OrderCount); i++ )
	    {
            if(gRecoveryBootingType == BOOT_OPTION_BOOTING)
            {
            	pBootData = BootGetBootData(pBootOrder[i]); // Normal bootoption
            }
            else
            {
            	pBootData = RecoveryGetRecoveryData(pBootOrder[i], gRecoveryBootingType ); // Recovery Bootoption
            }
    ...
    if(pBootOrder) //For MemoryLeak Fix
   	 MemFreePointer((void **) &pBootOrder);
}
