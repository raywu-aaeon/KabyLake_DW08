...
//      suppressScopeCount = suppressScopeCount? ++suppressScopeCount : 0;
    ...
    case EFI_IFR_ONE_OF_OPTION_OP:
		...
		if((condition != COND_SUPPRESS) && (condition != COND_DISABLE))
        {
...
#else	
	status = gHiiDatabase->GetPackageListHandle(gHiiDatabase,(EFI_HII_HANDLE)(UINTN)ReadUnaligned64((const UINT64 *)&( VariableInfo->VariableHandle)), &driverHandle);
#endif	
...
EFI_STATUS ProcessBrowserActionRequest(EFI_BROWSER_ACTION_REQUEST ActionReq)
{
  ...
  case EFI_BROWSER_ACTION_REQUEST_EXIT:
    ...
    HandleExitApplicationNoMsg();//if no user changes present then exit without showing any MsgBox
    if(!gApp->Quit)
  ...
  if((control != NULL) && (control->ControlPtr != NULL))
  {
	IfrPtr = control->ControlPtr; 
		 ...
		 ((Flags & EFI_QF_DATE_STORAGE) == QF_DATE_STORAGE_NORMAL)   )//  Incorrect logic used to check Time and Date control types
  	{
        ...
        HandleWriteOpOnChanged(control);				//Evaluating Write opcode for a current control when the control value is edited
        HandleReadOpOnChanged(control->ControlPageID);		//Evaluating Read opcode for all the controls in current page to update to the latest edited value
...
EFI_STATUS UefiRefershQuestionValueNvRAM(CONTROL_INFO *ControlData)
{
	...
	if(NvBuf == NULL){ // No Variable Assosiated then Just refrsh the page
        if(UefiIsInteractive(ControlData))
            CallFormCallBack(ControlData,UefiGetControlKey(ControlData),0,EFI_BROWSER_ACTION_RETRIEVE);
		return EFI_SUCCESS;
    }
...
//	EFI_GUID 	DriverHealthProtocolGuid = EFI_DRIVER_HEALTH_PROTOCOL_GUID;
	...
	if (!DriverName)
	{
		...
		if (!EFI_ERROR (Status))
		{
		    ...
		    RetStatus = gBS->LocateProtocol(&DevPathToTextGuid,NULL,(void**)&DevPathToTextProtocol);
...
VOID DisplayMsgListMessageBox (TSE_EFI_DRIVER_HEALTH_HII_MESSAGE *MessageList)
{
    ...
    while (MessageList [Count].HiiHandle)
    {
        ...
        if (String)
        { //If messagecode is non zero only, append it to String
            if(MessageList[Count].MessageCode)
            {
                ...
                String = MemReallocateZeroPool (
                        String,
                        ( (EfiStrLen (String) + 1) * sizeof (CHAR16) ),
                        ( ((EfiStrLen (MessageCode) + 2) * sizeof (CHAR16)) + ((EfiStrLen (String) + 1) * sizeof (CHAR16)) )    //2 for /n and NULL character
                );	
                EfiStrCat (String, MessageCode);
            }
            if (String)
            {
                CHAR16 *text = NULL;
                text = HiiGetString (gHiiHandle, STRING_TOKEN (STR_DRV_HLTH_TITLE));
                SETUP_DEBUG_UEFI("\n[TSE] DRIVERHEALTH STATUS:%s",String);
...
VOID DoDriverHealthOperation (VOID *Tempcontrol, UINT16 ControllerEntry)
{
	...
	EFI_HII_HANDLE				FormHiiHandle = NULL;
	UINT8						MsgBoxSel = 0;
...
//				    PAGE_DATA **page;                   //To display the configuration page in advanced menu itself
				    ...
				    control->ControlDestPageID = (UINT16)i;				    
...
EFI_FILE_PROTOCOL * CreateFile(CHAR16 *filename, UINT32 index)
{
	...
	if(!EFI_ERROR(Status))
    {
	 	Status = gBS->HandleProtocol( gSmplFileSysHndlBuf[index], &guidSimpleFileSystemProtocol, (void**)&pSFP );
...
DONE:
	if(formSet != PackData) //To Avoid the Freeing Satic Memory.
...
VOID DebugLibShowHiiControlState(UINT32 formID, UINT32 ctrlIndex, VOID *passedCtrlInfo)
{
	...
	switch(ctrlCond){
		...
		case COND_SUPPRESS:
			...
			break;
        case COND_DISABLE:
            SETUP_DEBUG_TSE("Control Condition: COND_DISABLE\n");
            break;
		case COND_HIDDEN:
  ...
  for(index = 0; index < count; index++)
  {
    ...
    if(ctrlInfo->ControlType == CONTROL_TYPE_POPUP_STRING && DefaultsFlag)
    {
      ...
      if(defaultValue)
      {
        ...
        {
          StrCpyS((CHAR16 *)&nvPtr[ctrlInfo->ControlVariable].Buffer[questionHdr->VarStoreInfo.VarOffset], (UINTN)UefiGetMaxValue(ctrlInfo->ControlPtr), defaultValue);		//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
          *ValChanged = TRUE;
    ...
    {
      UINT64 defaultval = 0;
      UINT16 size = EvaluateControlDefault(ctrlInfo, &defaultval);
      ...
      if(DefaultsFlag)
      {
        ...
        if(MemCmp((UINT8*)&nvPtr[ctrlInfo->ControlVariable].Buffer[questionHdr->VarStoreInfo.VarOffset], (UINT8*)&defaultval, size) != 0)
        {
          //defaults and Memory value donot match. Copying defaults to memory value
          MemCopy( &nvPtr[ctrlInfo->ControlVariable].Buffer[questionHdr->VarStoreInfo.VarOffset], &defaultval, size);
          *ValChanged = TRUE;
...
BOOLEAN _InconsistenceCheck(PAGE_INFO *PgInfo, UINT16 *ErrStrToken)
{
  ...
  for(i=0; i < PgInfo->PageControls.ControlCount; i++)
  {
    ...
    if(ctrlInfo->ControlType == CONTROL_TYPE_MSGBOX && ctrlInfo->ControlFlags.ControlVisible)
    {
...
//	gProcessedForms[gProFormOffset++] = FormID;
	...
	for(index = 0; index < count; index++)
	{
		...
		if(!ctrlInfo->ControlFlags.ControlVisible)
			continue;
...
//		MemSet(gProcessedForms, MAX_FORMS_TO_PROCESS, 0);
	...
	else//for single form ID
	{
		...
		for(i=0; i < PgInfo->PageControls.ControlCount; i++)
		{
			...
			ctrlInfo = (CONTROL_INFO*)((UINTN)(gControlInfo) + PgInfo->PageControls.ControlList[i]);
			if(ctrlInfo->ControlFlags.ControlVisible && (_NoSubmitCheck(ctrlInfo, ErrToken) == TRUE))
			{
				flag = TRUE;
				break;
			}
...
//	gProcessedForms [gProFormOffset++] = FormID;
	...
	for(index = 0; index < count; index++)
	{
		...
		{
			MemFreePointer ((VOID **)&tempLoadVarBuff); //Before Continue,Free the memory allocated for tempLoadVarBuff 
			continue;
		}
...
/**
    function to check for the control having Disabledif Condition
...
{
    EFI_IFR_OP_HEADER *ifrPtr,*CtrifrPtr = NULL;
    UINTN ScopeCount = 0;
    UINT8 Scope = 0;
    
    ifrPtr = (EFI_IFR_OP_HEADER *)controlInfo->ControlConditionalPtr;
    CtrifrPtr = (EFI_IFR_OP_HEADER *)controlInfo->ControlPtr;
    if( (NULL == ifrPtr) || (NULL == CtrifrPtr) || (ifrPtr >= CtrifrPtr))
        return FALSE;
	...
	do
    {
        if(ifrPtr->Scope)
        {
            PushScope(ifrPtr->OpCode);
            ScopeCount++;
        }
	    else if (EFI_IFR_END_OP == ifrPtr->OpCode)
	    {
	        PopScope(&Scope);
	        ScopeCount--;
	    }
		ifrPtr = (EFI_IFR_OP_HEADER*)((UINT8*)ifrPtr + ifrPtr->Length);        
    }while((ifrPtr != CtrifrPtr) && ScopeCount);


	while(ScopeCount && (EFI_SUCCESS == PopScope(&Scope)))
	{
	    if(EFI_IFR_DISABLE_IF_OP == Scope)
	    {
	        ResetScopeStack();
	        return TRUE;
	    }
