				...
				OpHeader=(EFI_IFR_OP_HEADER *)controlInfo->ControlPtr;
				  ...
				  else
				  {
					...
					if (!EFI_ERROR(Status))
					{
						...
						if(questionValue) //For MemoryLeak Fix [Freeing Memory Allocated by VarGetValue() for questionValue]
...
*/
	...
	if(found)
	{
		if (NULL != controlInfo->ControlPtr)      //For inconsistent and nosubmitif controlptr will be NULL
		{
			...
			if(varId == VARIABLE_ID_AMITSESETUP && 
			       (questionHdr->VarStoreInfo.VarOffset == GetUserPasswordOffset()||
			       questionHdr->VarStoreInfo.VarOffset == GetAdminPasswordOffset()))
			{
			    UINT8 i, *PwdBuffer = NULL;
			    dataWidth = TsePasswordVariableLength * sizeof(CHAR16);
			    
			    PwdBuffer = (UINT8*)EfiLibAllocateZeroPool(dataWidth);
                if(PwdBuffer == NULL)
                {
                    Status = EFI_OUT_OF_RESOURCES;
                    goto DONE;
                }
			    Status = VarGetValue(varId, questionHdr->VarStoreInfo.VarOffset, dataWidth, (VOID*)PwdBuffer); 
			    if(!EFI_ERROR(Status))
			    {
			        (*Value)->Value.u64 = 0;
...
/**
    Travels till the scope of the current Expression ends and returns the offset

    @param ControlCondPtr Pointer to the Expression beginning.

    @retval UINTN Offsets to skip the current expression.
**/
UINTN _SkipEntireScopeExpression(UINT8 *ControlCondPtr)
{
  EFI_IFR_OP_HEADER *OpHeader = ( EFI_IFR_OP_HEADER * )ControlCondPtr;
  UINTN offset = 0;
  INTN inScope = 0;
  ...
  if(!OpHeader->Scope)
  {
	// The start opcode should have Scope so that can stop at its End opcode.
	goto DONE;
  }
...
/**
    Checks whether the control having disable if condition over it.

    @param EFI_IFR_OP_HEADER *OpHeader

    @retval BOOLEAN.
**/
BOOLEAN IsExpressionDepondsOnVariable(EFI_IFR_OP_HEADER *OpHeader)
{
	UINTN ScopeCount = 0;
	
	if(!OpHeader->Scope) // Starting opcode should have scope.
		return FALSE;
...
/**
    Function to Evalaute a disbaleif expression which is over a form and returns a expression result.
...
**/
BOOLEAN EvaluateDisableifOverForm(EFI_IFR_OP_HEADER *OpHeader)
{

	EFI_IFR_OP_HEADER *tempopHdr = OpHeader;
	BOOLEAN MultipleExpression = FALSE;
	UINTN DisableScope = 1;
	
	
	if(tempopHdr->OpCode != EFI_IFR_DISABLE_IF_OP)
		return FALSE;
	
	if(IsExpressionDepondsOnVariable(tempopHdr))
		return TRUE;
	
	//Start from the expression.
	tempopHdr = (EFI_IFR_OP_HEADER*)((UINT8*)tempopHdr + tempopHdr->Length);

	//Check for multiple expression, since the starting expression will have a scope if it is multiple expression.
	if(tempopHdr->Scope)
		MultipleExpression = TRUE;

	while(DisableScope && tempopHdr)
	{
		if(tempopHdr->Scope)
		{
			DisableScope++;
			if(MultipleExpression)
				MultipleExpression = FALSE;
			else
				break;
		}

		if(tempopHdr->OpCode == EFI_IFR_END_OP)
			DisableScope--;
	...
	if(DisableScope && tempopHdr) // To avoid dummy disable if.
	{
		UINTN Off = 0, Size = 0;
		EFI_HII_VALUE Value, *StackPtr = NULL;
		CONTROL_INFO *DummyControl = (CONTROL_INFO*)EfiLibAllocateZeroPool(sizeof(CONTROL_INFO));
		EFI_STATUS SaveStackStatus = EFI_UNSUPPORTED, EvaluateStatus = EFI_UNSUPPORTED;
		
		//Fill it in dummy ControlInfo since EvaluateExpression takes ControlInfo as argument. 
		DummyControl->ControlPtr = (VOID*)tempopHdr; 
		DummyControl->ControlConditionalPtr = (VOID*)OpHeader;

		MemSet(&Value, sizeof(EFI_HII_VALUE),0);

		SaveStackStatus = SaveStack (&StackPtr, &Size); //Save stack before EvaluateExpresison() since it clears the stack at its entry.
		EvaluateStatus = EvaluateExpression(DummyControl, &Value, &Off);
		if (!EFI_ERROR (SaveStackStatus))
		{
			RestoreStack (StackPtr, Size);
			MemFreePointer((void**)&StackPtr);    	  
		}

		if(EFI_ERROR(EvaluateStatus) || Value.Value.b)
			return TRUE;
	}
	return FALSE;
}
