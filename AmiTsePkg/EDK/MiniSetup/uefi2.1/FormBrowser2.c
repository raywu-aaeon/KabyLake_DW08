...
// Function Prototypes
//---------------------------------------------------------------------------
EFI_STATUS _StorageToConfigResp(VARIABLE_INFO *VariableInfo, VOID *Buffer, CHAR16 *ConfigHdr, CHAR16 **ConfigResp,CHAR16 **TempResultsData);
EFI_STATUS _ConfigRespToStorage(VARIABLE_INFO *varInfo, UINT8 **buffer, UINTN *Size, UINT32 *Offset,CHAR16 **configResp);
...
EFI_STATUS UpdateBBSOrderFromCofigResp(UINTN size, UINT8 *buffer );
EFI_STATUS GetResultsDataDetails(CHAR16 **ConfigResp,UINT32 *Offset,UINTN* Size);
//---------------------------------------------------------------------------
...
/**
    Register Tse Defaults event to notify DefaultsEvaluateExpression().

    @param VOID 

    @retval EFI_STATUS
**/
EFI_STATUS TseDefaultEventForEvaluateDefault()
{
    VOID        *Defaults4FirstBootVar          = NULL;
    UINTN       VarSize                         = 0;
    EFI_STATUS  Status                          = EFI_UNSUPPORTED;
    VOID        *DefaultsEvaluateExpressionEventRegistration = NULL;
    static EFI_EVENT   DefaultsEvaluateExpressionEvent = NULL;
    EFI_GUID    gDefaultsEvaluateExpressionGuid = TSE_DEFAULTS_EVENT_GUID;
    
    Defaults4FirstBootVar = (VOID *)VarGetNvramName (L"EvaluateDefaults4FirstBoot", &gEvaluateDefaults4FirstBootGuid, NULL, &VarSize);
    if((Defaults4FirstBootVar == NULL) && (DefaultsEvaluateExpressionEvent == NULL))
    {
        Status = gBS->CreateEvent(
                    ...
                    NULL,
                    &DefaultsEvaluateExpressionEvent
             );
        if(!EFI_ERROR(Status))
        {
            Status = gBS->RegisterProtocolNotify(
                    &gDefaultsEvaluateExpressionGuid,
                    DefaultsEvaluateExpressionEvent,
                    &DefaultsEvaluateExpressionEventRegistration
                    );
        }
    }
    return Status;
...
EFI_STATUS InstallFormBrowserProtocol(EFI_HANDLE Handle)
{
  ...
  if(IsDefaultConditionalExpression())
      TseDefaultEventForEvaluateDefault();
...
NOT_SUPPORTED:
	...
	if(gST && gST->ConOut && (CurrentTextMode != (UINT32)gST->ConOut->Mode->Mode)) 
	{
	...
	if(SendFormBeforeSetup)
	{
		...
		if (gConsoleControl)
        {
            gConsoleControl->SetMode(gConsoleControl, ConsoleMode);
        }
#endif		
	}
...
DONE:
	...
	if(IsMouseSupported())// Re-Initializing the mouse when mouse for TSE and POST when if it's present
	{
	    EFI_STATUS EsaInterfaceStatus = EFI_SUCCESS;

		EsaInterfaceStatus = InitEsaTseInterfaces();
		
		//TSE mouse should not be updated for sendform inside ESA setup.
		if(EFI_ERROR(EsaInterfaceStatus) || SendFormBeforeSetup)
		{
	        if(SendFormBeforeSetup && IsSoftKbdSupported())
	        {// To update softkb for post. This is only for the senform before post
...
//  CONTROL_INFO  *ctrlInfo = NULL; Unused
  ...
  BOOLEAN       found;
  CHAR16        *configResp = NULL,*tempconfigResp = NULL;
  CHAR16        *strPtr = NULL;
  CHAR16        *configHdr = NULL;
  UINT8         *buffer = NULL;
  UINTN         bufferSize,TempResultsSize = 0;
  UINTN         size = 0;
  UINTN         length = 0;
  UINT32         varIndex = 0;
  UINT32         index = 0;
  CHAR16 *TempResultsData = NULL;
  UINT32 Offset = 0;
	...
	SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Entering BrowserCallback() : Requested for - %s\n\n", (RetrieveData == 1) ? L"RetrieveData" : L"SetData");
  ...
  if(RetrieveData)
  {
	  SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Displaying Retrieve Data Buffer, VariableName: %s,VariableGuid: %g, VariableHandle: 0x%x\n\n", varInfo->VariableName,&varInfo->VariableGuid, varInfo->VariableHandle  );

	  if(*ResultsDataSize && ResultsData != NULL && (EfiStrStr(ResultsData,L"OFFSET=") != NULL))
	  {
		  TempResultsData = (CHAR16 *) EfiLibAllocateZeroPool((EfiStrLen(ResultsData) + 2) * sizeof(CHAR16));
		  EfiCopyMem(TempResultsData,ResultsData,EfiStrLen(ResultsData) * sizeof(CHAR16));
		  SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Input ResultsData with Request element : %s\n\n", TempResultsData  );
	  }
	  ...
	  do
	  {
		  ...
		  // Generate <ConfigResp>
		  //
		  status = _StorageToConfigResp(varInfo, buffer, configHdr, &configResp,&TempResultsData);
		  ...
		  {
			  if(TempResultsSize == 0) // First Config Body
				  strPtr = configResp + EfiStrLen (configHdr) + 1;
			  else
				  strPtr = configResp + EfiStrLen (configHdr);// Add & for Second+ config body
		  }
		  else
		  {
			  strPtr = configResp;
		  ...
		  if (*ResultsDataSize < (TempResultsSize + bufferSize))
		  {
			  *ResultsDataSize = TempResultsSize + bufferSize;
		...
		if(TempResultsSize == 0)
			EfiCopyMem ((VOID*)((UINT8*)ResultsData + TempResultsSize), strPtr, bufferSize);
		else // to append from the byte '\0' of first Config body
			EfiCopyMem ((VOID*)((UINT8*)ResultsData + TempResultsSize - sizeof(CHAR16)), strPtr, bufferSize);

		SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] Result Data : %s\n\n", strPtr );

		TempResultsSize += bufferSize;

	  }while(TempResultsData != NULL);//Check whether Request element pending
#if TSE_DEBUG_MESSAGES
    DebugShowBufferContent(bufferSize, ResultsData);
#endif
	  *ResultsDataSize = TempResultsSize;
    //gBS->FreePool(configResp);
  } else
  {
    //
    ...
    tempconfigResp = configResp;
    
    SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] Displaying Browser Uncommited Data Buffer, VariableName: %s, VariableGuid: %g, VariableHandle: 0x%x\n", varInfo->VariableName, &varInfo->VariableGuid, varInfo->VariableHandle  );
    SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] Result Data : %s\n\n", ResultsData );
    ...
    do{
    	...
    	//
    	status = _ConfigRespToStorage (varInfo, &buffer, &size,&Offset, &tempconfigResp);
    	...
    	SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] Updating Browser Uncommited Data : Offset = 0x%x  Size  = 0x%x\n\n",Offset, size );
    	if (varIndex == VARIABLE_ID_BBS_ORDER)
    	{
    		status = UpdateBBSOrderFromCofigResp(size,buffer);
    	}
    	else
    		status = VarSetValue(varIndex, Offset, size, buffer);
    }while(tempconfigResp != NULL); // Check whether reaches end of config response.
  }
DONE:
//MEMFREE
  if(configResp)
...
EFI_STATUS _StorageToConfigResp(VARIABLE_INFO *VariableInfo, VOID *Buffer, CHAR16 *ConfigHdr, CHAR16 **ConfigResp,CHAR16 **TempResultsData)
{
  ...
  if(*TempResultsData && (EfiStrStr(*TempResultsData,L"OFFSET=") != NULL)) //For Request element 
  {
	  status = GetResultsDataDetails(TempResultsData,(UINT32 *)&Offset,&Size);
	  if(EFI_ERROR(status))
	  {
		  *TempResultsData = NULL;
		  Offset = VAR_ZERO_OFFSET;
		  Size =  VariableInfo->VariableSize;
	  }
	*TempResultsData = EfiStrStr(*TempResultsData,L"OFFSET="); // Move it to Next Request element if any
  }
  else
  { 
	  Offset = VAR_ZERO_OFFSET;
	...
	SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] _StorageToConfigResp : Offset = 0x%x  Size  = 0x%x\n\n",Offset, Size );

  //
  // Generate <BlockName>
  //
  status = GetBlockName(&configHdr, &length, Offset, Size);
  if(EFI_ERROR(status))
  {
    goto DONE;
  }
...
/**
    ...
    @retval EFI_STATUS
**/
EFI_STATUS _ConfigRespToStorage(VARIABLE_INFO *VariableInfo, UINT8 **Buffer, UINTN *Size,UINT32 *Offset, CHAR16 **ConfigResp)
{
  EFI_STATUS status = EFI_SUCCESS;
  ...
  }else
  {
    ...
    *Size = 0;
    *Offset = 0;// Reset Size and Offset

    status = GetResultsDataDetails(ConfigResp,Offset,Size);
	if(EFI_ERROR(status))
		goto DONE;
    ...
    //  Get Buffer Data
    //
    for(stringPtr = *ConfigResp; *stringPtr != 0 && EfiStrnCmp(stringPtr, L"&VALUE=", EfiStrLen(L"&VALUE=")) != 0; stringPtr++);
    stringPtr += EfiStrLen(L"&VALUE=");
    status = GetHexStringAsBuffer(Buffer, stringPtr);
    
    // Check whether next request element present, If yes update ConfigResp to that address.
    *ConfigResp = EfiStrStr(stringPtr,L"OFFSET=");
  }
...
//Notification Handler Ends
	...
	temp->FakeVariableCount = gFakeVariableCount;
...
//Notification Handler Ends	
	...
	gFakeVariableCount = temp->FakeVariableCount;
...
/**
        Procedure	:	GetResultsDataDetails

        Description	:	To get the Size and Offset details from ConfigResp. Will shift the ConfigResp to "&Value" so that it will continue from getting value details from CofigResp.

        Param		:	ConfigResp,Offset,Size
...
EFI_STATUS GetResultsDataDetails(CHAR16 **ConfigResp,UINT32 *Offset,UINTN* Size)
{
    ...
    UINTN length = 0;
    UINT16 *sizeBuf = NULL,*offsetBuf = NULL;
    CHAR8 *Buffer = NULL;
    EFI_STATUS status = EFI_UNSUPPORTED;
    ...
    //
    for(stringPtr = *ConfigResp; *stringPtr != 0 && EfiStrnCmp(stringPtr, L"OFFSET=", EfiStrLen(L"OFFSET=")) != 0; stringPtr++);
    stringPtr += EfiStrLen(L"OFFSET=");
    tempPtr = stringPtr;
   
	// Find the no. of digits(length)
    for(; *stringPtr != 0 && EfiStrnCmp(stringPtr, L"&WIDTH=", EfiStrLen(L"&WIDTH=")) != 0; length++, stringPtr++);
    offsetBuf = (CHAR16*)EfiLibAllocateZeroPool((length + 1) * sizeof(CHAR16));
    if(offsetBuf == NULL)
	{
      status = EFI_OUT_OF_RESOURCES;
	  goto DONE;
	...
	//Convert String to Buffer and update Offset
	EfiCopyMem(offsetBuf, tempPtr, length * sizeof(CHAR16));
	status = GetHexStringAsBuffer((UINT8**)&Buffer, offsetBuf);
    MemFreePointer((void**)&offsetBuf);
    if(!EFI_ERROR(status) || (Buffer != NULL))
    {
    	EfiCopyMem (
          Offset,
          Buffer,
          (((length + 1) / 2) < sizeof (UINT32)) ? ((length + 1) / 2) : sizeof (UINT32)
          );
    	 MemFreePointer((void**)&Buffer);
    }
    ...
    //
    for(; *stringPtr != 0 && EfiStrnCmp(stringPtr, L"&WIDTH=", EfiStrLen(L"&WIDTH=")) != 0; stringPtr++);
   ...
   // Find the no. of digits(length)
    ...
    sizeBuf = (CHAR16*)EfiLibAllocateZeroPool((length + 1) * sizeof(CHAR16));
    if(sizeBuf == NULL)
	{
      status = EFI_OUT_OF_RESOURCES;
	  goto DONE;
	}
    //Convert String to Buffer and update Size
    EfiCopyMem(sizeBuf, tempPtr, length * sizeof(CHAR16));
    status = GetHexStringAsBuffer((UINT8**)&Buffer, sizeBuf);
    ...
    if(!EFI_ERROR(status) && (Buffer != NULL))
    {
    	 ...
    	 *ConfigResp = stringPtr; // Move to "&VALUE=" so that parent function will continue from there
    }
DONE:
    return status;
...
/**
        Procedure	:	UpdateBBSOrderFromCofigResp

        Description	:	To update the ConfigResp data into BBSOrder
...
EFI_STATUS UpdateBBSOrderFromCofigResp(UINTN size, UINT8 *buffer )
{
	UINT16 CurLegBootDataValid = 0,CurrentBootOption = 0;
	UINT32 i;
	EFI_STATUS status = EFI_NOT_FOUND;
	...
	if(gCurrLegacyBootData)
		status = VarSetValue(VARIABLE_ID_BBS_ORDER, 0, size, buffer);
