...
EFI_GUID 			gRefreshEventPageGroupId;
...
UINTN gFakeVariableCount = 0; // will be cleared in MiniSetupExit.
...
extern BOOLEAN IsFSParamChecksEnforced(void);
extern BOOLEAN EvaluateDisableifOverForm(EFI_IFR_OP_HEADER *OpHeader);
extern UINTN _SkipEntireScopeExpression(UINT8 *ControlCondPtr);
//---------------------------------------------------------------------------
...
VOID EFIAPI RefreshGroupOnCallBack(EFI_EVENT Event, VOID *Context);
...
EFI_STATUS WriteOrderListDefaultsInNVRAM(EFI_IFR_DEFAULT *DefaultOp,UINTN  OffSetAdd,UINT8 OrderlistSize,UINTN Default_Size,UINT64 *OneofArray,UINT32 CtrlVar, void **DefBuffer);
#if TSE_BUILD_AS_APPLICATION
BOOLEAN IsVariableExistInApplicationArray(UINT16 VarId);
#endif
BOOLEAN IsSFGuidMatchClassId(TSE_EFI_IFR_FORM_SET* formSet, EFI_GUID *SFGuid);
UINT32 CreateFakeVariable (EFI_HII_HANDLE Handle, void *IfrData);
EFI_STATUS GetCurrentSetupPageID( UINT16 *PageID );
VOID CreateEventForRefreshIdControl(PAGE_INFO* PageInfo);
/**
...
EFI_STATUS _AddVariable(UINT8 *IFRData, EFI_HII_HANDLE Handle)
{
	...
	AMI_EFI_IFR_VARSTORE_EFI *efivarstore = NULL;
	...
	case EFI_IFR_VARSTORE_OP:
		...
		i = 0;
		for(varNamePtr = varName; (*str2 && i < 40); *varNamePtr = *str2, str2++, varNamePtr++, i++)	//Checking for the array index to avoid overlapping the boundary
			;
		break;
...
UINT32 _GetVarNumFromVarID (UINT32 ID, EFI_HII_HANDLE Handle, void *IfrData)
{
    ...
    if(ID == 0)
    {
        return CreateFakeVariable(Handle,IfrData);//Create Fake variable to avoiding update in "Setup" variable.
    }
                ...
                if (VARIABLE_ATTRIBUTE_NAMEVALUE == (Variable->ExtendedAttibutes & VARIABLE_ATTRIBUTE_NAMEVALUE))
                {
                    ...
                    if (varString)
                    {
                        if (0 == (EfiStrLen (Variable->VariableName)))
                        {
                            ...
                            StrCpyS (Variable->VariableName, VARIABLE_NAME_LENGTH, varString);	//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
                        }
                        else
                        {
                            MemSet (&newVariable, sizeof (VARIABLE_INFO), 0);
                            ...
                            StrCpyS (newVariable.VariableName, VARIABLE_NAME_LENGTH, varString);	//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
...
EFI_STATUS _InitFormsetLinks(char *buff,UINTN InitFormNum)
{
	...
	UINTN ScopeCount = 0, count = 0, FormScope = 0;
	...
	do
	{
		...
		switch(Header->OpCode )
		{
            case EFI_IFR_FORM_OP:
				...
				{
					ScopeCount++;
					FormScope = ScopeCount; ////opcode inside form
					InsideForm = TRUE;
				}
            ...
            case EFI_IFR_FORM_MAP_OP: 
				...
				if(Header->Scope)
				{
					FormScope = ScopeCount; ////opcode inside form map
					InsideForm = TRUE;
					ScopeCount++;
				}
				break;
			case EFI_IFR_REF_OP:
				...
				if(Header->Scope)//Ref5 with default will have scope
                {
                    ScopeCount++;
                }
		       break;
			case EFI_IFR_END_OP:
				ScopeCount--;
				if(InsideForm && (ScopeCount < FormScope) ) //opcode outside form
				{
					FormScope = 0;
					InsideForm = FALSE;
				}
				break;
			case EFI_IFR_DISABLE_IF_OP: //To avoid adding form deatils into FSetLinks from whihc goto control destination pageid will be udpated.
				{
					if(!InsideForm) // Evaluate only if disableif outside form.
					{
		                BOOLEAN IsDisabled = FALSE;
		                
		                gDisableFlag = EvaluateDisableOpControls; // To evaluate Disableif over form.
		                IsDisabled = EvaluateDisableifOverForm(Header);
		                gDisableFlag = 0;
		                
						if(IsDisabled)
						{
							buff += _SkipEntireScopeExpression((UINT8*)Header);
							continue;
						}
					}
				}
...
//    UINT16 RecommandedDefaultSize=0; // unused - GCC warning
    ...
    BOOLEAN MfgDefsInOpcodeUpdated = FALSE;     //To track the priority of updating Manufacturing Default
    BOOLEAN StdDefsInOpcodeUpdated = FALSE;     //To track the priority of updating Optimal Default
    BOOLEAN SetMfgDefsThruCheckFlag = FALSE;    //To track default is updated thru checkbox falg
    
    BOOLEAN ConditionaldefFound = FALSE;
    int TempIndex = 0;
    EFI_IFR_OP_HEADER *TempOpHeader = NULL;
    ...
    do
	{
		...
		case EFI_IFR_ONE_OF_OPTION_OP:
			...
			if(((Flags & EFI_IFR_OPTION_DEFAULT) == EFI_IFR_OPTION_DEFAULT) && !StdDefsInOpcodeUpdated)
			{
				GetDefaultValue(OneOfOption->Type, &(OneOfOption->Value), size, Optimal,(UINT8*)Data + i);
				StdDefsInOpcodeUpdated = TRUE;
			}

			if(((Flags & EFI_IFR_OPTION_DEFAULT_MFG) == EFI_IFR_OPTION_DEFAULT_MFG) && !MfgDefsInOpcodeUpdated)
			{
				GetDefaultValue(OneOfOption->Type, &(OneOfOption->Value), size, Failsafe,(UINT8*)Data + i);
				MfgDefsInOpcodeUpdated = TRUE;
			}
			if(IsOrderList)
			{
		...
		case EFI_IFR_DEFAULT_OP:
			...
			TempIndex = i;              //Maintaining the offset in temp variable to not disturbing the original flow
            TempOpHeader = OpHeader;    //Maintaining the Opheader in temp variable to not disturbing the original flow
            
            //If scope available, then searching for the EFI_IFR_CONDITIONAL_OP till END opcode found. If found seting a flag. 
            //If Conditional OP found set default value as 0 in default.bin
            if(TempOpHeader->Scope)
            {
                do {
                    TempIndex += TempOpHeader->Length;
                    TempOpHeader = (EFI_IFR_OP_HEADER*)((UINT8*)Data + TempIndex);

                    if (TempOpHeader->OpCode == EFI_IFR_CONDITIONAL_OP || TempOpHeader->OpCode == EFI_IFR_MAP_OP)
                    {
                        ConditionaldefFound = TRUE;
                        break;
                    }
                }while(TempOpHeader->OpCode != EFI_IFR_END_OP);
            }
			...
			else if(DefaultOp->DefaultId == EFI_HII_DEFAULT_CLASS_STANDARD)
			{
			    if (IsOrderList)
			    {
			        if (OrderlistSize*Default_Size > sizeof (UINT64))
			        {
			            Optimal = MemReallocateZeroPool (Optimal, sizeof (UINT64), (UINTN) OrderlistSize*Default_Size);
			            DefaultSize = (UINT8)(OrderlistSize*Default_Size);
			        }
			...
			)
			{
			    if (IsOrderList)
                {
                    if ((OrderlistSize*Default_Size) > sizeof (UINT64))
                    {
                        Failsafe = MemReallocateZeroPool (Failsafe, sizeof (UINT64), (UINTN) (OrderlistSize*Default_Size));
                        DefaultSize = (UINT8)(OrderlistSize*Default_Size);
                    }
                }
			...
			if(IsOrderList && (DefaultOp->Type == EFI_IFR_TYPE_BUFFER))
			{
			    if (DefaultOp->DefaultId == EFI_HII_DEFAULT_CLASS_STANDARD)
			    {
			        WriteOrderListDefaultsInNVRAM((EFI_IFR_DEFAULT*)((UINT8*)Data + i),OffSetAdd,OrderlistSize,Default_Size,OneofArray,CtrlVar, &Optimal);
			    }
			    else if (EFI_IFR_OPTION_DEFAULT_MFG == DefaultOp->DefaultId || 
                        EFI_HII_DEFAULT_CLASS_MANUFACTURING == DefaultOp->DefaultId)
			    {
	                WriteOrderListDefaultsInNVRAM((EFI_IFR_DEFAULT*)((UINT8*)Data + i),OffSetAdd,OrderlistSize,Default_Size,OneofArray,CtrlVar, &Failsafe);
			    }
			}
//			DefaultSize = 0;			//If defaultstore 2 is present then changing to 0 will mal function so commented.
        ...
        case EFI_IFR_CONDITIONAL_OP:
            //
            //For conditional expressions size need to set 0 and flag need to set to TRUE 
            //so that TSE will not set IFR data as default for opti/mfg in CTRL_INFO memory.
            //and TSE will construct/load defaults after evaluating expression used
            //
            if (ConditionaldefFound) 		//If cond opcode found in the scope of default value then set ControlEvaluateDefault else no need.
            {
                Control_Flags->ControlEvaluateDefault = TRUE;
                ConditionaldefFound = FALSE;
            }
            *size = 0;
            break;
		default:
	...
	if(CtrlVar)
	{
		...
		if(varInfo->ExtendedAttibutes)
		{
			...
			{
				StrCpyS(varInfo->VariableName, VARIABLE_NAME_LENGTH, varString);	//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
			}
			MemFreePointer( (VOID **)&varString );
		}
	}
	
    if (!MfgDefsInOpcodeUpdated && !SetMfgDefsThruCheckFlag)  //If -dms flag is coming and manufacuturing default values are missing then update optimal as manufacturing.
    {
        if (IsOrderList && (OrderlistSize*Default_Size) > sizeof(UINT64))
        {
            Failsafe = MemReallocateZeroPool (Failsafe, sizeof (UINT64), (UINTN) (OrderlistSize*Default_Size));
            MemCpy(Failsafe,(UINT8*)Optimal,(OrderlistSize*Default_Size));
        }
        else
            MemCpy(Failsafe,(UINT8*)Optimal,sizeof(UINT64));
    }
...
VOID EvaluateReadWrite(UINT8 *buf, CONTROL_INFO *ControlInfo, EFI_HII_VALUE *Value1)
{
  ...
  EFI_HII_VALUE   ExpressionResult;
  UINTN Offset = 0;
  CHAR16                  *StrPtr = NULL;
  ...
  EfiZeroMem (&ExpressionResult, sizeof (EFI_HII_VALUE));
  Value = &Data3;
  ...
  do
  {
    ...
    case EFI_IFR_EQ_ID_VAL_OP:		//Ideqval opcode need to be handled for Read\Write
    ...
    case EFI_IFR_EQ_ID_ID_OP:			//Ideqid opcode need to be handled for Read\Write
    ...
    case EFI_IFR_EQ_ID_LIST_OP:		//Ideqvallist opcode need to be handled for Read\Write
        ...
        break;
    case EFI_IFR_RULE_REF_OP:			//Ruleref opcode need to be handled for Read\Write
        {
          ...
          MemCopy(Value,&ExpressionResult,sizeof(EFI_HII_VALUE));
          break;
        }
    case EFI_IFR_QUESTION_REF1_OP:			//questionref opcode need to be handled for Read\Write
    case EFI_IFR_THIS_OP:
        ...
        Value = questionValue;
        break;
    case EFI_IFR_TO_BOOLEAN_OP:				//boolval opcode need to be handled for Read\Write
        //
        // Pop an expression from the expression stack
        //
        Status = PopExpression (Value);
        ...
        break;
    case EFI_IFR_TO_UINT_OP:		//unintval opcode need to be handled for Read\Write
    ...
    case EFI_IFR_GET_OP:
    {
        ...
        GetDefaultValue(IfrGet->VarStoreType,&questionValue1, (UINT16 *)&Size, &def, (UINT8 *)ifrData);
        Status = GetRWVariableID (IfrGet->VarStoreId, ControlInfo, &VarStoreId);
        if(EFI_ERROR(Status))
        {
           goto DONE;
        }
        varInfo = VarGetVariableInfoIndex (VarStoreId);

        if (varInfo != NULL) {      //Handling Varstore\EfiVarstore in same case.
            if(varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_VARSTORE || varInfo->ExtendedAttibutes & VARIABLE_ATTRIBUTE_EFI_VARSTORE)
            {
                //
                // Get value from Edit Buffer
                //
                Value->Type = IfrGet->VarStoreType;
            ...
            else if (varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_NAMEVALUE)        //Added code to handle NameValue varstore. Not able to test as of now, as vfrcompiler gives error.
            {
                if (IfrGet->VarStoreType != EFI_IFR_TYPE_STRING) {
                    //
    ...
    case EFI_IFR_SET_OP:
    {
        ...
        GetDefaultValue(IfrSet->VarStoreType,&questionValue1, (UINT16 *)&Size, &def, (UINT8 *)ifrData);
        ...
        if (varInfo != NULL) {      //Handling Varstore\EfiVarstore in same case.
            ...
            {
                if (IfrSet->VarStoreType != EFI_IFR_TYPE_STRING) {
                    qValue = Data1.Value.u64;
                    if(Data1.Type != EFI_IFR_TYPE_UNDEFINED){
                        Status = VarSetValue(VarStoreId, IfrSet->VarStoreInfo.VarOffset, (( Size > sizeof(UINT64))? sizeof(UINT64) : Size ), &qValue);
                    }
    ...
    case EFI_IFR_MAP_OP:
        ...
        if(Result != 0)
        {//Push EFI_IFR_TYPE_UNDEFINED if no match found
            Value->Type = EFI_IFR_TYPE_UNDEFINED;
            Value->Value.b = 0;
        }
    ...
    case EFI_IFR_LESS_THAN_OP:
      ...
      GetValueFromOpCode(ifrData,Value,Result);
      break;
    case EFI_IFR_TRUE_OP:		//TRUE opcode need to be handled for Read\Write
         Value->Value.b = TRUE;
         break;
    case EFI_IFR_FALSE_OP:		//FALSE opcode need to be handled for Read\Write
        Value->Value.b = FALSE;
        break;
    case EFI_IFR_UINT8_OP:
    case EFI_IFR_UINT16_OP:
...
/**
    ...
    @retval UINTN
**/
UINTN _AddHpkControls(EFI_HII_HANDLE Handle, UINT8 *buff,UINTN Size, PAGE_INFO **NewPgInfo, UINT32 *AllocatedPageSize, UINT32 *PgOffset)
{
   ...
   UINTN  i =0, end=0,j=0,index=0, itr = 0;
    ...
    {
    case EFI_IFR_REFRESH_ID_OP:
      (*NewPgInfo)->PageFlags.PageRefreshID = 1 ;
      MemCopy (&gRefreshEventPageGroupId, &(((AMI_EFI_IFR_REFRESH_ID *)(opHeader))->RefreshEventGroupId), sizeof (EFI_GUID));
      addControl = FALSE;
      break;
    case EFI_IFR_SUBTITLE_OP:
    ...
    case EFI_IFR_MODAL_TAG_OP: //Modal Support
      (*NewPgInfo)->PageFlags.PageModal = 1;
      break;	 
    case EFI_IFR_RESET_BUTTON_OP:
      ...
      break;
		...
		if(addControl)
		{
	      defaults_size =0;
			 ...
			 */
			control_Info->ControlLabelCount = (*NewPgInfo)->PageIdIndex;
			/*
			...
			//READ/WRITE can be used for formmap and form, no filtering needed here.
	...
	/*		// Storing the default information by using Memcopy
			...
			*/
			...
			CreatePage(NewPgInfo, AllocatedPageSize, PgOffset, &controlOffset, sizeof(UINT32));
		...
		if((Inconsistant) || (NoSubmitIf) || (gWarningIf))
		{
			...
			_AddHpkControls(Handle, buff + i + opHeader->Length, j - opHeader->Length,
			        NewPgInfo, AllocatedPageSize, PgOffset);
...
/**
    ...
    @retval EFI_STATUS
        EFI_SUCCESS
**/
EFI_STATUS AddPageToList(PAGE_INFO *NewPgInfo, UINT32 PageSize)
{
	EFI_STATUS status = EFI_SUCCESS;
	UINT32 offset;
	UINT32 ReallocSize=4096;

    SETUP_DEBUG_UEFI( "\n[TSE] Entering AddPageToList(),  PageFormID: %d \n", NewPgInfo->PageFormID );

#if TSE_DEBUG_MESSAGES
    if((gDbgPrint & PRINT_UEFI_PARSE)== PRINT_UEFI_PARSE)
        DebugShowPageInfo(NewPgInfo->PageFormID, NewPgInfo) ;
#endif 
    

  if(gIFRChangeNotify)
  {
    status = _AdvAddPageToList(NewPgInfo, PageSize);
	...
	MemCopy((UINT8 *)PageInfoPtr + PageInfoOffset, NewPgInfo, PageSize);
...
EFI_STATUS ParseForm(SETUP_LINK *Setup_Link)
{
   ...
   AMI_EFI_IFR_FORM_MAP *formMap = (AMI_EFI_IFR_FORM_MAP *) NULL;
    ...
    EFI_GUID  PlatformsetupHiiGuid = EFI_HII_PLATFORM_SETUP_FORMSET_GUID;
    extern EFI_GUID *gFSetGuid;
    UINT16 FormSetTitle = 0 ;
	UINT32 ifOverformCounter = 0;
	CONTROL_INFO *control_Info;
  ...
  while(i < pkgHdr->Length)
	{
		...
		case EFI_IFR_SUPPRESS_IF_OP:
		case EFI_IFR_GRAY_OUT_IF_OP:
			if(updateformcondvars == 1)//Support suppressif,grayoutif over form
			{
			...
			ifOverformCounter++;
			break;
		case EFI_IFR_DISABLE_IF_OP: //Skip form if disbaleif is true. don't add this opcode as starting of ConditionalPtr for controls.
			{
			    BOOLEAN IsDisabled = FALSE;
			    
			    gDisableFlag = EvaluateDisableOpControls; // To evaluate Disableif over form.
			    IsDisabled = EvaluateDisableifOverForm(opHeader);
			    gDisableFlag = 0;
				
			    if(IsDisabled)
				{
					i += _SkipEntireScopeExpression((UINT8*)opHeader);
					continue;
				}
				PushScope(opHeader->OpCode);
			}
...
*/
        ...
        case EFI_IFR_FORM_MAP_OP: 
			...
			if(NewPageInfo)
				MemFreePointer((void**)&NewPageInfo); //For MemoryLeak Fix - free memory allocated in HiiFxiupdata, if any.
	...
	//                }    
	            ...
	            if((NULL != gSfHandles))
	            {
	            	if(IsFSParamChecksEnforced())/*Compare Guid/Classguid only if TSE_ENFORCE_SENDFORM_PARAM_CHECK is enabled*/
	            	{
						if ((NULL == gFSetGuid) && IsSFGuidMatchClassId((TSE_EFI_IFR_FORM_SET*)formSet, &PlatformsetupHiiGuid))
						{
							tmpPgInfo->PageFlags.PageVisible = FALSE;
						}
						else if ((NULL != gFSetGuid) && (EfiCompareGuid (&formSet->Guid, gFSetGuid) || IsSFGuidMatchClassId((TSE_EFI_IFR_FORM_SET*)formSet, gFSetGuid)))
						{
							tmpPgInfo->PageFlags.PageVisible = FALSE;
						}
						else
						...
						tmpPgInfo->PageFlags.PageVisible = FALSE; // Show all formsets passed to SendForm.
	            }
            }
			/*
			...
			//Check for refreshID control, and create refresh event if available
			CreateEventForRefreshIdControl(NewPageInfo);
			if(NewPageInfo->PageFlags.PageRefreshID)
			{
				CreateEventForPageRefresh (NewPageInfo,gRefreshEventPageGroupId);
			}
    ...
    case EFI_IFR_VARSTORE_EFI_OP:    // 0x26
		...
		case EFI_IFR_END_OP:
			PopScope(&opCode);
			if(EFI_IFR_SUPPRESS_IF_OP == opCode || EFI_IFR_GRAY_OUT_IF_OP == opCode)
			{
...
**/
EFI_STATUS _AdvAddPageToList(PAGE_INFO *NewPgInfo, UINT32 PageSize)
{
  EFI_STATUS status = EFI_SUCCESS;
  UINT32 ii = 0;
  UINT16 pageNo=0;
  ...
  for(ii = PARSE_START_INDEX; ii < PageListPtr->PageCount; ii++)
  {
    ...
    {
      if(pageInfo->PageFormID == NewPgInfo->PageFormID)
      {
        if(pageInfo->PageControls.ControlCount == NewPgInfo->PageControls.ControlCount)
        {
		  pageNo = NewPgInfo->PageID;
          pageInfo->PageHandle = NewPgInfo->PageHandle;      
		  // Updated in old page location itself
          status = _ReplacePageWithNewPage(pageInfo, NewPgInfo, PageSize);
        }else
        {
          status = _InvalidateExistingPage(pageInfo, NewPgInfo);
          status = EFI_UNSUPPORTED;
        }
        break;
      }
  ...
  if(pageNo)
  {
	...
	{
		if(FSetLinks->PageLink[ii].FormID != NewPgInfo->PageFormID)
		{
			if(FSetLinks->PageLink[ii].ParentPageID == pageNo)
			{
	  			FSetLinks->PageLink[ii].ParentPageID = NewPgInfo->PageID;
			}
		...
		{
			//FSetLinks->PageLink[ii].ParentPageID = NewPgInfo->PageParentID;
			FSetLinks->PageLink[ii].PageNum = NewPgInfo->PageID;
		}
	...
	{
		PAGE_INFO *pageInfo = (PAGE_INFO *)((UINT8 *)PageInfoPtr + PageListPtr->PageList[ii]);
		if(pageInfo->PageHandle == NewPgInfo->PageHandle)
		{
			if(pageInfo->PageParentID == pageNo)
	  			pageInfo->PageParentID = NewPgInfo->PageID;
	...
	UpdateCtrlDestPageID(NewPgInfo->PageHandle, pageNo);
  }
...
/**
    ...
    @retval 
**/
EFI_STATUS _ReplacePageWithNewPage(PAGE_INFO *OldPageInfo, PAGE_INFO *NewPgInfo, UINT32 PageSize)
{
  EFI_STATUS status = EFI_SUCCESS;
  UINT32 page = PARSE_START_INDEX;
  UINT32 control = 0;
  ...
  // Go thru NewPgInfo CtrlList and update Control PageId
  for(;control < NewPgInfo->PageControls.ControlCount; control++)
  {
    CONTROL_INFO *controlInfo = NULL;

    controlInfo = (CONTROL_INFO *)((UINTN)gControlInfo + NewPgInfo->PageControls.ControlList[control]);
  ...
  // Go thru all PageList with the same Handle as NewPgInfo and
  ...
  for(;page < PageListPtr->PageCount; page++)
  {
    ...
    if(pgInfo->PageHandle == NewPgInfo->PageHandle)
    {
      for(control = 0;control < pgInfo->PageControls.ControlCount; control++)
      {
        CONTROL_INFO *controlInfo = NULL;
        ...
        controlInfo = (CONTROL_INFO*)((UINT8 *)gControlInfo + pgInfo->PageControls.ControlList[control]);
        if(_GetPageNumFromFormID(NewPgInfo->PageFormID) == controlInfo->ControlDestPageID)
        {
          controlInfo->ControlDestPageID = OldPageInfo->PageID;
        }
      }

	  if(pgInfo->PageParentID == NewPgInfo->PageID)
		  pgInfo->PageParentID = OldPageInfo->PageID;
    }
  }

  // Update NewPgInfo PageId and ParentPageId
  NewPgInfo->PageID = OldPageInfo->PageID;
  // Copy NewPgInfo to pageInfo
  MemCopy(OldPageInfo, NewPgInfo, PageSize);
...
/**
    ...
    @retval 
**/
EFI_STATUS _InvalidateExistingPage(PAGE_INFO *OldPageInfo, PAGE_INFO *NewPgInfo)
{
  EFI_STATUS status = EFI_SUCCESS;
  UINT32 page = 0;
  ...
  for(page = PARSE_START_INDEX; page < PageListPtr->PageCount; page++)
  {
    ...
    if(pgInfo->PageHandle == NewPgInfo->PageHandle)
    {
      for(control = 0; control < pgInfo->PageControls.ControlCount; control++)
      {
        ...
        if(controlInfo->ControlDestPageID == _GetPageNumFromFormID(NewPgInfo->PageFormID))
        {
          controlInfo->ControlDestPageID = NewPgInfo->PageID;
        }
      }
    }
  }
  //May cause circular page reference.
  //NewPgInfo->PageParentID = OldPageInfo->PageParentID;
...
VOID EFIAPI RefreshGroupOnCallBack(EFI_EVENT Event, VOID *Context)
{
	...
	//refresh the control
	UefiRefershQuestionValueNvRAM (control_Info);

	//redraw the current page
	if ((gApp !=NULL) 
	         && ((PAGE_INFO*)((UINTN)gApplicationData + gPages->PageList[gApp->CurrentPage]))->PageID == control_Info->ControlPageID)
	{	
		gApp->CompleteRedraw = TRUE;
        gApp->OnRedraw = DEACTIVATE_CONTROLS;
	}
	Status = GetCurrentSetupPageID(&PageID);
	if(!EFI_ERROR(Status) && (PageID == control_Info->ControlPageID))
...
VOID EFIAPI RefreshGroupOnCallBackForPage( EFI_EVENT Event, VOID *Context)
{
	EFI_STATUS 	Status = EFI_SUCCESS;
	...
	for(Index = 0 ;Index < page_info->PageControls.ControlCount;Index++) 		//For refreshing all the controls in the page and sub-pages
	{
		...
		if(EFI_ERROR(Status)) // if pack update happens _GetControlInfo() will return EFI_NOT_FOUND since PageHandle will be null for that pageid.
			return;
		
		//refresh the control
		Status = UefiRefershQuestionValueNvRAM (control_info);
		
		if(CurrentAppData != gApplicationData)//pack update has happened in callback. Return. 
		    return;
	}
	//redraw the current page
	if ( (gApp !=NULL)
	        && ((PAGE_INFO*)((UINTN)gApplicationData + gPages->PageList[gApp->CurrentPage]))->PageID == page_info->PageID)
	{	
		gApp->CompleteRedraw = TRUE;	
        gApp->OnRedraw = DEACTIVATE_CONTROLS;
	}
	Status = EFI_UNSUPPORTED;
	Status = GetCurrentSetupPageID(&PageID);//Get the current Displaying PageID
	if(!EFI_ERROR(Status) && (PageID == page_info->PageID))
	    EfiLibNamedEventSignal(&gSetupCompleteRedrawGuid); // Signal to ESA
}
...
/**
    ...
    @retval EFI_STATUS
**/
EFI_STATUS WriteOrderListDefaultsInNVRAM(EFI_IFR_DEFAULT *DefaultOp,UINTN  OffSetAdd,UINT8 OrderlistSize,UINTN Default_Size,UINT64 *OneofArray,UINT32 CtrlVariable, void **DefBuffer)
{
	...
	varInfo = (VARIABLE_INFO*)((UINT8 *)VariableInfoPtr + VariableListPtr->VariableList[CtrlVariable]);
	...
	if(!(EFI_ERROR (Status)) /*&& !MemCmp(Buffer,TempBuffer, OrderlistSize*Default_Size)*/)//Compare weather NVRAM having Some Data in that offset,if NVRAM having data,if condition Will Fail
	{
		...
		if (!MemCmp(Buffer,TempBuffer, OrderlistSize*Default_Size))
		{
            for(j=0;j<OrderlistSize;j++)
            {
		...
		{
		    Index = (UINT32)(i*Default_Size);
            MemCpy((UINT8 *)*DefBuffer+Index,(UINT8*)(DefsArray+i),Default_Size);
		}
...
EFI_STATUS ValidateParseForm(UINT8*	FormSet)
{
	...
	if ((inScope) || (i != FormSetLength))
	{
	    ...
	    if (IsTSEDisplayFormsetParsingError())
	    {
			...
			Message = HiiGetString( gHiiHandle, STRING_TOKEN(STR_PARSING_ERROR)); 	
			mPostMgr->DisplayMsgBox (
								Title,
								Message,
								MSGBOX_TYPE_OK,
								NULL);
			MemFreePointer((VOID **)&Title);//For MemoryLeak Fix
			MemFreePointer((VOID **)&Message);//For MemoryLeak Fix
	    }
	}	
...
/**
	Checks whether SendForm's ForsetGuid matches with Formset's Classguid list
    @param TSE_EFI_IFR_FORM_SET*, EFI_GUID *

    @retval BOOLEAN
**/
BOOLEAN IsSFGuidMatchClassId(TSE_EFI_IFR_FORM_SET* formSet, EFI_GUID *SFGuid)
{
	UINTN i, ClassIdCount;
	
	if(formSet == NULL || SFGuid == NULL)
	{
		SETUP_DEBUG_UEFI("[TSE] IsSFGuidMatchClassId() Invalid parameter\n");
		return FALSE;
	}
	ClassIdCount = formSet->Flags & MAX_CLASS_ID_LIST; // Bits 0 and 1 refers no of classguid
	
	for(i = 0; i < ClassIdCount; i++)
	{
		if(EfiCompareGuid (&formSet->ClassGuid[i], SFGuid))
		{
			return TRUE;
		}
	}
...
/** 
 *  This function will create Fake variable
 **/
UINT32 CreateFakeVariable (EFI_HII_HANDLE Handle, void *IfrData)
{
    EFI_GUID FakeVariableGuid = TSE_FAKE_VARIABLE_GUID; 
    CHAR16 FakeVariableName[200];
    VARIABLE_INFO Variable;
    EFI_IFR_OP_HEADER *OpHeader = (EFI_IFR_OP_HEADER *)IfrData;
    UINT32 i;
    ...
    MemCopy((&Variable.VariableGuid),&FakeVariableGuid,sizeof(EFI_GUID)); // Have common guid for Fake variables
    Variable.VariableAttributes = 0x7; // BT+RT+NV
    Variable.VariableHandle = Handle;
    Variable.ExtendedAttibutes = AMI_SPECIAL_VARIABLE_NO_SET | AMI_SPECIAL_VARIABLE_NO_GET ; // To avoid save into NvRam
    Variable.VariableSize = UefiGetWidth(IfrData);
    
    MemSet(FakeVariableName,sizeof(FakeVariableName),0);
    SPrint(FakeVariableName, sizeof(FakeVariableName),L"%s %d",L"NonStorageVarDynamic",gFakeVariableCount++);

    StrCpyS(Variable.VariableName, VARIABLE_NAME_LENGTH, FakeVariableName);		//Using StrCpyS instead of EfiStrCpy to avoid exceeding boundary
...
/** 
 *  This function will check whether any controls in the page having RefreshID under it and will create 
 *  event if available
 **/
VOID CreateEventForRefreshIdControl(PAGE_INFO* PageInfo)
{
    ...
    if(!PageInfo || (PageInfo->PageHandle == NULL) || (PageInfo->PageHandle == (VOID*)(UINTN)0xFFFF))
        return;
    
    for(Control = 0; Control < PageInfo->PageControls.ControlCount; Control++)
    {
        CONTROL_INFO *CtrlInfo = (CONTROL_INFO*)((UINT8*)gControlInfo + PageInfo->PageControls.ControlList[Control]);
        
        if(CtrlInfo && CtrlInfo->ControlFlags.RefreshID)
            CreateEventforIFR(CtrlInfo);
    }
