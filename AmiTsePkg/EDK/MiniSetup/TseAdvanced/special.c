...
#define MAX_SPECIAL_CTRL_COUNT 50
...
UINT16 GetSpecialCtrlToken(VOID* handle, CHAR16 *String);
...
UINTN           gSpecialTokenCount;
UINT16          *gSpecialTokenArray;
	...
	for (count = 0; count < max; count++)
	{
		...
		if (gSetupHideDisableBootOptions)
		{
			...
			{		
				...
				{
					MemFreePointer((void**)&newControlInfo->ControlConditionalPtr);//For MemoryLeak Fix - If skipping disabled boot options to show under Boot override menu
					MemFreePointer((void**)&newControlInfo);
					continue;
				}
			}
	    ...
	    if (UefiIsOneOfControl (ifrPtr))
		{
		    ...
		    UefiSetPromptField(ifrPtr, GetSpecialCtrlToken( controlInfo->ControlHandle, newText ));
...
VOID _SpecialFixupLanguages( CONTROL_INFO *control )
{
	...
	UefiSetPromptField (control->ControlPtr,GetSpecialCtrlToken( gHiiHandle, tempPtr ));

	MemFreePointer( (VOID **)&tempPtr );
	tempPtr = HiiGetString( control->ControlHandle, UefiGetHelpField(ifrPtr) );

	control->ControlHelp = GetSpecialCtrlToken( gHiiHandle, tempPtr );
...
UINT16 GetSpecialBootoptionToken(VOID* handle, UINTN Index) 
{
	...
	if (gBootPageHandle == handle)
	{
		if((FALSE == gSetupUpdated) || !gBootOptionTokenArray)
		{
			...
			if(gBootOptionTokenCount < gBootOptionCount)
			{ // Boot options added. Update gBootOptionTokenArray
				gBootOptionTokenArray = MemReallocateZeroPool(gBootOptionTokenArray, (sizeof(UINT16) * gBootOptionTokenCount), (sizeof(UINT16) * gBootOptionCount));
				for(i= gBootOptionTokenCount; i< gBootOptionCount ;i++)
				{
...
UINT16 _GetSpecialDriveroptionToken (VOID* handle, UINTN Index) 
{
	...
	if (gDriverPageHandle == handle)
	{
		if(FALSE == gSetupUpdated || !gDriverOptionTokenArray)
		{
			...
			//If the some control modified with respect to boot page..
			if (gDriverOptionTokenCount < gDriverOptionCount)
			{ // Drivers options added. Update gDriverOptionTokenArray
				gDriverOptionTokenArray = MemReallocateZeroPool (gDriverOptionTokenArray, (sizeof (UINT16) * gDriverOptionTokenCount), (sizeof (UINT16) * gDriverOptionCount) );
				for (i = gDriverOptionTokenCount; i < gDriverOptionCount ; i ++)
				{
...
VOID _SpecialFixupBootOrder (CONTROL_INFO *control, UINT16 value)
{
  ...
  //Create one of option for Disabled
	...
	option = GetSpecialCtrlToken (control->ControlHandle, text);
...
VOID _SpecialFixupBBSOrder( CONTROL_INFO *control, UINT16 value )
{
  ...
  length = UefiGetIfrLength(templatePtr);
    ...
    UINT16 bbsOptionVal = 0;
		...
		MemCopy( oneOf, templatePtr, length );
    flag = (i == value)? (UINT8)HiiGetDefaultMask() : 0;
		option = GetSpecialCtrlToken( control->ControlHandle, BBSGetOptionName( &bbsOptionVal,(UINT16)i ) );
		MemCopy( oneOf, templatePtr, length );
    // Add one of option
...
VOID SpecialFixupDelBootOption (CONTROL_INFO *control)
{
	...
	for ( i = 0; i < gBootOptionCount; i++ )
	{
		...
		else
		{
			...
			MemCopy( oneOf, templatePtr, length );
			option = GetSpecialCtrlToken (control->ControlHandle, BootGetOptionName (&(gBootData[i])));
			// Add one of option
			UefiSetOneOfOption (oneOf, gBootData[i].Option, BOOT_OPTION_SIZE, flag, option);
			oneOf = (UINT8*)oneOf +length;
		}
...
VOID _SpecialFixupAddBootOption( CONTROL_INFO *control)
{
    ...
    MemCopy( oneOf, templatePtr, length );
    // Add one of option
		option = GetSpecialCtrlToken( control->ControlHandle, gFsList[i].FsId);
...
VOID SpecialFixupDelDriverOption (CONTROL_INFO *control)
{
	...
	for (i = 0; i < gDriverOptionCount; i++)
	{
		...
		else
		{
			...
			UINT8 	flag = 0;
			MemCopy (oneOf, templatePtr, length);
			option = GetSpecialCtrlToken (control->ControlHandle, DriverGetOptionName (&(gDriverData [i])));
			// Add one of option
			UefiSetOneOfOption (oneOf, gDriverData [i].Option, BOOT_OPTION_SIZE, flag, option);
			oneOf = (UINT8*)oneOf +length;
		}
    ...
    if ( value != (UINT16)-2 )
	{
		switch(ctrlData->ControlData.ControlType) 
		{
            case CONTROL_TYPE_SUBMENU:// submenu control
                ...
                {
                    token = GetSpecialCtrlToken( ctrlData->ControlData.ControlHandle, BootGetBootNowName(value,gShowAllBbsDev,BootNowInBootOrderSupport()));
						...
						if((pageInfo->PageFlags.PageDynamic == TRUE) && (pageInfo->PageHandle != NULL))
					 	{
							if(Index == value)
							{
		                        ...
		                        token = GetSpecialCtrlToken( ctrlData->ControlData.ControlHandle, text);
                        		UefiSetPromptField(ctrlData->ControlData.ControlPtr, token);
                        		 MemFreePointer( (VOID **)&text ); 
								...
								pageInfo->PageParentID = (UINT16)gApp->CurrentPage;
                                gDynamicParentPage = (UINT16)gApp->CurrentPage;
...
/**/
				...
				else if (GUID_KEY_DYNAMIC_PAGE_GROUP == guidInfo->GuidKey)
                {
                    ...
                    if (NULL != help)
                    {
                        token = GetSpecialCtrlToken (ctrlData->ControlData.ControlHandle, help);
                        UefiSetHelpField (ctrlData->ControlData.ControlPtr, token);
                        ctrlData->ControlData.ControlHelp = token ;
                    }
...
{
    UINTN i = 0, j = 0;
    static UINTN SpecialCtrlCount = MAX_SPECIAL_CTRL_COUNT;

    if(!gSpecialTokenCount || !gSpecialTokenArray)
    {
        // Create the gSpecialTokenArray
        gSpecialTokenArray = EfiLibAllocateZeroPool (sizeof(UINT16) * MAX_SPECIAL_CTRL_COUNT);
        gSpecialTokenCount = MAX_SPECIAL_CTRL_COUNT;
        for(i=0;i< gSpecialTokenCount;i++)
        {
            gSpecialTokenArray[i] = INVALID_TOKEN;
        }
    }
    
    for (i = 0; i < gSpecialTokenCount; i++)
    {
        if (gSpecialTokenArray)
        {
            if(gSpecialTokenArray[i] == INVALID_TOKEN)
            {
                break;
            }
            else 
            {
                CHAR16 *text;
                text = HiiGetString( handle, gSpecialTokenArray[i]);
                     
                if (NULL == text) 
                    continue;
                         
                if(EfiStrCmp(text, String) == 0)
                {
                    MemFreePointer((VOID **)&text);
                    return gSpecialTokenArray[i];
                }
                else
                {
                    MemFreePointer((VOID **)&text);
    ...
    if (i < SpecialCtrlCount)
    {
        gSpecialTokenArray[i] = HiiAddString(handle, String);
    }
    else 
    {
        SpecialCtrlCount += MAX_SPECIAL_CTRL_COUNT;
        gSpecialTokenArray = MemReallocateZeroPool(gSpecialTokenArray, (sizeof(UINT16) * gSpecialTokenCount), (sizeof(UINT16) * SpecialCtrlCount));
        if (!gSpecialTokenArray)
        {
            return INVALID_TOKEN;
        }
        for(j= gSpecialTokenCount; j< SpecialCtrlCount ;j++)
        {
            gSpecialTokenArray[j] = INVALID_TOKEN;
        }
        gSpecialTokenCount = SpecialCtrlCount;
        gSpecialTokenArray[i] = HiiAddString(handle, String);
    }
