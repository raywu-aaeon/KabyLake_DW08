...
VOID copy_size (CHAR16 *Dst, UINT64 sz, UINTN MaxLen) {
	...
	if (sz < 0x100000) {	//up to 1MB
		UnicodeSPrintAsciiFormat (Dst, max, " %dKB",  Shr64(sz,10));
		
		
		return;
	}
	if (sz < 0x40000000) {  //up to 1GB
		UnicodeSPrintAsciiFormat (Dst, max, " %dMB", Shr64(sz,20));
		return;
	}
	if (sz < 0x10000000000) { //up to 1TB
		UnicodeSPrintAsciiFormat (Dst, max, " %dGB", Shr64(sz,30));
		return;
	}
	UnicodeSPrintAsciiFormat (Dst, max, " %dTB", Shr64(sz,40));
}
