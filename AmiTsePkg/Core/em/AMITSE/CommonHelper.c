...
EFI_STATUS TseCheckForIDEPasswordInstalled(VOID *popuppassword);
...
extern EFI_STATUS EFIAPI PostManagerInitProgressBar( UINTN x, UINTN y, UINTN w, UINTN h,	UINTN delta);
extern VOID SetProgressBarQuiteBootActive(BOOLEAN quiteBootActive);
...
EFI_STATUS TseLaunchFileSystem (UINT32 Variable)
{
	...
	FileBrowserChooseFile(Variable);
...
EFI_STATUS HiiInitializeProtocol( VOID )
{
	...
	return EFI_SUCCESS; // Uefi21HiiInitializeProtocol();
...
#if !OVERRIDE_CheckSystemPasswordPolicy 
//This HOOK decides whether to authenticate password or not based on token set
...
{
//Here we allow efikey,ps2scancode and unicode as char.
...
#endif
#endif//#if (TSE_USE_AMI_EFI_KEYCODE_PROTOCOL && SETUP_STORE_KEYCODE_PASSWORD)
}
#endif

#if !OVERRIDE_CheckIsAllowedPasswordCharCombination
/**
    Check is the entered password string allowed for the password control.
...
**/
BOOLEAN CheckIsAllowedPasswordCharCombination(CHAR16 *string)
{
return TRUE;
}
#endif
...
// the caller of PasswordUpdate has to make sure that Size is enough for encoding if encoding is used
...
GetGraphicsBitMapFromFV ( IN EFI_GUID *FileNameGuid, OUT VOID **Image, OUT UINTN *ImageSize )
{
	...
	EFI_FV_FILE_ATTRIBUTES		Attrib 			= 0;
...
EFI_STATUS OsUpdateCapsuleWrap (BOOT_FLOW *bootFlowPtr)
{
    ...
    Status = gBS->LocateProtocol (&ReflashGuid, NULL, (VOID**)&ReFlash);
    if (!EFI_ERROR (Status))
    {
        ...
        {																														//Recovery module have to provide the X,Y coordinates too
            ...
            if (BMP_Logo ==  LogoType)			//Draw only for BMP image
            {
                ...
                {
                    unBufferWidth = Width;
                    //this needs to be customized if the gif used is interfering with reflash
                    //they may want to stop animation or move the coordinates
                    /*
				    if(GifImageFlag) 
				    {   
				        StopGIFTimer();
                    ...
                    if ( gGOP)
                    {
                        ...
                        gGOP->Blt (
                                ...
                                Width, Height,
                                unBufferWidth * sizeof (EFI_UGA_PIXEL)
                        );
                    }


                    SETUP_DEBUG_TSE ("\nDrawing Reflash logo, size (%d x %d) at (%d, %d)",Width,Height,CoordinateX,CoordinateY);
                    GetScreenResolution(&HorizontalResolution, &VerticalResolution);
                    if (Height<VerticalResolution-CoordinateY-10)
                    { // UINTN x, UINTN y, UINTN w, UINTN h,UINTN delta  
                        PostManagerInitProgressBar( HorizontalResolution/5, 
                                CoordinateY+Height+1,
                                //calculate, put in the middle between logo and the end of the screen 
                                3*HorizontalResolution/5, 
                                10,
                                100);
                    }
                    else  // if it doesn't fit nicely, display at the last 10 px on the bottom of the screen, on top of the logo
                    {   PostManagerInitProgressBar( HorizontalResolution/5, 
...
#if !OVERRIDE_GetCurrentSetupPageID
...
/**
    Function to clear the ControlFlags and Conditional Ptr to stop processing the controls.  
    Sometimes, the ControlInfo passing to this may be from CheckControlCondition may be Frame control's data.
    Since the Frame creates copy of gControlInfo's ControlInfo, so need to clear the actual control info.

    @param  CONTROL_INFO* ControlInfo, BOOLEAN Checkin_gControlInfo
