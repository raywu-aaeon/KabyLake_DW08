  ...
  )
{
  ...
  UINT16  WordOfCount;
  //*** AMI PORTING BEGIN ***//
  // Bug fix: add buffer overrun check(refer code below that uses MaxTableLength).
  // This and other fixes are based on edk2 bz 686 and 1317.
  ...
  for (Index = 0; Index < NumOfChar; Index++) {
    //*** AMI PORTING BEGIN ***//
    // Bug fix: add buffer overrun check.
    if (BitLen[Index] > 16) {
