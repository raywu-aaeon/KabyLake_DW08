  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // Enhancement: If platform provides PopUp drawing protocol, use it
  typedef VOID (EFIAPI *AMI_UEFI_LIB_CREATE_POPUP_VA_LIST) (
    IN  UINTN          Attribute,
    OUT EFI_INPUT_KEY  *Key,      OPTIONAL
    IN VA_LIST         Args
    );
  ...
  AMI_UEFI_LIB_POPUP_PROTOCOL *PopupProtocol;
  EFI_GUID gAmiUefiLibPopupProtocolGuid = { 0xf31fcbb5, 0xb53d, 0x4215, { 0x81, 0xdb, 0x61, 0x33, 0xe4, 0xd4, 0x98, 0x6a } };
  
  if ( !EFI_ERROR( gBS->LocateProtocol(&gAmiUefiLibPopupProtocolGuid, NULL, (VOID **)&PopupProtocol) ) ){
	  VA_START (Args, Key);
	  PopupProtocol->CreatePopUpVaList(Attribute, Key, Args);
	  VA_END (Args);
	  return;
  }
  //*** AMI PORTING END *****//
