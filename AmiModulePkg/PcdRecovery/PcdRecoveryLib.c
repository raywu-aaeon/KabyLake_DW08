  ...
  )
{
    ...
    Status = gBS->AllocatePool (EfiBootServicesData, NewHobListLength, &NewHobList);
    if (EFI_ERROR(Status))
    {
        return Status;
    }
    ...
    CreateNewPeiPcdHob ((UINT16)(BackupPeiDataLength + sizeof (EFI_HOB_GUID_TYPE) ), &NewHob);
    // fill with PCD PEI data base
    gBS->CopyMem (&NewHob->Name, &gPcdDataBaseHobGuid, sizeof(EFI_GUID));
    gBS->CopyMem (NewHob+1, PcdPeiBuffer, PcdSize);
