...
#pragma pack(pop)
    ...
    DEBUG((DEBUG_BLKIO, "gNvmeInt13BinData : %lX\n", gNvmeInt13BinData));
...
)
{
    ...
    DEBUG((DEBUG_INFO, "Installing NVMe INT13 device %lx\n", NvmeLegacyMassDevice));
    ...
    if (Index >=NVMEDEVS_MAX_ENTRIES) {
        DEBUG((EFI_D_ERROR, "BBS entry creation failed for DeviceIndex:%d as it exceeds the maximum NVMe BBS entry count :%d", Index,NVMEDEVS_MAX_ENTRIES));
