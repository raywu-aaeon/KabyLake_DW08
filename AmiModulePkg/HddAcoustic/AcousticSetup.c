...
)
{
    ...
    Status = pBS->AllocatePool(EfiBootServicesData, 50,(VOID **) &NewString);
    if (EFI_ERROR(Status)) {
    ...
    for(Num = 0; Num < ACOUSTIC_MAXIMUM_CONTROLLER_SUPPORTED; Num++) {
        if(AcousticDeviceData.ControllerPresent[Num] == 1){
            StrRef = ControllerAddrInfo[Num];
            Sprintf(NewString, \
                    "SATA Controller %d - PCI Bus %02X Dev %02X Fun %02X", \
                    Num, \
                    AcousticDeviceData.BusNum[Num],\
                    AcousticDeviceData.DevNum[Num],\
                    AcousticDeviceData.FunNum[Num]);
                ...
                if((BusNum == AcousticDeviceData.BusNum[Num]) && \
                   ...
                   (FuncNum == AcousticDeviceData.FunNum[Num])) {
                    ...
                    if(IdentifyDriveInfo != NULL) {
                        ...
                        Status = pBS->AllocatePool(EfiBootServicesData, 43, (void**)&NewString);
                        if(EFI_ERROR(Status)) {
                            ASSERT_EFI_ERROR(Status);
                            continue;
                        }
