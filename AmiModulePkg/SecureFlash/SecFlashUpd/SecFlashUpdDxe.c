...
//<AMI_PHDR_START>
//----------------------------------------------------------------------------
// Procedure: DxeSmmReadyToLockeCallback
//
// Description: Callback on End Of Dxet
...
//
// Description: Callback on Flash Lock Complete event.
  ...
  )
{
    ...
    DEBUG ((SecureMod_DEBUG_LEVEL,"\nSecure Fl Upd: After_Flash_Lock callback\n"));
    ...
    Status = pRS->GetVariable (AMI_SECURE_FLASH_SETUP_VAR, 
            ...
            &Size,
            &SecureFlashSetupVar);
    if(!EFI_ERROR (Status) && SecureFlashSetupVar.Lock == 0) {
//    
