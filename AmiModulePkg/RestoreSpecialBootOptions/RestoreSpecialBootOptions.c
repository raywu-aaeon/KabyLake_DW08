...
)
{
    ...
    if(SourceOption->Description)
    {
        ...
        {
            FailToCopyFlag = TRUE;
            TRACE((-1, "Fails to copy (DestOption->Description)\n"));
        }
    }
    //
    // Copy FilePathList
    ...
    if(SourceOption->FilePathListLength)
    {
        ...
        {
            FailToCopyFlag = TRUE;
            TRACE((-1, "Fails to copy (DestOption->FilePathList)\n"));
        }
    }
    //
    // Copy OptionalData
    ...
    if(SourceOption->OptionalDataSize)
    {
        ...
        {
            FailToCopyFlag = TRUE;
            TRACE((-1, "Fails to copy (DestOption->OptionalData)\n"));
        }
    }
    //
    // Copy BbsEntry
    ...
    if(SourceOption->BbsEntry)
    {
        ...
        {
            FailToCopyFlag = TRUE;
            TRACE((-1, "Fails to copy (DestOption->BbsEntry)\n"));
        }
    }
...
)
{
    ...
    if(Policy == ORPHAN_BOOT_OPTIONS_POLICY_DELETE)
    {
        ...
        {
            TRACE((-1, "Fails to MallocZ (BootOptionLink or BootOption)\n"));
    ...
    if(!EFI_ERROR(Status))
    {
...
#ifdef EFI_DEBUG
        {
            ...
            if(After)
            {
                ...
                TRACE((-1, "After Description(%S)\n", O->Description));
            }
            else
            {
                TRACE((-1, "After is NULL\n"));
            }
            TRACE((-1, "BackupOptionName(%S)\n", OptionLink->Option->Description));
...
VOID AddBackupBootOptionBack(VOID)
{
    ...
    {
        TRACE((-1, "BootOptionLinkList->Size is 0\n"));
        return;
    }
