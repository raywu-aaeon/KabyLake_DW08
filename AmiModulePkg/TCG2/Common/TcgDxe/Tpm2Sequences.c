...
)
{
    ...
    if(TreeProtocol == NULL)return EFI_INVALID_PARAMETER;
    ...
    Status = TreeProtocol->SubmitCommand(TreeProtocol, CmdSize, (UINT8 *)&Cmd, ResultBufSize, (UINT8 *)&Res);
    DEBUG ((EFI_D_ERROR, "HashSequenceStart: ResultsStatus = %r \r\n", Status));
    //printbuffer((UINT8 *)&Res, ResultBufSize );
    if (EFI_ERROR(Status))
    {
        return Status;
    }
...
)
{
    ...
    DEBUG ((EFI_D_ERROR, "SequenceUpdate: ResultsStatus = %r \r\n", Status));
    //printbuffer((UINT8 *)&Res, ResultBufSize );
...
)
{
    ...
    Status = TreeProtocol->SubmitCommand(TreeProtocol, CmdSize, (UINT8 *)&Cmd, ResultBufSize, (UINT8 *)&Res);
    //Status = Tpm2SubmitCommand (CmdSize, (UINT8 *)&Cmd, &ResultBufSize, (UINT8 *)&Res);
    DEBUG ((EFI_D_ERROR, "SequenceComplete: Results Status = %r \r\n", Status));
    //printbuffer((UINT8 *)&Res, ResultBufSize );
