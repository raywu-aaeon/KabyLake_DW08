...
/**
    ...
    @param[in] LpcPciIo           Pointer to LPC PCI IO Protocol.
    @param[in] Base               I/O base address.
    @param[in] Length             I/O Length.
    @param[in] Enabled            Enable/Disable the generic decode range register.
...
/**
    ...
    @param[in] LpcPciIo     Pointer to LPC PCI IO Protocol(NULL in PEI).
    @param[in] Base         I/O base address, if Base is 0 means disabled the decode of the device.
    @param[in] DevUid       The device Unique ID.
    @param[in] Type         Device Type, please refer to AMISIO.h.
