...
static EFI_STATUS BuildMadtByProtocol(IN UINT32 TableVer, OUT VOID **Entries, OUT UINT32 *NumEntries, OUT UINT32 *TableSize)
{
    ...
    *TableSize = 0;
    ...
    if (gAmiCpu2Info == NULL) {
        Status = gBS->LocateProtocol(&gAmiCpuInfo2ProtocolGuid, NULL, (VOID**)&gAmiCpu2Info);
    ...
    *NumEntries = NumProtEntries * 2;    //Each CPU logical processor will have 2 entries: 1 APIC and 1 NMI.
    ...
    for (Index = 0; Index < NumProtEntries; ++Index) {
        if (AcpiProcInfo[Index].LocalApicType == ACPI_PROC_INFO_x2APIC) {
            ...
            CreateLx2ApicNmiEntry(&MadtEntry, AcpiProcInfo[Index].ProcId, (UINT16)AcpiProcInfo[Index].NmiFlags, (UINT8)AcpiProcInfo[Index].LintnPin);
        } else {
            CreateLxApicEntry(&MadtEntry, (UINT8)AcpiProcInfo[Index].ProcId, (UINT8)AcpiProcInfo[Index].ApicId, EFI_ACPI_4_0_LOCAL_APIC_ENABLED);
            CreateLxApicNmiEntry(&MadtEntry, (UINT8)AcpiProcInfo[Index].ProcId, (UINT16)AcpiProcInfo[Index].NmiFlags, (UINT8)AcpiProcInfo[Index].LintnPin);
        }
    }

    ASSERT((UINTN)(MadtEntry - (UINT8*)*Entries) == *TableSize);
...
static EFI_STATUS BuildMadtByHob(IN UINT32 TableVer, OUT VOID **Entries, OUT UINT32 *NumEntries, OUT UINT32 *TableSize)
{
    ...
    *NumEntries = PopulatedCpus;
    ...
    for (Index = 0; Index < PopulatedCpus; ++Index) {
        CpuNumApicId[Index].CpuNum = (UINT8)Index;
...
//------Update code, if LAPIC_QUANTITY > 3 - Add more structures
    ...
    ASSERT((UINTN)(MadtEntry-Table) == *TableSize);
