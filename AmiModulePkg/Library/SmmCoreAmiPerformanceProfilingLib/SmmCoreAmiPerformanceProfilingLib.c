  ...
  )
{
    ...
    if(ProfilingName != NULL && Handle != NULL)
    {
        ...
        else if(Status == EFI_NOT_FOUND)
        {
            ...
            {
                AMI_PERFORMANCE_PROFILING_ENTRY *TempEntry;

                TempEntry = AllocatePool(
                    sizeof(AMI_PERFORMANCE_PROFILING_ENTRY)*(MaxEntries + 20)
                );
                if (TempEntry==NULL) return EFI_OUT_OF_RESOURCES;
                SetMem(&(TempEntry[MaxEntries]), sizeof(AMI_PERFORMANCE_PROFILING_ENTRY)*20, 0);
                CopyMem(TempEntry, EntryList, sizeof(AMI_PERFORMANCE_PROFILING_ENTRY)*MaxEntries);
...
EFI_STATUS
EFIAPI
AmiSmmPerformanceProfilingHandlerEx (
...
{
    AMI_SMM_PERFORMANCE_PROFILING_COMMUNICATE_EX *AmiSmmCommunicateEx;
    UINTN Size;

    if (CommBufferSize == NULL) return EFI_SUCCESS;

    AmiSmmCommunicateEx = (AMI_SMM_PERFORMANCE_PROFILING_COMMUNICATE_EX*)CommBuffer;

    if (   *CommBufferSize < sizeof(AMI_SMM_PERFORMANCE_PROFILING_COMMUNICATE_EX)
         || EFI_ERROR(AmiValidateMemoryBuffer(AmiSmmCommunicateEx, *CommBufferSize))
    ) return EFI_SUCCESS;

    if( AmiSmmCommunicateEx->Command != AMI_SMM_PERFORMANCE_PROFILING_GET_NUMBER_OF_ENTRIES &&
            AmiSmmCommunicateEx->Command != AMI_SMM_PERFORMANCE_PROFILING_GET_ENTRIES )
    {
        AmiSmmCommunicateEx->Status = EFI_INVALID_PARAMETER;
        return EFI_SUCCESS;
    }

    if( AmiSmmCommunicateEx->Command == AMI_SMM_PERFORMANCE_PROFILING_GET_NUMBER_OF_ENTRIES)
    {
        AmiSmmCommunicateEx->Status = EFI_BUFFER_TOO_SMALL;
        AmiSmmCommunicateEx->NumberOfEntries = CurrentEntry;
        AmiSmmCommunicateEx->EntryNo = 0;
    }
    else
    {
        Size = sizeof(AMI_PERFORMANCE_PROFILING_ENTRY)*AmiSmmCommunicateEx->NumberOfEntries;
        AmiSmmCommunicateEx->Status = AmiValidateMemoryBuffer(AmiSmmCommunicateEx->ProfilingEntry, Size);

        if (EFI_ERROR(AmiSmmCommunicateEx->Status))
            return EFI_SUCCESS;

        if (*CommBufferSize < Size + sizeof(AMI_SMM_PERFORMANCE_PROFILING_COMMUNICATE_EX) - sizeof(UINTN))
        {
            AmiSmmCommunicateEx->Status = EFI_BUFFER_TOO_SMALL;
            AmiSmmCommunicateEx->NumberOfEntries = 0;

            return EFI_SUCCESS;
        }
        if (AmiSmmCommunicateEx->EntryNo > CurrentEntry)
        {
            AmiSmmCommunicateEx->Status = EFI_INVALID_PARAMETER;
            AmiSmmCommunicateEx->NumberOfEntries = 0;
            return EFI_SUCCESS;
        ...
        CopyMem((VOID*)AmiSmmCommunicateEx->ProfilingEntry,
                (VOID*)((UINTN)EntryList + sizeof(AMI_PERFORMANCE_PROFILING_ENTRY)*AmiSmmCommunicateEx->EntryNo),
                Size);
        AmiSmmCommunicateEx->Status = EFI_SUCCESS;
    }
    return EFI_SUCCESS;
}
  ...
  )
{
    ...
    if (    Size < sizeof(AMI_SMM_PERFORMANCE_PROFILING_COMMUNICATE)
         || EFI_ERROR(AmiValidateMemoryBuffer(AmiSmmCommunicate, Size))
    ) return EFI_SUCCESS;
    ...
    else
    {
        ...
        AmiSmmCommunicate->Status = AmiValidateMemoryBuffer(AmiSmmCommunicate->ProfilingEntry, Size);
  ...
  )
{
    ...
    EntryList = AllocateZeroPool(sizeof(AMI_PERFORMANCE_PROFILING_ENTRY)*AMI_PERFORMANCE_PROFILING_INITIAL_ENTRIES_SMM);
    if(EntryList!=NULL)
    {
        MaxEntries = AMI_PERFORMANCE_PROFILING_INITIAL_ENTRIES_SMM;
        CurrentEntry = 0;

        gSmst->SmmInstallProtocolInterface(&Handle,
                                           &gAmiPerformanceProfilingGuid,
                                           EFI_NATIVE_INTERFACE,
                                           (VOID*)&AmiPerformanceProtocol
                                          );

        gSmst->SmiHandlerRegister (AmiSmmPerformanceProfilingHandler, &gAmiPerformanceProfilingGuid, &Handle);
        gSmst->SmiHandlerRegister (AmiSmmPerformanceProfilingHandlerEx, &gAmiPerformanceProfilingExGuid, &Handle);
    }
}
