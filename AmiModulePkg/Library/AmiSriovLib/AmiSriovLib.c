...
//-------------------------------------
    ...
    DEBUG((DEBUG_VERBOSE,"PciBus: Found SRIOV Device @ B%X|D%X|F%X; ", addr.Addr.Bus, addr.Addr.Device, addr.Addr.Function));
...
//    ASSERT(Device->PciExpress->SriovData!=NULL);
    ...
    ASSERT_EFI_ERROR(Status);
    DEBUG((DEBUG_VERBOSE,"PageSize: Sup=0x%X; ", PageSize));
    if(PageSize){
    ...
    DEBUG((DEBUG_VERBOSE,"Progr=0x%X(0x%X); ", i, PageSize));
    ...
    DEBUG((DEBUG_VERBOSE,"VfMaxBus=%X; ", VfMaxBus));
...
EFI_STATUS SriovAllocateSriovData(PCI_DEV_INFO *Device, UINT32 SriovCapOffset){
    EFI_STATUS         Status;
//--------------------------------
    DEBUG((DEBUG_VERBOSE,"PciE2: Found SRIOV Ext Cap Header @ offset 0x%X\n", SriovCapOffset));
    ...
    //DEBUG((DEBUG_VERBOSE,"PciE2: SRIOVProbeDevice Status=%r\n", Status));
    //ASSERT_EFI_ERROR(Status);
...
/**
    This function will convert Resources
