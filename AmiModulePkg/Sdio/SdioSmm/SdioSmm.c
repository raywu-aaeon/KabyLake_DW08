  ...
  )
{
    ...
    if (*SdioSwSmiSignature == SIGNATURE_64 ('S', 'D', 'S', 'M', 'I', 'U', 'R', 'G')) {           
...
)
{
    ...
    Status = pSmst2->SmmRegisterProtocolNotify (&gEfiSmmEndOfDxeProtocolGuid, 
...
)
{
    ...
    for(i=0;i<MAX_SDIO_DEVICES;i++) {
        if((gSdioData->SdioDev[i].DeviceAddress == DeviceAddress) && \
            (gSdioData->SdioDev[i].DevEntryUsed == TRUE)) {
            return i;
        }
    }
...
)
{
    ...
    // corruption through SMI handlers
    Status = AmiValidateMemoryBuffer((VOID*)(UINTN)BufferAddress, 
