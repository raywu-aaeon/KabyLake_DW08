...
)
{
    ...
    for (i = 0; i< NumHandles; ++i) {
        ...
        gBS->HandleProtocol(HandleBuffer[i],&gSdioBusInitProtocolGuid,(VOID**)&SdioBusInterface);
...
{
    EFI_STATUS  Status;
    EFI_EVENT   SdioAllDriverConnectedEvent;
    VOID        *SdioAllDriverConnectedRegistration;
    ...
    Status = gBS->CreateEvent( 
                            EVT_NOTIFY_SIGNAL,
                            TPL_CALLBACK,
                            SdioAllDriverConnectedCallback,
                            NULL,
                            &SdioAllDriverConnectedEvent );
    ASSERT_EFI_ERROR(Status);
    Status = gBS->RegisterProtocolNotify(&gAmiBdsAllDriversConnectedProtocolGuid,
                                            SdioAllDriverConnectedEvent,
                                            &SdioAllDriverConnectedRegistration);
    ASSERT_EFI_ERROR (Status);
...
)
{
    ...
    if((AmiSdEmmcVendorDpNode->VendorDevicePath.Header.SubType == HW_VENDOR_DP)) {
...

{
    ...
    if  ( !(Status == EFI_SUCCESS || Status == EFI_ALREADY_STARTED)) {
        ...
        Status = InstallSdioBusProtocol(Controller, 
                      SdioBusInterface,
...
ErrorPath:
    if(AdmaDescriptorAdd) {
        if(PciIo!= NULL) {
            ...
            gBS->FreePages(AdmaDescriptorAdd,
    ...
    if(NumberOfChildren) {
        ...
        if(SdioBusInterface->AdmaDescriptorAddress && (SdioBusInterface->AdmaDescriptorAddressUnMap != NULL)) {
            if(PciIo!= NULL) {
                ...
                gBS->FreePages(SdioBusInterface->AdmaDescriptorAddress,
                               EFI_SIZE_TO_PAGES (SDIO_SIZE));
            }
        }
        ...
        if(--gCounterSdioEnumTimer == 0) {
            ...
            if(gSdioData!= NULL) {
                ...
                Status = gBS->UninstallProtocolInterface ( gSdioDataHandle,
                                                 &SdioRuntimeDataGuid,
                                                 gSdioData
                                                 );
                ASSERT_EFI_ERROR(Status);
...
)
{
    ...
    if(SdioBusInterface->ControllerType == NonPciIoType) {
        ...
        for( SlotNo = 0;SlotNo < (SdioBusInterface->NumberOfSlotsImplemented); SlotNo++ ) {
            Status = AmiSdEmmcControllerInitProtocol->GetSlotBaseAddress(AmiSdEmmcControllerInitProtocol,\
                                                                 SlotNo,\
...
)
{
    ...
    gBS->ConnectController(SdioDevInterface->SdioDeviceHandle,NULL,NULL,TRUE);
...
)
{
    ...
    if (gSdioReadData == 0) {
        ...
        Status = gBS->AllocatePages(AllocateAnyPages, EfiBootServicesData,
                                    1, (EFI_PHYSICAL_ADDRESS*)&BufferAddress);
 ...
 )
{
    ...
    Status = gRT->GetVariable(L"SdioDevConfiguration",&gSetupGuid,
    ...
    if(SdioDevInterface != NULL) {
        ...
        if(SdioDevInterface->DeviceState == DEVICE_CONFIGURED_SUCCESSFULLY) {
            ...
            if((PresentState &PSTATE_CardInsert)== 0) {
                ...
                // Before uninstalling BLOCKIO check whether it is installed or not
                Status = gBS->OpenProtocol( SdioDevInterface->SdioDeviceHandle,
                                           &gEfiBlockIoProtocolGuid,
                                           NULL,
                                           gSdioDriverBinding.DriverBindingHandle,
                                           SdioDevInterface->SdioDeviceHandle,
...
)
{
    ...
    if (SDIOGetFormatType( \
           SdioDevInfo, Port, (UINT8 *)gSdioReadData, &wEmulationType) == SDIO_ERROR) {
        //
	...
	)
{
    ...
    UINT32         DataBuffer;
    UINT8          IOEx;
    UINT8          Index, TimeOut = COMMAND_EXECUTION_TIMEOUT;
...
/**@internal
 * 
    Callback function to pass signature in smm region when all drivers are connected
...
)
{
    ...
    EFI_SMM_COMMUNICATION_PROTOCOL   *SmmCommunication;
    EFI_SMM_COMMUNICATE_HEADER       *CommHeader;
    UINT64                           SdioSwSmiSignature = SIGNATURE_64 ('S', 'D', 'S', 'M', 'I', 'U', 'R', 'G');    
