...
)
{
    ...
    for(i=0;i<=COMMAND_EXECUTION_TIMEOUT;i++) {
        ...
        if(Temp16 & NINTSTS_EI) {
			...
			// Intel Hs400 eMMC tuning funtion expects DATA_END_BIT_ERROR should
			// be retunred with Status EFI_CRC_ERROR
            if((Temp16 & CMD_CRC_ERROR) || (Temp16 & DATA_CRC_ERROR) || (Temp16 & DATA_END_BIT_ERROR)) {
...
)
{
    ...
    //
    Status= SdMmcWaitXferComplete(SdioDevInfo,Port,(VOID**)&TransferAddress,BlockSize,Read);
    if(EFI_ERROR(Status)) {
    ...
    while(1){
        ...
        if(Temp16 & NINTSTS_DMAINT) {
            ...
            if( ErrorInt16 & ADMA_ERROR) {
				...
				// Intel Hs400 eMMC tuning funtion expects DATA_END_BIT_ERROR should
				// be retunred with Status EFI_CRC_ERROR
                if((ErrorInt16 & CMD_CRC_ERROR) || (ErrorInt16 & DATA_CRC_ERROR) || (ErrorInt16 & DATA_END_BIT_ERROR)) {
...
)
{
    ...
    *BufferAddress= (VOID*)(((UINT8*)*BufferAddress)+BytesToBeTransferd);
    if(SdioDevInfo->Bit64Support == TRUE) {
        ...
        SDIO_WRITE_REG32(SdioBaseAddr,ADMAADR + 4,(UINT64)*BufferAddress >> 32);
    } else {
...
)
{
    ...
    //
    Status= SdMmcWaitXferComplete(SdioDevInfo,Port,(VOID**)&BufferAddress,SdioDevInfo->wBlockSize,SDMMC_READ);
    if(EFI_ERROR(Status)) {
        SdMmcRestoreStatus(SdioDevInfo,Port);
        goto ReadError;
    }
...
)
{
    ...
    //
    Status= SdMmcWaitXferComplete(SdioDevInfo,Port,(VOID**)&BufferAddress,SdioDevInfo->wBlockSize,SDMMC_WRITE);
    if(EFI_ERROR(Status)) {
        SdMmcRestoreStatus(SdioDevInfo,Port);
        goto WriteError;
    }
...
)
{
    ...
    if(SdioDevInfo->Bit64Support == TRUE) {
        ...
        for(i=0; i< TotalDescEntries;i++) {
            Adma128BitDescrip[i].Reserved =0;
            Adma128BitDescrip[i].EntryAddress = (UINT64)((UINT64*)BufferAddress)+(i*MAXIMUM_ADMA_LENGTH);
            Adma128BitDescrip[i].Attribute.ValidBit =1;
            Adma128BitDescrip[i].Attribute.EndBit =0;
            Adma128BitDescrip[i].Attribute.IntBit =0;
            Adma128BitDescrip[i].Attribute.Act1Bit =0;
            Adma128BitDescrip[i].Attribute.Act2Bit =1;
            Adma128BitDescrip[i].Attribute.Act0Bit =0;
            Adma128BitDescrip[i].Attribute.Length_06_07 =0;
            Adma128BitDescrip[i].Attribute.Length_08_15 =0;
            Adma128BitDescrip[i].Attribute.Length_16_31 =0;
        }
        
        // if TotalBytes is not evenly divisible(no remainder) by  MAXIMUM_ADMA_LENGTH(64kb) 
        // then the last entry will hold remainder.
        if (TotalBytes % MAXIMUM_ADMA_LENGTH) { 
            Adma128BitDescrip[i-1].Attribute.Length_16_31 = (TotalBytes % MAXIMUM_ADMA_LENGTH);
        } 
    
        // The endBit of the last entry should be set to 1
        Adma128BitDescrip[i-1].Attribute.EndBit =1;
        SDIO_WRITE_REG32(SdioBaseAddr,ADMAADR,(UINT32)(UINTN)AdmaDescrip);
        SDIO_WRITE_REG32(SdioBaseAddr,ADMAADR + 4,(UINT64)AdmaDescrip >> 32);
    } else {
        ...
        for(i=0; i< TotalDescEntries;i++) {
            Adma64BitDescrip[i].EntryAddress = ((UINT32)(UINTN)((UINT32*)BufferAddress))+(i*MAXIMUM_ADMA_LENGTH);
            Adma64BitDescrip[i].Attribute.ValidBit =1;
            Adma64BitDescrip[i].Attribute.EndBit =0;
            Adma64BitDescrip[i].Attribute.IntBit =0;
            Adma64BitDescrip[i].Attribute.Act1Bit =0;
            Adma64BitDescrip[i].Attribute.Act2Bit =1;
            Adma64BitDescrip[i].Attribute.Act0Bit =0;
            Adma64BitDescrip[i].Attribute.Length_06_07 =0;
            Adma64BitDescrip[i].Attribute.Length_08_15 =0;
            Adma64BitDescrip[i].Attribute.Length_16_31 =0;
        }
    
        // if TotalBytes is not evenly divisible(no remainder) by  MAXIMUM_ADMA_LENGTH(64kb) 
        // then the last entry will hold remainder.
        if (TotalBytes % MAXIMUM_ADMA_LENGTH) { 
            Adma64BitDescrip[i-1].Attribute.Length_16_31 = (TotalBytes % MAXIMUM_ADMA_LENGTH);
        } 
    
        // The endBit of the last entry should be set to 1
        Adma64BitDescrip[i-1].Attribute.EndBit =1;
