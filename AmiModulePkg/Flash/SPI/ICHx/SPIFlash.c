...
) 
{
    if( SPIRegs->BusDevFuncRegr != 0 )
    {
#if defined(SMM_BUILD)
        {
            ...
            SpiMmioAddr = (VOID*)(UINTN)(PciRead32( CF8_TO_PCI(SPIRegs->BusDevFuncRegr) ) & (~(UINT32)SPI_BAR0_MASK));
            if( SpiMmioAddr != (VOID*)gSPIBASE )
                return FALSE;
        }
#else
        gSPIBASE = (UINT8*)(UINTN)(PciRead32( CF8_TO_PCI(SPIRegs->BusDevFuncRegr) ) & (~(UINT32)SPI_BAR0_MASK));
#endif
    }
...
)
{
    ...
    for (i = 0; i < 8; i++){
    if (*(volatile UINT8*)(gSPIBASE + SPIRegs->OpCodeMenu + i) == 0){
            //Update opcode
            *(volatile UINT8*)(gSPIBASE + SPIRegs->OpCodeMenu + i) = FlashInfo->ReadId.Opcode;
            //Update opcode type
            CurOpType = *((volatile UINT16*)( gSPIBASE + SPIRegs->OpCodeType ));
            CurOpType &= (UINT16)(~(0x3 << (i * 2)));
            CurOpType |= (UINT16)(FlashInfo->ReadId.OpcodeType << (i * 2));
            *((volatile UINT16*)( gSPIBASE + SPIRegs->OpCodeType )) = CurOpType;
        }
    }

    // If zero in PreOpcode menu, use Write Enable Opcode instead
    if (*(volatile UINT8*)(gSPIBASE + SPIRegs->PreOpCode ) == 0)
        *(volatile UINT8*)(gSPIBASE + SPIRegs->PreOpCode) = FlashInfo->WriteEnable.Opcode;

    if (*(volatile UINT8*)(gSPIBASE + SPIRegs->PreOpCode + 1) == 0)
        *(volatile UINT8*)(gSPIBASE + SPIRegs->PreOpCode + 1) = FlashInfo->WriteEnable.Opcode;
#endif
}
/**
...
)
{
    ...
    if( *(volatile UINT8*)(gSPIBASE + SPIRegs->OpCodeMenu + 1) == 0 )
    ...
    {  
        case FlashRegionErase:
            Status = CommonSpiEraseCommandEx( FlashPartOffset, Length );
            break;
        ...
        case FlashRegionWrite:
            ...
            for( Index = 0; Index < Length; Index = (Length - TempLength) )
            {
                if( Operation == FlashRegionRead )
                    CommonSpiReadCommand( FlashPartOffset + Index, Buffer + Index, &TempLength );
                if( Operation == FlashRegionWrite )
                {
                    BOOLEAN Result;
                    Result = CommonSpiProgramCommand( FlashPartOffset + Index, Buffer + Index, &TempLength );    
                    if( Result == FALSE )
                    {
...
#if SPI_USE_HARDWARE_SEQUENCE == 2
    // Check WRSDIS bit
    wSpiFlashCtl = *(volatile UINT16*)(gSPIBASE + SPIRegs->HwSeqStsCtl);
    if (wSpiFlashCtl & BIT11) return;
...
)
{
    ...
    for (i = 0; i < Length; TempLength = RemainBytes, i = (Length - RemainBytes)) {
        ...
        if (SpiCycle) CommonSpiReadCommand (pDestination + i, Buffer, &RemainBytes);
            ...
            MemCpy(Buffer, (VOID*)(pDestination + i), TempLength - RemainBytes);
        }
...
)
{
    ...
    if(((UINT32)pAddress & BIT31) == 0)  //In this case, the address won't map to 0xFE000000~0xFFFFFFFF, so use the offset directly
    {
...
)
{
    ...
    if( InitializeSpiEnvironment( &mExFlashPart.FlashCommandMenu ) == FALSE )
        return EFI_INVALID_PARAMETER;
    ...
    Status = EFI_SUCCESS;
    SectorAddr = (UINTN)pBlockAddress;
    MaxAddress = (UINTN)pBlockAddress + Length;
    for( ; SectorAddr < MaxAddress ; SectorAddr += ProcessLength )
    {
#if SPI_USE_HARDWARE_SEQUENCE == 2
        if( (SectorAddr % SECTOR_SIZE_64KB == 0) &&
            ((MaxAddress - SectorAddr) >= SECTOR_SIZE_64KB) )
        {
            //If address is alignment on 64K and rest length is large than 64K
        ...
        if( EFI_ERROR(CommonConvertSpiAddress( (volatile UINT8*)SectorAddr, (UINT32*)(&PhyAddress) )) )
            return EFI_INVALID_PARAMETER;
        if( IsEqualOrClean( (volatile UINT8*)SectorAddr, NULL, ProcessLength, TRUE ) )
            continue;
        for( EraseRetry = 0; EraseRetry < FLASH_RETRIES; EraseRetry++ )
        {
            // Wait for spi hardware/software sequence cycle completed.
            WaitForSpiCycleDone( TRUE, TRUE );
            *(volatile UINT32*)(gSPIBASE + SPIRegs->FlashAddr) = PhyAddress;
#if SPI_USE_HARDWARE_SEQUENCE != 0
            if( ProcessLength == SECTOR_SIZE_4KB )
            {
                // Flash SPI Cycle type 3 generated to "4K Block Erase"
                SpiCmd = (SPI_HW_SEQ_ERASE_4K << 1) + BIT00;
            }
#if SPI_USE_HARDWARE_SEQUENCE == 2
                ...
                (ProcessLength == SECTOR_SIZE_64KB) )
            {
                ...
                SpiCmd = (SPI_HW_SEQ_ERASE_64K << 1) + BIT00;
            }
#endif // #if SPI_USE_HARDWARE_SEQUENCE == 2
            *(volatile UINT16*)(gSPIBASE + SPIRegs->HwSeqStsCtl + 2) = SpiCmd;
            // Wait for spi hardware sequence cycle completed.
            WaitForSpiCycleDone( TRUE, FALSE );
#else // #if SPI_USE_HARDWARE_SEQUENCE == 0
            // opcode index 2 is programmed for erase command.
            // Set BIT1 (Go), BIT2(Atomic w/ Prefix)
            SpiCmd = (SPI_OPCODE_ERASE_INDEX << 4) + BIT01 + BIT02;
            *(volatile UINT16*)(gSPIBASE + SPIRegs->SwSeqCtrl) = SpiCmd;
            // Wait for spi software sequence cycle completed.
            WaitForSpiCycleDone( FALSE, TRUE );
            ...
            if( ProcessLength == SECTOR_SIZE_64KB )
                WriteBackInvalidateDataCacheRange( (VOID*)SectorAddr, SECTOR_SIZE_64KB );
            else
                WriteBackInvalidateDataCacheRange( (VOID*)SectorAddr, FlashAPI->FlashSectorSize );
#endif

            // write operation appeared to succeed, now read back byte
            // and compare.
            CommonSpiIsEraseCompleted( (volatile UINT8*)SectorAddr, ProcessLength, NULL, &Status );
...
#if ENABLE_ROM_CACHE_IN_DXE
        // Clear cache for this range.
        if(pByteAddress >= (UINT8*)FLASH_DEVICE_BASE) WriteBackInvalidateDataCacheRange ((VOID *) pByteAddress, wProgBytes);
#endif
...
{
    if( IsEqualOrClean( pBlockAddress, NULL, Length, TRUE ) == FALSE )
    {
