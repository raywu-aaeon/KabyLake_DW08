...
BOOLEAN FlashErase (volatile UINT8* BlockAddress, UINT32 Length);
 ...
 * 
 * @param BlockAddress Pointer to an address contained in the block.
 * @param Length The number of bytes that need to be erased
 * 
 * @return BOOLEAN
 * @retval TRUE Erase completed successfully
 * @retval FALSE Erase did not complete successfully
**/
BOOLEAN AmiFlashErase(VOID* BlockAddress, UINT32 Length)
{
    return FlashErase((UINT8*)BlockAddress, Length);
}
 ...
 * @retval FALSE Erase did not complete successfully
**/
CHAR8* AmiGetFlashPartName(VOID)
{
    return (CHAR8*)pFlashDeviceNumber;
}
 ...
 */
EFI_STATUS EFIAPI PeiAmiFlashLibConstructor (
