...
/**
    This function returns the next valid Var after passed in NVAR record pointer.

    @param pVar pointer to NVRAM record
    @param pInfo pointer to NVRAM store structure
...
/**
    This function returns the next valid Var after passed in NVAR record pointer keeping
    into account if system is at runtime. If system has transitioned to runtime,
    boot time variables are skipped. That's the key difference between this function
    and NvGetNextValid.

    @param Var      pointer to NVRAM record
    @param Info     pointer to NVRAM store structure
    @param Runtime  TRUE when system has transitioned to Runtime; FALSE otherwise

    @retval NVAR pointer to next valid record after Var, NULL - if none
**/
NVAR* NvGetNextValidEx(NVAR* Var, NVRAM_STORE_INFO *Info, BOOLEAN Runtime){
    Var = NvGetNextValid(Var,Info);
    if (Runtime){
        while (Var && !(Var->flags & NVRAM_FLAG_RUNTIME))
            Var=NvGetNextValid(Var,Info);
    }
    return Var;
}
...
)
{
    ...
    // No overflow check here. This is an internal function that is called
    // after initial NVAR record size validation that is typically performed by NvIsVariable.
    Size1 = NvSize(Nvar1) - (Name1 - (UINT8*)Nvar1);
    Size2 = NvSize(Nvar2) - (Name2 - (UINT8*)Nvar2);
    MaxNameSize = MIN(Size1,  Size2);
    ...
    if (AsciiName)
    {
        ...
        if (*Name1!=*Name2 || MaxNameSize<=0) return FALSE;
    }
    ...
    else
    {
        ...
        if (Name1[0]!=Name2[0] || Name1[1]!=Name2[1] || MaxNameSize<=0) return FALSE;
    }
...
BOOLEAN NvVarEq(NVAR* pNvar, CHAR16* sName, EFI_GUID* pGuid, UINTN* pNameSize, NVRAM_STORE_INFO *pInfo)
{
    ...
    // No overflow check here. This is an internal function that is called
    // after initial NVAR record size validation that is typically performed by NvIsVariable.
    MaxNameSize = NvSize(pNvar) - (pNameStart - (UINT8*)pNvar);

    //-- Check name if ASCII
    if (pNvar->flags&NVRAM_FLAG_ASCII_NAME)
    {
        while (*pN && *pN==*sName && MaxNameSize>0) {
            pN++; sName++; MaxNameSize--;
        }

        if (*pN!=*sName || MaxNameSize<=0) return FALSE;
    ...
    else   //-- Check name if Unicode
    {
        ...
        while (ReadUnaligned16(sN) && ReadUnaligned16(sN)==*sName && MaxNameSize>0) {
            sN++; sName++; MaxNameSize-=sizeof(CHAR16);
        }

        if (ReadUnaligned16(sN)!=*sName || MaxNameSize<=0) return FALSE;
...
)
{
    ...
    *DataSize=Size;
    ...
    if (Flags) *Flags=(UINT8)pNvar->flags;
...
)
{
    ...
    if (pNvar->flags&NVRAM_FLAG_ASCII_NAME)
    {
        while (*pN++ && MaxNameSize-- > 0);
        if (MaxNameSize<=0) return EFI_VOLUME_CORRUPTED;
        NameSize = (pN - pNameStart)*2;
    }

    else
    {
        while ((pN[0]||pN[1]) && MaxNameSize > 0){ pN+=2; MaxNameSize-=2;}
        if (MaxNameSize<=0) return EFI_VOLUME_CORRUPTED;
    ...
    *VariableNameSize=NameSize;
    ...
    else
    {
        ...
        while ((*VariableName++=ReadUnaligned16(sN++))!= 0);
    }
...
/**
    This function searches for an NVRAM record with a valid name starting from StartNvar and returns variable name and size
...
**/
EFI_STATUS NvGetValidNameFromNvar(
    IN NVAR *StartNvar, IN OUT CHAR16 *VariableName,
    IN OUT UINTN *VariableNameSize,
    IN OUT EFI_GUID *VendorGuid, NVRAM_STORE_INFO *pInfo,
    BOOLEAN Runtime
)
{
    ...
    do{
        Status = NvGetNameFromNvar(
            StartNvar, VariableName, VariableNameSize, VendorGuid, pInfo
        );
        if (Status==EFI_VOLUME_CORRUPTED){
            StartNvar=NvGetNextValidEx(StartNvar,pInfo,Runtime);
            if (StartNvar==NULL) return EFI_NOT_FOUND;
        }
    }while(Status == EFI_VOLUME_CORRUPTED);
    return Status;
}
...
)
{
    ...
    return NvGetValidNameFromNvar(
        pNvar, VariableName, VariableNameSize, VendorGuid, pInfo, Runtime
    );
}
...
)
{
    ...
    for ( ; i<InfoCount; i++)
    {
            ...
            while ( Nvar ){
                ...
                for (j = 0; j < i; j++)
                    if (NvFindVariableByNvar(Nvar, &pInfo[i], &pInfo[j] ))
                        break;
                if (j==i) break;
                Nvar=NvGetNextValidEx(Nvar,&pInfo[i], Runtime);
            }
            if (!Nvar)
            {
        ...
        if (!EFI_ERROR(Status))
        {
            Status = NvGetValidNameFromNvar(
                Nvar, VariableName, VariableNameSize, VendorGuid, &pInfo[i], Runtime
            );
            if (Status!=EFI_NOT_FOUND) {
                //Restore first letter in case it was set to 0 before going to new Varstore
                if (EFI_ERROR(Status)) VariableName[0] = OrgName;
...
/**
    This function analyzes file state of main and backup store and checks if main store is valid.


    @param MainState Main NVRAM storage FFS file state
    @param MainState Backup NVRAM storage FFS file state
...
BOOLEAN IsMainNvramStoreValid(
    ...
    return CheckNvramStoreState(MainState, BackupState, BackupStoreValidPtr);
}
