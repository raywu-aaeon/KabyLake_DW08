    ...
    IN BOOT_OPTION *Option )
{
    ...
    if (BCP_DEBUG_MESSAGES == 1) DEBUG((-1, "BcpBootOrderItem BbsType = %X Option DeviceType = %X\n", BcpBootOrderItemInfo->BbsType, Option->BbsEntry->DeviceType));
    
    if (BcpBootOrderItemInfo->BbsType != Option->BbsEntry->DeviceType)
    {
    	if (BCP_DEBUG_MESSAGES == 1) DEBUG((-1, "Wrong BBS Type\n"));
		return FALSE;
    }
    ...
    if (BcpBootOrderItemInfo->MsgDpSubType != BCP_UNDEFINED) {
        ...
        if (EFI_ERROR(Status)) 
        {
        	if (BCP_DEBUG_MESSAGES == 1) DEBUG((-1, "No device Path\n"));
        	return FALSE;
        }
        ...
        {        	
        	if (BCP_DEBUG_MESSAGES == 1) DEBUG((-1, "Bad device Path\n"));
            return FALSE;
        }
...
VOID SetDefaultBcpBootOptionPriorities(VOID)
{
    ...
    if (BCP_DEBUG_MESSAGES == 1) DUMP_BOOT_OPTION_LIST(BootOptionList,"\nAfter first sort of Boot Order");
    ...
    for ( i = 0; i < BcpSection->NumberOfOptions; i++ ) {
        ...
        if (BCP_DEBUG_MESSAGES == 1) DEBUG((-1, "\nBCP Type %d\n", BcpSection->Option[i]));
        ...
        FOR_EACH_BOOT_OPTION(BootOptionList,Link,Option)
        {
            ...
            if (BcpSection->Option[i] == BCP_UEFI_EMBEDDED_SHELL)
            {
                ...
                {
                	if (BCP_DEBUG_MESSAGES == 1) DEBUG((-1, "Found Shell!!\n"));
            ...
            if (Option->GroupHeader) 
            {
                ...
                {
                	if (BCP_DEBUG_MESSAGES == 1) DEBUG((-1, "Found a group header!!\n"));
                	Option->Priority = Priority++;
                }
                continue;
            }
            ...
            if ( IsDeviceMatch(&BcpBootOrderItemInfo[j],Option) ) {
            	if (BCP_DEBUG_MESSAGES == 1) DEBUG((-1, "Found: Device matched!!\n"));
                Option->Priority = Priority++;
            }
        }
    }
