    ...
    while (!Updated && Offset < PackageHeader.Length) {
        ...
        *TempBufferSize += IfrOpHdr->Length;
        ...
        case EFI_IFR_GUID_OP :
            ...
            DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "EFI_IFR_GUID_OP 1\n"));
            ...
            DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "EFI_IFR_GUID_OP 2 ExtendOpCode=%x LabelNumber=%x Label=%x\n",ExtendOpCode, LabelNumber, Label));
            ...
            if (Insert) {
                ...
                //
                DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "EFI_IFR_GUID_OP 3\n"));
            ...
            //
            if (Data != NULL) {
                DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "EFI_IFR_GUID_OP 4\n"));
...
)
{
    ...
    DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "[FixedBootOrderHii.c] ExportPackageLists...%r\n",  status));
    ...
    if ( status == EFI_BUFFER_TOO_SMALL)
    {
        ...
        //
        status = gHiiDatabase->ExportPackageLists (gHiiDatabase, HiiHandle, &FormPackageLength, hiiFormPackage);
        DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "[FixedBootOrderHii.c] Again ExportPackageLists...%r size=%x\n",  status, FormPackageLength));
    }
...
UINT16 FIndVariableID(CHAR8 *VarName )
{
    ...
    while ( offset < ((EFI_HII_PACKAGE_LIST_HEADER*)hiiFormPackage)->PackageLength )
    {
        ...
        DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "[FixedBootOrderHii.c] pkgHdr=%x Type=%x Length=%x\n", (UINT8*)pkgHdr-(UINT8*)hiiFormPackage,
               pkgHdr->Type,
               pkgHdr->Length ));
