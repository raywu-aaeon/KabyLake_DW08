...
VOID FboBuildDefaultSceOrder()
{
    ...
    if(!EFI_ERROR(Status))
    {
        ...
        if (SceOrder) 
        {
            ...
            pRS->SetVariable(L"FboLegacySceOrder", &gFixedBootOrderGuid, VARIABLE_ATTRIBUTES, (UINTN)SceWalker - (UINTN)SceOrder, SceOrder);
    ...
    if(!EFI_ERROR(Status))
    {
        ...
        if (SceOrder) 
        {
            ...
            pRS->SetVariable(L"FboUefiSceOrder", &gFixedBootOrderGuid, VARIABLE_ATTRIBUTES, (UINTN)SceWalker - (UINTN)SceOrder, (VOID*)SceOrder);
            pBS->FreePool(SceOrder);
        }
    ...
    if(SceVar->IsUefi)
    {
        ...
        if(!EFI_ERROR(Status))
        {
            ...
            if(NewUefiOrder)
            {
                ...
                {
                    DevDataIndex = SearchDevMapByType(DevMap, OrderWalker->Type);
                    // Skip if get invalid Index
                    if(DevDataIndex == 0xFFFF)
                        continue;
                    Offset = FIXED_BOOT_ORDER_SUBMENU_MAX_NUM * DevDataIndex;
                    ...
                    if(EFI_ERROR( ExamineFixedBootIndex(ItemIndex, Count)))
                    {
                        ...
                        DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "ExamineFixedBootIndex failed\n"));
                        continue;
                    }
                ...
                pRS->SetVariable(L"UefiDevOrder", &gFixedBootOrderGuid, VARIABLE_ATTRIBUTES, Size, NewUefiOrder);
...
#ifdef EFI_DEBUG
                {
                    ...
                    DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "New UefiDevOrder order : "));
                    for(i = 0; i < Size; i++)
                        DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "%02X ", data[i]));
...
VOID FboSyncWithSce(FBO_SCE_VAR *SceVar)
{
            ...
            && SceVar->FixedBootSize)
    {
                ...
                && SceVar->SceOrderSize >= sizeof(FBO_SCE_ORDER))
        {
            ...
            {
                DevDataIndex = SearchDevMapByType(DevMap, SceWalker->Header.Type);
                // Skip if get invalid Index
                if(DevDataIndex == 0xFFFF)
				{
                ...
                {
#ifdef EFI_DEBUG
                    DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "FboSync SceWalker->Index[%X](%X) : Index[%X](%X)\n", i, SceWalker->Index[i], Offset+i, Index[Offset+i]));
#endif
                    if(SceWalker->Index[i] != Index[Offset+i])
                    {
                        DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "Fbo SCE Running!!!\n"));
