...
///backup skip table pointer
    ...
    Status = gBS->AllocatePool(EfiBootServicesData, gSkipStrucNum*sizeof(USB_SKIP_LIST), (VOID**)&SkipStruc);
    if(!EFI_ERROR(Status)){
        ...
        DEBUG((DEBUG_INFO,"FB: get SkipStatus(%r) BackupSkipTable(0x%X)\n", Status, gBackupSkipTable));    
    ...
    if (gFastBootPolicy->SataSupport != 2) {
        ...
        if (!EFI_ERROR(Status)) {
            ...
            if (!guidcmp(&DiskInfo->Interface, &DiskInfoIdeGuid) || !guidcmp(&DiskInfo->Interface,&DiskInfoAhciGuid)) {
                Status = gBS->AllocatePool(EfiBootServicesData, sizeof(UINT8) * MAX_SERIAL_COUNT, &DetectedHddSerialNum);
                if (EFI_ERROR(Status)) return Status;
...
FastLegacyBoot(VOID)
{
    ...
    for (Index=0;Index<BbsCount;Index++){
        ...
        if (BbsTable[Index].IBV1 == (UINT32)gLegacyBootDeviceHandle) break;
...
{
    EFI_STATUS      Status;
    UINT64          CheckPointFB = 0xFB;
    EFI_GUID        PciRootBridgeIoProtocolGuid = EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL_GUID;
    
    //output 80port to show system boot with fast boot path.
    Status = gBS->LocateProtocol (&PciRootBridgeIoProtocolGuid, NULL, (VOID **) &gPciRootBridgeIo);
    if(!EFI_ERROR(Status)){
        gPciRootBridgeIo->Io.Write (gPciRootBridgeIo, EfiPciWidthUint8, PORT_80, 1, &CheckPointFB);
        DEBUG((DEBUG_INFO, "FB: In Fast boot path\n"));
    }
}
...
{
    EFI_STATUS  Status;
    EFI_GUID gGuidBootFlow = BOOT_FLOW_VARIABLE_GUID;    
