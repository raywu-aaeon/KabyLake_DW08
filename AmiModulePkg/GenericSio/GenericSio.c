...
/**
    Upadate the SIO Configuration Mode script data.

    @param[out] SdlBytreArray           Point to SIO device private configuration mode script data.
    @param[in] CompatibleScritpData     Point to script data list.
**/
...
/**
    ...
    @param[in] ChildHandle      The handle of the child controller to retrieve the name of.
    @param[in] Language         A pointer to a Null-terminated ASCII string array indicating the language.
    @param[out] ControllerName  A pointer to the string to return.
    
    @retval EFI_UNSUPPORTED     Unsupported this function.
**/ 
EFI_STATUS EFIAPI AmiSioComponentNameGetControllerName(IN EFI_COMPONENT_NAME2_PROTOCOL *This,
						IN  EFI_HANDLE                   ControllerHandle,
						IN  EFI_HANDLE                   ChildHandle        OPTIONAL,
...
/**
    ...
    @param[in] This                 A pointer to the EFI_COMPONENT_NAME2_PROTOCOL instance.
    @param[in] Language             A pointer to a Null-terminated ASCII string array indicating the language.
    @param[out] DriverName          A pointer to the string to return.
    
    @retval EFI_SUCCESS             The string for the user readable name in the language
                                    specified by Language for the driver specified by This was
                                    returned in DriverName.
    @retval EFI_UNSUPPORTED         This does not support the language specified by Language.
...
/**
    ...
    @param[in] This                 Pointer to the EFI_SIO_PROTOCOL instance.
    @param[in] Write                Action Selector.
    @param[in] ExitCfgMode          Action Selector 2 if TRUE will exit SIO Config Mode after access operation.
    @param[in] Register             SIO LD Register to access.
    @param[in,out] Value            Pointer to the Data value/buffer to write/read.
    
    @retval EFI_SUCCESS             If everything good.
    @retval EFI_INVALID_PARAMETER   Value or This is NULL or Register > SIO_MAX_REG.
**/ 
EFI_STATUS EFIAPI AmiSioRegister(IN AMI_SIO_PROTOCOL		*This,
						IN BOOLEAN				Write,
						IN BOOLEAN				ExitCfgMode,					
...
/**
    This is a routine provides Set/Get current resources of the SIO LDs.

    @param[in] This                 Pointer to the EFI_SIO_PROTOCOL instance.
    @param[in] Set                  Action set or get resources.
    @param[in,out] Resources        Pointer to the Data value/buffer to write/read.
    
    @retval EFI_SUCCESS             If everything good.
    @retval EFI_INVALID_PARAMETER   Instance is NULL or Resources is NULL.
**/ 
EFI_STATUS EFIAPI AmiSioCRS(IN AMI_SIO_PROTOCOL	*This,
				    IN BOOLEAN 				Set,
					IN OUT T_ITEM_LIST		**Resources)
