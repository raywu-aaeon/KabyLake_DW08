...
/** @file
    ...
    authenticated variable service in UEFI2.3.1c
**/
/**
...
//            break;
// UEFI: Only a digest algorithm of SHA-256 is accepted
        case SHA256:
...
// must have Auth attribute to go deeper
                ...
                CertData->TimeStamp.Minute, CertData->TimeStamp.Second, &TimeStamp ))
    {
        ...
        if(NonZeroTime || ((Attributes & EFI_VARIABLE_APPEND_WRITE) == 0)) {
              AVAR_TRACE((EFI_D_ERROR,"Invalid Time Stamp\n"));
              return EFI_SECURITY_VIOLATION;
        }
    }        
    AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"Update Time Stamp\nOrg=%lx\nNew=%lx\n", ExtFlags->Mc, TimeStamp));
