...
//    DEBUG((SecureBoot_DEBUG_LEVEL,"Get Raw image %s(%r), sz = %x\n",EFI_IMAGE_SECURITY_DATABASE, Status, DataSize));
    if (!EFI_ERROR(Status)) {
        Status = AmiSetSecureBootVariable((SET_SECURE_VARIABLE_DEL | SET_SECURE_VARIABLE_SET), EFI_IMAGE_SECURITY_DATABASE, NULL, &gEfiImageSecurityDatabaseGuid, Data, DataSize );
