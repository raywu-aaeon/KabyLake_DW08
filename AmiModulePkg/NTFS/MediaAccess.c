...
// Name: MediaAccess.c
//
// Description:  Handles all Media access for the NTFS driver
//
//<AMI_FHDR_END>
//**********************************************************************
...
//    
    ...
    if (DataRegion == DATA_REGION) {
        Status= vi->DiskIo->ReadDisk (vi->DiskIo, vi->MediaID,
                                      AbsoluteOffset, ByteCount, Buffer);
...
//
// REGION is MFT or DIRECTORY
//
    for ( ; ByteCount; ) {
        Status = CheckCached(vi, AbsoluteOffset, ByteCount, &cBuffer,
                             &cBufferLength, &ch, DataRegion);
    ...
    )
{
    ...
    if (!IsCacheHit(vi, DataRegion, AbsoluteOffset,
        ByteCount, cBuffer, cBufferLength, &ch1)) {
//
...
//
        ...
        *(UINT32 *)&AlignedAbsoluteOffset = *(UINT32 *)&AbsoluteOffset & Mask;
        ...
        Status= vi->DiskIo->ReadDisk (vi->DiskIo, vi->MediaID,
                                      AlignedAbsoluteOffset,
                                      AlignedBytecount, ch1->Buffer);
    ...
    )
{
    ...
    for ( ; dlink; ) {
        ...
        if (ch->DataRegion == DataRegion && \
            AbsoluteOffset >= ch->AbsoluteOffset && \
            AbsoluteOffset < ch->AbsoluteOffsetEnd) {

            *cBuffer = (UINT8 *)(ch->Buffer + \
                        (UINT32) (AbsoluteOffset - ch->AbsoluteOffset));
...
//###DEBUG Not used -- No writes
/*        
        if (ch->DIRTY_FLAG) {
            Status= vi->DiskIo->WriteDisk (vi->DiskIo, vi->MediaID,
                                           ch->AbsoluteOffset,
                                           (UINT32) (ch->AbsoluteOffsetEnd - \
                                           ch->AbsoluteOffset), ch->Buffer);
...
//###DEBUG
    /*
       //  Check for cache hit
        do {
            if (!IsCacheHit (vi, DataRegion, AbsoluteOffset,
                ByteCount, &cBuffer, &cBufferLength, &ch)) {
       //          Cache the region, before writing it
                for (AbsoluteOffset1 = AbsoluteOffset,
                    ByteCount1 = ByteCount; ByteCount1; ) {
                    Status = CheckCached(vi, AbsoluteOffset1,
                                         ByteCount1, &cBuffer1,
                                         &cBufferLength1, DataRegion);
                    if (EFI_ERROR(Status)) {
                        HandleDiskIoError(vi, Status);
                        return Status;
                    }
...
//###DEBUG END
    do {
        Status = CheckCached(vi, AbsoluteOffset, ByteCount,
                             &cBuffer, &cBufferLength, &ch, DataRegion);
...
//
    ...
    if (vi->NTFSName) pBS->FreePool (vi->NTFSName);
    ...
    )
{
    ...
    if (fhi->HandleInstanceStatus != EFI_SUCCESS)
        return fhi->HandleInstanceStatus;

    if (!Pfh) return EFI_NOT_FOUND;

    if (Pfh->HandleStatus != EFI_SUCCESS) return Pfh->HandleStatus;

    if (EFI_ERROR(Pfh->VolumeInterface->VolumeStatus))
        return Pfh->VolumeInterface->VolumeStatus;
    ...
    )
{
    ...
    if (fhi->HandleInstanceStatus | \
        Pfh->HandleStatus | \
        vi->VolumeStatus) return EFI_UNSUPPORTED;

    if (Pfh->DirectoryEntry.DIR_FileFlags & \
        ATTR_DIRECTORY) return EFI_UNSUPPORTED;
    ...
    )
{
    ...
    if (fhi->HandleInstanceStatus | Pfh->HandleStatus | vi->VolumeStatus)
...
//    
    if (fhi->Position == -1) fhi->Position = Pfh->DirectoryEntry.DIR_AllocSize;

    if (fhi->Position == 0) {
        if (Pfh->ResidentFile == 0)
        {   // Cluster information only valid for non-resident files.
            TempRunList = &Pfh->FileRunList[0];
            Status = GetRunListElementData (&TempRunList, &TempLength,
                                            &TempCluster, FALSE);
            if (EFI_ERROR(Status)) return Status;
            fhi->CurrentCluster = TempCluster;
            fhi->CurrentClusterOffset = 0;
        } else {
            fhi->CurrentCluster = 0;
            fhi->CurrentClusterOffset = 0;
        }
    } else {
//        
// Convert postion to cluster information.
//
        AbsCluster = Div64(Position, vi->BytesPerCluster, &Remainder);
        TempRunList = fhi->pRunList;
        Status = GetRunListElementData (&TempRunList, &AbsLength,
                                        &TempCluster, TRUE);
        if (EFI_ERROR(Status)) return Status;
        OldLength = 0;
        while (AbsLength < AbsCluster)
        {
            Status = GetRunListElementData (&TempRunList, &TempLength,
                                            &TempCluster, TRUE);
            if (EFI_ERROR(Status)) break; // End of file reached
            OldLength = AbsLength;
            AbsLength += TempLength;
        }
        fhi->CurrentCluster = TempCluster + (AbsCluster - OldLength);
        fhi->CurrentClusterOffset = (UINT32)Remainder;
    }
...
//    
    if (fh->Parent) DListDelete (&(fh->Parent->ChildList),
                                 &(fh->ChildLink));
    ...
    )
{
    ...
    while ((*FileName)[Index] !=0 && (*FileName)[Index] != '\\') {
        ...
        if (Index > 260) return EFI_INVALID_PARAMETER;
    }
    ...
    )
{
    ...
    for ( ; dlink ; ) {
        ...
        if (Buffer >= (void *)(Mem->BufferStart) && \
            Buffer < (void *)(Mem->BufferEnd)) {

            StartBlock  = (UINT32) (((UINTN)Buffer - Mem->BufferStart) \
                                    / MEMALLOCSIZE);

            Status = EFI_SUCCESS;

            for (Count = 1; ; Count++, StartBlock++)
                if (((UINT8 *)(Mem->AddresMap))[StartBlock] == Count)
                    ((UINT8 *)(Mem->AddresMap))[StartBlock] = 0;
...
//----------------------------------------------------------------------
//
// Procedure:       AddBlocktoInternalMemManager
//
// Description:
//
// Parameters:
