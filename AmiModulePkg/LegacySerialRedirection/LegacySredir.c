...
)
{
    ...
    {
        if (!MemCmp((UINT8*)(UINTN)Addr16, RpSBCSig, 4))
        {
            Addr16 = Addr16+4;          // Skip the Signature "$SBC"
            Addr = (UINT16*)(UINTN) Addr16;
    ...
    while(Addr16 < 0xf0000)
    {
        if (!MemCmp((UINT8*)(UINTN)Addr16, RpSBFSig, 4))
        {
...
)
{
    ...
    *(UINT16*)0x40e = (UINT16)NewEbdaSeg;
    ...
    pBS->CopyMem((VOID*)(UINTN)NewEbdaAdd,(VOID*)(UINTN)(EbdaSeg<<4),EbdaSize);
    
    // Copy the Sredir.bin Base Memory.
    pBS->CopyMem((VOID*)(UINTN)(NewEbdaAdd+EbdaSize), gSreDirImage, gSreDirImageSize);
...
)
{
    ...
    CommHeader->MessageLength = sizeof(UINTN);
    pBS->CopyMem (&(CommHeader->Data[0]), &MmioBaseAddress, sizeof(UINTN));
...
)   
{
    ...
    if(!LegacySredir) {
        Status=pBS->LocateProtocol(&gEfiLegacySredirProtocolGuid, NULL, (VOID *)&LegacySredir);
        if(EFI_ERROR(Status)) {
            return Status;
        }
    }
...
)
{
    ...
    if(OpRomStartEndProtocol != NULL) {
        ...
        // Skip Video OpRom
        if((PciClass == PCI_CL_DISPLAY) || (OpRomStartEndProtocol->ExecuteThisRom == FALSE)) {
            return;
        }
    }
...
)
{
    ...
    Status = CreateLegacyBootEvent(\
                  TPL_CALLBACK,\
                  (VOID *)Legacy_OS_SerialRedirection,\
                  NULL,\
                  &gSreDir\
        );
    ASSERT_EFI_ERROR(Status);
