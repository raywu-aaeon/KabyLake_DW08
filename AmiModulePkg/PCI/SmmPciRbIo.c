...
//--------------------------
    ...
    DEBUG((DEBUG_VERBOSE,"SmmRb: Get PCI RB SMM Handoff Protocol - Status=%r.\n",Status));
    ...
    DEBUG((DEBUG_VERBOSE,"SmmRb: Initialize SMM RbIo ... RbCnt=%d; Status=%r.\n",gSmmRbIoCnt,Status));
    if(EFI_ERROR(Status)) return Status;
    ...
    for(i=0;i<gSmmRbIoCnt; i++){
        ...
        if(smmrb->NotPresent) {
            DEBUG((DEBUG_VERBOSE,"SmmRb: RootBridge #%d - Not Present - Skipping...\n",i));
            continue; 
        }
        ...
        DEBUG((DEBUG_VERBOSE,"SmmRb: Installing SMM_PCI_RB_IO_PROTOKOL for Rb #%d; Handle=0x%X; Status=%r.\n",
