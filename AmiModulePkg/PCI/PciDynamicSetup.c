...
/**
    ...
    @param PciDevGotoData Pointer to the  PCI Device setup goto Data.
...
/**
    This function generates Pci Goto Items.
...
//--------------------------------------------
    ...
    DEBUG((DEBUG_VERBOSE,"PciSetup: PCI Dev varstoreid=0x%X.\n",vsid));
...
//---------------------------
    DEBUG((DEBUG_VERBOSE,"PciSetup: BrowserCallback(Action %x, KeyValue %x, Type %x).\n", Action, KeyValue, Type));
...
VOID EFIAPI InitPciIfr(IN EFI_EVENT Event, IN VOID *Context)
{
    EFI_STATUS            Status;
//------------------------------------
    DEBUG((DEBUG_VERBOSE, "PciSetup: InitSioIfr(Event 0x%X, Context @0x%X) = ",Event, Context));
...
//DEBUG//DEBUG//DEBUG//DEBUG//DEBUG//DEBUG
    ...
    DEBUG((DEBUG_VERBOSE,"PciSetup: Found %d PCI Devices With Setup Pages... %r.\n",gPciIfrInfo.PciDevCount, Status));
