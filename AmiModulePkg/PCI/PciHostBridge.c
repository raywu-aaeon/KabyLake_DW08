...
//    if( TmpBot[ResType] != 0 ) bot=TmpBot[ResType];
    ...
    DEBUG((DEBUG_VERBOSE,"  Ajusting ACPI MMIO MAP [0x%lX...[0x%lX...0x%lX]...0x%lX]\n",bot,cb,ct,top));
...
//--------------------
    ...
    switch(ResType){
        ...
        case raBus:
           ...
           DEBUG((DEBUG_VERBOSE,"  Ajusting ACPI BUS MAP [0x%lX...[0x%lX...0x%lX]...0x%lX]\n",
...
/**
    ...
    @retval EFI_DEVICE_ERROR SetResources failed due to HW error.
**/
EFI_STATUS EFIAPI ForceSetResources(PCI_ROOT_BRG_DATA *RootBrg, ASLR_QWORD_ASD *Resources,
            ASLR_TYPE_ENUM ResType, BOOLEAN Replace)
{
    UINTN               i, cnt;
    ASLR_QWORD_ASD      *res;
...
//--------------------------------------------------------
    ...
    switch(Phase){
          ...
          case EfiPciHostBridgeEndEnumeration:
              if(hbdata->AllocPhase+1!=Phase) return EFI_INVALID_PARAMETER;
               Status=HbNotifyCspEndEnumeration(&hbdata->ResAllocProtocol, &pp[0], hbdata->RootBridgeCount);
               ASSERT_EFI_ERROR(Status);
               if(EFI_ERROR(Status)) return Status;
               break;
...
//-----------------------------
    ...
    if(EFI_ERROR(Status)){
        DEBUG((DEBUG_ERROR,"Hb.SetBusNumbers - Invalid Resoutce Descriptor(s) Passed\n"));
        //ASSERT_EFI_ERROR(Status);
        //return Status;
    ...
    if(EFI_ERROR(Status)){
        ...
        if ( Status == EFI_UNSUPPORTED ){
          if (PcdGetBool(AmiPcdPciOutOfResourcesStatus)) {// BUS OOR Happend and Platform code didn't' implement HbCspSetBusNnumbers() to adjust the Bus Numbers.
...
//-----------------------------
    ...
    if(EFI_ERROR(Status)){
        DEBUG((DEBUG_ERROR,"Hb.SubmitResources - Invalid Resoutce Descriptor(s) Passed\n"));
        //ASSERT_EFI_ERROR(Status);
        //return Status;
        return EFI_INVALID_PARAMETER;
    }
