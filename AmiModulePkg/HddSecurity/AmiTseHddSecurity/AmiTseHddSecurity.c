...
)
{
    ...
    for (Index=0; Index<gHddSecurityCount; Index++) {
        ...
        if ( (NULL != DataPtr ) && (!DataPtr->Locked) && (DataPtr->Validated) ) {
            pBS->ConnectController(DataPtr->DeviceHandle,
                                          NULL,
                                          NULL,
                                          TRUE);
        }
...
)
{
    ...
    for ( i = 0; i < Count; i++ ) {
        ...
        if ((Security->BusInterfaceType == AmiStorageInterfaceIde) || \
            (Security->BusInterfaceType == AmiStorageInterfaceAhci)) {
...
)
{
    ...
    if ( !EFI_ERROR(Status) ) {
        ...
        if ((Security->BusInterfaceType != AmiStorageInterfaceIde) && \
            (Security->BusInterfaceType != AmiStorageInterfaceAhci) && \
            (Security->BusInterfaceType != AmiStorageInterfaceNvme)) {
            return FALSE;
        }
...
)
{
    ...
    *bHddStatus = FALSE;
    ...
    if (SecurityStatus & 0x10) {
        *bHddStatus = TRUE;
        DEBUG((DEBUG_VERBOSE, "HddSecurity: HDD unlock attempts reached maximum\n"));
    }
