...
)
{
    ...
    if (gFirstCall) {
        ...
        if (EFI_ERROR(Status)) {
            ...
            if (EFI_ERROR(Status)) {
                USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "AllocAlignedMemory AllocateAnyPages %r\n", Status);
                return NULL;
            }
        }
        ...
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Usb Memroy Allocation: 0x%0lx - 0x%0lx, Data size: 0x%0lx\n", 
