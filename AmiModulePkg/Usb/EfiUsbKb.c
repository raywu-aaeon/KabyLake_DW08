...
)
{
    ...
    gParameters->bFuncNumber         = USB_API_LIGHTEN_KEYBOARD_LEDS;
...
)
{
    ...
    if (NewEfiKey.Key.ScanCode == SCAN_NULL && NewEfiKey.Key.UnicodeChar == 0 &&
        ...
        if (NewEfiKey.KeyState.KeyShiftState & ~SHIFT_STATE_VALID) {
            ...
            if (!(NewEfiKey.KeyState.KeyToggleState & KEY_STATE_EXPOSED)) {
...
)
{
    ...
    if (!EFI_ERROR(Status)) {
        if ((EfiKey.Key.UnicodeChar == 0) && (EfiKey.Key.ScanCode == SCAN_NULL) &&
            ...
            // If the Key ShiftState has valid key but KEY_STATE_EXPOSED bit not enabled,
            // don't insert key into buffer
            if (!(EfiKey.KeyState.KeyToggleState & KEY_STATE_EXPOSED)) {
                InProgress = FALSE;
                return EFI_UNSUPPORTED;
            }
        }
...
)
{
    ...
    ToggleState = *KeyToggleState & (SCROLL_LOCK_ACTIVE | NUM_LOCK_ACTIVE | CAPS_LOCK_ACTIVE);

    if (*KeyToggleState & KEY_STATE_EXPOSED) {
        ToggleState |= KEY_STATE_EXPOSED;
    } else {
        ToggleState &= ~KEY_STATE_EXPOSED;
    }
...
)
{
    ...
    Status = gBS->CreateEvent(EFI_EVENT_NOTIFY_SIGNAL, TPL_CALLBACK,
...
)
{
    ...
    ASSERT(AmiKeyCode);
    if (AmiKeyCode == NULL) return EFI_INVALID_PARAMETER;
