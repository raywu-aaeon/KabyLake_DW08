...
UINT8   OHCI_GlobalSuspend (HC_STRUC*); 
...
//    UINT32          OhciControlReg = 0;
    ...
    Status = AmiUsbValidateMmioBuffer((VOID*)BaseAddr, HcStruc->BaseAddressSize);
...
)
{
    ...
    if (((UINTN)DescPtrs->fpTDRepeat < (UINTN)gUsbDataList->MemBlockStart) ||
    ...
    if (((UINTN)DescPtrs->fpEDRepeat < (UINTN)gUsbDataList->MemBlockStart) ||
...
//        USB_DEBUG(DEBUG_INFO, 3, "after OHCI_ProcessRootHubStatusChange\n");
    ...
    //
    for (;;) {
        Td = (OHCI_TD*)(UINTN)(((OHCI_HCCA_PTRS*)HcStruc->FrameList)->dHccaDoneHead);
        ((OHCI_HCCA_PTRS*)HcStruc->FrameList)->dHccaDoneHead = 0;
...
)
{
    ...
    if (((UINTN)DescPtrs->fpEDControl < (UINTN)gUsbDataList->MemBlockStart) ||
        (((UINTN)DescPtrs->fpEDControl + sizeof(OHCI_ED)) > (UINTN)MemBlockEnd)) {
        return 0;
    }
    if (((UINTN)DescPtrs->fpTDControlSetup < (UINTN)gUsbDataList->MemBlockStart) ||
        (((UINTN)DescPtrs->fpTDControlSetup + sizeof(OHCI_TD)) > (UINTN)MemBlockEnd)) {
        return 0;
    }
    if (((UINTN)DescPtrs->fpTDControlData < (UINTN)gUsbDataList->MemBlockStart) ||
        (((UINTN)DescPtrs->fpTDControlData + sizeof(OHCI_TD)) > (UINTN)MemBlockEnd)) {
        return 0;
    }
    if (((UINTN)DescPtrs->fpTDControlStatus < (UINTN)gUsbDataList->MemBlockStart) ||
...
)
{
    ...
    if (((UINTN)DescPtrs->fpEDBulk < (UINTN)gUsbDataList->MemBlockStart) ||
        (((UINTN)DescPtrs->fpEDBulk + sizeof(OHCI_ED)) > (UINTN)MemBlockEnd)) {
        return 0;
    } 

    if (((UINTN)DescPtrs->fpTDBulkData < (UINTN)gUsbDataList->MemBlockStart) ||
    ...
    for (;BytesRemaining != 0; BytesRemaining -= BytesToTransfer) {
        ...
        DescPtrs->fpTDBulkData->CallBackIndex = USB_InstallCallBackFunction(OHCI_GeneralTDCallback);
...
)
{
    ...
    ((OHCI_ED*)DevInfo->PollEdPtr)->dControl = Data;
    ((OHCI_ED*)DevInfo->PollEdPtr)->fpHeadPointer = (UINT32)(UINTN)DevInfo->PollTdPtr;
    ((OHCI_ED*)DevInfo->PollEdPtr)->fpHeadPointer |= DatToggle << 1;
    ((OHCI_ED*)DevInfo->PollEdPtr)->fpEDLinkPointer = OHCI_TERMINATE;
    ((OHCI_ED*)DevInfo->PollEdPtr)->fpTailPointer = OHCI_TERMINATE;
    ...
    ((OHCI_TD*)DevInfo->PollTdPtr)->dControlStatus = (UINT32)(GTD_BUFFER_ROUNDING | GTD_IN_PACKET | GTD_IntD |
                GTD_NO_ERRORS | (GTD_NOT_ACCESSED << 28));
    ((OHCI_TD*)DevInfo->PollTdPtr)->dCSReloadValue = (UINT32)(GTD_BUFFER_ROUNDING | GTD_IN_PACKET | GTD_IntD |
                GTD_NO_ERRORS | (GTD_NOT_ACCESSED << 28));
    ((OHCI_TD*)DevInfo->PollTdPtr)->fpCurrentBufferPointer = (UINT32)(DevInfo->PollDataBuffer); 
     ((OHCI_TD*)DevInfo->PollTdPtr)->fpBufferEnd  = (UINT32)(DevInfo->PollDataBuffer + DevInfo->PollingLength - 1);
    ((OHCI_TD*)DevInfo->PollTdPtr)->fpLinkPointer = OHCI_TERMINATE;
    ...
    ((OHCI_TD*)DevInfo->PollTdPtr)->bActiveFlag = TRUE;
    ((OHCI_ED*)DevInfo->PollEdPtr)->dControl &= ~ED_SKIP_TDQ;
...
)
{
    ...
    UsbUpdateDataToggle(DevInfo, DevInfo->IntInEndpoint, 
      (UINT8)((((OHCI_ED*)DevInfo->PollEdPtr)->fpHeadPointer & ED_TOGGLE_CARRY) >> 1));
    ...
    if ((((OHCI_TD*)Td)->dControlStatus & GTD_STATUS_FIELD) == GTD_NO_ERROR) {
        ...
        if (((OHCI_TD*)Td)->fpCurrentBufferPointer != 0) {
            BytesTransferred = (UINT16)(((OHCI_TD*)Td)->fpCurrentBufferPointer -
    ...
    ((OHCI_TD*)Td)->fpCurrentBufferPointer = (UINT32)(UINTN)(DevInfo->PollDataBuffer);  
    ((OHCI_ED*)DevInfo->PollEdPtr)->fpHeadPointer &= ED_TOGGLE_CARRY;
    ((OHCI_ED*)DevInfo->PollEdPtr)->fpHeadPointer |= (UINTN)((OHCI_TD*)Td);
...
)
{
    ...
    //
    // Save the 8 ED/TD in their proper position. Note: fpHCStruc->DescPtrs.OhciDescPtrs
    ...
    DescPtrs->fpEDRepeat->dControl = ((UINT32)(gUsbData->MaxDevCount+1) |
...
//    DescPtrs->fpEDRepeat->Interval = REPEAT_INTERVAL;
    if (gUsbData->RepeatIntervalMode){
        DescPtrs->fpEDRepeat->Interval = REPEAT_INTERVAL_16MS;
    } else {
        DescPtrs->fpEDRepeat->Interval = REPEAT_INTERVAL_8MS;
    }
    ...
    DescPtrs->fpTDRepeat->fpBufferEnd =
                (UINT32)(UINTN)DescPtrs->fpTDRepeat->aSetupData;
    DescPtrs->fpTDRepeat->fpLinkPointer = OHCI_TERMINATE;
    DescPtrs->fpTDRepeat->CallBackIndex = USB_InstallCallBackFunction(OHCI_RepeatTDCallBack);
    DescPtrs->fpTDRepeat->bActiveFlag = FALSE;
